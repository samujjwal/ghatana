...
@DisplayName("InMemory implementations — integration tests [GH-90000]")
class IncidentServiceLauncherIntegrationTest extends EventloopTestBase {

    @Test
    @DisplayName("InMemoryKillSwitchService activate and deactivate work correctly [GH-90000]")
    void inMemoryKillSwitchService_activateDeactivate_works() { // GH-90000
        ...
        boolean isActive = runPromise(() -> service.isActive("tenant-1 [GH-90000]"));
        assertThat(isActive).isTrue(); // GH-90000
        
        runPromise(() -> service.deactivate("tenant-1", "resolved")); // GH-90000
        isActive = runPromise(() -> service.isActive("tenant-1 [GH-90000]"));
        assertThat(isActive).isFalse(); // GH-90000
    }

    @Test
    @DisplayName("InMemoryKillSwitchService global activate works [GH-90000]")
    ...
    @Test
    @DisplayName("InMemoryGracefulDegradationManager set and get mode work correctly [GH-90000]")
    ...
    @Test
    @DisplayName("InMemoryGracefulDegradationManager defaults to FULL [GH-90000]")
    ...
    @Test
    @DisplayName("InMemory implementations handle multiple tenants independently [GH-90000]")
    void inMemoryImplementations_multipleTenantsIndependent() { // GH-90000
        ...
        boolean tenant1Active = runPromise(() -> killSwitch.isActive("tenant-1 [GH-90000]"));
        boolean tenant2Active = runPromise(() -> killSwitch.isActive("tenant-2 [GH-90000]"));
        boolean tenant3Active = runPromise(() -> killSwitch.isActive("tenant-3 [GH-90000]"));
        ...
        DegradationMode mode1 = runPromise(() -> degradation.getMode("tenant-1 [GH-90000]"));
        DegradationMode mode2 = runPromise(() -> degradation.getMode("tenant-2 [GH-90000]"));
        DegradationMode mode3 = runPromise(() -> degradation.getMode("tenant-3 [GH-90000]"));
