 ...
 * @doc.pattern Test
 */
@DisplayName("Concurrent UserProfile Update Tests [GH-90000]")
class ConcurrentUpdateTest extends EventloopTestBase {
    ...
    @Test
    @DisplayName("concurrent upserts for 50 distinct users all persist successfully [GH-90000]")
    ...
    @Test
    @DisplayName("concurrent upserts for same user result in exactly one stored entry [GH-90000]")
    void concurrentUpsertsForSameUserResultInOneEntry() throws InterruptedException { // GH-90000
        ...
        for (int i = 0; i < count; i++) { // GH-90000
            ...
            Thread.ofVirtual().start(() -> { // GH-90000
                UserProfile profile = UserProfile.builder() // GH-90000
                        .userId("user-same [GH-90000]")
                        .tenantId("tenant-single [GH-90000]")
                        .email("same@example.com [GH-90000]")
        ...
        assertThat(storage.containsKey("tenant-single|user-same [GH-90000]")).isTrue();
    }

    // ── Concurrent update + read consistency ──────────────────────────────────

    @Test
    @DisplayName("concurrent read always finds a stored profile after upsert completes [GH-90000]")
    void concurrentReadFindsProfileAfterUpsert() throws InterruptedException { // GH-90000
        ...
        UserProfile base = UserProfile.builder() // GH-90000
                .userId("user-read-conc [GH-90000]")
                .tenantId("tenant-read-conc [GH-90000]")
                .email("read@conc.example.com [GH-90000]")
                .build(); // GH-90000
        ...
        for (int i = 0; i < readerCount; i++) { // GH-90000
            Thread.ofVirtual().start(() -> { // GH-90000
                UserProfile found = storage.get("tenant-read-conc|user-read-conc [GH-90000]");
    ...
    @Test
    @DisplayName("concurrent delete and upsert for same user key stabilizes without errors [GH-90000]")
    void concurrentDeleteAndUpsertStabilizes() throws InterruptedException { // GH-90000
        UserProfile profile = UserProfile.builder() // GH-90000
                .userId("user-del-upsert [GH-90000]")
                .tenantId("tenant-del-upsert [GH-90000]")
                .email("delup@example.com [GH-90000]")
                .build(); // GH-90000
        ...
        Optional<UserProfile> finalState = Optional.ofNullable( // GH-90000
                storage.get("tenant-del-upsert|user-del-upsert [GH-90000]"));
