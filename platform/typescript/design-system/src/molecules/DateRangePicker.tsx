...
const DEFAULT_PRESETS: DateRangePreset[] = [
  {
    ...
    getValue: () => {
      const now = new Date();
      const today = toIsoDate(new Date(now.setHours(0, 0, 0, 0)));
  ...
  },
  {
    ...
    getValue: () => {
      ...
      const start = toIsoDate(new Date(now.setDate(now.getDate() - 7)));
  ...
  },
  {
    ...
    getValue: () => {
      const now = new Date();
      const end = toIsoDate(now);
      const start = toIsoDate(new Date(now.setDate(now.getDate() - 30)));
      return { startDate: start, endDate: end };
    },
  },
  {
  ...
  },
  {
    ...
    getValue: () => {
      const now = new Date();
      const start = toIsoDate(new Date(now.getFullYear(), now.getMonth() - 1, 1));
      const end = toIsoDate(new Date(now.getFullYear(), now.getMonth(), 0));
      return { startDate: start, endDate: end };
    },
  },
];
