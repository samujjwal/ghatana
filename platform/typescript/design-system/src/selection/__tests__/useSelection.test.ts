...
describe('useSelection', () => {
  it('initializes with empty selection', () => {
    ...
    const { result } = renderHook(() => useSelection({ items, keyFn: (item) => item.id }));
  ...
  it('toggles selection for a single item', () => {
    ...
    expect(result.current.isAllSelected).toBe(false);
    // 1 of 2 items selected → partial selection → isIndeterminate is true
  ...
  it('updates selectedItems when items change', () => {
    ...
    const { result, rerender } = renderHook(
      ({ items }: { items: TestItem[] }) => useSelection({ items, keyFn: (item) => item.id }),
    ...
    expect(result.current.selectedItems).toEqual([{ id: '1', name: 'Updated Item 1' }]);
  });
});
