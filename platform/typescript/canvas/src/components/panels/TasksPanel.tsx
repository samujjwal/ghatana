...
export const TasksPanel: React.FC<TasksPanelProps> = ({ onClose }) => {
  ...
  const tasks = useMemo(() => {
    ...
    return taskElements.map((el): Task => ({
      id: el.id,
      title: String(el.data?.label ?? el.data?.title ?? "Untitled Task"),
      description: typeof el.data?.description === "string" ? el.data.description : undefined,
      status: (el.data?.status as TaskStatus) || "todo",
      priority: (el.data?.priority as TaskPriority) || "medium",
      phase: typeof el.data?.phase === "string" ? el.data.phase : currentPhase,
      assignee: typeof el.data?.assignee === "string" ? el.data.assignee : undefined,
      dueDate: typeof el.data?.dueDate === "string" ? el.data.dueDate : undefined,
      createdAt:
        typeof el.data?.createdAt === "string"
          ? el.data.createdAt
          : new Date().toISOString(),
      updatedAt:
        typeof el.data?.updatedAt === "string"
          ? el.data.updatedAt
          : new Date().toISOString(),
    }));
  }, [currentPhase]);
