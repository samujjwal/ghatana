...
function deploymentVerifierStatusLabel(
...
export default function DeploymentsPage(): ReactElement {
  ...
  const deploymentManifestStatus =
    snapshot.manifestLoadState.deploymentManifest.status;
  const deploymentManifestMessage =
    snapshot.manifestLoadState.deploymentManifest.message;
  const verifyHealthStatus =
    snapshot.manifestLoadState.verifyHealthReport.status;
  const deploymentManifestRef =
    snapshot.selectedRun?.manifestRefs?.["deployment-manifest"];
  const verifyHealthRef =
    snapshot.selectedRun?.manifestRefs?.["verify-health-report"] ??
    snapshot.selectedRun?.healthSnapshotRef;
  ...
  return (
    <section className="space-y-6" aria-labelledby="deployments-title">
      ...
      {deploymentManifestStatus === "missing" ||
      deploymentManifestStatus === "unavailable" ||
      deploymentManifestStatus === "corrupt" ||
      deploymentManifestStatus === "unauthorized" ||
      verifyHealthStatus === "missing" ||
      verifyHealthStatus === "unavailable" ? (
        <article
          className="rounded-md border border-red-200 bg-red-50 p-4"
          aria-label="deployment-manifest-state"
        >
        ...
        >
          ...
          <p className="text-xs text-gray-600">
            verify-health-report: {verifyHealthStatus}
          </p>
          {deploymentManifestStatus !== "loaded" && (
          ...
          <ul className="space-y-1 text-xs text-gray-600">
            ...
            <li className="font-mono">
              ...
              {verifyHealthRef ?? t("studio.route.lifecycle.notAvailableValue")}
            </li>
          </ul>
        </article>
      )}
        ...
        <article className="studio-card space-y-3">
          ...
          <dl className="space-y-2 text-sm text-gray-600">
            <div className="flex justify-between gap-3">
              ...
              <dd>
                {deploymentManifest?.target ??
                  deploymentManifest?.environment ??
                  t("studio.route.lifecycle.notAvailableValue")}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              ...
              <dd>
                {serviceNames.length > 0
                  ? serviceNames.join(", ")
                  : t("studio.deployments.servicesNotReported")}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="font-medium text-gray-900">
              ...
              <dd>
                {deploymentVerifierStatusLabel(
                  deploymentManifest?.verifierResult?.valid,
                  t,
                )}
              </dd>
            </div>
