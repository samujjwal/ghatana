...
describe('ToolchainAdapterRegistryBridge', () => {
  ...
  it('includes registered adapter ids when bridge lookup fails', () => {
    ...
    expect(() => bridge.getAdapter('missing')).toThrow(/Registered adapters: \[gradle-java-service\]/);
  });
  ...
  it('preserves full toolchain execution evidence through the bridge', async () => {
    class EvidenceAdapter implements ToolchainAdapter {
      ...
      async execute(_context: ToolchainAdapterContext): Promise<ToolchainExecutionResult> {
        return {
          status: 'failed',
          steps: [
            {
          ...
          coverageResults: { lineCoverage: 50, branchCoverage: 40, instructionCoverage: 45 },
          durationMs: 12,
          failure: { stepId: 'docker-build', message: 'build failed', cause: 'exit 1' },
    ...
    const result = await bridge.getAdapter('evidence-adapter').execute({
      ...
      phase: 'package',
      surface: { type: 'backend-api', adapter: 'evidence-adapter', path: 'products/api' },
      dryRun: false,
      surfaceConfig: {},
      phaseConfig: {},
      logger: { info: () => {}, warn: () => {}, error: () => {}, debug: () => {} },
  ...
  it('infers bridged artifact types by phase and surface type', async () => {
    const cases = [
      { phase: 'deploy', surfaceType: 'backend-api', expectedType: 'deployment-manifest' },
      { phase: 'verify', surfaceType: 'backend-api', expectedType: 'verify-health-report' },
      { phase: 'test', surfaceType: 'backend-api', expectedType: 'test-report' },
      { phase: 'build', surfaceType: 'web', expectedType: 'static-web-bundle' },
      { phase: 'build', surfaceType: 'backend-api', expectedType: 'jvm-service' },
      { phase: 'package', surfaceType: 'web', expectedType: 'static-web-bundle', artifactRef: 'web/dist' },
    ] as const;
    ...
    for (const testCase of cases) {
      ...
      const result = await bridge.getAdapter(adapter.id).execute({
        ...
        surface: { type: testCase.surfaceType, adapter: adapter.id, path: 'surface/path' },
        dryRun: false,
        surfaceConfig: {},
        phaseConfig: {},
        logger: { info: () => {}, warn: () => {}, error: () => {}, debug: () => {} },
