...
describe("DockerBuildxAdapter", () => {
  ...
  beforeEach(async () => {
    repoRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "ghatana-docker-buildx-"),
    );
    await fs.mkdir(path.join(repoRoot, "products/digital-marketing/dm-api"), {
      recursive: true,
    });
    await fs.writeFile(
    ...
    commandRunner = new FakeCommandRunner([
      { exitCode: 0, stdout: "", stderr: "", durationMs: 100 },
      {
        exitCode: 0,
        stdout:
          '["ghatana/digital-marketing-api@sha256:abc123"]|sha256:imageid',
        stderr: "",
        durationMs: 10,
      },
      {
  ...
  describe("plan()", () => {
    ...
    it("includes -t <image>:<tag> in the command", async () => {
      ...
      expect(step.command[tIndex + 1]).toBe(
        "ghatana/digital-marketing-api:local",
      );
    });
    ...
    it("includes --build-arg pairs when buildArgs are configured", async () => {
      ...
      const buildArgCount = step.command.filter(
        (a) => a === "--build-arg",
      ).length;
      expect(buildArgCount).toBe(2);
    });

    it("includes an explicit buildx builder when configured", async () => {
      const [step] = await adapter.plan(
        makeContext({
          surfaceConfig: {
            dockerfile: "Dockerfile",
            context: ".",
      ...
      expect(step.command).toContain("--builder");
      expect(step.command).toContain("default");
    });
    ...
    it("requires dockerfile, context, image, and tag", async () => {
      const ctx = makeContext({
        surfaceConfig: { image: "test/img", tag: "v1", context: "." },
      });

      await expect(adapter.plan(ctx)).rejects.toThrow(
        "surfaceConfig.dockerfile",
      );
    });
  });
  ...
  describe("execute()", () => {
    it("calls docker buildx build and returns succeeded on exit 0", async () => {
      ...
      expect(result.artifacts).toContain(
        "ghatana/digital-marketing-api@sha256:abc123",
      );
      expect(result.evidenceRefs).toContain(
        "container-image:ghatana/digital-marketing-api@sha256:abc123",
      );
    ...
    it("returns failed when image inspect cannot find the built image", async () => {
      ...
      expect(result.failure?.message).toContain(
        "not found in local Docker daemon",
      );
    });
    ...
    it("falls back to image ref when inspect digest segment is empty", async () => {
      ...
      expect(result.status).toBe("failed");
      expect(result.failure?.message).toContain(
        "container-image-digest-missing",
      );
      expect(result.artifacts).toContain("ghatana/digital-marketing-api:local");
    });
    ...
    it("falls back to image ref when inspect digest payload is not an array", async () => {
      commandRunner = new FakeCommandRunner([
        ...
        {
          exitCode: 0,
          stdout: '{"Id":"sha256:imageid"}|sha256:imageid',
          stderr: "",
          durationMs: 10,
        },
        {
    ...
    it("fails before execution when Dockerfile or context is missing", async () => {
      ...
      expect(result.status).toBe("failed");
      expect(result.failure?.message).toContain(
        "surfaceConfig.dockerfile-not-found",
      );
      expect(commandRunner.invocations).toHaveLength(0);
    });
    ...
    it("redacts build arg values in logs and command runner options", async () => {
      ...
      expect(
        commandRunner.invocations[0].options.redact?.(
          "--build-arg API_TOKEN=secret-token",
        ),
      ).toBe("--build-arg API_TOKEN=<redacted>");
    });
  });
});
