/**
 * DataCloudMemoryProvider - Data Cloud-backed memory persistence.
 *
 * @doc.type class
 * @doc.purpose Data Cloud-backed memory provider for Kernel platform mode
...
export interface DataCloudMemoryProviderOptions {
...
export class DataCloudMemoryProvider implements LifecycleMemoryProvider {
  ...
  constructor(options: DataCloudMemoryProviderOptions) {
  ...
  async recordMemory(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return fail(`Failed to record memory to Data Cloud: ${response.status} ${errorText}`, options.required);
      }

      const result = await response.json() as { ref?: string; id?: string };
      return { success: true, ref: result.ref || `data-cloud-memory:${result.id}` };
    } catch (error) {
      return fail(`Data Cloud memory write failed: ${error instanceof Error ? error.message : String(error)}`, options.required);
    }
  }
  ...
  async listMemory(query: LifecycleProviderQuery): Promise<readonly LifecycleMemoryRecord[]> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to list memory from Data Cloud: ${response.status}`);
      }

      const data = await response.json() as { records?: LifecycleMemoryRecord[] };
      return data.records || [];
    } catch (error) {
      throw new Error(`Data Cloud memory list failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
