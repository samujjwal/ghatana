...
describe("buildRoundTripDiffReport", () => {
  it("marks generated files as semantically equivalent when re-import preserves model shape", () => {
    ...
    expect(
      report.paritySections?.find((section) => section.kind === "component")
        ?.status,
    ).toBe("passed");
    expect(
      report.paritySections?.find((section) => section.kind === "import-graph")
        ?.findings[0],
  ...
  it("marks missing re-imported nodes as not semantically equivalent", () => {
    ...
    const report = buildRoundTripDiffReport({
      ...
      originalSources: [{ relativePath: "src/Button.tsx", content: SOURCE }],
      generatedSources: [
        { relativePath: "src/Button.tsx", content: "export const Broken = 1;" },
      ],
...
`.trim();
    ...
    const report = buildRoundTripDiffReport({
      ...
      originalSources: [{ relativePath: "src/Button.tsx", content: SOURCE }],
      generatedSources: [
        { relativePath: "src/Button.tsx", content: generated },
      ],
    });
  ...
  it("marks JSX event-handler changes as not semantically equivalent", () => {
    ...
    const report = buildRoundTripDiffReport({
      ...
      model: decompiled.model,
      originalSources: [
        { relativePath: "src/Clickable.tsx", content: original },
      ],
      generatedSources: [
        { relativePath: "src/Clickable.tsx", content: generated },
      ],
    });
  ...
  it("includes generated artifact validation results in the round-trip report", () => {
    ...
    expect(
      report.paritySections?.find((section) => section.kind === "validation"),
    ).toMatchObject({
      status: "warning",
  ...
  it("defaults missing generated validation to a blocking not-run result", () => {
    const decompiled = decompileTsx({
      label: "diff-test-validation-default",
      modelId: "diff-model-validation-default",
      files: [{ relativePath: "src/Button.tsx", content: SOURCE }],
    });
    const compiled = compileReact(decompiled.model);

    const report = buildRoundTripDiffReport({
      reportId: "diff-report-validation-default",
      model: decompiled.model,
      originalSources: [{ relativePath: "src/Button.tsx", content: SOURCE }],
      generatedSources: compiled.emittedFiles,
    });
    ...
    expect(report.validation).toMatchObject({
      targetId: "diff-model-validation-default",
      passed: false,
      findings: [expect.objectContaining({ code: "validation/not-run" })],
    });
    expect(report.isLossless).toBe(false);
    expect(
      report.paritySections?.find((section) => section.kind === "validation")
  ...
  it("surfaces design-token parity drift as a structured warning section", () => {
    ...
    expect(
      report.paritySections?.find((section) => section.kind === "design-token"),
    ).toMatchObject({
      status: "warning",
      findings: [
