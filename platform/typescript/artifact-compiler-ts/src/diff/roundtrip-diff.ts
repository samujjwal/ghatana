...
export function buildRoundTripDiffReport(
  ...
  const validation =
    ...
    createNotRunValidationPipelineResult({
      targetId: options.model.modelId,
      reason:
        "Generated artifact validation was not provided to the round-trip diff builder.",
    });
  const originalByPath = new Map(
    options.originalSources.map((source) => [source.relativePath, source]),
  );
  ...
  return {
    ...
    fidelity:
      options.fidelity ??
      computeFidelityReport([], options.model.modelId, "pipeline"),
    residuals: options.residuals ?? createResidualIslandReport([]),
    validation,
    paritySections,
    isLossless:
      diffs.every(
        (diff) => diff.semanticallyEquivalent && diff.hunks.length === 0,
      ) && validation.passed,
    ...
    findings: [
      {
        ...
        category: "other",
        suggestion:
          "Run generated project install, typecheck, lint, test, and build gates before marking the round-trip production-ready.",
      },
    ],
    errorCount: 0,
    warningCount: 1,
...
function diffSourcePair(options: {
  ...
  const hunk = buildSingleSummaryHunk(
    originalLines,
    generatedLines,
    unchangedLines,
  );
...
function buildRoundTripParitySections(options: {
  ...
  const importGraphFailures = buildImportGraphFindings(
    options.model,
    options.reimportedModel,
  );
  const componentFindings = buildComponentParityFindings(
    options.model,
    options.reimportedModel,
  );
  const apiFindings = buildApiParityFindings(
    options.model,
    options.reimportedModel,
  );
  const designTokenFindings = buildDesignTokenParityFindings(
    options.originalSources,
    options.generatedSources,
  );
  ...
  return [
    ...
    },
    {
      kind: "import-graph",
      status:
        options.reimportedModel === undefined
          ? "not-run"
    ...
    },
    {
      ...
      findings:
        options.validation?.findings.map(
...
function areAstEquivalent(
  ...
  if (originalAst === null || generatedAst === null) {
    return (
      normalizeSource(originalContent) === normalizeSource(generatedContent)
    );
  }
  ...
  try {
    ...
    const visit = (node: ts.Node): void => {
      if (
        ...
        importSignatures.push(
          `${importClause?.isTypeOnly === true ? "type" : "value"}:${node.moduleSpecifier.text}:${bindingShape}`,
        );
      }
      ...
      if (ts.isJsxElement(node)) {
        ...
        node.openingElement.attributes.properties.forEach((attribute) => {
          if (ts.isJsxAttribute(attribute)) {
            const attributeName = jsxAttributeNameText(
              attribute.name,
              sourceFile,
            );
            ...
            if (
              ...
              jsxBindingExpressions.push(
                `${attributeName}:${normalizeSource(attribute.initializer.expression.getText(sourceFile))}`,
              );
            }
      ...
      if (ts.isJsxExpression(node) && node.expression) {
        jsxBindingExpressions.push(
          normalizeSource(node.expression.getText(sourceFile)),
        );
      }
    ...
    return JSON.stringify({
      imports: importSignatures.sort((left, right) =>
        left.localeCompare(right),
      ),
      exports: exportSignatures.sort((left, right) =>
        left.localeCompare(right),
      ),
      jsxNodes: jsxNodeKinds.sort((left, right) => left.localeCompare(right)),
      jsxAttributes: jsxAttributeNames.sort((left, right) =>
        left.localeCompare(right),
      ),
      calls: callExpressionNames.sort((left, right) =>
        left.localeCompare(right),
      ),
      jsxEventHandlers: jsxEventHandlerNames.sort((left, right) =>
        left.localeCompare(right),
      ),
      jsxBindings: jsxBindingExpressions.sort((left, right) =>
        left.localeCompare(right),
      ),
      styleReferences: styleReferences.sort((left, right) =>
        left.localeCompare(right),
      ),
    });
  } catch {
...
function buildSingleSummaryHunk(
  ...
  return {
    ...
    lineCount: Math.max(
      1,
      Math.max(originalLines.length, generatedLines.length) - unchangedLines,
    ),
    originalSnippet: originalLines
      .slice(unchangedLines, unchangedLines + 12)
      .join("\n")
      .slice(0, 500),
    generatedSnippet: generatedLines
      .slice(unchangedLines, unchangedLines + 12)
      .join("\n")
      .slice(0, 500),
  };
...
function isSemanticallyEquivalent(
  ...
  const shapeEquivalent =
    ...
    JSON.stringify([...originalNode.exportedSymbols].sort()) ===
      JSON.stringify([...reimportedNode.exportedSymbols].sort()) &&
    JSON.stringify(normalizeRecord(originalNode.inferredProps)) ===
      JSON.stringify(normalizeRecord(reimportedNode.inferredProps)) &&
    JSON.stringify(normalizeSourceImportShape(originalNode.sourceImports)) ===
      JSON.stringify(
        normalizeSourceImportShape(reimportedNode.sourceImports),
      ) &&
    JSON.stringify(nodeEdgeShape(model, nodeId)) ===
      JSON.stringify(nodeEdgeShape(reimportedModel, nodeId));

  if (!shapeEquivalent) return false;

  // Import graph parity (new)
  const importGraphEquivalent = isImportGraphEquivalent(
    model,
    reimportedModel,
    nodeId,
  );
...
function isImportGraphEquivalent(
  ...
  const originalEdges = originalModel.edges.filter(
    (edge) => edge.fromId === nodeId,
  );
  const reimportedEdges = reimportedModel.edges.filter(
    (edge) => edge.fromId === nodeId,
  );
  ...
  return (
    JSON.stringify(originalNormalized) === JSON.stringify(reimportedNormalized)
  );
}
...
function buildImportGraphFindings(
  ...
  const mismatches = nodeIds.filter(
    (nodeId) =>
      !isImportGraphEquivalent(originalModel, reimportedModel, nodeId),
  );
  ...
  findings.push(
    ...mismatches.map((nodeId) => `Import graph differs for node "${nodeId}".`),
  );
  return findings;
}
...
function buildComponentParityFindings(
  ...
  const originalNodeIds = Object.keys(originalModel.nodes).sort((left, right) =>
    left.localeCompare(right),
  );
  const reimportedNodeIds = Object.keys(reimportedModel.nodes).sort(
  ...
  for (const nodeId of originalNodeIds) {
    ...
    if (originalNode.kind !== reimportedNode.kind) {
      findings.push(
        `Component node "${nodeId}" kind changed from "${originalNode.kind}" to "${reimportedNode.kind}".`,
      );
    }
    if (originalNode.displayName !== reimportedNode.displayName) {
      findings.push(
        `Component node "${nodeId}" displayName changed from "${originalNode.displayName}" to "${reimportedNode.displayName}".`,
      );
    }
  }
  for (const nodeId of reimportedNodeIds) {
    if (originalModel.nodes[nodeId] === undefined) {
      findings.push(
        `Component node "${nodeId}" was introduced during re-import.`,
      );
    }
  }
  return findings;
...
function buildApiParityFindings(
  ...
  if (reimportedModel === undefined) return [];
  const findings: string[] = [];
  for (const nodeId of Object.keys(originalModel.nodes).sort((left, right) =>
    left.localeCompare(right),
  )) {
    const originalNode = originalModel.nodes[nodeId];
    const reimportedNode = reimportedModel.nodes[nodeId];
    if (originalNode === undefined || reimportedNode === undefined) continue;
    const originalExports = JSON.stringify(
      [...originalNode.exportedSymbols].sort(),
    );
    const reimportedExports = JSON.stringify(
      [...reimportedNode.exportedSymbols].sort(),
    );
    ...
    if (
      JSON.stringify(normalizeSourceImportShape(originalNode.sourceImports)) !==
      JSON.stringify(normalizeSourceImportShape(reimportedNode.sourceImports))
    ) {
...
function buildDesignTokenParityFindings(
  ...
  for (const token of originalTokens) {
    if (!generatedTokens.has(token)) {
      findings.push(
        `Design token reference "${token}" is missing from generated sources.`,
      );
    }
  }
  for (const token of generatedTokens) {
    if (!originalTokens.has(token)) {
      findings.push(
        `Design token reference "${token}" was introduced in generated sources.`,
      );
    }
  }
  return findings;
...
function collectDesignTokenReferences(
  sources: readonly RoundTripDiffSourceFile[],
): ReadonlySet<string> {
...
function normalizeSourceImportShape(
  sourceImports: LogicalArtifactModel["nodes"][string]["sourceImports"],
): readonly string[] {
  if (sourceImports === undefined) return [];
  return sourceImports
    .map(
      (record) =>
        `${record.isTypeOnly ? "type" : "value"}:${record.moduleSpecifier}:${record.importClauseText ?? ""}`,
    )
    .sort((left, right) => left.localeCompare(right));
}

function nodeEdgeShape(
  model: LogicalArtifactModel,
  nodeId: string,
): readonly string[] {
  return model.edges
