...
export class YjsMonacoBinding {
   ...
   */
  private debouncedSyncFromMonacoToYjs(
    event: editor.IModelContentChangedEvent
  ): void {
  ...
  private syncFromMonacoToYjs(event: editor.IModelContentChangedEvent): void {
    ...
    try {
      ...
      for (const change of event.changes) {
        ...
        const startOffset = this.getOffsetAtPosition(
          range.startLineNumber,
          range.startColumn
        );
        const endOffset = this.getOffsetAtPosition(
          range.endLineNumber,
          range.endColumn
        );
      ...
      this.metrics.averageSyncTime =
        (this.metrics.averageSyncTime * (this.metrics.syncOperations - 1) +
          syncTime) /
        this.metrics.syncOperations;
    ...
    try {
      ...
      for (const delta of currentDelta) {
        ...
        if (delta.delete) {
          ...
          const endPos = this.getPositionAtOffset(
            currentPosition + delta.delete
          );
   ...
   */
  private getPositionAtOffset(offset: number): {
    line: number;
    column: number;
  } {
    const model = this.editor.getModel();
