...
describe('KernelLifecycleApiHandlers', () => {
  ...
  it('persists and reloads Studio workflow state by tenant/workspace/project scope', async () => {
    const handlers = new KernelLifecycleApiHandlers({ service: createService(), requireAuthentication: false });
    const body = {
      state: {
  ...
  it('keeps Studio workflow state isolated between scopes', async () => {
    ...
    await handlers.putStudioWorkflowState({ headers: scopedHeaders('corr-studio-3'), body });
  ...
  it('persists Studio evidence immutably and treats identical writes as idempotent', async () => {
    ...
    const first = await handlers.putStudioWorkflowEvidence({
      headers: { ...scopedHeaders('corr-studio-6'), 'Idempotency-Key': 'idem-1' },
      body: evidence,
    });
    const second = await handlers.putStudioWorkflowEvidence({
      headers: { ...scopedHeaders('corr-studio-7'), 'Idempotency-Key': 'idem-1' },
      body: evidence,
    });
    ...
    expect(conflicting.body).toMatchObject({ reasonCode: 'evidence-immutable' });
  });
  ...
  it('creates scoped Studio repository source acquisition jobs', async () => {
    ...
    const jobId = (response.body as { readonly acquisitionJob: { readonly jobId: string } }).acquisitionJob.jobId;
  ...
  it('rejects repository source acquisition for mismatched or credentialed hosts', async () => {
    ...
    const mismatchedHost = await handlers.createStudioRepositorySourceAcquisition({
    ...
    const credentialedUrl = await handlers.createStudioRepositorySourceAcquisition({
    ...
    expect(mismatchedHost.body).toMatchObject({ reasonCode: 'invalid-request', correlationId: 'corr-source-3' });
    expect(credentialedUrl.statusCode).toBe(400);
    expect(credentialedUrl.body).toMatchObject({ reasonCode: 'invalid-request', correlationId: 'corr-source-4' });
  });
  ...
  it('rejects unsafe or oversized archive source acquisition requests', async () => {
    ...
    expect(unsafeName.body).toMatchObject({ reasonCode: 'invalid-request', correlationId: 'corr-source-5' });
    expect(oversized.statusCode).toBe(400);
    expect(oversized.body).toMatchObject({ reasonCode: 'invalid-request', correlationId: 'corr-source-6' });
  });
  ...
  it('updates Studio source acquisition jobs through worker lifecycle states', async () => {
    ...
    const jobId = (createResponse.body as { readonly acquisitionJob: { readonly jobId: string } }).acquisitionJob.jobId;
  ...
  it('rejects invalid Studio source acquisition worker transitions and terminal payloads', async () => {
    ...
    expect(invalidComplete.body).toMatchObject({ reasonCode: 'invalid-request' });
    expect(failed.statusCode).toBe(200);
    expect(failed.body).toMatchObject({ status: 'failed', errorMessage: 'clone failed' });
    expect(invalidAfterTerminal.statusCode).toBe(409);
    expect(invalidAfterTerminal.body).toMatchObject({ reasonCode: 'invalid-acquisition-job-transition' });
  });
});
