...
describe('StudioSourceAcquisitionWorker', () => {
  it('materializes a TAR archive and marks the job complete', async () => {
    ...
    const worker = new StudioSourceAcquisitionWorker(jobStore, writer, fixedClock());
    ...
    const result = await worker.executeArchive({
      ...
      fileName: 'source.tar',
      bytes: makeTar([
        { path: 'src/App.tsx', bytes: new TextEncoder().encode('export const App = true;') },
        { path: 'src/.hidden.tsx', bytes: new TextEncoder().encode('export const Hidden = true;') },
      ]),
    });
    ...
    expect(result.files.map((file) => file.relativePath)).toEqual(['src/App.tsx']);
    expect(writer.filesByJob.get('job-1')?.map((file) => file.relativePath)).toEqual(['src/App.tsx']);
  });
  ...
  it('materializes a repository archive fetched by an injected server-side fetcher', async () => {
        ...
        fetchArchive: async (request) => {
          ...
          return {
            ...
            bytes: gzipSync(makeTar([
              { path: 'repo/src/App.tsx', bytes: new TextEncoder().encode('export const App = true;') },
            ])),
          };
        },
      },
    ...
    expect(result.files.map((file) => file.relativePath)).toEqual(['repo/src/App.tsx']);
  });

  it('marks the job failed when archive entries attempt path traversal', async () => {
    const jobStore = new TestJobStore();
    await jobStore.putJob(createJob('job-3', 'archive'));
    const worker = new StudioSourceAcquisitionWorker(jobStore, new InMemoryStudioSourceWorkspaceWriter(), fixedClock());
    ...
    const result = await worker.executeArchive({
      ...
      fileName: 'source.tar',
      bytes: makeTar([
        { path: '../escape.tsx', bytes: new TextEncoder().encode('export const Escape = true;') },
      ]),
    });
  ...
  it('marks the job failed when archive limits are exceeded', async () => {
    ...
    const result = await worker.executeArchive({
      ...
      fileName: 'source.tar',
      bytes: makeTar([
        { path: 'src/Large.tsx', bytes: new TextEncoder().encode('x'.repeat(32)) },
      ]),
...
describe('HttpStudioRepositoryArchiveFetcher', () => {
  it('fetches GitHub tarballs with bearer token injection and size checks', async () => {
    ...
    expect(capturedUrl).toBe('https://github-api.local/repos/samujjwal/ghatana/tarball/main');
  ...
  it('fetches GitLab archive tarballs through the project archive endpoint', async () => {
    ...
    expect(capturedUrl).toBe('https://gitlab-api.local/api/v4/projects/group%2Fproject/repository/archive.tar.gz?sha=release');
  ...
  it('rejects unsupported or credentialed repository URLs before fetching', async () => {
    ...
    await expect(fetcher.fetchArchive({
  ...
  it('fails closed for HTTP errors and oversized responses without exposing tokens', async () => {
    const failingFetcher = new HttpStudioRepositoryArchiveFetcher({
      ...
      fetchFn: async () => new Response('rate limited secret-token', { status: 429 }),
    });
    await expect(failingFetcher.fetchArchive({
      repositoryUrl: 'https://github.com/samujjwal/ghatana',
      maxBytes: 10,
    ...
    await expect(oversizedByHeader.fetchArchive({
      repositoryUrl: 'https://github.com/samujjwal/ghatana',
      maxBytes: 10,
    ...
    await expect(oversizedByBody.fetchArchive({
      repositoryUrl: 'https://github.com/samujjwal/ghatana',
      maxBytes: 2,
    })).rejects.toThrow(/exceeds maximum size/);
  });
...
describe('file-backed Studio source acquisition adapters', () => {
  it('persists acquisition jobs and materialized files across adapter instances', async () => {
    ...
    try {
      const jobStore = new FileSystemStudioSourceAcquisitionJobStore(join(root, 'jobs'));
      const writer = new FileSystemStudioSourceWorkspaceWriter(join(root, 'workspaces'));
      await jobStore.putJob(createJob('studio-acquisition:archive:durable', 'archive'));
      const worker = new StudioSourceAcquisitionWorker(jobStore, writer, fixedClock());
      ...
      const reloadedStore = new FileSystemStudioSourceAcquisitionJobStore(join(root, 'jobs'));
      const reloadedJob = await reloadedStore.getJob(scope, 'studio-acquisition:archive:durable');
      const writtenFile = await readFile(join(result.job.localWorkspacePath ?? '', 'src', 'App.tsx'), 'utf8');
  ...
  it('executes stored archive payloads from durable scoped payload storage', async () => {
    ...
    try {
      const jobStore = new FileSystemStudioSourceAcquisitionJobStore(join(root, 'jobs'));
      const payloadStore = new FileSystemStudioSourceAcquisitionPayloadStore(join(root, 'payloads'));
      const writer = new FileSystemStudioSourceWorkspaceWriter(join(root, 'workspaces'));
      ...
      const reloadedPayloadStore = new FileSystemStudioSourceAcquisitionPayloadStore(join(root, 'payloads'));
      const worker = new StudioSourceAcquisitionWorker(jobStore, writer, fixedClock());
      ...
      const writtenFile = await readFile(join(result.job.localWorkspacePath ?? '', 'src', 'App.tsx'), 'utf8');
      const deletedPayload = await reloadedPayloadStore.getArchivePayload(scope, jobId);
      const crossScopePayload = await payloadStore.getArchivePayload({ ...scope, tenantId: 'tenant-2' }, jobId);
  ...
  it('marks stored archive jobs failed when decoded payload size does not match metadata', async () => {
    ...
    try {
      ...
      await payloadStore.putArchivePayload({
        ...
        fileName: 'source.tar',
        size: 999,
        contentBase64: Buffer.from(makeTar([
          { path: 'src/App.tsx', bytes: new TextEncoder().encode('export const App = true;') },
        ])).toString('base64'),
        receivedAt: '2026-05-21T00:00:00.000Z',
      });
  ...
  it('claims the oldest pending archive job and executes it through the queue runner', async () => {
    ...
    try {
      ...
      const writer = new FileSystemStudioSourceWorkspaceWriter(join(root, 'workspaces'));
      const worker = new StudioSourceAcquisitionWorker(jobStore, writer, fixedClock());
      const runner = new StudioSourceAcquisitionQueueRunner(jobStore, worker, fixedClock());
      const jobId = 'studio-acquisition:archive:queued';
      const archiveBytes = makeTar([
        { path: 'src/Queued.tsx', bytes: new TextEncoder().encode('export const Queued = true;') },
      ]);
      ...
      const writtenFile = await readFile(join(result?.job.localWorkspacePath ?? '', 'src', 'Queued.tsx'), 'utf8');
  ...
  it('claims queued repository jobs and executes them with the injected fetcher', async () => {
    ...
    try {
      ...
      const result = await runner.runNext({
        ...
        repositoryFetcher: {
          fetchArchive: async (request) => {
            expect(request.repositoryUrl).toBe('https://github.com/samujjwal/ghatana');
  ...
  it('reclaims expired running jobs with bounded retry attempts', async () => {
    ...
    try {
      ...
      const archiveBytes = makeTar([
        { path: 'src/Recovered.tsx', bytes: new TextEncoder().encode('export const Recovered = true;') },
      ]);
  ...
  it('fails expired running jobs that exceed retry attempts and continues to the next claimable job', async () => {
    ...
    try {
      ...
      const archiveBytes = makeTar([
        { path: 'src/Next.tsx', bytes: new TextEncoder().encode('export const Next = true;') },
      ]);
      ...
      expect(exhaustedJob).toMatchObject({
        status: 'failed',
        errorMessage: expect.stringContaining('exceeded maximum retry attempts'),
      });
      expect(result?.job).toMatchObject({
        jobId: nextJobId,
        status: 'complete',
  ...
  it('reports queue health snapshots for production monitoring', async () => {
    ...
    try {
      ...
      const snapshot = await jobStore.getQueueSnapshot('2026-05-21T00:10:00.000Z');
  ...
  it('rejects workspace writes that would escape the scoped job directory', async () => {
    ...
    try {
      ...
      await expect(writer.writeFiles({
        scope,
        jobId: 'job-escape',
        files: [
          {
  ...
  it('encodes hostile scope and job path segments before writing files', async () => {
    ...
    try {
      ...
      const writtenFile = await readFile(join(result.localWorkspacePath, 'src', 'App.tsx'), 'utf8');
  ...
  it('enforces lifecycle transitions in the file-backed job store', async () => {
    ...
    try {
      ...
      await expect(jobStore.updateJob(scope, 'job-terminal', {
        status: 'running',
        startedAt: '2026-05-21T00:01:00.000Z',
      })).rejects.toThrow(/Cannot transition Studio source acquisition job from complete to running/);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
...
class TestJobStore implements StudioSourceAcquisitionJobStore {
  ...
  async getJob(_scope: StudioWorkflowStoreScope, jobId: string): Promise<StudioSourceAcquisitionJob | null> {
...
function createJob(jobId: string, kind: 'github' | 'gitlab' | 'archive'): StudioSourceAcquisitionJob {
  return {
    jobId,
    status: 'pending',
    descriptor: {
      kind,
      uri: kind === 'archive' ? 'archive://source.tar' : 'https://github.com/samujjwal/ghatana',
      label: kind === 'archive' ? 'source.tar' : 'https://github.com/samujjwal/ghatana',
    },
    createdAt: '2026-05-21T00:00:00.000Z',
    scope,
  };
...
function makeTar(entries: readonly { readonly path: string; readonly bytes: Uint8Array }[]): Uint8Array {
