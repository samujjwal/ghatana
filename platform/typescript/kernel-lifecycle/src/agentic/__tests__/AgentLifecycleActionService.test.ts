...
const request: AgentLifecycleActionRequest = {
  ...
  requiredVerification: [
    { verificationId: "verify-health", kind: "health", required: true },
  ],
...
function lifecycleResult(
  status: ProductLifecycleResult["status"] = "succeeded",
): ProductLifecycleResult {
  return {
    productId: "digital-marketing",
    phase: "deploy",
...
function createService(
  overrides: Partial<
    ConstructorParameters<typeof AgentLifecycleActionService>[0]
  > = {},
) {
  const planner = {
...
describe("AgentLifecycleActionService", () => {
  ...
  it("executes after approval and writes provenance through provider context", async () => {
    const recordProvenance = vi.fn(async () => ({
      success: true,
      ref: "provenance/agent.json",
    }));
    const recordRuntimeTruth = vi.fn(async () => ({
      success: true,
      ref: "runtime-truth/agent.json",
    }));
    const recordMemory = vi.fn(async () => ({
      success: true,
      ref: "memory/agent.json",
    }));
    ...
    expect(result.evidenceRefs).toContain(
      ".kernel/out/run-1/lifecycle-result.json",
    );
    ...
    expect(recordProvenance).toHaveBeenCalledWith(
      expect.objectContaining({
        provenanceId: "agent-lifecycle:agent-request-1",
      }),
  ...
  it("fails contract validation for raw command requests", async () => {
    ...
    await expect(
      service.handle({ ...request, proposedPlanRef: "pnpm deploy" }),
    ).rejects.toThrow();
    expect(planner.plan).not.toHaveBeenCalled();
  });
