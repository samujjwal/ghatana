...
describe('ExecutionResultCollector', () => {
  it('preserves lifecycle truth metadata in collected results', () => {
    ...
    const result = collector.collect('digital-marketing', 'build', '/tmp/out', 'run-1', {
  ...
  it('collects artifact and gate truth while preserving current state access', () => {
    ...
    expect(logger.debug).toHaveBeenCalledWith('Added gate result for typecheck', {
      status: 'passed',
    });
  });
  ...
  it('resets collected execution state', () => {
    ...
    expect(logger.debug).toHaveBeenCalledWith('Reset execution result collector');
  });
  ...
  it('omits failure causes when failed steps do not expose stderr', () => {
    ...
    expect(collector.collect('digital-marketing', 'build', '/tmp/out', 'run-1').failure).toEqual({
      reasonCode: 'adapter-failed',
      stepId: 'build-web',
      message: 'Step build-web failed',
    });
  ...
  it('marks runs with only skipped steps as skipped', () => {
    ...
    expect(collector.collect('digital-marketing', 'build', '/tmp/out', 'run-1').status).toBe('skipped');
  });
});
