 ...
 * @doc.pattern Test
 */
@DisplayName("Real AI Model Call Integration Tests [GH-90000]")
...
class RealModelCallIntegrationTest extends EventloopTestBase {
    ...
    static class SimulatedModelAdapter {
        ...
        ModelResponse call(ModelRequest request) { // GH-90000
            ...
            if (simulateRateLimit && callCount == 1) { // GH-90000
                throw new RuntimeException("rate_limit_exceeded: Too many requests [GH-90000]");
            }
            if (simulateTimeout) { // GH-90000
                throw new RuntimeException("timeout: Request timed out after 30s [GH-90000]");
            }
            if (simulateFailure) { // GH-90000
                throw new RuntimeException("api_error: Internal server error [GH-90000]");
            }
    ...
    @Nested
    @DisplayName("model response parsing [GH-90000]")
    class ModelResponseParsing {

        @Test
        @DisplayName("valid response includes non-empty content and token counts [GH-90000]")
        void validResponse_includesNonEmptyContentAndTokenCounts() { // GH-90000
            ...
            assertThat(response.finishReason()).isEqualTo("stop [GH-90000]");
        }

        @Test
        @DisplayName("response finish_reason=stop indicates normal completion [GH-90000]")
        ...
        @Test
        @DisplayName("response finish_reason=length indicates truncation [GH-90000]")
        void response_finishReasonLength_indicatesTruncation() { // GH-90000
            ...
            assertThat(response.finishReason()).isEqualTo("length [GH-90000]");
    ...
    @Nested
    @DisplayName("model error handling [GH-90000]")
    class ModelErrorHandling {

        @Test
        @DisplayName("API error surfaces as categorized exception [GH-90000]")
        void apiError_surfacesAsCategorizedExceptionWithMessage() { // GH-90000
            ...
            assertThat(thrown.getMessage()).contains("api_error [GH-90000]");
        }

        @Test
        @DisplayName("timeout error surfaces with descriptive message [GH-90000]")
        void timeoutError_surfacesWithDescriptiveMessage() { // GH-90000
            ...
            assertThat(thrown).isNotNull(); // GH-90000
            assertThat(thrown.getMessage()).contains("timeout [GH-90000]");
        }
    }
    ...
    class RetryLogic {
        ...
        @Test
        @DisplayName("rate limit triggers retry and succeeds on second attempt [GH-90000]")
        ...
        @Test
        @DisplayName("non-retryable error does not retry [GH-90000]")
        void nonRetryableError_doesNotRetry() { // GH-90000
            ...
            try {
                ...
                throw new RuntimeException("invalid_request: Malformed input [GH-90000]"); // non-retryable
            } catch (RuntimeException e) { // GH-90000
        ...
        @Test
        @DisplayName("max retries exhausted surfaces final error to caller [GH-90000]")
        void maxRetriesExhausted_surfacesFinalErrorToCaller() { // GH-90000
            ...
            assertThat(finalError.getMessage()).contains("rate_limit_exceeded [GH-90000]");
        }
    }
    ...
    class RateLimiting {
        ...
        @Test
        @DisplayName("rate limit error contains retry-after information [GH-90000]")
        void rateLimitError_containsRetryAfterInformation() { // GH-90000
            ...
            assertThat(rateLimitMessage).contains("rate_limit_exceeded [GH-90000]");
            assertThat(rateLimitMessage).contains("retry after [GH-90000]");
        }

        @Test
        @DisplayName("request within rate limit succeeds without error [GH-90000]")
