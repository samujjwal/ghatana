 ...
 * @doc.pattern Test
 */
@DisplayName("Identity Lifecycle Tests [GH-90000]")
...
class IdentityLifecycleTest extends EventloopTestBase {
    ...
    @Nested
    @DisplayName("agent identity registration [GH-90000]")
    class AgentIdentityRegistration {

        @Test
        @DisplayName("registered identity is resolvable by tenant and agent ID [GH-90000]")
        void registeredIdentity_isResolvableByTenantAndAgentId() { // GH-90000
            ...
            assertThat(result.get().agentId()).isEqualTo("agent-001 [GH-90000]");
            assertThat(result.get().tenantId()).isEqualTo("tenant-a [GH-90000]");
        }

        @Test
        @DisplayName("unregistered agent ID returns empty Optional [GH-90000]")
        ...
        @Test
        @DisplayName("agent registered in one tenant is not visible in another tenant [GH-90000]")
        void agentInOneTenant_notVisibleInAnotherTenant() { // GH-90000
            AgentIdentity identity = new AgentIdentity( // GH-90000
                    "tenant-a", "agent-001", "code-agent", Set.of("read [GH-90000]"), Instant.now());
            resolver.register(identity); // GH-90000
    ...
    class AgentDeregistration {
        ...
        @Test
        @DisplayName("deregistered agent is no longer resolvable [GH-90000]")
        void deregisteredAgent_isNoLongerResolvable() { // GH-90000
            AgentIdentity identity = new AgentIdentity( // GH-90000
                    "tenant-a", "agent-002", "temp-agent", Set.of("read [GH-90000]"), Instant.now());
            resolver.register(identity); // GH-90000
        ...
        @Test
        @DisplayName("deregistering non-existent agent is a no-op [GH-90000]")
    ...
    @Nested
    @DisplayName("credential issuance [GH-90000]")
    class CredentialIssuance {

        @Test
        @DisplayName("credential issued for valid agent has all required fields [GH-90000]")
        void credentialIssued_forValidAgent_hasAllRequiredFields() { // GH-90000
            AgentIdentity identity = new AgentIdentity( // GH-90000
                    "tenant-a", "agent-003", "data-agent", Set.of("read [GH-90000]"), Instant.now());
            resolver.register(identity); // GH-90000
            ...
            assertThat(token.agentId()).isEqualTo("agent-003 [GH-90000]");
        ...
        @Test
        @DisplayName("freshly issued token is not expired [GH-90000]")
        void freshlyIssuedToken_isNotExpired() { // GH-90000
            AgentIdentity identity = new AgentIdentity( // GH-90000
                    "tenant-a", "agent-004", "fresh-agent", Set.of("read [GH-90000]"), Instant.now());
            resolver.register(identity); // GH-90000
        ...
        @Test
        @DisplayName("TTL beyond max is capped at 1 hour [GH-90000]")
        void ttlBeyondMax_isCappedAtOneHour() { // GH-90000
            AgentIdentity identity = new AgentIdentity( // GH-90000
                    "tenant-a", "agent-005", "capped-agent", Set.of("read [GH-90000]"), Instant.now());
            resolver.register(identity); // GH-90000
