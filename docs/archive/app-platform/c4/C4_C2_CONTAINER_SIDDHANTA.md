...
> This C4 Level 2 Container diagram describes the **Capital Markets (Siddhanta)** domain pack on AppPlatform. Platform-level architecture is in `DOMAIN_PACK_INTERFACE_SPECIFICATION.md`.
...
### Multi-Calendar Service (K-15)

A dedicated kernel container provides pluggable calendar conversion via the K-15 Multi-Calendar Service. Canonical timestamps are stored as ISO 8601 UTC. The Nepal Capital Markets T1 calendar pack adds Bikram Sambat representations via the `CalendarDate` type. The K-05 Event Bus standard envelope carries `timestamp` (UTC, required) and an optional `calendar_date` (CalendarDate) field.
