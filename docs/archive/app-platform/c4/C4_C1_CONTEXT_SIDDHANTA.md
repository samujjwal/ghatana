> **📦 DOMAIN PACK DOCUMENT — Capital Markets (Siddhanta) · Nepal Instantiation**  
> This C4 Level 1 Context diagram describes the **Capital Markets (Siddhanta)** domain pack on AppPlatform. Platform-level architecture is in `DOMAIN_PACK_INTERFACE_SPECIFICATION.md`.
...
All timestamps, settlement dates, reporting periods, and audit trails use a canonical ISO 8601 UTC `timestamp`. The K-15 Multi-Calendar Service generates a `CalendarDate` record (including Bikram Sambat and Gregorian representations for Nepal deployments) on demand via the Capital Markets T1 calendar pack. Every domain event in the K-05 Event Bus standard envelope carries the optional `calendar_date` field alongside the required `timestamp` (UTC). Other calendar systems (Hijri, Thai Solar, etc.) are extensible via additional T1 packs.
