# AppPlatform — Core Kernel Review & Implementation Plan
...
AppPlatform is designed as a **multi-domain operating system** with a clear separation between:
...
3. **Multi-Calendar at Data Layer** — Pluggable `CalendarDate` type (open map of CalendarId→CalendarDateTime) in every domain-significant timestamp, enriched by K-15 Multi-Calendar Service
...
- **Ghatana Reuse (85%):** `:platform:java:security` (Nimbus JOSE+JWT, OAuth2, RBAC, BouncyCastle, Caffeine); `:platform:java:governance` (TenantExtractionFilter, TenantGrpcInterceptor); `shared-services/auth-service` (OAuth2/OIDC — deployed); `shared-services/auth-gateway` (JWT validation proxy — deployed). **Net new:** beneficial ownership graph (GAP-002), TOTP MFA, break-glass access, refresh-token family tracking.
...
- **Purpose:** 5-level hierarchical config (GLOBAL→JURISDICTION→TENANT→USER→SESSION), maker-checker approval, hot-reload, multi-calendar effective dates (`CalendarDate`), CQRS
- **Ghatana Reuse (70%):** `:platform:java:config` (Typesafe Config HOCON/YAML, networknt schema validation); `:platform:java:database` (PostgreSQL LISTEN/NOTIFY hot-reload, Redis Jedis/Lettuce cache); `:platform:java:governance` (tenant-scoped config isolation). **Net new:** 5-level hierarchy merge logic, CalendarDate effective dates, air-gap Ed25519 signing, maker-checker workflow.
- **Key FRs:** Schema registration/validation, 5-level hierarchy with deep merge, hot-reload via PostgreSQL LISTEN/NOTIFY or Kafka, canary rollout for config changes with auto-rollback, multi-calendar effective dates (`CalendarDate`), temporal queries (as-of-date), air-gap bundle generation with Ed25519 signing, maker-checker workflow, CQRS write/read sides
...
- **Ghatana Reuse (60%):** `:platform:java:plugin` (plugin/sandbox isolation base for rule packs, event-cloud + ai-integration enabled); `:products:aep:platform` (Tier-J/S rule agent dispatch, resilience); `:platform:java:agent-resilience` (circuit breaker on evaluation path). **Net new:** OPA/Rego gRPC wrapper, T2 V8/WASM sandbox, jurisdiction routing, dry-run evaluation.
...
- **Ghatana Reuse (80%):** `:platform:java:plugin` (plugin loading, Ed25519 manifest verification, hot-swap, capability management; integrates event-cloud + ai-integration + governance natively). **Net new:** T1/T2/T3 tier enforcement rules, capability approval workflow, per-plugin resource quotas, data exfiltration prevention.
...
- **Ghatana Reuse (90%):** `:products:aep:platform` (**sole event backend** — never call `:platform:java:event-cloud` directly); `:platform:java:connectors` (KafkaConnector exactly-once, PostgreSQL outbox SKIP LOCKED relay); `:platform:java:schema-registry` (Avro/Protobuf schema compatibility enforcement via `:products:data-cloud:spi`); `:products:data-cloud:spi` (EventLogStore append interface). **Net new:** finance event envelope (`AiAnnotation`, `CalendarDate`), saga policy definitions, 100K TPS tuning, consumer backpressure config.
...
- **Ghatana Reuse (~100%):** `:platform:java:observability` (Micrometer, Prometheus, OpenTelemetry, AspectJ); `:platform:java:observability-http` (Prometheus /metrics + health endpoints, split per ADR-007). **Net new:** SLO/SLA error budget tracking framework, PII masking rules, ML anomaly detection (via `:platform:java:ai-integration` gateway).
...
- **Ghatana Reuse (90%):** `:platform:java:audit` (AuditEvent abstractions, event-cloud-backed persistence, JPA for audit record storage); `:products:data-cloud:platform` (durable EventLogStore-backed write backend for audit records). **Net new:** SHA-256 hash chain computation (genesis=`"0"×64`), Merkle root anchoring (every 1,000 events / 60 min), blockchain/TSA external anchor, 7yr/5yr retention policy enforcement.
...
- **Ghatana Reuse (95%):** `:products:data-cloud:platform` (data catalog, lineage tracking, classification, quality rules, residency enforcement, right-to-erasure, PII masking; backed by `:platform:java:audit`, `:platform:java:database`, `:platform:java:security`). **Net new:** finance-specific classification taxonomy (MNRE, SEBON categories), automated regulatory breach response workflow.
...
- **Ghatana Reuse (80%):** `:platform:java:ai-integration` (gateway, registry, feature-store, evaluation, observability, training, promotion sub-modules); `:platform:java:agent-framework` (HITL workflow agent patterns, LangChain4j); `:platform:java:agent-learning` (evaluation gates, consolidation, retention); `shared-services/ai-registry` (model registry — deployed); `shared-services/ai-inference-service` (inference gateway — deployed); `shared-services/feature-store-ingest` (feature ingestion — deployed). **Net new:** SHAP/LIME explainability wrappers, drift detector (60s auto-rollback), bias monitoring per demographic slice, ONNX-only T3 enforcement.
...
- **Ghatana Reuse (50%):** `:platform:java:runtime` (ActiveJ service lifecycle management). Infra layer: Kubernetes HPA with custom K-05 consumer lag metrics from Prometheus. **Net new:** deployment orchestrator service, canary strategy engine (10-min error_rate/latency gate), IaC drift scanner, environment registry, deployment audit via K-07.
...
- **Ghatana Reuse (80%):** `:platform:java:http` (ActiveJ HTTP server/client, OkHttp, Guava, Jackson middleware patterns); `shared-services/auth-gateway` (JWT validation proxy + tenant rate limiting — deployed, built on `:platform:java:http` + `:platform:java:security`); `:platform:java:governance` (tenant-aware routing). **Net new:** Envoy/Istio ingress config, OWASP Top 10 WAF rules, jurisdiction-based routing, schema validation at gateway via `:platform:java:schema-registry`.
...
- **Ghatana Reuse (100% composition):** Aggregates all `:platform:java:*` modules + `:products:aep:platform` + `:products:data-cloud:spi`. SDK wraps and re-exports capabilities; no net-new platform logic. **Net new:** OpenAPI + event schema code generation tooling, PACT contract test harness, domain pack scaffold generator, developer portal, SDK certification test suite.
...
- **Ghatana Reuse (0% Java — new Node.js stack):** Frontend: React 18 + Tailwind CSS + Jotai (app state) + TanStack Query (server state). BFF: Node.js + Fastify + Prisma (aligns with Ghatana hybrid backend rule for user API surfaces). **Net new:** entire UI shell, micro-frontend composition, plugin registry UI, maker-checker task center, K-02 config management UI.
...
- **Ghatana Reuse (60%):** `:platform:java:security` (BouncyCastle AES-256-GCM for local encrypted file provider; Nimbus JOSE+JWT for JWT signing key management; Caffeine + Jedis for secret caching); `:platform:java:database` (PostgreSQL for secret version history persistence via JPA/Flyway). **Net new:** HashiCorp Vault KV v2 adapter, PKCS#11 HSM integration, auto-rotation scheduler, break-glass forensic workflow (K-07 audit).
...
- **Purpose:** Multi-calendar (pluggable `CalendarDate`) support at the data layer; BS↔Gregorian is the first implementation via T1 Config Pack
- **Ghatana Reuse (0% — net contributor):** This module contributes back to the platform: `CalendarDate` open-map type definition is contributed to `:platform:java:domain`; holiday calendar persistence uses `:platform:java:database` (JPA/Flyway). **Net new:** entire JDN-based BS↔Gregorian math library, T+n settlement date calculator, storage enrichment middleware, comprehensive 36,500-day accuracy test suite.
- **Key FRs:** JDN-based BS↔Gregorian conversion library (2000-2100 BS range), REST API, `CalendarDate` type (open map of `CalendarId→CalendarDateTime`), storage enrichment middleware, holiday calendar CRUD, business day calculation, fiscal year boundary calculation, fiscal quarter/period, T+n settlement date calculation, configurable weekend/holiday skip logic, edge case handling (32-day months, Chaitra→Baisakh, year boundary), comprehensive accuracy test suite (36,500 days verified)
- **Dependencies:** K-02, K-05, K-07
...
- **Ghatana Reuse (40%):** `:platform:java:database` (JPA/Hibernate for DECIMAL(28,12) schema, Flyway migrations, HikariCP connection management, PostgreSQL); `:platform:java:audit` (hash chain integrity pattern reference for per-account tamper detection); `:platform:java:governance` (tenant-isolated account namespacing). **Net new:** entire double-entry accounting engine, Money value object, multi-currency reconciliation engine, chart-of-accounts maker-checker.
...
- **Ghatana Reuse (85%):** `:platform:java:connectors` (KafkaConnector for exactly-once Kafka publish; PostgreSQL SKIP LOCKED outbox table relay); `:products:aep:platform` (saga coordination engine — uses AEP saga orchestration patterns); `:platform:java:database` (idempotency store table + outbox table via JPA/Flyway). **Net new:** finance-specific saga policies, version vector causal ordering data structure, DTC saga monitoring REST API.
...
- **Ghatana Reuse (90%):** `:platform:java:agent-resilience` (circuit breaker state machine, retry with jitter, bulkhead, health monitoring decorators for TypedAgent); `:platform:java:config` (K-02 hot-reload for runtime profile switching). **Net new:** STRICT/STANDARD/RELAXED named profile definitions, TRADING_CRITICAL composite profile, `X-Request-Deadline` cascading timeout header, semaphore-based bulkhead for async Node.js.
...
- **Ghatana Reuse (90%):** `:products:aep:platform` (dead-letter handling infrastructure + replay engine built into AEP); `:platform:java:ai-integration` (ML failure pattern classifier sub-module); `:platform:java:connectors` (Kafka DLQ topic management). **Net new:** finance-specific DLQ routing policy, payload inspector + transformation UI, DLQ SLA dashboard, poison pill detection algorithm.
...
| K05-004 | Add multi-calendar timestamp enrichment (CalendarDate via K-15) | 2 | 1 |
...
| K02-010 | Implement multi-calendar effective date support (`CalendarDate`) | 3 | 2 |
...
| K15-004 | Implement `CalendarDate` type and generation utility | 2 | 1 |
...
| K-15 Multi-Calendar | K-15 (13 stories) | 37 | Conversion, holidays, business days, settlement, fiscal year, batch |
...
| INT (Integration) | 8 | K-05 events, K-07 audit, K-15 multi-calendar |

### 5.4 K-05 Event Bus (68 TCs)

| Group | TCs | Coverage |
|-------|-----|---------|
| EE (Event Envelope) | 10 | Validation, multi-calendar enrichment, causality/correlation IDs |
...
| Group | TCs | Coverage |
|-------|-----|---------|
| AE (Audit Event Mgmt) | 10 | Schema validation, multi-calendar timestamps, PII detection/redaction |
...
### 5.6 K-15 Multi-Calendar (37 TCs)

| Group | TCs | Coverage |
|-------|-----|---------|
| DC (Date Conversion) | 9 | BS→Gregorian, Gregorian→BS, round-trip, bounds, leap year |
| CD (CalendarDate Generation) | 5 | From timestamp, formatting, component extraction |
...
| K-13 Portal | 4 | Auth, RBAC, multi-calendar UI, maker-checker |
...
| FR10 | Multi-Calendar Edge Cases | K-15 | 500+ assertions (2000-2100 BS) |
...
| K-15 Multi-Calendar | K-02, K-05 | 12+ modules |
...
|--------|---------|-----|-------|
| K-05 Event Bus | K05-001 to K05-004 | 15 | Event store schema (append-only), appendEvent API, getEvents API, multi-calendar enrichment |
...
| K-15 Calendar | K15-001 to K15-007 | 17 | JDN conversion library, BS month lookup, REST API, `CalendarDate` type, storage middleware, holiday CRUD, business day calc |
...
| K-02 Config | K02-007 to K02-017 | 26 | Hot-reload, canary, rollback, multi-calendar dates (`CalendarDate`), temporal queries, air-gap bundles, maker-checker, CQRS |
...
2. **Multi-Calendar Consistency** (K-15): Every domain-significant timestamp uses `CalendarDate` — an open map of `CalendarId→CalendarDateTime`; UTC is always the canonical field
