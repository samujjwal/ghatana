...
Create logger: `observability.logger.info/warn/error/debug(message, data)`. JSON structured output. Auto-enriched with: timestamp, calendar_date (CalendarDate, optional, via K-15 T1 calendar pack), correlation_id, trace_id, service, level, pid. Log level configurable per service via K-02.
