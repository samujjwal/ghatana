...
- `audit_log` table: id, timestamp_utc, calendar_date JSONB, actor_id, actor_type, action, resource_type, resource_id, old_value JSONB, new_value JSONB, metadata JSONB, previous_hash VARCHAR(64), entry_hash VARCHAR(64)
...
- Automatic enrichment: actor_id from request context (JWT), correlation_id, calendar_date (CalendarDate, optional, via K-15), timestamp_utc
...
- `CalendarDate` type: `{ primary: string (ISO8601 UTC), timezone: string (IANA), calendars: Record<CalendarId, CalendarDateTime> }` — open string CalendarId supports any registered calendar
- `toCalendarDate(date: Date, packCalendars): CalendarDate` — generates all registered calendar representations
