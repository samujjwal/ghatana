...
8. **FR8 Multi-Calendar Timestamps:** All incident timestamps, SLA deadlines, and report dates use `CalendarDate` via K-15.
...
- **New Entities:**
  - `Incident`: `{ incident_id: UUID, type: Enum, severity: Enum(P0, P1, P2), status: Enum, summary: String, affected_systems: List<String>, detected_at: CalendarDate, resolved_at: CalendarDate }`
  - `IncidentNotification`: `{ notification_id: UUID, incident_id: UUID, recipient: String, channel: Enum, status: Enum(SENT, DELIVERED, ACKNOWLEDGED, ESCALATED), sent_at: CalendarDate, acknowledged_at: CalendarDate }`
  - `EscalationRecord`: `{ escalation_id: UUID, incident_id: UUID, from_level: Int, to_level: Int, reason: String, escalated_at: CalendarDate }`
...
- **Multi-Calendar Fields:** `detected_at`, `resolved_at`, `sent_at`, `acknowledged_at`, `escalated_at` use `CalendarDate` (K-15 enriched).
