...
- **Multi-Calendar Fields:** `calculated_at` in `InstrumentPrice` uses `CalendarDate`.
...
| Payload           | `{ "instrument_id": "...", "price": 245.5, "type": "EOD", "timestamp": "2025-03-02T10:30:00Z", "calendar_date": {} }` |
...
- **AI Hook Type:** Predictive Model / Anomaly Detection / Ensemble Fusion
- **Primary Use Case:** (a) AI-estimated fair value for illiquid and OTC instruments where deterministic model cannot converge; (b) Confidence-interval pricing — every AI-sourced price carries a distributional estimate (mean ± σ), not just a point estimate; (c) Real-time price anomaly detection to flag prices that are statistical outliers against peer instruments and recent history (AI prices feed D-08 Surveillance); (d) Ensemble blending for liquid instruments: AI-derived microstructure signal weighted against the deterministic model by an instrument-specific liquidity score (high liquidity → low AI weight; low liquidity → high AI weight).
- **Inference Mode:** Synchronous for (a)/(b)/(d) — called on the pricing query path (budget: 3ms P99 AI component); Asynchronous for (c) — anomaly annotation on emitted `PriceUpdated` events.
- **Workflow Steps Exposed:** (a) Pricing query for illiquid instruments; (b) EOD price validation pass; (c) Real-time quote tick enrichment; (d) Ensemble blend computation.
- **Model Registry Usage:** `illiquid-pricer-v1` (comparables-based fair value via K-09 Embedding Service for instrument similarity search), `price-confidence-estimator-v1` (quantile regression → P10, P50, P90 price estimates), `price-anomaly-detector-v1` (Z-score + ML cross-sectional outlier detection), `ensemble-blender-v1` (liquidity-weighted combining of AI and deterministic prices)
- **Input Data / Feature Set:** Instrument attributes (sector, credit rating, maturity, coupon, issuer); recent comparable trades (K-09 Feature Store); order book depth; macro indicators; instrument-specific liquidity score.
- **Output / AI Annotation:** `{ fair_value_mid, price_p10, price_p50, price_p90, liquidity_score, ai_weight, comparable_instruments: [{ isin, weight, last_price }] }`. `PriceUpdated` event carries `ai_annotation` if anomaly detected: `{ anomaly_score, z_score, peer_deviation_pct }`.
- **Explainability Requirement:** Illiquid pricing: AI must output the top-5 comparable instruments and their weights. Anomaly detection: must state the Z-score and identify which dimension (price, volume, spread) drove the anomaly. Ensemble blend: must show the liquidity score and resulting AI weight.
- **Confidence Threshold:** Illiquid pricer confidence < 0.70 → mark price as `ESTIMATED_LOW_CONFIDENCE`; must not be used for EOD NAV without manual validation. Anomaly score > 3σ → emit `SuspiciousPriceAnomaly` event to D-08 Surveillance.
- **Human Override Path:** Trader or pricing analyst can override any AI-estimated price (override reason required, audited in K-07). Overridden prices are excluded from AI training data for that day.
- **Feedback Loop:** Eventual actual trade prices for instruments that had AI-estimated prices are automatically ingested as ground truth and fed to `illiquid-pricer-v1` fine-tuning pipeline.
- **Drift Monitoring:** Mean absolute error (MAE) of AI prices vs. eventual trade prices tracked weekly per instrument class. If MAE exceeds 1.5% for any class, model recalibration triggered.
- **Fallback Behavior:** Last-known traded price. For EOD: system flags instrument for manual pricing. Ensemble blend defaults to 100% deterministic model.
- **Tenant Isolation:** Pricing computations are shared-model but tenant-isolated feature extraction. Comparable instrument universe is configurable per tenant T2 pack (e.g., Nepal-only tenant restricts comparables to NEPSE-listed instruments).
