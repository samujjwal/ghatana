...
6. **FR6 Multi-Calendar Dates:** Record dates, ex-dates, and payment dates must use `CalendarDate` (K-15) for jurisdiction-native date display.
...
- **New Entities:**
  - `CorporateAction`: `{ ca_id: UUID, issuer_id: UUID, type: Enum, record_date: CalendarDate, ex_date: CalendarDate, details: JSON }`
  - `Entitlement`: `{ entitlement_id: UUID, ca_id: UUID, account_id: UUID, gross_amount: Decimal, tax_withheld: Decimal, net_amount: Decimal }`
- **Multi-Calendar Fields:** `record_date`, `ex_date`, `payment_date` use `CalendarDate` (K-15 enriched).
...
- **AI Hook Type:** Copilot Assist / Classifier / Verifier
- **Primary Use Case:** (a) AI classifies ambiguous corporate action announcements into structured event types (bonus issue, rights, dividend, merger, etc.) when announcement text is non-standard; (b) AI verifies entitlement calculations by cross-checking the computed entitlement formula against the announcement terms and the K-16 ledger balance for each account; (c) AI assists in reconciling discrepancies between D-12 calculated entitlements and external depository records (CDSC/NSDL) by suggesting likely root causes and resolution actions; (d) AI validates that ex-date and record-date calculations are correct across BS and AD calendars for jurisdiction-specific instruments.
- **Inference Mode:** (a) Synchronous on CA announcement ingestion (budget: 2 seconds); (b) Asynchronous post-calculation, before entitlement letter generation; (c) On-demand when reconciliation breaks are detected; (d) Synchronous at CA event creation.
- **Workflow Steps Exposed:** (a) Announcement ingestion → AI type classification suggestion; (b) Entitlement batch complete → AI verification pass; (c) Reconciliation break queue → AI root cause copilot; (d) CA event creation → AI calendar date validator.
- **Model Registry Usage:** `ca-classifier-v1` (LLM: regulatory announcement text → structured CA type + parameters), `ca-entitlement-verifier-v1` (rule-guided LLM: verify calculation correctness vs. announcement terms), `ca-reconciliation-copilot-v1` (retrieval-augmented reasoning: suggest break root cause from historical reconciliation patterns), `ca-calendar-verifier-v1` (deterministic + AI: validate ex-date/record-date across BS/AD with calendar library)
- **Input Data / Feature Set:** CA announcement text (from exchange/depository feeds); CA parameters (ratio, ex-date, record-date); account positions on record date; calculated entitlement amounts; depository reconciliation feed (CDSC/NSDL); CalendarDate values for ex/record dates.
- **Output / AI Annotation:** (a) Structured CA event: `{ ca_type, ratio, ex_date_bs, ex_date_ad, record_date_bs, record_date_ad, confidence }`. (b) Verification result: `{ verified: Boolean, discrepancies: [String], confidence }`. (c) Break root cause: `{ category: DATA_MISMATCH|TIMING|ENTITLEMENT_FORMULA|OTHER, confidence, suggested_resolution }`. (d) Calendar validation: `{ valid: Boolean, computed_ex_date_bs, computed_ex_date_ad, discrepancy_flag }`.
- **Explainability Requirement:** CA classifier: must quote the specific announcement sentence(s) that justify each extracted parameter. Entitlement verifier: must show the step-by-step calculation it performed and where it diverges. Reconciliation copilot: must cite historical break cases with similar patterns.
- **Confidence Threshold:** CA classification confidence < 0.80 → flag as NEEDS_REVIEW before proceeding with entitlement calculation; halt auto-processing. Entitlement verification discrepancy found → halt entitlement letter generation pending human review. Calendar discrepancy detected → always hard-stop; never auto-proceed.
- **Human Override Path:** CA Operations team reviews and approves all AI suggestions. No CA event is published without explicit human sign-off at each stage.
- **Feedback Loop:** Operator corrections to CA classifications contribute fine-tuning data to `ca-classifier-v1`. Approved break resolutions feed `ca-reconciliation-copilot-v1` retrieval knowledge base.
- **Drift Monitoring:** Monthly: CA classification accuracy vs. operator corrections; entitlement verification false alarm rate (verified correct but operator modified); reconciliation copilot resolution acceptance rate.
- **Fallback Behavior:** Standard manual announcement keying, manual entitlement calculation verification, and manual reconciliation matching. Calendar dates computed deterministically via registered calendar library without AI verification layer.
