...
10. **FR10 Multi-Calendar Timestamping:** Secret expiry dates and rotation schedules support all active calendar systems via K-15 `CalendarDate` for operational planning.
...
- **New Entities:**
  - `SecretMetadata`: `{ secret_id: String, path: String, version: Int, created_at: CalendarDate, expires_at: CalendarDate, rotation_policy: String, owner_service: String }`
  - `SecretAccessLog`: `{ access_id: UUID, secret_id: String, accessor: String, timestamp: CalendarDate, operation: Enum }`
  - `CertificateMetadata`: `{ cert_id: String, subject_dn: String, issuer: String, valid_from: CalendarDate, valid_until: CalendarDate, status: Enum }`
- **Multi-Calendar Fields:** `created_at`, `expires_at`, `valid_from`, `valid_until` use `CalendarDate` (from K-15; calendar systems determined by active T1 calendar config). All store the canonical UTC `primary` timestamp; additional calendar representations are populated on demand.
...
| Payload           | `{ "secret_id": "...", "old_version": 5, "new_version": 6, "rotation_type": "AUTOMATIC", "timestamp": "2025-03-02T10:30:00Z", "calendar_date": {} }` |
