...
7. **FR7 Multi-Calendar Timestamps:** All transaction log entries must include `CalendarDate` timestamps via K-15 when active T1 calendar config is present.
...
- **New Entities:**
  - `Outbox`: `{ outbox_id: UUID, aggregate_type: String, aggregate_id: String, event_type: String, payload: JSON, created_at: CalendarDate, published: Boolean, published_at: Timestamp }`
  - `SagaInstance`: `{ saga_id: UUID, definition_id: String, version: String, status: Enum(RUNNING, COMPLETED, COMPENSATING, STUCK, TIMED_OUT), steps: List<SagaStep>, started_at: CalendarDate, completed_at: CalendarDate }`
  ...
  - `TransactionLog`: `{ log_id: UUID, saga_id: UUID, step_id: UUID, action: String, timestamp: CalendarDate, details: JSON }`
- **Multi-Calendar Fields:** `started_at`, `completed_at` in `SagaInstance`; `timestamp` in `TransactionLog` use `CalendarDate` (K-15 enriched).
