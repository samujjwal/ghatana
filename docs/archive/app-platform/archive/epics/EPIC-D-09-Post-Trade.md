...
6. **FR6 Multi-Calendar Settlement Dates:** Settlement dates are tracked using a UTC `settlement_date` plus an optional `settlement_date_calendar` (CalendarDate from K-15) for jurisdiction-native display.
...
- **New Entities:**
  - `SettlementObligation`: `{ obligation_id: UUID, account_id: UUID, asset_id: String, net_qty: Decimal, settlement_date: Date, settlement_date_calendar: CalendarDate | null, status: Enum }`
- **Multi-Calendar Fields:** `settlement_date_calendar` (CalendarDate) in `SettlementObligation` for jurisdiction-native settlement date.
...
- **AI Hook Type:** Predictive Model / Risk Scoring / Process Optimization
- **Primary Use Case:** (a) Settlement failure prediction by recognizing EDIS authorization behavioral patterns (who typically authorizes late, which brokers have high fail rates, which instruments cluster around corporate actions); (b) Counterparty credit risk scoring for intraday exposure management — AI estimates default probability based on net settlement obligation and counterparty financial health signals; (c) Netting optimization — AI suggests bilateral netting arrangements that minimize total settlement obligations; (d) Recall and buy-in timing optimization — AI recommends the optimal time to initiate a buy-in based on current market liquidity.
- **Inference Mode:** (a) Near-real-time after trade confirmation (< 2 min post-trade); (b) Updated every 30 minutes during settlement session; (c) Nightly batch (T-1 for next-day settlements); (d) On-demand when settlement fails.
- **Workflow Steps Exposed:** (a) Trade matched → AI immediately annotates with fail-risk score; (b) Counterparty exposure dashboard updated continuously; (c) Netting recommendation presented to operations team before cut-off; (d) Buy-in scheduling suggestion shown to operations desk.
- **Model Registry Usage:** `settlement-fail-predictor-v1` (gradient boosted: fail risk per trade based on EDIS pattern + counterparty history), `counterparty-exposure-scorer-v1` (neural network: real-time counterparty default probability), `netting-optimizer-v1` (combinatorial optimization with ML-guided initial solutions), `buy-in-timing-advisor-v1` (reinforcement learning: optimal buy-in initiation time vs. market liquidity cost)
- **Input Data / Feature Set:** Trade confirmation attributes; EDIS authorization history per broker/account; counterparty net position; counterparty credit CDS spreads (external data via K-08 connector); instrument liquidity (from D-05); corporate action calendar.
- **Output / AI Annotation:** `TradeConfirmed` event enriched with `ai_annotation: { fail_risk_score, fail_risk_category: LOW|MEDIUM|HIGH, top_risk_factors }`. Netting recommendation: `{ proposed_net_pairs, obligation_reduction_pct, confidence }`. Buy-in advice: `{ optimal_window_start, optimal_window_end, expected_market_impact_bps }`.
- **Explainability Requirement:** Fail predictor: must list the top-3 risk factors (e.g., "Broker X has 25% fail rate on T+2 for equity settlements", "EDIS authorization pending since T+0.5 hours"). Counterparty scorer: must show which financial signals drove the score. Buy-in advisor: must explain liquidity conditions justifying the proposed window.
- **Confidence Threshold:** Fail risk HIGH (probability ≥ 0.70) → proactive early intervention alert to operations desk at T+30min. Fail risk MEDIUM (0.40–0.69) → flag in monitoring list, no immediate action. Counterparty score ≥ 0.65 (elevated default probability) → suspend further matching with that counterparty pending human review.
- **Human Override Path:** Operations supervisor can override AI fail risk classification. Counterparty suspension requires Ops Head approval. All overrides audited in K-07.
- **Feedback Loop:** Actual settlement outcomes (failed/cleared) vs. predictions form the primary training signal for `settlement-fail-predictor-v1`. Actual buy-in costs vs. expected costs feed `buy-in-timing-advisor-v1`.
- **Drift Monitoring:** Weekly: fail predictor precision/recall; counterparty scorer Brier score; netting optimizer realized savings vs. projected savings.
- **Fallback Behavior:** Standard reactive settlement failure handling. Counterparty exposure via static limit matrix. Netting via standard bilateral agreement without optimization.
- **Tenant Isolation:** Settlement and counterparty data are strictly tenant-isolated. Netting suggestions only cross counterparty boundaries within the same CCP participant group (as configured in T2 pack).
