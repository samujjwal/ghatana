...
6. **FR6 Multi-Calendar Tracking:** Manifest activation times must be recorded using `CalendarDate` (from K-15).
...
- **New Entities:**
  - `PlatformManifest`: `{ manifest_version: Int, active_components: List<ComponentRef>, activated_at: CalendarDate, signature: String }`
  - `ComponentRef`: `{ component_id: String, type: Enum, version: String, hash: String }`
- **Multi-Calendar Fields:** `activated_at` uses `CalendarDate`.
...
| Multi-Calendar Correctness | Correct `CalendarDate` activation logging                        |
