...
6. **FR6 Multi-Calendar Timestamps:** All execution timestamps must use `CalendarDate` (from K-15). The kernel envelope carries `timestamp` (ISO 8601 UTC mandatory); the optional `calendar_date` field is enriched by K-15 when the active domain pack's T1 calendar config declares additional calendar systems (e.g., BS for Nepal, Hijri for Gulf markets).
...
- **New Entities:**
  ...
  - `Fill`: `{ fill_id: UUID, route_id: String, price: Decimal, qty: Int, executed_at: CalendarDate, venue_ref: String }`
- **Multi-Calendar Fields:** `executed_at` uses `CalendarDate` (populated by K-15 when domain pack T1 calendar config is active).
...
| Payload           | `{ "order_id": "...", "fill_qty": 100, "fill_price": 245.5, "venue": "NEPSE", "timestamp": "2025-03-02T10:30:00Z", "calendar_date": {} }` |
