...
- **New Entities:**
  - `LedgerTransaction`: `{ transaction_id: UUID, posting_date: Timestamp, posting_date_calendar: CalendarDate | null, source_module: String, entries: List<LedgerEntry> }`
  - `LedgerEntry`: `{ account_id: String, asset_id: String, amount: Decimal, type: Enum(DEBIT, CREDIT) }`
- **Multi-Calendar Fields:** `posting_date_calendar` (CalendarDate, K-15 enriched) stores jurisdiction-native date representations of `posting_date` to support fiscal-year reporting in BS and other calendars.
...
| Payload           | `{ "transaction_id": "...", "posting_date": "2025-03-02T00:00:00Z", "posting_date_calendar": { "bs": "2081-11-18" }, "entries": [...] }` |
...
- **Events Emitted:** `LedgerPostedEvent`, `ReconciliationBreakDetectedEvent`, `LedgerImbalanceDetectedEvent`, `AccountCreated`, `ReconciliationCompleted` — all conform to K-05 standard envelope (`event_id`, `event_type`, `aggregate_id`, `aggregate_type`, `sequence_number`, `timestamp`, `calendar_date?`, `payload`)
