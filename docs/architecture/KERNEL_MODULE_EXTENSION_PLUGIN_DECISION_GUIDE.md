...
public final class HealthcareConsentKernelExtension implements KernelExtension {
    ...
    @Override public void onModuleInitialized(KernelContext context) {
        context.registerService(HealthcareConsentPolicyEvaluator.class,
                new DefaultHealthcareConsentPolicyEvaluator());
    }
    // ...
}
```
...
| `plugin-billing-ledger` | Double-entry ledger with idempotency |
...
Platform plugins (`plugin-audit-trail`, `plugin-billing-ledger`, `plugin-consent`) come in two variants:
