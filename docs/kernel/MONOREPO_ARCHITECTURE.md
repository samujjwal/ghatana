...
### 4.1 Kernel Architecture Layers

The kernel is organized into four distinct, isolated layers. Each layer may only depend on the layer directly below it. Cross-layer direct imports are forbidden.

```
┌─────────────────────────────────────────────────────────────────────┐
│  Products  (phr, finance, aep, yappc, data-cloud, …)               │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  Product Domain Packs (supplied per-product)                  │  │
│  │  - BoundaryPolicyStore  (e.g. PhrBoundaryPolicyStore)         │  │
│  │  - ComplianceRulePack   (e.g. PhrComplianceRulePack)          │  │
│  │  - DomainPackManifest   (domain-pack-manifest.yaml)           │  │
│  │  - SchemaRegistry       (schema-registry.yaml)                │  │
│  └───────────────────────────────────────────────────────────────┘  │
└────────────────────────────┬────────────────────────────────────────┘
                             │  plugin SPI bindings + bridge adapters
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│  Platform Plugins  (platform-plugins/)                             │
│  - plugin-audit-trail      - plugin-compliance                     │
│  - plugin-consent          - plugin-fraud-detection                │
│  - plugin-human-approval   - plugin-ledger                         │
│  - plugin-risk-management  - core-observability                    │
│                                                                     │
│  Rules:                                                             │
│  ✗ No product domain terms (PHR, CLINICAL, Finance, SOX, …)        │
│  ✗ No cross-plugin implementation imports                           │
│  ✗ No product-scoped default data in manifests                      │
└────────────────────────────┬────────────────────────────────────────┘
                             │  kernel SPI (BoundaryPolicyStore, etc.)
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│  Platform Kernel  (platform-kernel/)                               │
│  - kernel-core   kernel-plugin   kernel-persistence   kernel-bom   │
│                                                                     │
│  Rules:                                                             │
│  ✗ No product imports, no product constants                         │
│  ✗ No CompletableFuture (ActiveJ Promise only)                      │
│  ✗ No plugin.impl imports from kernel packages                      │
└────────────────────────────┬────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│  Platform Libraries  (platform/java/*, platform/contracts)         │
│  - core   observability   security   database   http   testing …   │
└─────────────────────────────────────────────────────────────────────┘
```
...
| Layer boundary | Gate |
|---|---|
| Kernel ↛ products | `checkKernelPurity` Gradle task + ArchUnit `ARCH-001` |
| Kernel ↛ plugin.impl | ArchUnit `ARCH-002` |
| Plugins ↛ products | `checkPluginPurity` Gradle task + ArchUnit `ARCH-010` |
| Plugins ↛ sibling plugins | ArchUnit `ARCH-011` through `ARCH-017` |
| Products → kernel (public API only) | ArchUnit product boundary tests |
