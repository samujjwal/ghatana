...
The Kernel plugin platform enables platform and product plugins to hook into product lifecycle events. Plugins remain product-neutral and are registered in the kernel plugin registry.

## Plugin Registry

Plugins are registered in `config/kernel-plugin-registry.json`:
...
- `platform-plugin` - Platform-level plugins available to all products
- `product-plugin` - Product-specific plugins

Plugins remain product-neutral and do not import from `products/**`.
