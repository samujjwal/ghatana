...
| 13 | Partial | Shared product shell contract is in use for PHR, but FlashIt/other UI surfaces are not fully migrated to one shell yet. |
...
| 25 | Partial | Product/plugin dependency governance exists, but full manifest-driven plugin pruning is not complete for every product. |
...
| 31 | Partial | Shared `PageHeader` primitive now exists and some product wrappers compose it, but Data Cloud and other local layout primitives remain. |
...
| 44 | Open | Canonical action/resource schema enforcement for product-defined policy actions is still missing. |
| 45 | Partial | In-memory policy-store usage is documented and checked, but non-dev launcher/runtime protection is not fully integration-tested everywhere. |
...
| 48 | Open | A full Kernel product scaffolder has not been implemented yet. |
| 49 | Completed | Finance product shape is explicitly declared as backend-only. |
| 50 | Partial | FlashIt client conformance checks exist for shared shell/tokens/route contracts, but not every client surface is fully migrated. |
...
| 57 | Partial | Finance still needs deeper dependency decomposition so the root product module is not a broad aggregator. |
...
- Kernel platform expansion: `44`, `48`
- Manifest/plugin ownership cleanup: `25`
