...
- `platform`: the top-level folder/path to start auditing from
- `platform-lib-audit-report.md`: path to the Markdown report file to generate, for example:
...
### 5. Test strategy and coverage review
Assess all existing and missing test layers. For each library/folder, determine what is required and what is missing across:
...
### 6. 100% coverage requirement
Your goal is to enforce **100% coverage for all code paths that should exist for the evaluated scope**, but do not confuse line coverage with real quality.
...
- Produce a **concrete test plan** to reach true, meaningful 100% coverage of:
  - functionality,
  - branches,
  - failure modes,
  - states,
...
# 4. Test plan to reach real 100% coverage
Create a repository-wide plan that includes:
- test tiers required by library
- missing scenarios by library
- branch/failure/state/contract coverage gaps
...
- Focus on **intent, completeness, correctness, real test coverage, performance, AI/ML appropriateness, o11y, security, privacy, auditability, and production readiness**.
