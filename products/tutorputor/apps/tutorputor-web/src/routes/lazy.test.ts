...
describe("lazyRoutes", () => {
  ...
  it("should not have placeholder /content or /content/explore routes", () => {
    const contentRoute = lazyRoutes.find((r) => r.path === "/content");
    const contentExploreRoute = lazyRoutes.find(
      (r) => r.path === "/content/explore",
    );

    expect(contentRoute).toBeUndefined();
    expect(contentExploreRoute).toBeUndefined();
  });

  it("should have lazy loading for heavy routes", () => {
    const animationEditorRoute = lazyRoutes.find(
      (r) => r.path === "/animations/editor/:id?",
    );
...
describe("prefetchConfig", () => {
  it("should have idle prefetch routes", () => {
    ...
    expect(prefetchConfig.idlePrefetch).toContain("/search");
  });
