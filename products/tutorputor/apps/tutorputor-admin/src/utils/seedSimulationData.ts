...
async function seedDomain(
    ...
    if (existingDomainsRes.ok && existingDomainsRes.data?.domains) {
        const existingDomain = existingDomainsRes.data.domains.find((d: { domain: string; id: string; name: string; concepts?: { id: string; name: string }[] }) => d.domain === domain.domain);
        if (existingDomain) {
            console.log(`⚠️  Domain already exists: ${domain.name} (${existingDomain.id})`);
            return {
                domainId: existingDomain.id,
                concepts: existingDomain.concepts?.map((c: { id: string; name: string }) => ({ id: c.id, name: c.name })) || []
            };
        }
    }
