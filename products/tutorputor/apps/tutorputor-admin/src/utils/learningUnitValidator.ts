...
export function validateLearningUnit(lu: LearningUnit): ValidationResult {
  ...
  const prereqMap = new Map<string, string[]>(
    lu.claims.map((c) => [
      c.id,
      (c.prerequisites ?? []).filter((p) => claimIds.has(p)),
    ]),
  );
  ...
  function hasCycle(nodeId: string, path: string[]): boolean {
    ...
    for (const neighbour of prereqMap.get(nodeId) ?? []) {
      ...
      if (
        colour.get(neighbour) === WHITE &&
        hasCycle(neighbour, [...path, nodeId])
      ) {
        return true;
      }
    }
  ...
  lu.claims.forEach((claim) => {
    ...
    if (!hasEvidence) {
      errors.push(
        `Claim "${claim.text.substring(0, 20)}...": No evidence defined for this claim.`,
      );
    }
  });
  ...
  lu.tasks.forEach((task) => {
    if (!task.claimRef) {
      errors.push(
        `Task "${task.prompt.substring(0, 20)}...": Must reference a claim.`,
      );
    }
    if (!task.evidenceRef) {
      errors.push(
        `Task "${task.prompt.substring(0, 20)}...": Must reference an evidence type.`,
      );
    }
  });
