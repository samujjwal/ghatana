...
import { createMultiProviderAIService, type AIProviderConfig } from "./ai-providers";
...
Ensure all required fields are present and the manifest is valid.`;
    ...
    prompt += "Output format: Return ONLY valid JSON matching the SimulationManifest schema.\n\n";
    prompt += "Constraints:\n";
  ...
  return {
    async generateManifest(request: GenerateManifestRequest): Promise<GenerateManifestResult> {
      ...
      const userPrompt = buildUserPrompt(domain, request.prompt, request.constraints);
      ...
      if (needsReview && validation.warnings.length > 0) {
        manifest.reviewNotes = validation.warnings.map(w => w.message).join("; ");
      }
    ...
    async refineManifest(request: RefineManifestRequest): Promise<GenerateManifestResult> {
...
Return the complete refined manifest as JSON.`;
    ...
    async suggestParameters(request: SuggestParametersRequest): Promise<SuggestParametersResult> {
...
Suggest appropriate parameter values.`;
    ...
    async validateManifest(manifest: SimulationManifest): Promise<ManifestValidationResult> {
