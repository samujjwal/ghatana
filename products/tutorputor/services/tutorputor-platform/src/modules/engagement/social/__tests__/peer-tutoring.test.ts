...
describe("PeerTutoringServiceImpl", () => {
  ...
  describe("getTutorProfile", () => {
    it("returns null for non-existent profile", async () => {
      ...
      const profile = await service.getTutorProfile({
        tenantId: "t1" as any,
        userId: "u1" as any,
      });
    ...
    it("returns profile when found", async () => {
      (prisma.tutorProfile.findFirst as any).mockResolvedValue(
        makeProfileRow(),
      );
  ...
  describe("toggleAvailability", () => {
    it("toggles tutor availability", async () => {
      (prisma.tutorProfile.update as any).mockResolvedValue(
        makeProfileRow({ isAvailable: false }),
      );
  ...
  describe("createRequest", () => {
    it("creates a request and notifies matching tutors", async () => {
      (prisma.tutoringRequest.create as any).mockResolvedValue(
        makeRequestRow(),
      );
      (prisma.tutorProfile.findMany as any).mockResolvedValue([
        makeProfileRow(),
  ...
  describe("listOpenRequests", () => {
    it("returns open requests sorted by urgency", async () => {
      (prisma.tutoringRequest.findMany as any).mockResolvedValue([
        makeRequestRow(),
      ]);
      ...
      expect(result.items).toHaveLength(1);
      const callArgs = (prisma.tutoringRequest.findMany as any).mock
        .calls[0][0];
  ...
  describe("scheduleSession", () => {
    it("rejects cross-tenant request ids", async () => {
      (prisma.tutoringRequest.findUnique as any).mockResolvedValue(null);
  ...
  describe("listSessions", () => {
    it("scopes session listings to the supplied user id", async () => {
      (prisma.tutoringSession.findMany as any).mockResolvedValue([]);
      (prisma.tutoringSession.count as any).mockResolvedValue(0);
      ...
      const callArgs = (prisma.tutoringSession.findMany as any).mock
        .calls[0][0];
      expect(callArgs.where.studentId).toBe("u2");
    });
  });
});
