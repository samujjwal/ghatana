...
export const knowledgeBaseModule: FastifyPluginAsync = async (fastify) => {
  ...
  const knowledgeBaseService = new KnowledgeBaseService(prisma, {
    ...
    openStaxApiUrl:
      process.env.KNOWLEDGE_BASE_OPENSTAX_API_URL || "https://openstax.org/api",
    khanAcademyApiUrl:
  ...
  const config: KnowledgeBaseModuleConfig = {
    ...
    sources: {
      wikipedia: {
        ...
        rateLimitPerMinute: parseInt(
          process.env.KNOWLEDGE_BASE_WIKIPEDIA_RATE_LIMIT || "60",
          10,
        ),
      },
      openStax: {
        ...
        apiUrl:
          process.env.KNOWLEDGE_BASE_OPENSTAX_API_URL ||
          "https://openstax.org/api",
        rateLimitPerMinute: parseInt(
          process.env.KNOWLEDGE_BASE_OPENSTAX_RATE_LIMIT || "30",
          10,
        ),
      },
      khanAcademy: {
        ...
        apiUrl:
          ...
          "https://www.khanacademy.org/api",
        rateLimitPerMinute: parseInt(
          process.env.KNOWLEDGE_BASE_KHAN_ACADEMY_RATE_LIMIT || "30",
          10,
        ),
      },
    },
    caching: {
      ...
      maxEntries: parseInt(
        process.env.KNOWLEDGE_BASE_CACHE_MAX_ENTRIES || "10000",
        10,
      ),
    },
    validation: {
      ...
      confidenceThreshold: parseFloat(
        process.env.KNOWLEDGE_BASE_CONFIDENCE_THRESHOLD || "0.7",
      ),
      riskThresholds: {
  ...
  if (config.enabled) {
    ...
    fastify.get("/api/knowledge-base/metrics", async (_request, _reply) => {
      return {
