...
export class ContentNeedsAnalyzer {
  ...
  private applyBloomLevelHeuristics(
    bloomLevel: string,
    needs: ContentNeeds,
  ): void {
  ...
  private applyDomainHeuristics(
    ...
    if (domainLower.includes("physics") || domainLower.includes("science")) {
      if (
        claimLower.includes("force") ||
        claimLower.includes("motion") ||
    ...
    // Mathematics domain
    if (
      domainLower.includes("math") ||
      domainLower.includes("algebra") ||
      domainLower.includes("calculus")
    ) {
      needs.examples.required = true;
    ...
    // Chemistry domain
    if (
      domainLower.includes("chemistry") ||
      claimLower.includes("molecule") ||
      claimLower.includes("reaction")
    ) {
      needs.simulation.required = true;
      needs.animation.required = true;
      needs.animation.type = "process_visualization";
    ...
    // Biology domain
    if (
      domainLower.includes("biology") ||
      claimLower.includes("cell") ||
      claimLower.includes("organism")
    ) {
      needs.examples.required = true;
  ...
  private applyGradeLevelHeuristics(
    gradeRange: string,
    needs: ContentNeeds,
  ): void {
  ...
  private async analyzeConceptType(
    claimText: string,
    needs: ContentNeeds,
  ): Promise<void> {
  ...
  private generateReasoning(
    ...
    if (needs.examples.required) {
      reasons.push(
        `Examples required (${needs.examples.count}x) for ${needs.examples.complexity} complexity with ${needs.examples.scaffolding} scaffolding`,
      );
    }

    if (needs.simulation.required) {
      reasons.push(
        `Simulation required for ${needs.simulation.interactionType} interaction at ${needs.simulation.complexity} level`,
      );
    }

    if (needs.animation.required) {
      reasons.push(
        `Animation required (${needs.animation.type}, ${needs.animation.durationSeconds}s) for visualization`,
      );
    }
  ...
  private generateSuggestions(needs: ContentNeeds): string[] {
    ...
    if (needs.examples.required) {
      suggestions.push(
        `Create ${needs.examples.count} ${needs.examples.types.join(", ")} examples`,
      );
    }

    if (needs.simulation.required) {
      suggestions.push(
        `Design ${needs.simulation.complexity} simulation with ${needs.simulation.interactionType} interaction`,
      );
    }

    if (needs.animation.required) {
      suggestions.push(
        `Produce ${needs.animation.complexity} ${needs.animation.type} animation`,
      );
    }
  ...
  private async persistAnimation(
    claim: ClaimForGeneration,
    needs: ContentNeeds,
  ): Promise<unknown> {
  ...
  private async generateAnimation(
    ...
    return {
      ...
      keyframes: [
        ...
        {
          time_ms: needs.animation.durationSeconds * 1000,
  ...
  private requireExperience(claim: ClaimForGeneration): {
    tenantId: string;
    domain: string;
  } {
    if (!claim.experience) {
  ...
  private extractGradeRange(experience: {
    gradeAdaptations?: unknown;
  }): string {
...
function toSimulationDomain(domain: string): SimulationDomain {
  ...
  if (normalized.includes("CS") || normalized.includes("COMPUT"))
    return "CS_DISCRETE";
  return "MATHEMATICS";
}
