...
type AuditHashChainDelegate = {
  findFirst(
    args: Record<string, unknown>,
  ): Promise<AuditHashChainRecord | null>;
  findMany(args: Record<string, unknown>): Promise<AuditHashChainRecord[]>;
  create(args: Record<string, unknown>): Promise<AuditHashChainRecord>;
};
...
export class AuditHashChain {
  ...
  private get auditHashChain(): AuditHashChainDelegate | null {
    return (
      (
        this.prisma as PrismaClient & {
          auditHashChain?: AuditHashChainDelegate;
        }
      ).auditHashChain ?? null
    );
  }
  ...
  async appendEntry(
    ...
    const auditHashChain = this.auditHashChain;
    if (!auditHashChain) {
      throw new Error("Audit hash chain storage is not configured");
    }

    // Get latest chain entry for this tenant
    const latest = await auditHashChain.findFirst({
      where: { tenantId },
    ...
    const entry = await auditHashChain.create({
      data: {
        entryId,
        tenantId,
        sequenceNumber,
  ...
  async verifyChain(
    ...
    if (!auditHashChain) {
      return {
        valid: false,
        checkedCount: 0,
        error: "Audit hash chain storage is not configured",
      };
    }

    const entries = await auditHashChain.findMany({
      where: { tenantId },
