 ...
 * Tenant Service
 *
 * Manages tenant-specific configurations, domain packs, settings,
 * and multi-tenant platform administration.
 *
 * @doc.type service
 * @doc.purpose Multi-tenant management
 * @doc.layer product
...
export function createTenantService(prisma: TutorPrismaClient) {
  ...
  return {
    async getTenantConfig(tenantId: TenantId): Promise<TenantConfig> {
      ...
      const settings: TenantSettings = dbSettings
        ? {
            ...
            simulationQuotas: parseJSON(
              dbSettings.simulationQuotas,
    ...
    async updateTenantConfig(
      ...
      const currentQuotas = existing
        ? parseJSON(
            existing.simulationQuotas,
            DEFAULT_SETTINGS.simulationQuotas,
          )
        : DEFAULT_SETTINGS.simulationQuotas;
      ...
      if (updates.requireEmailVerification !== undefined)
        data.requireEmailVerification = updates.requireEmailVerification;
      if (updates.defaultUserRole !== undefined)
        data.defaultUserRole = updates.defaultUserRole;
      if (updates.maxUsersPerClassroom !== undefined)
        data.maxUsersPerClassroom = updates.maxUsersPerClassroom;
      if (updates.enabledFeatures !== undefined)
        data.enabledFeatures = JSON.stringify(updates.enabledFeatures);
      if (updates.enabledDomainPacks !== undefined)
        data.enabledDomainPacks = JSON.stringify(updates.enabledDomainPacks);
      if (updates.simulationQuotas !== undefined)
        data.simulationQuotas = JSON.stringify({
          ...currentQuotas,
          ...updates.simulationQuotas,
        });
    ...
    async listDomainPacks(
      tenantId: string,
      filter?: DomainPackFilter,
    ): Promise<{ packs: DomainPack[]; total: number }> {
      ...
      if (filter?.search) {
        ...
        packs = packs.filter(
          (pack) =>
            ...
            (pack.metadata.description?.toLowerCase().includes(searchLower) ??
              false) ||
    ...
    async updateDomainPack(
      id: string,
      updates: Partial<DomainPack>,
