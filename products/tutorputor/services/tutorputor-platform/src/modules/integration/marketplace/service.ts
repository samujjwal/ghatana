  ...
  return {
    ...
    async updateTemplate({ tenantId, templateId, updatedBy, patch }) {
      ...
      const data: Record<string, unknown> = {
        ...
        ...(patch.authorName !== undefined
          ...
          : {}),
        ...(patch.authorAvatarUrl !== undefined
          ? { authorAvatarUrl: patch.authorAvatarUrl }
          : {}),
    ...
    async deleteTemplate({ tenantId, templateId }) {
      ...
      if (existing.status === "PUBLISHED") {
        warnings.push(
          "Deleting a published template will remove it from the marketplace.",
        );
      }
    ...
    async getMarketplaceStats({ tenantId }) {
      ...
      return {
        totalListings: (Object.values(stats) as number[]).reduce(
    ...
    async getTemplateStats({ tenantId }) {
      const [totalCount, verifiedCount, premiumCount, domainCounts] =
        await Promise.all([
