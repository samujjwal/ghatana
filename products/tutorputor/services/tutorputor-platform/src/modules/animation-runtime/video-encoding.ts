...
export class VideoEncodingService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to initialize encoding libraries: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async generateFrames(
    ...
    const totalFrames = Math.floor(
      (animation.durationSeconds * 1000) / (1000 / options.fps),
    );
  ...
  private interpolateProperties(
    ...
    const progress =
      (currentTime - previousKeyframe.timeMs) /
      (nextKeyframe.timeMs - previousKeyframe.timeMs);
  ...
  private async encodeMP4(
    ...
    try {
      ...
      this.ffmpeg.setProgress((progress: { ratio: number }) => {
        if (onProgress && startTime) {
          onProgress({
            ...
            timeElapsed: Date.now() - startTime,
            timeEstimated: Math.round(
              (Date.now() - startTime) / (progress.ratio || 0.1),
            ),
          });
        }
      });
      ...
      throw new Error(
        `MP4 encoding failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private buildFFmpegCommand(
    ...
    const command = [
      ...
      `frame_%06d.png`,
      "-c:v",
      this.getVideoCodec(options.format, options.quality),
      "-preset",
  ...
  estimateEncodingTime(
    ...
    const qualityMultiplier =
      options.quality === "high" ? 2 : options.quality === "low" ? 0.5 : 1;

    const avgTime =
      totalFrames * baseTimePerFrame * complexity * qualityMultiplier;
  ...
  private getAnimationComplexity(animation: AnimationSpec): number {
    ...
    const maxProperties = Math.max(
      ...animation.keyframes.map((kf) => Object.keys(kf.properties).length),
    );
