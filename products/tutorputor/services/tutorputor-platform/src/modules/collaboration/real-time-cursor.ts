...
export class RealTimeCollaboration extends EventEmitter {
  ...
  async getStats(): Promise<{
    ...
    const averageSessionDuration =
      totalParticipants > 0 ? totalDuration / totalParticipants : 0;
   ...
   */
  private throttledBroadcastCursor(
    sessionId: string,
  ...
  static getCursorType(context: string): CursorPosition["cursorType"] {
    ...
    if (
      contextLower.includes("crosshair") ||
      contextLower.includes("precise")
    ) {
      return "crosshair";
    }
