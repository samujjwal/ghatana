...
export const autoRevisionModule: FastifyPluginAsync = async (fastify) => {
  ...
  const config: AutoRevisionModuleConfig = {
    ...
    driftMonitoring: {
      ...
      intervalHours: parseInt(
        process.env.AUTO_REVISION_DRIFT_INTERVAL_HOURS || "6",
        10,
      ),
    },
    regeneration: {
      ...
      maxConcurrentJobs: parseInt(
        process.env.AUTO_REVISION_MAX_CONCURRENT_JOBS || "3",
        10,
      ),
    },
    abTesting: {
      ...
      minSampleSize: parseInt(
        process.env.AUTO_REVISION_MIN_SAMPLE_SIZE || "100",
        10,
      ),
      significanceThreshold: parseFloat(
        process.env.AUTO_REVISION_SIGNIFICANCE_THRESHOLD || "0.05",
      ),
    },
  };
  ...
  if (config.enabled) {
    ...
    fastify.log.info(
      "Auto-Revision module initialized with background workers",
    );
  } else {
