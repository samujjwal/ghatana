...
describe("AutoRevisionService", () => {
  ...
  describe("detectDrift", () => {
    ...
    it("throws when analytics not found", async () => {
      ...
      await expect(service.detectDrift("nonexistent")).rejects.toThrow(
        "Analytics not found",
      );
    });
  });
  ...
  describe("generateImprovedVersion", () => {
    it("calls content studio for regeneration", async () => {
      ...
      await service.generateImprovedVersion("exp-1", [
        {
  ...
  describe("signal metadata", () => {
    it("includes detectedAt timestamp", async () => {
      ...
      expect(signals[0].detectedAt.getTime()).toBeGreaterThanOrEqual(
        before.getTime(),
      );
      expect(signals[0].detectedAt.getTime()).toBeLessThanOrEqual(
        after.getTime(),
      );
    });
