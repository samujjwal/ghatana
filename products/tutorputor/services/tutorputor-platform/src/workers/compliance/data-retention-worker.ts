...
const DEFAULT_POLICIES: Omit<RetentionPolicy, "tenantId">[] = [
  ...
  {
    resourceType: "deleted_user_data",
    retentionDays: 30,
    archiveBeforeDelete: false,
    enabled: true,
  },
...
export class DataRetentionWorker {
    ...
    for (const tenant of tenants) {
      ...
      for (const policy of policies) {
        ...
        try {
          ...
          errors.push(
            `Policy ${policy.resourceType}@${policy.tenantId}: ${errMsg(err)}`,
          );
        }
      }
    }
  ...
  async enforcePolicy(policy: RetentionPolicy): Promise<RetentionRunResult> {
    ...
    try {
      if (policy.archiveBeforeDelete) {
        archivedCount = await handler.archive(
          policy.tenantId,
          cutoffDate,
          this.batchSize,
        );
      }
      deletedCount = await handler.delete(
        policy.tenantId,
        cutoffDate,
        this.batchSize,
      );
  ...
  private async getPoliciesForTenant(
    tenantId: string,
  ...
  private getResourceHandler(resourceType: string): {
    ...
    const handlers: Record<
      string,
      ReturnType<typeof this.getResourceHandler>
    > = {
      session_data: {
        archive: async () => 0,
