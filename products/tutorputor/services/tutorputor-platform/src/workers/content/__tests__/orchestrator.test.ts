...
function makeNeeds(overrides: Record<string, any> = {}) {
  return {
    ...
    simulation: {
      required: true,
      interactionType: "parameter_tuning",
      complexity: "medium",
    },
    animation: {
      required: false,
...
describe("ContentGenerationOrchestrator", () => {
  ...
  describe("orchestrateForClaim", () => {
    ...
    it("dispatches examples when required", async () => {
      const result = await orchestrator.orchestrateForClaim(
        makeRequest({
          needs: makeNeeds({
            examples: { required: true, count: 3, types: ["worked"] },
          }),
        }),
      );
  ...
  describe("orchestrateForExperience", () => {
    it("processes multiple claims and returns results array", async () => {
      const results = await orchestrator.orchestrateForExperience(
        "exp-1",
        "t1",
        [
          {
    ...
    it("stops when cost limit is reached", async () => {
      ...
      const lowCostOrchestrator = new ContentGenerationOrchestrator(
        prisma,
        logger,
        { ...makeConfig(), maxCostUsd: 0 },
      );

      const results = await lowCostOrchestrator.orchestrateForExperience(
        "exp-1",
        "t1",
        [
          {
      ...
      // With maxCostUsd=0 the pre-dispatch guard should skip every claim.
