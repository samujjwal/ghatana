...
describe("Invariant: Modality Priority Order", () => {
  ...
  describe("compareModalityPriority", () => {
    it("should return negative when first has higher priority", () => {
      expect(
        selector.compareModalityPriority("simulation", "animation"),
      ).toBeLessThan(0);
      expect(
        selector.compareModalityPriority("simulation", "example"),
      ).toBeLessThan(0);
      expect(
        selector.compareModalityPriority("animation", "example"),
      ).toBeLessThan(0);
    });

    it("should return positive when second has higher priority", () => {
      expect(
        selector.compareModalityPriority("animation", "simulation"),
      ).toBeGreaterThan(0);
      expect(
        selector.compareModalityPriority("example", "simulation"),
      ).toBeGreaterThan(0);
      expect(
        selector.compareModalityPriority("example", "animation"),
      ).toBeGreaterThan(0);
    });

    it("should return 0 when priorities are equal", () => {
      expect(selector.compareModalityPriority("simulation", "simulation")).toBe(
        0,
      );
      expect(selector.compareModalityPriority("animation", "animation")).toBe(
        0,
      );
