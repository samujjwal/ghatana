...
export const moduleSchemas = {
  create: z.object({
    ...
    learningObjectives: z.array(z.string().min(1).max(500)).max(20, 'Too many objectives').default([]),
...
export class InputValidator {
  ...
  static validate<T>(schema: z.ZodSchema<T>, data: unknown, context?: ValidationContext): T {
    try {
      ...
      if (typeof sanitizedData === 'string' && detectSuspiciousPatterns(sanitizedData)) {
