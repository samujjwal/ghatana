...
function sanitizeOptionalText(input: string | undefined): string | undefined {
  return input === undefined ? input : sanitizeText(input);
}
...
export const moduleSchemas = {
  create: z.object({
    ...
    description: baseSchemas.description.transform(sanitizeOptionalText),
    domain: z.enum([
      "MATHEMATICS",
      "SCIENCE",
      "LANGUAGE_ARTS",
    ...
    estimatedTimeMinutes: z.coerce
      ...
      .max(10080, "Cannot exceed 1 week"),
    prerequisites: z
      .array(baseSchemas.id)
      .max(50, "Too many prerequisites")
      .default([]),
    learningObjectives: z
      .array(z.string().min(1).max(500).transform(sanitizeText))
  ...
  search: z.object({
    ...
    sort: z
      .enum(["title", "created_at", "updated_at", "difficulty"])
      .default("created_at"),
  }),
};
...
export const assessmentSchemas = {
    ...
    questions: z
      .array(
        z.object({
          ...
          text: z.string().min(1).max(1000).transform(sanitizeText),
          ...
          explanation: z.string().max(1000).transform(sanitizeText).optional(),
        }),
      )
      .min(1)
      .max(50),
  ...
  grade: z.object({
    ...
    feedback: z.string().max(2000).transform(sanitizeText).optional(),
    passed: z.boolean(),
  }),
};
...
export const simulationSchemas = {
  ...
  results: z.object({
    ...
    summary: z.string().max(1000).transform(sanitizeText).optional(),
  }),
};
...
export const searchSchemas = {
  global: z.object({
    query: z.string().min(1).max(100).transform(sanitizeText),
    types: z
      .array(z.enum(["modules", "assessments", "simulations", "users"]))
      .max(4)
      .default(["modules"]),
    page: baseSchemas.page,
    limit: baseSchemas.limit,
  ...
  advanced: z.object({
    query: z.string().max(100).transform(sanitizeText).optional(),
  ...
  static validate<T>(
    ...
    try {
      ...
      if (
        ...
        this.detectSuspiciousPatterns(sanitizedData)
      ) {
    ...
    return dataArray.map((data: any, index: any) => {
      try {
        return this.validate(schema, data, {
          ...context,
          requestId: `${context?.requestId}-${index}`,
        });
      } catch (error) {
        if (error instanceof ValidationError) {
          throw new ValidationError(
