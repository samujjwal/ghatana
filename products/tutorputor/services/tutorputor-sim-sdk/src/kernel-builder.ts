...
class BuiltKernel<TState = Record<string, unknown>>
  ...
  protected initializeEntities(entities: SimEntityBase[]): SimEntityBase[] {
    return this.entityInit(entities);
  }

  protected initializeCustomState(manifest: SimulationManifest): Record<string, unknown> {
  ...
  protected executeStep(
    ...
    const result = this.stepExecutor(this.state.entities, this.customState, stepDef);
...
export class KernelBuilder<TState = Record<string, unknown>> {
  private stepExecutor: StepExecutor<TState> = (entities, state) => ({ entities, state });
 ...
 */
export function createKernel<TState = Record<string, unknown>>(): KernelBuilder<TState> {
