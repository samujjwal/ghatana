...
> implements SimKernelService {
  ...
  constructor(config?: TConfig, hooks?: KernelHooks<TConfig>) {
    this.config = config;
    this.hooks = hooks ?? {};
  ...
  initialize(manifest: SimulationManifest): void {
    try {
      ...
      this.state.entities = this.initializeEntities(manifest.initialEntities as TEntity[]);
   ...
   */
  protected initializeCustomState(_manifest: SimulationManifest): Record<string, unknown> {
    return {};
  }
  ...
  protected calculateTimestamp(t: number): number {
    ...
    for (let i = 0; i < this.state.currentStep; i++) {
      time += this.getStepDuration(this.manifest.steps[i]);
    }
    const currentStepDuration = this.getStepDuration(this.manifest.steps[this.state.currentStep]);
  ...
  reset(): void {
    ...
    if (this.manifest) {
      ...
      this.state.entities = this.initializeEntities(this.manifest.initialEntities as TEntity[]);
  ...
  getAnalytics(): Record<string, unknown> {
    const avgStepTime =
      this.analytics.stepTimings.length > 0
        ? this.analytics.stepTimings.reduce((a, b) => a + b, 0) / this.analytics.stepTimings.length
        : 0;
  ...
  protected incrementMetric(name: string, amount = 1): void {
    this.analytics.customMetrics[name] = (this.analytics.customMetrics[name] ?? 0) + amount;
  }
}
