...
5. **File**: `apps/api-gateway/src/routes/simulation.ts`
   ...
   **Remove TODO, wire `POST /simulations/refine`**:
   ...
   fastify.post("/simulations/refine", async (request, reply) => {
     ...
     // Call NLService
     const result = await fastify.nlService.refine(
       sessionId,
       userInput,
       manifest,
     );
...
6. **File**: `apps/api-gateway/src/routes/simulation.ts`
   ...
   **Remove TODO, wire `POST /simulations/sessions`**:
   ...
   fastify.post("/simulations/sessions", async (request, reply) => {
     ...
     if (
       manifest.lifecycle?.status !== "published" &&
       manifest.lifecycle?.status !== "validated"
     ) {
    ...
    **Test Suite 1: NL Generation Flow**
    ...
    describe("Simulation NL Generation Flow", () => {
      it("should generate manifest from NL prompt", async () => {
        ...
        expect(result.manifest.lifecycle.status).toBeOneOf([
          "draft",
          "validated",
        ]);
    ...
    describe("CMS Publishing Flow", () => {
      ...
      it("should validate simulationId reference", async () => {
        ...
        expect(response.errors).toContain(
          "Invalid simulationId: template not found",
        );
      });
    });
    ```
...
19. **File**: `services/tutorputor-domain-loader/src/cli.ts`
    ...
    **Add CLI command**:
    ...
    program
      ...
      .description(
        "Generate simulation templates for all concepts in curriculum",
      )
      ...
      .action(async (curriculumId, options) => {
        ...
        const report = await generateSimulationsForCurriculum(
          curriculumId,
          options,
        );
...
20. **File**: `apps/api-gateway/src/routes/marketplace.ts`
    ...
    **Add simulation template routes**:
    ...
    fastify.post(
      "/marketplace/simulations/:id/clone",
      async (request, reply) => {
        const { id } = request.params;
...
22. **File**: `apps/api-gateway/src/routes/marketplace.ts`
    ...
    **Add governance endpoints**:
    ...
    fastify.put(
      "/marketplace/simulations/:id/submit",
      async (request, reply) => {
        const { id } = request.params;
        const userId = request.user.id;

        const result = await marketplaceService.submitTemplateForReview(
          id,
          userId,
        );
    ...
    fastify.put(
      "/marketplace/simulations/:id/review",
      {
    ...
    fastify.put(
      "/marketplace/simulations/:id/deprecate",
      {
        preHandler: [requireRole("admin")],
      },
      async (request, reply) => {
...
23. **File**: `services/tutorputor-marketplace/src/versioning.ts` (NEW)
    ...
    export async function getTemplateVersionHistory(
      ...
      const versions = await prisma.simulationTemplate.findMany({
        where: {
          OR: [{ id: templateId }, { parentVersionId: templateId }],
        },
        orderBy: { createdAt: "desc" },
      });
...
**Lifecycle Contract Reference:** [`docs/kernel/PRODUCT_LIFECYCLE_CONTRACT.md`](../../../../../docs/kernel/PRODUCT_LIFECYCLE_CONTRACT.md)
