...
export async function generateContentBlocks(
  ...
  if (concept.pedagogicalMetadata.learningObjectives.length > 0) {
    await createLearningObjectivesBlock(
      prisma,
      moduleId,
      concept,
      orderIndex++,
  ...
  if (
    ...
    (concept.keywords.length > 0 ||
      concept.pedagogicalMetadata.competencies.length > 0)
  ) {
  ...
  if (
    includeExercise &&
    concept.pedagogicalMetadata.learningObjectives.length > 0
  ) {
  ...
  if (verbose) {
    logger.info(
      {},
      `Created ${orderIndex} content blocks for module ${moduleId}`,
    );
  }
...
function generateSuggestedQuestions(concept: DomainConcept): string[] {
  ...
  for (const objective of concept.pedagogicalMetadata.learningObjectives.slice(
    0,
    2,
  )) {
  ...
  for (const competency of concept.pedagogicalMetadata.competencies.slice(
    0,
    1,
  )) {
...
function generateExercises(concept: DomainConcept): ExerciseItem[] {
  ...
  for (
    let i = 0;
    i < concept.pedagogicalMetadata.learningObjectives.length;
    i++
  ) {
...
function inferTaxonomyLevel(objective: string): string {
  ...
  // Create level
  if (
    /design|create|develop|formulate|construct|build|generate|produce/.test(
      lower,
    )
  ) {
    return "create";
  ...
  // Analyze level
  if (
    /analyze|compare|contrast|differentiate|distinguish|examine|investigate/.test(
      lower,
    )
  ) {
    return "analyze";
  ...
  // Apply level
  if (
    /apply|use|implement|solve|compute|calculate|demonstrate|execute/.test(
      lower,
    )
  ) {
    return "apply";
  ...
  // Understand level
  if (
    /explain|describe|interpret|summarize|understand|classify|discuss/.test(
      lower,
    )
  ) {
    return "understand";
