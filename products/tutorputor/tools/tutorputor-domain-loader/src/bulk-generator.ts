...
} from "./generators/manifest-generator.js";
  ...
  async getJobResults(jobId: string): Promise<BulkGenerationResult | null> {
    ...
    return {
      ...
      manifests: manifests.map(
        (m) => m.manifest as unknown as SimulationManifest,
      ),
      errors: job.errors,
    };
  }
        ...
        batch.map(async (concept) => {
          try {
            ...
            const enrichedManifest = {
              ...
              ecd: {
                claims:
                  concept.pedagogicalMetadata?.learningObjectives?.map(
                    (obj, idx) => ({
                      id: `claim-${idx}`,
                      description: obj,
            ...
            console.error(
              `Failed to generate simulation for concept ${concept.id}:`,
              error,
            );
            job.failureCount++;
            job.processedConcepts++;
      ...
      return concepts.map(
        (c) =>
          ({
            ...
            level: c.level,
            prerequisites: [],
            audienceTags: JSON.parse(
              c.audienceTags,
            ) as DomainConcept["audienceTags"],
            keywords: JSON.parse(c.keywords) as string[],
            simulationMetadata:
              c.simulationMetadata as unknown as DomainConcept["simulationMetadata"],
            crossDomainLinks:
              c.crossDomainLinks as unknown as DomainConcept["crossDomainLinks"],
            learningObjectMetadata:
              c.learningObjectMetadata as unknown as DomainConcept["learningObjectMetadata"],
            pedagogicalMetadata:
              c.pedagogicalMetadata as unknown as DomainConcept["pedagogicalMetadata"],
 ...
 */
export function createBulkSimulationGenerator(
  prisma: TutorPrismaClient,
): BulkSimulationGenerator {
