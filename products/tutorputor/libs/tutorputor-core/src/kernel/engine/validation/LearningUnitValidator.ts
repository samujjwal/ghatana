...
export class LearningUnitValidator {
  ...
  private validateClaim(claim: Claim, index: number): void {
    ...
    if (!claim.text || claim.text.length < 10) {
      this.addError(
        `${path}.text`,
        "Claim text must be at least 10 characters",
      );
    }
    ...
    if (!hasActionVerb) {
      this.addWarning(
        `${path}.text`,
        "Claim should use an action verb (predict, explain, construct, etc.)",
      );
    }
  ...
  private checkEvidence(lu: LearningUnit): void {
    if (!lu.evidence || lu.evidence.length === 0) {
      this.addError(
        "evidence",
        "At least one evidence specification is required",
      );
      return;
    }
    ...
    const claimsWithoutEvidence = [...claimIds].filter(
      (id) => !evidenceClaimRefs.has(id),
    );
    if (claimsWithoutEvidence.length > 0) {
      this.addError(
        "evidence",
        `Claims without evidence: ${claimsWithoutEvidence.join(", ")}`,
      );
    } else {
  ...
  private validateEvidence(
    ...
    if (!evidence.id || !evidence.id.match(/^E\d+$/)) {
      this.addError(
        `${path}.id`,
        "Evidence ID must match pattern E1, E2, etc.",
      );
    }
    ...
    if (!evidence.claimRef || !claimIds.has(evidence.claimRef)) {
      this.addError(
        `${path}.claimRef`,
        `Invalid claim reference: ${evidence.claimRef}`,
      );
    }
  ...
  private checkTasks(lu: LearningUnit): void {
    ...
    lu.tasks.forEach((task, index) => {
      ...
      if (!claimIds.has(task.claimRef)) {
        this.addError(
          `${path}.claimRef`,
          `Invalid claim reference: ${task.claimRef}`,
        );
      }
      if (!evidenceIds.has(task.evidenceRef)) {
        this.addError(
          `${path}.evidenceRef`,
          `Invalid evidence reference: ${task.evidenceRef}`,
        );
      }
      ...
      if (task.type === "prediction") {
        ...
        if (!predTask.confidenceRequired) {
          this.addError(
            `${path}.confidenceRequired`,
            "Prediction tasks must require confidence",
          );
        ...
        if (!predTask.options || predTask.options.length < 2) {
          this.addError(
            `${path}.options`,
            "Prediction tasks must have at least 2 options",
          );
        }
      }
      ...
      if (task.type === "simulation") {
        if (!("simulationRef" in task) || !task.simulationRef) {
          this.addError(
            `${path}.simulationRef`,
            "Simulation tasks must reference a simulation",
          );
        }
        if (!("successCriteria" in task) || !task.successCriteria) {
          this.addError(
            `${path}.successCriteria`,
            "Simulation tasks must define success criteria",
          );
        }
      }
    });
    ...
    lu.artifacts.forEach((artifact, index) => {
      ...
      artifact.claims.forEach((claimRef) => {
        if (!claimIds.has(claimRef)) {
          this.addError(
            `${path}.claims`,
            `Invalid claim reference: ${claimRef}`,
          );
        }
      });
      ...
      if (artifact.type === "explainer_video") {
        if (!artifact.scaffolds || artifact.scaffolds.length === 0) {
          this.addWarning(
            `${path}`,
            "Explainer videos should scaffold at least one task (cannot be terminal content)",
          );
          ...
          artifact.scaffolds.forEach((taskRef) => {
            if (!taskIds.has(taskRef)) {
              this.addError(
                `${path}.scaffolds`,
                `Invalid task reference: ${taskRef}`,
              );
            }
          });
        }
  ...
  private checkTelemetry(lu: LearningUnit): void {
    ...
    const hasMinimumEvents =
      lu.telemetry.events && lu.telemetry.events.length >= 3;

    if (!hasMinimumEvents) {
      this.addError(
        "telemetry.events",
        "At least 3 telemetry events are required",
      );
    } else {
    ...
    requiredEvents.forEach((event) => {
      if (!lu.telemetry.events.includes(event)) {
        this.addWarning(
          "telemetry.events",
          `Missing recommended event: ${event}`,
        );
      }
    });
  }
  ...
  private checkAssessment(lu: LearningUnit): void {
    ...
    // Check viva triggers
    if (
      !lu.assessment.vivaTrigger ||
      !lu.assessment.vivaTrigger.conditions?.length
    ) {
      this.addWarning(
        "assessment.vivaTrigger",
        "Consider adding viva trigger conditions for overconfidence detection",
      );
    }
  }
  ...
  private generateSuggestions(lu: LearningUnit): string[] {
    ...
    if (lu.claims.length === 1) {
      suggestions.push(
        "Consider adding more claims to break down the learning objectives",
      );
    }
    ...
    if (evidenceTypes.size === 1) {
      suggestions.push(
        "Consider using multiple evidence types (prediction + simulation + explanation)",
      );
    }
    ...
    if (!hasSimulationTask) {
      suggestions.push(
        "Consider adding a simulation task for interactive evidence collection",
      );
    }
    ...
    if (
      lu.level === "university" &&
      lu.claims.every((c) => !c.prerequisites?.length)
    ) {
      suggestions.push(
        "Consider adding prerequisite claims for university-level content",
      );
    }
