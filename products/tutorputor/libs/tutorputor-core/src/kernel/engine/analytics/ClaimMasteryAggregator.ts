      ...
      if (e.object.taskType === "explanation") {
        return {
          ...
          ecd: {
            ...
            studentModel: `claim:${claimId}`,
            reliability: e.result.score !== undefined ? 0.7 : 0.5,
...
export class ClaimMasteryAggregator {
  ...
  computeMastery(
    ...
    return {
      ...
      evidenceCount,
      lastUpdated: records[records.length - 1]?.timestamp ?? new Date(),
      breakdown,
      calibration,
    };
  }
  ...
  private computeTypeScore(
    type: EvidenceType,
    records: EnrichedEvidenceRecord[],
  ): number {
    switch (type) {
      ...
      case "explanation_quality": {
        ...
        const rubricScores = records
          .filter(
            (r) =>
              r.rubricScore !== undefined && r.maxRubricScore !== undefined,
          )
