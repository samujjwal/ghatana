...
export class VivaEngine {
  ...
  constructor(cohortStats: CohortStats, vivaConditions: VivaCondition[] = []) {
    this.cohortStats = cohortStats;
    this.vivaConditions =
      vivaConditions.length > 0 ? vivaConditions : this.defaultConditions();
  }
  ...
  private defaultConditions(): VivaCondition[] {
    return [
      ...
      { type: "speed_anomaly", completionTimePercentile: "<= 10" },
      {
        ...
        description: "Correct but random exploration",
      },
  ...
  identifyVivaCandidates(
    ...
    const byLearnerClaim = this.groupByLearnerClaim(
      predictions,
      simulations,
      explanations,
    );
  ...
  private evaluateTriggers(
    ...
    if (this.checkSimEvidenceContradiction(predictions, simulations)) {
      triggers.push({
        type: "sim_evidence_contradiction",
  ...
  private checkOverconfidentWrong(predictions: PredictionRecord[]): boolean {
    const condition = this.vivaConditions.find(
      (c) => c.type === "overconfident_wrong",
    );
  ...
  private checkPatternMismatch(
    ...
    const avgParamChanges =
      simulations.reduce((sum, s) => sum + s.parameterChanges, 0) /
      (simulations.length || 1);

    const isRandomExploration =
      avgParamChanges > this.cohortStats.avgParameterChanges * 2;
    ...
    const suspiciousConfidence =
      lowConfidenceCorrect > predictions.length * 0.5;
   ...
   */
  private checkExplanationAvoidance(
    explanations: ExplanationRecord[],
  ): boolean {
    ...
    if (attempted.length > 0) {
      const avgTime =
        attempted.reduce((sum, e) => sum + e.timeSpentSeconds, 0) /
        attempted.length;
  ...
  private checkGamingDetection(
    ...
    const totalParamChanges = simulations.reduce(
      (sum, s) => sum + s.parameterChanges,
      0,
    );
  ...
  private checkSimEvidenceContradiction(
    ...
    const simSuccessRate =
      simulations.filter((s) => s.goalAchieved).length / simulations.length;
