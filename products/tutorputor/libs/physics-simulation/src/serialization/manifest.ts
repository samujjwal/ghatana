...
export function createManifest(
    entities: PhysicsEntity[],
    physics: PhysicsConfig,
    metadata?: SimulationManifest['metadata']
): SimulationManifest {
 ...
 * @doc.pattern Serializer
 */
export function exportManifestToJSON(manifest: SimulationManifest): string {
 ...
 * @doc.pattern Utility
 */
export function downloadManifest(manifest: SimulationManifest, filename?: string): void {
...
export function parseManifest(json: string): {
    ...
    try {
        ...
        if (result.success) {
            return { success: true, manifest: result.data as SimulationManifest };
        }
 ...
 */
export function migrateManifest(oldManifest: Record<string, unknown>): SimulationManifest {
    const now = new Date().toISOString();

    // Handle v1.0 format (from SimulationBuilderPage)
    if (oldManifest.version === '1.0' && Array.isArray(oldManifest.entities)) {
        const migratedEntities: PhysicsEntity[] = (oldManifest.entities as any[]).map(
            (e: any, index: number) => ({
    ...
    return oldManifest as unknown as SimulationManifest;
}
        ...
        reader.onload = (event) => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    resolve({
                        success: false,
                        error: result.errors?.errors.map((e) => e.message).join(', '),
                    });
                }
            } catch (error) {
