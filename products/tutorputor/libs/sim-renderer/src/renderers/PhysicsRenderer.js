 ...
 */
import { drawRect, drawCircle, drawPolygon, drawText, drawSpring, drawVector, applyGlow, clearGlow, } from "../primitives";
// =============================================================================
// Rigid Body Renderer
// =============================================================================
/**
...
export const physicsBodyRenderer = {
    ...
    render(entity, context, isHovered = false, isSelected = false) {
        ...
        if (entity.velocityX !== undefined && entity.velocityY !== undefined) {
            const vMag = Math.sqrt(entity.velocityX * entity.velocityX + entity.velocityY * entity.velocityY);
            if (vMag > 0.1) {
                const vAngle = Math.atan2(entity.velocityY, entity.velocityX);
                drawVector(ctx, screen.x, screen.y, vMag, vAngle, { color: theme.success, width: 2 }, undefined, zoom * 2);
            }
        }
    },
...
export const physicsSpringRenderer = {
    ...
    render(entity, context, isHovered = false) {
        ...
        const anchorEntity = entities.get(entity.anchorId);
        ...
        drawSpring(ctx, anchor.x, anchor.y, attach.x, attach.y, coils, amplitude, { color: springColor, width: 2 * zoom });
        clearGlow(ctx);
    ...
    hitTest(entity, worldX, worldY, context) {
        ...
        if (lengthSq === 0)
            return false;
        const t = Math.max(0, Math.min(1, ((worldX - anchorEntity.x) * dx + (worldY - anchorEntity.y) * dy) /
            lengthSq));
 ...
 */
export const physicsVectorRenderer = {
    entityTypes: ["vector"],
    domain: "PHYSICS",
    render(entity, context, isHovered = false) {
        const { ctx, theme, zoom, entities, worldToScreen } = context;
        ...
        drawVector(ctx, screen.x, screen.y, entity.magnitude, entity.angle, { color, width: 3 * zoom }, label, scale);
        clearGlow(ctx);
    },
    hitTest(entity, worldX, worldY) {
        // Check if point is near the vector line
        const endX = entity.x + entity.magnitude * Math.cos(entity.angle) * 3;
        const endY = entity.y + entity.magnitude * Math.sin(entity.angle) * 3;
        const dx = endX - entity.x;
        const dy = endY - entity.y;
        const lengthSq = dx * dx + dy * dy;
        if (lengthSq === 0)
            return false;
        const t = Math.max(0, Math.min(1, ((worldX - entity.x) * dx + (worldY - entity.y) * dy) / lengthSq));
 ...
 */
export const physicsParticleRenderer = {
...
//# sourceMappingURL=PhysicsRenderer.js.map