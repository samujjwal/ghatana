...
export const medInfectionAgentRenderer: EntityRenderer<MedInfectionAgentEntity> = {
    ...
    render(entity, context, isHovered = false): void {
        ...
        const popRatio = Math.min(Math.log10((entity.population ?? 0) + 1) / 6, 1);
        ...
        const popText = (entity.population ?? 0) >= 1e6 ? `${((entity.population ?? 0) / 1e6).toFixed(1)}M` : (entity.population ?? 0) >= 1e3 ? `${((entity.population ?? 0) / 1e3).toFixed(1)}K` : String(entity.population ?? 0);
