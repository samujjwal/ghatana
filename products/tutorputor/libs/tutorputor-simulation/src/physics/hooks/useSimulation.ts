...
export function useSimulation(): UseSimulationResult {
  ...
  const [physicsConfig, setPhysicsConfigAtom] = useAtom(
    simulationPhysicsConfigAtom,
  );
  const [, setSelection] = useAtom(simulationSelectionAtom);
  const [isPreviewMode, setPreviewModeAtom] = useAtom(
    simulationPreviewModeAtom,
  );
  ...
  const loadManifest = useCallback(
    (manifest: unknown) => {
      ...
      dispatchLoadEntities({
        entities: parsedManifest.entities ?? [],
        physics: parsedManifest.physics,
      });
    },
    [dispatchLoadEntities],
  );
