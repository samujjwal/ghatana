...
describe("SystemDynamicsKernel", () => {
  ...
  describe("domain identification", () => {
    it("should have ECONOMICS as domain", () => {
      expect(kernel.domain).toBe("ECONOMICS");
    });

    it("should be able to execute ECONOMICS manifests", () => {
      const manifest = createSDManifest();
      expect(kernel.canExecute(manifest)).toBe(true);
    });

    it("should not execute non-ECONOMICS manifests", () => {
  ...
  describe("run()", () => {
    it("should return error for wrong domain", async () => {
      ...
      expect(result.errors).toContain("Manifest domain is not ECONOMICS");
...
describe("Population Growth Model", () => {
  ...
  it("should simulate logistic growth with carrying capacity", async () => {
    ...
    const entities = [
      createStock("population", "Population", 100, { x: 200, y: 200 }),
      createFlow("net_growth", "Net Growth", 0, {
        x: 100,
        y: 200,
        toStock: "population",
      }),
...
describe("Supply and Demand Model", () => {
  it("should simulate market equilibrium", async () => {
    ...
    const entities = [
      ...
      createVariable("demand", "Demand", 150, {
        x: 100,
        y: 200,
        formula: "200 - 2 * price",
      }),
      createVariable("supply", "Supply", 50, {
        x: 500,
        y: 200,
        formula: "-50 + 3 * price",
      }),
      createFlow("price_adjustment", "Price Adjustment", 0, { x: 200, y: 200 }),
      createFeedbackLoop("market", "negative", [
        "price",
        "demand",
        "supply",
        "price_adjustment",
...
describe("Investment and Capital Model", () => {
  it("should simulate capital accumulation with depreciation", async () => {
    ...
    const entities = [
      createStock("capital", "Capital Stock", 1000, {
        x: 300,
        y: 200,
        unit: "dollars",
      }),
      createFlow("investment", "Investment", 100, {
        x: 150,
        y: 200,
        toStock: "capital",
      }),
      createFlow("depreciation", "Depreciation", 0, {
        x: 450,
        y: 200,
        fromStock: "capital",
      }),
...
describe("Inventory Management Model", () => {
  it("should simulate inventory with orders and shipments", async () => {
    ...
    const entities = [
      createStock("inventory", "Inventory", 100, {
        x: 300,
        y: 200,
        unit: "units",
      }),
      createStock("backlog", "Order Backlog", 0, {
        x: 300,
        y: 100,
        unit: "units",
      }),
      createFlow("orders", "Customer Orders", 20, {
        x: 100,
        y: 100,
        toStock: "backlog",
      }),
      createFlow("shipments", "Shipments", 0, {
        x: 200,
        y: 150,
        fromStock: "backlog",
      }),
      createFlow("production", "Production", 0, {
        x: 400,
        y: 200,
        toStock: "inventory",
      }),
      ...
      createFeedbackLoop("inventory_control", "negative", [
        "inventory",
        "production",
        "shipments",
      ]),
      ...
      steps.push({
        ...
        actions: [
          ...
          } as unknown as SimAction,
          {
            ...
            flowId: "production",
            formula:
              "(desired_inventory - inventory) / adjustment_time + shipments",
          } as unknown as SimAction,
          {
            action: "APPLY_FLOWS",
            stockId: "inventory",
...
describe("Climate Carbon Cycle Model", () => {
  it("should simulate carbon flow between atmosphere and ocean", async () => {
    ...
    const entities = [
      createStock("atm_carbon", "Atmospheric Carbon", 850, {
        x: 200,
        y: 100,
        unit: "GtC",
      }),
      createStock("ocean_carbon", "Ocean Carbon", 38000, {
        x: 200,
        y: 300,
        unit: "GtC",
      }),
      createStock("land_carbon", "Land Carbon", 2000, {
        x: 400,
        y: 200,
        unit: "GtC",
      }),
      createFlow("emissions", "Fossil Fuel Emissions", 10, {
        x: 50,
        y: 100,
        toStock: "atm_carbon",
      }),
      createFlow("ocean_uptake", "Ocean Uptake", 0, {
        x: 200,
        y: 200,
        fromStock: "atm_carbon",
        toStock: "ocean_carbon",
      }),
      createFlow("photosynthesis", "Photosynthesis", 0, {
        x: 300,
        y: 150,
        fromStock: "atm_carbon",
        toStock: "land_carbon",
      }),
      createFlow("respiration", "Respiration", 0, {
        x: 300,
        y: 250,
        fromStock: "land_carbon",
        toStock: "atm_carbon",
      }),
    ];
...
describe("Feedback Loop Analysis", () => {
  it("should identify and annotate positive feedback", async () => {
    ...
    const entities = [
      ...
      createFlow("adoption", "New Adoptions", 0, {
        x: 100,
        y: 200,
        toStock: "customers",
      }),
  ...
  it("should identify and annotate negative feedback (goal-seeking)", async () => {
    ...
    const entities = [
      createStock("temperature", "Room Temperature", 20, {
        x: 200,
        y: 200,
        unit: "°C",
      }),
      createFlow("heating", "Heating", 0, {
        x: 100,
        y: 200,
        toStock: "temperature",
      }),
      createVariable("setpoint", "Thermostat Setting", 22),
      createVariable("gap", "Temperature Gap", 0, {
        formula: "setpoint - temperature",
      }),
      createFeedbackLoop("thermostat", "negative", [
        "temperature",
        "gap",
        "heating",
      ]),
