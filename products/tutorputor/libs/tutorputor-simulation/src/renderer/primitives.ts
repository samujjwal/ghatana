...
export function drawRoundedRect(
  ...
  const fillStyle: FillStyle | undefined =
    typeof fill === "string" ? { color: fill } : fill;
  const strokeStyle: LineStyle | undefined =
...
export function drawLine(
  ...
  const s: LineStyle =
    typeof style === "string" ? { color: style, width: lineWidth ?? 1 } : style;
  ctx.beginPath();
  ctx.moveTo(x1, y1);
  ctx.lineTo(x2, y2);
...
export function drawArrow(
  ...
  const s: LineStyle =
    typeof style === "string"
      ? {
          color: style,
          width: typeof headStyleOrWidth === "number" ? headStyleOrWidth : 1,
        }
      : style;
  const headStyle: ArrowHeadStyle =
    typeof headStyleOrWidth === "object" && headStyleOrWidth !== null
      ? headStyleOrWidth
      : {
          size: typeof headSizeOrCurved === "number" ? headSizeOrCurved : 10,
          angle: Math.PI / 6,
          filled: true,
        };
  const isCurved =
    typeof headSizeOrCurved === "boolean"
      ? headSizeOrCurved
      : (curved ?? false);
...
export function drawVector(
  ...
  if (typeof style === "string") {
    ...
    s = {
      color: style,
      width: typeof labelOrWidth === "number" ? labelOrWidth : 1,
    };
