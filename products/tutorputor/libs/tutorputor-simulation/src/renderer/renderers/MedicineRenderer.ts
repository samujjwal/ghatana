 ...
 */
export const medCompartmentRenderer: EntityRenderer<MedCompartmentEntity> = {
  ...
  render(entity, context, isHovered = false, isSelected = false): void {
    ...
    drawText(
      ctx,
      entity.compartmentType.toUpperCase(),
      screen.x,
    ...
    ctx.fillRect(
      screen.x - width / 2 + 5 * zoom,
      screen.y + height / 2 - 5 * zoom - fillHeight,
      width - 10 * zoom,
      fillHeight,
    );
 ...
 */
export const medDoseRenderer: EntityRenderer<MedDoseEntity> = {
  entityTypes: ["dose"],
  domain: "MEDICINE",
  ...
  render(entity, context, isHovered = false): void {
    ...
    let color: string;
    let shape: "syringe" | "pill" | "patch" | "drop";
    switch (entity.route) {
      case "iv":
        color = theme.danger;
        shape = "syringe";
    ...
    switch (shape) {
      case "syringe":
        ...
        // Plunger
        drawRect(ctx, screen.x - size * 1.2, screen.y, size * 0.4, size * 0.5, {
          color: theme.neutral,
        });
        // Needle
        drawLine(
    ...
    drawText(
      ctx,
      entity.route.toUpperCase(),
      screen.x,
      screen.y - size - 5 * zoom,
      {
        color: theme.neutral,
        fontSize: 8 * zoom,
      },
    ...
    if (entity.bioavailability !== undefined) {
      drawText(
        ctx,
        `F: ${(entity.bioavailability * 100).toFixed(0)}%`,
        screen.x,
        screen.y + size + 22 * zoom,
        {
          color: theme.neutral,
          fontSize: 8 * zoom,
        },
 ...
 */
export const medInfectionAgentRenderer: EntityRenderer<MedInfectionAgentEntity> =
  {
    ...
    render(entity, context, isHovered = false): void {
      ...
      const popRatio = Math.min(
        Math.log10((entity.population ?? 0) + 1) / 6,
        1,
      );
      ...
      switch (entity.agentType) {
        ...
        case "parasite":
          ...
          ctx.globalAlpha = 1;
          // Nucleus
          drawCircle(ctx, screen.x - size * 0.2, screen.y, size * 0.3, {
            color: theme.foreground,
            opacity: 0.5,
          });
          break;
        ...
        case "fungus":
          ...
          ctx.lineCap = "round";
          // Main stem
          drawLine(ctx, screen.x, screen.y + size, screen.x, screen.y - size, {
            color,
            width: 3 * zoom,
          });
          // Branches
      ...
      const popText =
        (entity.population ?? 0) >= 1e6
          ? `${((entity.population ?? 0) / 1e6).toFixed(1)}M`
          : (entity.population ?? 0) >= 1e3
            ? `${((entity.population ?? 0) / 1e3).toFixed(1)}K`
            : String(entity.population ?? 0);
      ...
      if (entity.reproductionRate !== undefined) {
        drawText(
          ctx,
          `R₀: ${entity.reproductionRate.toFixed(1)}`,
          screen.x,
          screen.y + size * 1.5 + 22 * zoom,
          {
