 ...
 */
import { drawCircle, drawText, drawBond, drawLine, drawArrow, drawBidirectionalArrow, applyGlow, clearGlow, } from "../primitives";
// =============================================================================
...
export const chemAtomRenderer = {
    ...
    render(entity, context, isHovered = false, isSelected = false) {
        ...
        // Draw atom circle
        drawCircle(ctx, screen.x, screen.y, size, { color, opacity: entity.opacity ?? 1 }, { color: theme.border, width: 2 });
        clearGlow(ctx);
        // Draw element symbol
        ...
        if (entity.charge !== undefined && entity.charge !== 0) {
            const chargeStr = entity.charge > 0
        ...
        if (entity.isotope !== undefined) {
            drawText(ctx, String(entity.isotope), screen.x - size * 0.7, screen.y - size * 0.7, {
                color: theme.neutral,
                fontSize: 9 * zoom,
            });
        }
...
export const chemBondRenderer = {
    ...
    render(entity, context, isHovered = false) {
        ...
        if (entity.stereochemistry === "up") {
            ...
            drawBond(ctx, start.x, start.y, end.x, end.y, entity.bondOrder, { color, width: 2 * zoom });
        }
        clearGlow(ctx);
    },
    hitTest(entity, worldX, worldY, context) {
        ...
        if (lengthSq === 0)
            return false;
        const t = Math.max(0, Math.min(1, ((worldX - atom1.x) * dx + (worldY - atom1.y) * dy) / lengthSq));
 ...
 */
export const chemMoleculeRenderer = {
    ...
    render(entity, context, isHovered = false, isSelected = false) {
        ...
        if (isHovered || isSelected) {
            ...
            ctx.lineWidth = 2;
            ctx.setLineDash([4, 4]);
            ctx.strokeRect(screen.x - 60 * zoom, screen.y - 40 * zoom, 120 * zoom, 80 * zoom);
            ctx.setLineDash([]);
        }
    },
    hitTest(entity, worldX, worldY) {
 ...
 */
export const chemReactionArrowRenderer = {
    ...
    render(entity, context, isHovered = false) {
        ...
        switch (entity.arrowType) {
            ...
            case "equilibrium":
                // Double arrows
                drawArrow(ctx, startX, y - 4 * zoom, endX, y - 4 * zoom, { color, width: 1.5 }, { size: 8 * zoom, angle: Math.PI / 6, filled: true });
                drawArrow(ctx, endX, y + 4 * zoom, startX, y + 4 * zoom, { color, width: 1.5 }, { size: 8 * zoom, angle: Math.PI / 6, filled: true });
                break;
            case "resonance":
 ...
 */
export const chemEnergyProfileRenderer = {
    ...
    render(entity, context) {
        ...
        const scaleX = (v) => left + ((v - minX) / (maxX - minX)) * width;
        const scaleY = (v) => top + height - ((v - minY) / (maxY - minY)) * height;
        // Draw axes
        ...
        if (entity.activationEnergy !== undefined) {
            drawText(ctx, `Ea = ${entity.activationEnergy}`, left + width - 40 * zoom, top + 15 * zoom, {
                color: theme.danger,
                fontSize: 10 * zoom,
            });
        }
        ...
        if (entity.deltaH !== undefined) {
            ...
            drawText(ctx, `ΔH = ${sign}${entity.deltaH}`, left + width - 40 * zoom, top + 30 * zoom, {
...
//# sourceMappingURL=ChemistryRenderer.js.map