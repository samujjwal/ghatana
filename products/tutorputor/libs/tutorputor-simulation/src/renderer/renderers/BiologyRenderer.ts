 ...
 */
export const bioCellRenderer: EntityRenderer<BioCellEntity> = {
  entityTypes: ["cell"],
  domain: "BIOLOGY",
  ...
  render(entity, context, isHovered = false, isSelected = false): void {
    ...
    if (
      entity.cellType === "eukaryote" ||
      entity.cellType === "plant" ||
      entity.cellType === "animal"
    ) {
      ctx.beginPath();
      ctx.ellipse(
        screen.x,
 ...
 */
export const bioOrganelleRenderer: EntityRenderer<BioOrganelleEntity> = {
  ...
  render(entity, context, isHovered = false): void {
    ...
    const color =
      entity.color ??
      ORGANELLE_COLORS[entity.organelleType] ??
      ORGANELLE_COLORS.default ??
      "#888888";
 ...
 */
export const bioCompartmentRenderer: EntityRenderer<BioCompartmentEntity> = {
  ...
  render(entity, context, isHovered = false): void {
    ...
    concentrations.forEach(([name, value], index) => {
      const barX =
        screen.x - width / 2 + 10 * zoom + index * barWidth + barWidth / 2;
      const maxHeight = height * 0.6;
      const barHeight = Math.min(value / 100, 1) * maxHeight;

      // Bar background
      drawRect(ctx, barX, screen.y + height / 4, barWidth * 0.6, maxHeight, {
        color: theme.border,
        opacity: 0.3,
      });
      ...
      drawRect(
        ctx,
        barX,
        screen.y + height / 4 + (maxHeight - barHeight) / 2,
        barWidth * 0.6,
        barHeight,
        {
          color: theme.primary,
 ...
 */
export const bioEnzymeRenderer: EntityRenderer<BioEnzymeEntity> = {
  entityTypes: ["enzyme"],
  domain: "BIOLOGY",
  ...
  render(entity, context, isHovered = false): void {
    ...
    const color =
      entity.color ?? (entity.active ? theme.success : theme.neutral);
 ...
 */
export const bioSignalRenderer: EntityRenderer<BioSignalEntity> = {
  entityTypes: ["signal"],
  domain: "BIOLOGY",
  ...
  render(entity, context): void {
    ...
    const size = baseSize * Math.min(1 + (entity.concentration ?? 0) / 100, 2);
  ...
  hitTest(entity, worldX, worldY): boolean {
    const size = 15 * Math.min(1 + (entity.concentration ?? 0) / 100, 2);
    const dx = worldX - entity.x;
    const dy = worldY - entity.y;
    return dx * dx + dy * dy <= size * size;
  },
 ...
 * Renderer for genes.
 */
export const bioGeneRenderer: EntityRenderer<BioGeneEntity> = {
  entityTypes: ["gene"],
  domain: "BIOLOGY",
  ...
  render(entity, context, isHovered = false): void {
    ...
    const color =
      entity.color ?? (entity.promoterActive ? theme.success : theme.neutral);
    ...
    if (entity.expressionLevel !== undefined) {
      ...
      drawRect(ctx, screen.x, barY, width, barHeight, {
        color: theme.border,
        opacity: 0.3,
      });
      drawRect(
        ctx,
        screen.x - width / 2 + exprWidth / 2,
        barY,
        exprWidth,
        barHeight,
        { color: theme.primary },
