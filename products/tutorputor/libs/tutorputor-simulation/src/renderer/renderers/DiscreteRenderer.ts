 ...
 */
export const discreteNodeRenderer: EntityRenderer<DiscreteNodeEntity> = {
  entityTypes: ["node"],
  domain: "CS_DISCRETE",
  ...
  hitTest(entity, worldX, worldY, _context): boolean {
 ...
 */
export const discreteEdgeRenderer: EntityRenderer<DiscreteEdgeEntity> = {
  entityTypes: ["edge"],
  domain: "CS_DISCRETE",
  ...
  hitTest(entity, worldX, worldY, context): boolean {
    ...
    const t = Math.max(
      ...
      Math.min(
        1,
        ((worldX - sourceNode.x) * dx + (worldY - sourceNode.y) * dy) /
          lengthSq,
      ),
    );
 ...
 */
export const discretePointerRenderer: EntityRenderer<DiscretePointerEntity> = {
  entityTypes: ["pointer"],
  domain: "CS_DISCRETE",
  ...
  render(entity, context, isHovered = false): void {
    ...
    if (entity.pointerLabel ?? entity.label) {
      drawText(
        ctx,
        entity.pointerLabel ?? entity.label ?? "",
        target.x,
        target.y + size + 35 * zoom,
        {
          color,
          fontSize: 12 * zoom,
          fontWeight: "600",
