...
export function createMockManifest(
  ...
  return {
    id: ("test-manifest-" +
      Math.random().toString(36).slice(2)) as SimulationId,
    version: "1.0.0",
    schemaVersion: "1.0.0",
    ...
    steps: [
      createMockStep({
        id: "step-1" as SimStepId,
        orderIndex: 0,
        description: "Step 1",
      }),
      createMockStep({
        id: "step-2" as SimStepId,
        orderIndex: 1,
        description: "Step 2",
      }),
    ],
...
export function createMockEntity(
  overrides: Partial<SimEntityBase> = {},
): SimEntityBase {
  return {
...
export class KernelTestHarness {
  ...
  assertSorted(ascending = true): this {
    ...
    for (let i = 1; i < values.length; i++) {
      const v_prev = values[i - 1]!;
      const v_curr = values[i]!;
      const comparison = ascending ? v_prev <= v_curr : v_prev >= v_curr;
      if (!comparison) {
        throw new Error(
          `Array not sorted at index ${i}: ${v_prev} ${ascending ? ">" : "<"} ${v_curr}`,
        );
      }
    }
  ...
  assertMetric(name: string, expected: unknown): this {
    ...
    if (actual !== expected) {
      throw new Error(
        `Expected metric ${name} to be ${expected}, got ${actual}`,
      );
    }
    return this;
  }
  ...
  assertEntityValue(entityId: string, expected: unknown): this {
    ...
    if (entityWithValue.value !== expected) {
      throw new Error(
        `Expected entity ${entityId} value to be ${expected}, got ${entityWithValue.value}`,
      );
    }
    return this;
  }
