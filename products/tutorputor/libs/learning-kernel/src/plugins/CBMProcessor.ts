 ...
 */
const DEFAULT_CBM_CONFIG: CBMConfig = (() => {
  const config = CANONICAL_CBM_SCORING;
  if (
    !config ||
    Object.keys(config).length === 0 ||
    config.correctHighConfidence === undefined
  ) {
    console.warn(
      "WARNING: CANONICAL_CBM_SCORING import failed or is empty. Using local fallback values.",
    );
    return {
...
export class CBMProcessor implements EvidenceProcessor {
  readonly metadata: PluginMetadata = {
    ...
    description:
      "Processes assessment submissions with confidence-weighted scoring and calibration metrics",
    author: "TutorPutor Core Team",
    tags: ["assessment", "confidence", "calibration", "mastery", "cbm"],
    enabled: true,
  };
    ...
    try {
      ...
      if (isCorrect === undefined || confidenceValue === undefined) {
        ...
        if (isCorrect !== undefined) {
          context.data[`cbm.pending.${evidence.evidenceId}.isCorrect`] =
            isCorrect;
        }
        if (confidenceValue !== undefined) {
          context.data[`cbm.pending.${evidence.evidenceId}.confidence`] =
            confidenceValue;
        }
  ...
  private calculateCBM(isCorrect: boolean, confidenceValue: number): CBMResult {
    ...
    // Safety check if config values are undefined
    if (score === undefined) {
      console.error(
        "CBM Score is undefined! Config:",
  ...
  private updateAggregates(
    ...
    const existing = (context.data[sessionKey] as CBMItemList | undefined) ?? {
      items: [],
    };
    ...
    if (
      aggregate.itemCount >= 3 &&
      Math.abs(aggregate.calibrationIndex) > 0.3
    ) {
      context.data["cbm.trigger.viva"] = {
        reason:
          aggregate.calibrationIndex > 0 ? "overconfidence" : "underconfidence",
