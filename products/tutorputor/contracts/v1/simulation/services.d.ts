...
import type { SimulationId, SimulationManifest, SimKeyframe, SimulationDomain, SimulationRunRequest, SimulationRunResult, GenerateManifestRequest, GenerateManifestResult, RefineManifestRequest, SuggestParametersRequest, SuggestParametersResult, ManifestValidationResult, SimulationAnalyticsEvent, SimulationSessionId } from "./types";
/**
...
export interface SimulationAuthorService {
     ...
     */
    validateManifest(manifest: SimulationManifest): Promise<ManifestValidationResult>;
}
/**
...
export interface DiscreteKernel extends SimulationKernel {
     ...
     */
    interpolateKeyframes(keyframes: SimKeyframe[], targetFps: number): SimKeyframe[];
}
/**
...
export interface PhysicsKernel extends SimulationKernel {
     ...
     */
    runDeterministic(request: SimulationRunRequest, seed: number): Promise<SimulationRunResult>;
}
/**
...
export interface SystemDynamicsKernel extends SimulationKernel {
     ...
     */
    runWithIntegration(request: SimulationRunRequest, method: "euler" | "rk4"): Promise<SimulationRunResult>;
    /**
...
export interface ChemistryKernel extends SimulationKernel {
     ...
     */
    validateReaction(reactants: string[], products: string[]): {
        valid: boolean;
...
export interface BiologyKernel extends SimulationKernel {
     ...
     */
    simulateDiffusion(compartments: Array<{
        id: string;
        concentration: number;
        volume: number;
    }>, permeability: number, timeStep: number): Array<{
        id: string;
        concentration: number;
    }>;
    /**
     ...
     */
    simulateEnzymeKinetics(substrate: number, enzyme: number, km: number, vmax: number, time: number): {
        substrateRemaining: number;
...
export interface MedicineKernel extends SimulationKernel {
     ...
     */
    runOneCompartmentPK(dose: number, volume: number, ke: number, timePoints: number[]): Array<{
        time: number;
        concentration: number;
    }>;
    /**
     ...
     */
    runTwoCompartmentPK(dose: number, v1: number, v2: number, k12: number, k21: number, ke: number, timePoints: number[]): Array<{
        time: number;
        c1: number;
        c2: number;
    }>;
     ...
     * Run Emax PD model.
     */
    runEmaxPD(concentrations: number[], emax: number, ec50: number, hill?: number): Array<{
        concentration: number;
        effect: number;
    }>;
    /**
     ...
     */
    runEpidemiologyModel(model: "sir" | "seir", population: number, initialInfected: number, beta: number, gamma: number, days: number, sigma?: number): Array<{
        day: number;
        susceptible: number;
        exposed?: number;
        infected: number;
...
export interface SimulationRuntimeService {
     ...
     */
    getCachedKeyframes(simulationId: SimulationId): Promise<SimKeyframe[] | null>;
    /**
...
export interface SimulationRendererService {
     ...
     */
    initialize(keyframes: SimKeyframe[], config: {
        canvasWidth: number;
        canvasHeight: number;
        targetFps?: number;
    }): void;
