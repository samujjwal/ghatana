...
public class DefaultMemoryTierRouter<R extends DataRecord> implements MemoryTierRouter<R> {
    ...
    public DefaultMemoryTierRouter(Map<MemoryTier, TierPolicy> customPolicies) {
        this();
        Objects.requireNonNull(customPolicies, "Custom policies must not be null");
        for (Map.Entry<MemoryTier, TierPolicy> entry : customPolicies.entrySet()) {
            MemoryTier tier = Objects.requireNonNull(entry.getKey(), "Tier must not be null");
            TierPolicy policy = Objects.requireNonNull(entry.getValue(), "Policy must not be null");
    ...
    private String extractTenantId(R record) {
        ...
        // Fall back to metadata for records that are not fully populated yet.
