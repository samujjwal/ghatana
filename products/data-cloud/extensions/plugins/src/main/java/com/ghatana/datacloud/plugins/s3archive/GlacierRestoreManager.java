...
public class GlacierRestoreManager {
        ...
        try {
            ...
            if (!isGlacierStorageClass(storageClass)) {
                log.info("Object {} is not in Glacier (storage class: {}), no restore needed",
                        key, storageClass);
                return Promise.of(new RestoreResult(key, RestoreStatus.ALREADY_AVAILABLE, null));
            }
            ...
            if (headResponse.restore() != null && headResponse.restore().contains("ongoing-request=\"true\"")) {
                ...
                return Promise.of(new RestoreResult(key, RestoreStatus.IN_PROGRESS, null));
            }
            ...
            if (headResponse.restore() != null && headResponse.restore().contains("ongoing-request=\"false\"")) {
                ...
                return Promise.of(new RestoreResult(key, RestoreStatus.COMPLETED, parseRestoreExpiry(headResponse.restore())));
            }
            ...
            return Promise.of(new RestoreResult(key, RestoreStatus.INITIATED, null));
        } catch (Exception e) {
    ...
    public Promise<Boolean> isRestoreComplete(String key) {
        try {
            ...
            if (restore == null) {
                ...
                StorageClass storageClass = headResponse.storageClass();
                return Promise.of(!isGlacierStorageClass(storageClass));
            }
    ...
    public Promise<RestoreResult> getRestoreStatus(String key) {
        try {
            ...
            if (restore == null) {
                return Promise.of(new RestoreResult(key, RestoreStatus.NOT_STARTED, null));
            }
            ...
            if (restore.contains("ongoing-request=\"false\"")) {
                ...
                return Promise.of(new RestoreResult(key, RestoreStatus.COMPLETED, expiry));
            }

            return Promise.of(new RestoreResult(key, RestoreStatus.UNKNOWN, null));
        } catch (Exception e) {
            return Promise.ofException(e);
        }
    }
