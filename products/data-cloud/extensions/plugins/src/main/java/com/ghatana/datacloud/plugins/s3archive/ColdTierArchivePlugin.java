...
public class ColdTierArchivePlugin implements StoragePlugin {
    ...
    public Promise<HealthStatus> healthCheck() {
        ...
        try {
            s3Client.headBucket(HeadBucketRequest.builder()
                    ...
                    .build());
            return Promise.of(HealthStatus.ok("S3 bucket accessible: " + config.getBucketName()));
        } catch (Exception e) {
            log.warn("Health check failed: {}", e.getMessage());
            return Promise.of(HealthStatus.error("S3 health check failed", e));
        }
    }
    ...
    public Promise<ArchiveResult> archiveBatch(List<Event> events) {
        ...
        try {
            ...
            return Promise.of(new ArchiveResult(archiveKey, events.size(), compressed.length));

        } catch (Exception e) {
            log.error("Failed to archive batch of {} events: {}", events.size(), e.getMessage());
            return Promise.ofException(e);
        } finally {
