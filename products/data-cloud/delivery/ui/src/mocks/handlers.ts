...
const mockCollections = MOCK_COLLECTIONS.map((c) => ({
  ...
  schema: {
    ...
    constraints: c.schema.constraints.map(
      (con) => ({ ...con }) as Record<string, unknown>,
    ),
  },
  tags: [] as string[],
...
function buildCollectionCostReport(collection: (typeof collections)[0]) {
  const totalSizeGb = Math.max(
    0.25,
    Number((collection.entityCount / 50_000).toFixed(2)),
  );
...
const collectionHandlers = [
  ...
  http.post(`${BASE}/entities/dc_collections`, async ({ request }) => {
    ...
    const existingIdx = body.id
      ? collections.findIndex((c) => c.id === body.id)
      : -1;
    if (existingIdx >= 0) {
      const updated = {
  ...
  http.get(`${BASE}/lineage/:collection`, async ({ params, request }) => {
    ...
    const direction =
      new URL(request.url).searchParams.get("direction") ?? "BOTH";
...
const pluginHandlers = [
  ...
  http.get(`${BASE}/plugins/:id`, async ({ params }) => {
    ...
    if (!plugin) {
      return HttpResponse.json(
        { code: "NOT_FOUND", message: "Plugin not found" },
        { status: 404 },
      );
    }
  ...
  http.post(`${BASE}/plugins/:id/enable`, async ({ params }) => {
    ...
    plugins = plugins.map((plugin) =>
      plugin.id === params.id
        ? { ...plugin, status: "enabled" as const }
        : plugin,
    );
  ...
  http.post(`${BASE}/plugins/:id/disable`, async ({ params }) => {
    ...
    plugins = plugins.map((plugin) =>
      plugin.id === params.id
        ? { ...plugin, status: "disabled" as const }
        : plugin,
    );
    return HttpResponse.json({ success: true });
  }),
  ...
  http.post(`${BASE}/plugins/:id/upgrade`, async ({ params }) => {
    ...
    plugins = plugins.map((plugin) =>
      plugin.id === params.id
        ? {
            ...plugin,
            version: plugin.version === "1.0.0" ? "1.0.1" : plugin.version,
          }
        : plugin,
    );
...
const storageFabricHandlers = [
  ...
  http.put(
    `${BASE}/data-fabric/profiles/:profileId`,
    async ({ params, request }) => {
      ...
      storageProfiles = storageProfiles.map((p, i) =>
        i === idx ? updated : p,
      );
  ...
  http.post(
    `${BASE}/data-fabric/profiles/:profileId/set-default`,
    async ({ params }) => {
      await delay(SIMULATED_DELAY_MS);
      storageProfiles = storageProfiles.map((p) => ({
        ...p,
        isDefault: p.id === params.profileId,
      }));
      const profile = storageProfiles.find((p) => p.id === params.profileId);
      if (!profile) {
        return HttpResponse.json(
...
const connectorHandlers = [
  ...
  http.get(
    `${BASE}/data-fabric/connectors/:connectorId`,
    async ({ params }) => {
      await delay(SIMULATED_DELAY_MS);
      const connector = connectors.find((c) => c.id === params.connectorId);
      if (!connector) {
        return HttpResponse.json(
          { code: "NOT_FOUND", message: "Connector not found" },
          { status: 404 },
        );
      }
  ...
  http.get(
    `${BASE}/data-fabric/connectors/:connectorId/statistics`,
    async () => {
      await delay(SIMULATED_DELAY_MS);
      return HttpResponse.json({
...
const runtimeCapabilitySchema = {
  ...
  metadata: {
    description:
      "Runtime capability schema for Data Cloud UI gating and compatibility checks",
        ...
        queries: [
          {
            ...
            template:
              "SELECT date_trunc('day', created_at) AS day, SUM(total) FROM revenue GROUP BY 1",
            explanation:
      ...
      nodes: [
        {
          ...
          status: "completed",
          startTime: new Date(Date.now() - 60 * 1000).toISOString(),
          endTime: new Date(Date.now() - 55 * 1000).toISOString(),
          duration: 5,
        },
        {
          id: "node-2",
          name: "End",
          status: "completed",
          startTime: new Date(Date.now() - 55 * 1000).toISOString(),
          endTime: new Date().toISOString(),
          duration: 55,
        },
      ],
    ...
    return HttpResponse.json([
      {
        ...
        level: "info",
        message: `Execution ${params.executionId} started`,
      },
      {
        timestamp: new Date().toISOString(),
        level: "info",
        message: `Execution ${params.executionId} completed`,
      },
    ]);
  }),
