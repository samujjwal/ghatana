...
export function useEventLogStream(
  options: UseEventLogStreamOptions,
): UseEventLogStreamReturn {
  const {
    serverUrl,
    tenantId,
    authToken,
    autoConnect = true,
    metricsRefreshInterval: _metricsRefreshInterval = 5000,
    initialNodes = [],
    initialEdges = [],
  } = options;
  ...
  }, [isConnected, nodes.length, requestSync]);

  return {
    nodes,
    edges,
    connectionState:
      connectionState as UseEventLogStreamReturn["connectionState"],
    error,
    connect,
    disconnect,
    requestSync,
...
export function useStreamMetrics(
  streamId: string,
  options: Pick<
    UseEventLogStreamOptions,
    "serverUrl" | "tenantId" | "authToken"
  >,
): {
  ...
  const { subscribe, isConnected, error } =
    useActiveJStream<MetricsUpdateEvent>(
      normalizedServerUrl,
      options.tenantId,
      "/eventlog/metrics",
      {
