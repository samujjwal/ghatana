...
export function DashboardKPI({
  ...
  const content = (
    <div
      className={`flex items-center justify-between ${colorClass} ${className}`}
    >
      ...
      {secondaryValue && (
        <div className="text-right">
          <p className="text-sm text-gray-900">{secondaryValue}</p>
        </div>
      )}
    </div>
  );
  ...
  if (link) {
    return (
      <Link
        to={link}
        className="block hover:bg-gray-50 transition-colors duration-150 -mx-6 px-6 py-4"
      >
        {content}
      </Link>
    );
