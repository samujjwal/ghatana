...
const suggestedPrompts = [
  {
    ...
    text: "Convert to SQL: Show me all users who signed up last week",
  },
  {
    icon: <Database className="h-4 w-4" />,
    text: "Explain the schema for user_events collection",
  },
  {
    icon: <GitBranch className="h-4 w-4" />,
    text: "Show lineage for the transactions table",
  },
  {
    icon: <Search className="h-4 w-4" />,
    text: "Find collections related to customer data",
  },
];
...
async function generateResponse(userMessage: string): Promise<ChatMessage> {
  ...
  if (
    ...
    const tableMatch =
      lowerMessage.match(/from ([a-z_]+)/) ||
      lowerMessage.match(/in ([a-z_]+)/);
  ...
  if (lowerMessage.includes("schema") || lowerMessage.includes("explain")) {
    return {
      ...
      role: "assistant",
      content:
        "Open the Data Fabric explorer and select a collection to inspect its live schema.",
      type: "text",
      timestamp: new Date(),
  ...
  if (lowerMessage.includes("lineage")) {
    return {
      ...
      role: "assistant",
      content:
        "Open the lineage preview in Data Explorer to review the current placeholder lineage experience. Live lineage APIs are not exposed by this launcher yet.",
      type: "lineage",
      timestamp: new Date(),
      metadata: { confidence: 0.2 },
    };
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            "Failed to process your request. Please ensure the analytics service is reachable.",
          type: "error",
          timestamp: new Date(),
        },
      ]);
              ...
              >
                ...
                {message.metadata?.sql && (
                  <div className="mt-2 rounded bg-gray-900 p-3 relative">
                    <button
                      onClick={() =>
                        handleCopy(message.metadata!.sql!, message.id)
                      }
