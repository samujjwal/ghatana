...
async function generateSQLFromNL(
  ...
  if (lowerQuery.includes("count") || lowerQuery.includes("how many")) {
    ...
    optimizations = [
      'Consider adding an index on the "active" column for faster counts.',
    ];
  } else if (
    lowerQuery.includes("last") &&
    (lowerQuery.includes("week") || lowerQuery.includes("7 days"))
  ) {
    sql = `SELECT *\nFROM ${collectionName}\nWHERE created_at >= NOW() - INTERVAL '7 days'\nORDER BY created_at DESC\nLIMIT 100;`;
    confidence = 0.92;
    explanation =
      "Fetching records created in the last 7 days, ordered by most recent.";
    optimizations = [
    ...
    const field = lowerQuery.includes("revenue")
      ? "revenue"
    ...
    optimizations = [
      "Composite index on (category, amount) will optimize this query.",
    ];
  } else {
    sql = `SELECT *\nFROM ${collectionName}\nWHERE 1=1\n-- Add your conditions here\nLIMIT 100;`;
    confidence = 0.7;
    explanation =
      "Generated a basic query template. Please refine your natural language description for better results.";
  }
...
export function SmartSQLAssistant({
  ...
  const _getConfidenceColor = (confidence: number): string => {
  ...
  return (
    <div className={cn(cardStyles.base, "overflow-hidden", className)}>
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          <label
            htmlFor="smart-sql-assistant-query"
          ...
          <div className="relative">
            <textarea
              ...
              className={cn(inputStyles.base, "min-h-[80px] resize-none pr-12")}
          ...
          <p className={cn(textStyles.xs, "mt-1")}>Press ⌘+Enter to generate</p>
        </div>
        ...
        {suggestion && (
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                {suggestion.estimatedCost && (
                  <span
                    className={cn(textStyles.xs, "flex items-center gap-1")}
                  >
                    <TrendingUp className="h-3 w-3" />~
                    {suggestion.estimatedCost.rows} rows,{" "}
                    {suggestion.estimatedCost.executionTimeMs}ms
                  </span>
                )}
              </div>
            </div>
            ...
            {suggestion.optimizations &&
              suggestion.optimizations.length > 0 && (
                <div className="p-3 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
                  <p
                    className={cn(
                      textStyles.small,
                      "font-medium text-yellow-800 dark:text-yellow-200 mb-1",
                    )}
                  >
