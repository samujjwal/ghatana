...
export function OperationHistory({
  ...
  return (
    <div className={cn('flex flex-col gap-3', className)} data-testid="operation-history">
      <div className="flex items-center justify-between">
        <span className={cn(textStyles.xs, 'font-semibold uppercase tracking-wide text-gray-500')}>
          Operation History
        </span>
      ...
      {visible.length === 0 ? (
        ...
        <ol className="relative border-l border-gray-200 dark:border-gray-700 pl-4 space-y-4">
          {visible.map((record) => (
            <li key={record.id} className="flex flex-col gap-0.5" data-testid="operation-history-entry">
              ...
              {record.detail && (
                <span className="text-xs text-red-600 dark:text-red-400">{record.detail}</span>
              )}
              <div className="flex items-center gap-2 mt-0.5">
                <span
                  className={cn(
                    'text-[10px] font-medium uppercase tracking-wide',
                    record.outcome === 'success' && 'text-green-600 dark:text-green-400',
                    record.outcome === 'failure' && 'text-red-600 dark:text-red-400',
                    record.outcome === 'pending' && 'text-amber-600 dark:text-amber-400',
                  )}
                >
                ...
                {record.user && (
                  <span className="text-[10px] text-gray-400">by {record.user}</span>
                )}
              </div>
            </li>
          ))}
