...
import { FlowCanvas } from "@ghatana/canvas/flow";
 ...
 */
const EventLogNodeComponent = memo(function EventLogNodeComponent(props: {
  data: EventCloudNodeData;
  selected?: boolean;
}) {
  const { data, selected: _selected } = props;
 ...
 */
const EventLogEdgeComponent = memo(function EventLogEdgeComponent(
  props: Record<string, unknown>,
) {
  return (
    <BaseTopologyEdge
      {...(props as unknown as React.ComponentProps<typeof BaseTopologyEdge>)}
      pathType="smoothstep"
      showThroughput={true}
    />
  );
      ...
      >
        ...
        >
          ...
          <p className={cn(textStyles.muted, "mt-2")}>
            Configure stream sources, processors, and sinks to visualize your
            data flow.
          </p>
        </div>
      </div>
    ...
    <div className={cn("h-full w-full", className)}>
      ...
      <div className="absolute bottom-2 left-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-2 z-10">
        <div className="flex items-center gap-3 text-xs">
          {(Object.entries(STATUS_COLORS) as [string, { dot: string }][])
            .slice(0, 4)
            .map(([status, colors]) => (
