...
import { AutonomyState, brainService } from "../../api/brain.service";
...
const DEFAULT_DOMAINS = [
  {
    ...
    name: "Data Ingestion",
    icon: Database,
    description: "Automatic data pipeline scaling",
  },
  {
    id: "quality",
    name: "Data Quality",
    icon: CheckCircle,
    description: "Quality issue detection and fixes",
  },
  {
    id: "security",
    name: "Security",
    icon: Shield,
    description: "Access control and PII handling",
  },
  {
    id: "optimization",
    name: "Optimization",
    icon: Activity,
    description: "Query and cost optimization",
  },
];
...
export function AutonomyControl({
  ...
  const domainStates = useQueries({
    queries: activeDomains.map((domainId) => ({
  ...
  const updatePolicy = useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["autonomy-state", variables.domain],
      });
            ...
            >
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {state && (
                    <div
                      className={`px-3 py-1 rounded-full text-xs font-semibold border-2 ${getModeColor(
                        state.mode,
                      )}`}
                    >
                      {state.mode}
              ...
              {isExpanded && state && (
                <div className="p-4 space-y-4 bg-white">
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <button
                      onClick={() =>
                        handleEnabledToggle(domain.id, !state.enabled)
                      }
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {AUTONOMY_MODES.map((modeOption) => (
                        <button
                          key={modeOption.mode}
                          onClick={() =>
                            handleModeChange(domain.id, modeOption.mode)
                          }
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label
                        htmlFor={`autonomy-confidence-${domain.id}`}
                    ...
                    <input
                      id={`autonomy-confidence-${domain.id}`}
                      type="range"
                      min="50"
                      max="100"
                      step="5"
                      value={Math.round(
                        (state.confidenceThreshold || 0.8) * 100,
                      )}
                      onChange={(e) =>
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-orange-800">
                            Full autonomy in security domain requires careful
                            monitoring. Consider using CHATTY mode for sensitive
                            operations.
                          </div>
                        </div>
                      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-blue-800">
                Control is not binary. Slide between <strong>Advisory</strong>{" "}
                (human approval), <strong>Chatty</strong> (act with
                notification), and <strong>Autonomous</strong> (self-driving)
                modes per domain. Confidence thresholds ensure safety.
              </p>
            </div>
          </div>
        </div>
