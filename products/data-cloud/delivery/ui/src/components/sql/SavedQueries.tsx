...
export function SavedQueries({
  ...
  const isSampleData = queries.every((q) =>
    SAMPLE_QUERIES.some((s) => s.id === q.id),
  );
  ...
  return (
    <div className={cn("flex flex-col h-full", className)}>
      ...
      <div className="p-3 border-b border-gray-200 dark:border-gray-700">
        ...
        {isSampleData && (
          <div
            className="mb-2 flex items-start gap-2 rounded-md bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-3 py-2"
            role="note"
          >
            <span className="mt-0.5 text-blue-500 text-xs">{"\u2139"}</span>
            <p className="text-xs text-blue-700 dark:text-blue-300">
              <span className="font-semibold">Sample queries</span> — these are
              examples only. Your saved queries will appear here once query
                  ...
                  >
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setActiveMenu(
                          activeMenu === query.id ? null : query.id,
                        );
                      }}
