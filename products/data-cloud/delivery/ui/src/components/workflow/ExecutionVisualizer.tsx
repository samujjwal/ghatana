...
export function ExecutionVisualizer({
  ...
  return (
    <div className={cn(cardStyles.base)}>
      {/* Header */}
      <div
        className={cn(cardStyles.header, "flex items-center justify-between")}
      >
        <div>
        ...
        <div className="flex items-center gap-2">
          ...
          {isRunning && onCancel && (
            <button
              onClick={onCancel}
              className={cn(buttonStyles.danger, buttonStyles.sm)}
            >
          ...
          {execution.nodes.map((node, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {/* Records */}
                {(node.inputRecords !== undefined ||
                  node.outputRecords !== undefined) && (
                  <p className={cn(textStyles.xs, "mt-1")}>
                    {node.inputRecords !== undefined &&
                      `In: ${node.inputRecords.toLocaleString()} `}
                    {node.outputRecords !== undefined &&
                      `Out: ${node.outputRecords.toLocaleString()}`}
                  </p>
                )}
