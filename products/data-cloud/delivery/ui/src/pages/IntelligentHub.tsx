 ...
 */
function _QuickActionCard({ action }: { action: QuickAction }) {
  return (
    <button
...
export function classifyOutcomeIntent(query: string): OutcomeIntent {
  ...
  if (
    /(workflow|pipeline|automate|sync|ingest|etl|approval)/.test(normalized)
  ) {
    return {
      kind: "workflow",
  ...
  if (
    /(trust|policy|retention|pii|privacy|redact|purge|governance|compliance|audit)/.test(
      normalized,
    )
  ) {
    return {
  ...
  if (
    /(failure|failed|incident|alert|outage|latency|degraded|broken)/.test(
      normalized,
    )
  ) {
    return {
...
function _buildQuickActions(
  ...
  logActivityMutation: {
    mutate: (data: Parameters<typeof logActivity>[0]) => void;
  },
  shellRole: ShellRole,
): QuickAction[] {
...
export function IntelligentHub(): React.ReactElement {
  ...
  const _shellRole = SessionBootstrap.getShellRole();
    ...
    ...(persona !== "primary"
      ? [
          ...
          },
          {
            ...
            value:
              brainStats?.totalRecordsProcessed ?? ("–" as string | number),
  ...
  const outcomeActions: OutcomeAction[] = useMemo(() => {
    const actions: OutcomeAction[] = [
      {
        ...
        description:
          "Start from natural language and land in the SQL workspace.",
      ...
      actions.push(
        {
          ...
          description:
            "Jump straight to retention, privacy, and audit actions.",
          icon: <Shield className="h-5 w-5 text-amber-500" />,
          supportingText:
            "Trust Center is the canonical place for policy actions and audit visibility.",
    ...
    if (persona === "admin") {
      actions.push({
        ...
        supportingText:
          "Admin workspace with privileged controls and blast-radius awareness.",
  ...
  const handleAskAnything = useCallback(
    (query: string) => {
      ...
      navigate(
        intent.destination,
        intent.state ? { state: intent.state } : undefined,
      );
    },
    ...
    <div className="flex h-full" data-testid="intelligent-hub-page">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header
          ...
          data-testid="intelligent-hub-header"
        >
          <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div
              className="hidden sm:flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1"
            ...
            >
              {(
                [
                  {
                    id: "primary" as Persona,
                    label: "Standard",
                    icon: <User className="h-3.5 w-3.5" />,
                  },
                  {
                    id: "operator" as Persona,
                    label: "Operator",
                ...
                ] as { id: Persona; label: string; icon: React.ReactNode }[]
              ).map((p) => (
                <button
                  key={p.id}
        ...
        <section className="flex-1 overflow-y-auto p-6">
          ...
          {continueWorking.length > 0 && (
            <section
              className="mb-8"
              data-testid="intelligent-hub-continue-working"
            >
          ...
          <section
            className="mb-8"
            data-testid="intelligent-hub-outcome-section"
          >
            <div className="mb-4">
