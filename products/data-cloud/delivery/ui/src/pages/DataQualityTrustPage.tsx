...
export function DataQualityTrustPage(): ReactElement {
  ...
  return (
    <main
      ...
      data-testid="data-quality-trust-page"
    >
      <div className="mx-auto max-w-6xl">
        <header className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Canonical Data Plane trust contract derived from collection registry
            metadata.
          </p>
        </header>
        ...
        {isError && (
          <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 dark:border-rose-900/40 dark:bg-rose-900/10 dark:text-rose-300">
            {error instanceof Error
              ? error.message
              : t("error.failedToLoadTrustScores")}
          </div>
        )}
        ...
        {data && (
          <section className="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
            <div className="border-b border-gray-100 px-4 py-3 text-xs text-gray-500 dark:border-gray-800 dark:text-gray-400">
              {data.count} collections · Snapshot{" "}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900/60">
                    <th className="px-4 py-2 text-left text-xs uppercase tracking-wide text-gray-500">
                      Collection
                    </th>
                    <th className="px-4 py-2 text-left text-xs uppercase tracking-wide text-gray-500">
                      Trust
                  ...
                  {data.scores.map((score) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        <ScoreBadge score={score.trustScore} />
                      </td>
                      <td className="px-4 py-2 text-gray-600 dark:text-gray-300">
                        {Math.round(score.qualityScore * 100)}%
                      </td>
                      <td className="px-4 py-2 text-gray-600 dark:text-gray-300">
                        {score.lifecycleStatus}
                      </td>
                      <td className="px-4 py-2 text-gray-600 dark:text-gray-300">
                        {score.operationalStatus}
                      </td>
                    </tr>
                  ))}
                </tbody>
