 ...
 */
const DATA_EXPLORER_VIEW_MODES = [
  "table",
  "lineage",
  "quality",
  "schema",
...
const advisoryUnavailable: QualityAdvisory = {
  ...
  confidence: 0.5,
  evidence: [
    "No advisory payload returned by /api/v1/action/ai/quality-advisories.",
  ],
};
...
function advisoryFromBackend(payload: AiQualityAdvisory): QualityAdvisory {
  const highestImpact = [...payload.advisories]
    ...
    .slice(0, 3);
  const topConfidence = payload.advisories.reduce(
  ...
  const evidence = highestImpact.map(
    (item) =>
...
function StatusBadge({ status }: { status: Collection["status"] }) {
  ...
  return (
    <span
      className={cn("inline-flex items-center gap-1 text-xs", colors[status])}
    >
 ...
 */
function OperationalStatusDot({
  status,
}: {
  status: Collection["operationalStatus"];
}) {
        ...
        <div className="flex items-center gap-3 mt-1 text-[11px] text-gray-500 dark:text-gray-400">
          ...
          {collection.qualityScore != null && (
            <span className="flex items-center gap-1">
              ...
              <strong
                className={
                  collection.qualityScore >= 0.8
                    ? "text-emerald-600"
                    : collection.qualityScore >= 0.5
                      ? "text-amber-600"
                      : "text-red-600"
                }
              >
                {collection.qualityScore.toFixed(2)}
              </strong>
            </span>
          )}
...
export function DataExplorer() {
  ...
  useEffect(() => {
    if (collectionIdFromRoute && collections.length > 0) {
      const collection = collections.find(
        (c) => c.id === collectionIdFromRoute,
      );
      if (
        collection &&
        (!selectedCollection || selectedCollection.id !== collectionIdFromRoute)
      ) {
  ...
  const { data: backendQualityAdvisory } = useQuery({
    ...
    queryFn: () =>
      aiOperationsService.getQualityAdvisories(advisoryCollectionId as string),
  ...
  return (
    <section
      className="p-6"
      data-testid="data-explorer-page"
    ...
    >
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Explore and manage your data collections with quality insights and
          lineage context
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <label htmlFor="collection-status-filter" className="sr-only">
          Filter by status
        </label>
        <select
          ...
          value={statusFilter}
          onChange={(e) =>
            setStatusFilter(e.target.value as Collection["status"] | "all")
          }
        ...
        <label htmlFor="collection-schema-filter" className="sr-only">
        ...
        <select
          ...
          onChange={(e) =>
            setSchemaTypeFilter(
              e.target.value as Collection["schemaType"] | "all",
            )
          }
          className="py-2 pl-3 pr-8 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-white"
        >
        ...
        <label htmlFor="collection-sort-field" className="sr-only">
          Sort by
        </label>
      ...
      <div
        ...
        data-testid="data-explorer-view-toggle"
      >
        ...
        <div className="flex gap-1">
          {viewModes.map((mode) => (
            <button
              ...
              title={
                mode.id === "quality" && !QUALITY_VIEW_ENABLED
                  ? "Quality mode is temporarily unavailable in this deployment"
                  : undefined
              }
            >
              {mode.icon}
        ...
        {collectionsPage && (
          <span className="ml-auto text-xs text-gray-500">
            {collectionsPage.total} collection
            {collectionsPage.total !== 1 ? "s" : ""}
          </span>
        )}
      </div>
            ...
            <CollectionRow
              ...
              onArchive={async () => {
                try {
                  ...
                  toast.success(
                    `Collection "${collection.name}" archived successfully`,
                  );
                  await refetch();
                } catch (error) {
                  toast.error(
                    `Failed to archive collection: ${error instanceof Error ? error.message : "Unknown error"}`,
                  );
                }
              }}
              viewMode={viewMode}
      ...
      {selectedCollection && (
        <div
          className="mt-8 p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg"
        ...
        >
          ...
          <Suspense
            fallback={<LazyLoading message="Loading detailed view..." />}
          >
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
                    ...
                    <p className="text-2xl font-bold mt-1 text-gray-900 dark:text-white">
                      {selectedCollection.qualityScore != null
                        ? selectedCollection.qualityScore.toFixed(2)
                        : "N/A"}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
                    ...
                    <p className="text-lg font-semibold mt-1 text-gray-900 dark:text-white">
                      {selectedCollection.lifecycleStatus}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
                    <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                    ...
                    <p className="text-lg font-semibold mt-1 text-gray-900 dark:text-white capitalize">
                      {selectedCollection.operationalStatus}
                    </p>
                  </div>
                </div>

                {/* Quality metrics breakdown */}
                {selectedCollection.qualityMetrics &&
                  Object.keys(selectedCollection.qualityMetrics).length > 0 && (
                    <div>
                      <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-3">
                        Quality Metrics
                      </h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        {Object.entries(selectedCollection.qualityMetrics).map(
                          ([key, value]) => (
                            <div
                              key={key}
                ...
                {selectedCollection.retentionPolicy && (
                  <DataSummary
                    ...
                    data={
                      selectedCollection.retentionPolicy as Record<
                        string,
                        unknown
                      >
                    }
                ...
                {!selectedCollection.qualityScore &&
                  !selectedCollection.qualityMetrics && (
                    <p className="text-sm text-gray-500">
                      No quality metrics computed for this collection yet. Run
                      schema inference or the quality pipeline to populate
                      metrics.
                    </p>
                  )}
              </div>
            ...
            {viewMode === "lineage" && (
              <div
                className="space-y-4"
                data-testid="data-explorer-lineage-panel"
              >
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p
                      className="font-medium text-gray-700 dark:text-gray-300"
                      data-testid="lineage-preview-title"
                    >
                      Lineage preview
                    </p>
                    <p className="text-sm mt-2 text-gray-500">
                      Live upstream and downstream lineage from the canonical
                      launcher route.
                    </p>
                  </div>
                  {impactAnalysis && (
                    <div className="text-right text-sm text-gray-600 dark:text-gray-300">
                      <p>
                        Impact level:{" "}
                        <strong>
                          {impactAnalysis.affectedDatasets > 1
                            ? "HIGH"
                            : impactAnalysis.affectedDatasets === 1
                              ? "MEDIUM"
                              : "LOW"}
                      ...
                      <p>
                        Affected datasets:{" "}
                        <strong>{impactAnalysis.affectedDatasets}</strong>
                      </p>
                    </div>
                  )}
                </div>
                ...
                {lineageLoading && (
                  <LazyLoading message="Loading lineage graph..." />
                )}
                  ...
                  <>
                    ...
                    {impactAnalysis && impactAnalysis.details.length > 0 && (
                      <div
                        ...
                        data-testid="lineage-impact-panel"
                      >
