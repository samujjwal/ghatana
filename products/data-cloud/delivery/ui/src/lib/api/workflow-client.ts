...
export class WorkflowApiClient {
  ...
  private pipelineToWorkflowDefinition(
    pipeline: Pipeline,
    fallbackCollectionId?: string,
  ): WorkflowDefinition {
    const extras = pipeline as Record<string, unknown>;
    const rawCollectionId =
      typeof extras.collectionId === "string" ? extras.collectionId.trim() : "";
    ...
    return {
      ...
      updatedBy:
        typeof updatedBy === "string"
          ? updatedBy
          : (pipeline.createdBy ?? this.userId),
      createdAt: pipeline.createdAt ?? new Date().toISOString(),
      updatedAt:
        pipeline.updatedAt ?? pipeline.createdAt ?? new Date().toISOString(),
    };
  }
  ...
  async listWorkflows(
    ...
    const rawResponse = await apiClient.get<PipelineListResponse>(
      "/action/pipelines",
      {
    ...
    return {
      workflows: response.pipelines.map((pipeline) =>
        this.pipelineToWorkflowDefinition(pipeline, collectionId),
      ),
  ...
  async getWorkflow(workflowId: string): Promise<WorkflowDefinition> {
    const rawResponse = await apiClient.get<Pipeline>(
  ...
  async createWorkflow(
    ...
    const rawResponse = await apiClient.post<Pipeline>(
      "/action/pipelines",
      request,
      {
        headers: this.getHeaders(),
  ...
  async getExecutionStatus(executionId: string): Promise<WorkflowExecution> {
    ...
    return {
      ...
      ) as WorkflowExecution["status"],
      progress:
        totalNodes > 0 ? Math.round((completedNodes / totalNodes) * 100) : 100,
      startedAt: String(response.startTime ?? new Date().toISOString()),
      completedAt:
        typeof response.endTime === "string" ? response.endTime : undefined,
      duration:
      ...
      nodeStatuses: nodes.map((node) => {
        ...
        return {
          ...
          state: String(
            typedNode.status ?? "PENDING",
          ) as WorkflowExecution["nodeStatuses"][number]["state"],
          startedAt:
            typeof typedNode.startTime === "string"
              ? typedNode.startTime
              : undefined,
          completedAt:
            typeof typedNode.endTime === "string"
              ? typedNode.endTime
              : undefined,
          duration:
            typeof typedNode.duration === "number"
              ? typedNode.duration
              : undefined,
          error:
            typeof typedNode.error === "string" && typedNode.error !== ""
              ? typedNode.error
              : undefined,
  ...
  async cancelExecution(executionId: string): Promise<void> {
    await apiClient.post(
      `/action/executions/${executionId}/cancel`,
      {},
      {
        headers: this.getHeaders(),
      },
