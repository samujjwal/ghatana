...
export function useDataCloudPlugins(options?: {
  ...
  void options;
  const installed = useMemo<DataCloudPlugin[]>(() => [], []);
  const marketplace = useMemo<DataCloudPlugin[]>(() => [], []);
  const healthMap = useMemo<Record<string, PluginHealth>>(() => ({}), []);
...
export function usePluginConfiguration(pluginId: string | undefined) {
  ...
  return {
    plugin,
    config: (plugin?.configSchema ? {} : undefined) as
      | Record<string, unknown>
      | undefined, // Current config would come from instance
...
const DataCloudPluginContext =
  createContext<DataCloudPluginContextValue | null>(null);
...
export function useDataCloudPluginContext() {
  ...
  if (!context) {
    throw new Error(
      "useDataCloudPluginContext must be used within DataCloudPluginProvider",
    );
  }
  return context;
}
