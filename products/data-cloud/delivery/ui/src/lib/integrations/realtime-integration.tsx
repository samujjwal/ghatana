  ...
  const filteredData = useMemo(() => {
    ...
    if (options?.filter?.entityTypes?.length) {
      result = result.filter(
        (e) =>
          e.entityType && options.filter!.entityTypes!.includes(e.entityType),
      );
    }

    if (options?.filter?.severities?.length) {
      result = result.filter(
        (e) => e.severity && options.filter!.severities!.includes(e.severity),
      );
    }
  ...
  const filteredData = useMemo(() => {
    ...
    if (options?.entityTypes?.length) {
      result = result.filter((e) =>
        options.entityTypes!.includes(e.entityType),
      );
    }

    if (options?.changeTypes?.length) {
      result = result.filter((e) =>
        options.changeTypes!.includes(e.changeType),
      );
    }
...
const DataCloudRealtimeContext =
  createContext<DataCloudRealtimeContextValue | null>(null);
...
export function useDataCloudRealtimeContext() {
  ...
  if (!context) {
    throw new Error(
      "useDataCloudRealtimeContext must be used within DataCloudRealtimeProvider",
    );
  }
  return context;
}
