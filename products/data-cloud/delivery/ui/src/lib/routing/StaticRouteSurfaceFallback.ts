/**
 * Static Route Surface Fallback (Fallback-Only)
 *
...
export const staticRouteSurfaceFallback: RouteSurfaceRegistry = {
  home: {
    path: "/",
    label: "Home",
...
export function getDiscoverableRouteSurfaces(
  ...
  return Object.values(staticRouteSurfaceFallback).filter(
    (route) =>
      route.discoverable &&
...
> {
  ...
  for (const route of Object.values(staticRouteSurfaceFallback)) {
...
export function getRouteSurfaceByPath(path: string): RouteSurface | undefined {
  return Object.values(staticRouteSurfaceFallback).find(
