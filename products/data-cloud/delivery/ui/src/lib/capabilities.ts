...
const RUNTIME_SCHEMA_ENDPOINTS = ["/api/v1/surfaces/schema"] as const;
...
const mockSchema: CapabilitySchema = {
  ...
  data_cloud_capabilities: [
    ...
    },
    {
      ...
      spi_interface:
        "com.ghatana.datacloud.spi.capability.WorkflowExecutionCapability",
    ...
    },
    {
      ...
      metadata: {
        ...
        notes:
          "Preview/demo-only with hardcoded demo metrics; needs production capability boundary",
      },
    },
    {
  ...
  aep_capabilities: [
    {
      ...
      products: ["aep"],
      description:
        "Durable event storage and processing with HTTP 503 for unavailable services in production",
      metadata: {
        fail_closed: "true",
        notes:
          "Production requires durable EventLog provider; in-memory only for dev/test with allow flag",
      },
    },
  ],
  ui_feature_gates: [
  ...
  try {
    ...
    for (const endpoint of RUNTIME_SCHEMA_ENDPOINTS) {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch capability schema from ${endpoint}: ${response.status} ${response.statusText}`,
          );
        }
        ...
        if (!envelope.data) {
          throw new Error(
            `Capability schema envelope from ${endpoint} is missing data`,
          );
        }
        ...
        if (endpoint !== RUNTIME_SCHEMA_ENDPOINTS[0]) {
          emitDataCloudDiagnostic(
            "Capabilities",
            "warn",
            "Capability schema fetched from compatibility endpoint",
            {
              endpoint,
    ...
    throw (
      lastError ??
      new Error(
        "Failed to fetch capability schema from all configured runtime endpoints",
      )
    );
  } catch (error) {
    emitDataCloudDiagnostic(
      "Capabilities",
      "error",
      "Failed to load capability schema from backend, falling back to mock",
      {
        error,
      },
    );
...
export function isCapabilityAvailable(
  ...
  if (
    !capability.products.includes("all") &&
...
export async function isFeatureGateEnabled(
  ...
  if (gate.capability_dependency) {
    const capabilityAvailable = isCapabilityAvailable(
      gate.capability_dependency,
      product,
    );
...
export function getProductCapabilities(product: string): Capability[] {
  ...
  return [
    ...
    (cap) => cap.products.includes("all") || cap.products.includes(product),
  );
}
...
export function getProductFeatureGates(product: string): FeatureGate[] {
  ...
  return cachedSchema.ui_feature_gates.filter(
    (gate) => gate.products.includes("all") || gate.products.includes(product),
  );
}
