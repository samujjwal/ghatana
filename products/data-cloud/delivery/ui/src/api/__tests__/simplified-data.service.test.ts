...
describe('SimplifiedDataService', () => {
  ...
  describe('constructor', () => {
    ...
    it('should normalize base URL by removing trailing slash', () => {
      const serviceWithSlash = new SimplifiedDataService('https://api.example.com/', tenantId);
  ...
  describe('search', () => {
    it('should perform search with query', async () => {
      const mockResponse = {
        items: [
          { id: '1', name: 'Test Entity', type: 'entity' }
        ],
        total: 1,
    ...
    it('should handle search with filters', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        'https://api.example.com/api/v1/simplified/search?q=test&type=entity',
        expect.any(Object)
      );
    });
  });
  ...
  describe('entities', () => {
    it('should fetch entities without collection filter', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        'https://api.example.com/api/v1/simplified/entities?limit=50&offset=0',
        expect.any(Object)
      );
    ...
    it('should fetch entities with collection filter', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        'https://api.example.com/api/v1/simplified/entities?collectionId=collection-123&limit=50&offset=0',
        expect.any(Object)
      );
    });
  ...
  describe('quick actions', () => {
    it('should fetch quick actions', async () => {
      const mockResponse = {
        actions: [
          { id: 'create-collection', name: 'Create Collection' }
        ]
      };
      ...
      expect(result).toEqual(mockResponse.actions);
    });
    ...
    it('should execute quick action', async () => {
      ...
      const result = await service.executeQuickAction('create-collection', { name: 'Test' });
  ...
  describe('error handling', () => {
    it('should handle network errors consistently', async () => {
      ...
      await expect(service.getDashboard()).rejects.toThrow('Network unavailable');
      await expect(service.getEntities()).rejects.toThrow('Network unavailable');
      await expect(service.getCollections()).rejects.toThrow('Network unavailable');
    });
