...
function makeBoundaryError(status: number): { status: number; message: string } {
...
describe('SettingsService', () => {
  ...
  describe('listApiKeys', () => {
    ...
    it('raises a boundary error on 404 (identity backend not deployed)', async () => {
      ...
      await expect(service.listApiKeys()).rejects.toThrow(SETTINGS_API_KEY_LIST_BOUNDARY_MESSAGE);
    });
  ...
  describe('createApiKey', () => {
    it('calls POST /settings/keys with validated request and returns key + secret', async () => {
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(
        '/settings/keys',
        { name: 'CI Pipeline Key', scopes: ['read:collections'], expiresInDays: 365 },
        { headers: { 'X-Tenant-ID': TENANT } },
      );
    ...
    it('raises a boundary error on 404', async () => {
      ...
      await expect(
        service.createApiKey({ name: 'Key', scopes: ['read:collections'], expiresInDays: null }),
  ...
  describe('revokeApiKey', () => {
    it('calls DELETE /settings/keys/:id/revoke and returns the revoked key', async () => {
      const revoked = makeApiKey({ status: 'revoked', revokedAt: '2026-04-24T12:00:00.000Z' });
      mockApiClient.delete.mockResolvedValue(revoked);

      const result = await service.revokeApiKey('key-001');

      expect(mockApiClient.delete).toHaveBeenCalledWith('/settings/keys/key-001/revoke', {
        headers: { 'X-Tenant-ID': TENANT },
      });
  ...
  describe('getProfile', () => {
    ...
    it('raises a boundary error on 404', async () => {
      ...
      await expect(service.getProfile()).rejects.toThrow(SETTINGS_PROFILE_BOUNDARY_MESSAGE);
    });
  });
  ...
  describe('getPreferences', () => {
    ...
    it('raises a boundary error on 404', async () => {
      ...
      await expect(service.getPreferences()).rejects.toThrow(SETTINGS_PREFERENCES_BOUNDARY_MESSAGE);
    });
  });
  ...
  describe('updatePreferences', () => {
    it('calls PATCH /settings/preferences with validated fields', async () => {
      ...
      const result = await service.updatePreferences({ theme: 'light', dateFormat: 'US' });
