...
describe('analytics client — OpenAPI route alignment (DC-P1-006)', () => {
  ...
  describe('implemented paths match OpenAPI spec', () => {
    const implementedPaths: Array<{ description: string; clientPath: string; openapiPath: string | null }> = [
      {
      ...
      },
      {
        ...
        description: 'Policy evaluation for a query (client extension, pending OpenAPI declaration)',
  ...
  it('all analytics API calls propagate the X-Tenant-ID header', () => {
    ...
    const callSitePattern = /apiClient\.(post|get|delete)<[^>]*>\s*\(\s*'\/analytics/g;
