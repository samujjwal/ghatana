...
describe("releaseReadinessService", () => {
  ...
  it("posts runtime-bound release readiness evidence", async () => {
    ...
    const result =
      await releaseReadinessService.produceReleaseReadiness(payload);
  ...
  it("fetches release readiness stats for the tenant cockpit", async () => {
    ...
    const result =
      await releaseReadinessService.getReleaseReadinessStats("tenant-123");

    expect(result.passedReleases).toBe(1);
    expect(fetch).toHaveBeenCalledWith(
      "/api/v1/release-readiness/stats?tenantId=tenant-123",
    );
  });
});
