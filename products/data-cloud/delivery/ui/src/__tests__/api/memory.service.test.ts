...
describe("memoryService", () => {
  ...
  it("fails closed before network calls when memory surface gate is disabled", async () => {
    ...
    await expect(memoryService.listMemoryItems()).rejects.toThrow(
      MEMORY_SURFACE_BOUNDARY_MESSAGE,
    );
    await expect(
      memoryService.searchMemory("agent-1", "rollback"),
    ).rejects.toThrow(MEMORY_SURFACE_BOUNDARY_MESSAGE);
  ...
  it("unwraps memory search responses from the launcher envelope", async () => {
    ...
    const items = await memoryService.searchMemory(
      "agent-1",
      "rollback",
      "tenant-a",
    );
  ...
  it("unwraps agent summary counts from the canonical /memory/:agentId route", async () => {
    ...
    const summary = await memoryService.getAgentMemorySummary(
      "agent-9",
      "tenant-a",
    );

    expect(mockApiClient.get).toHaveBeenCalledWith("/memory/agent-9", {
      params: { tenantId: "tenant-a" },
    });
  ...
  it("lists agent-root memory items when only agentId is provided", async () => {
    ...
    expect(mockApiClient.get).toHaveBeenCalledWith("/memory/agent-2", {
      params: { tenantId: "tenant-a" },
    });
