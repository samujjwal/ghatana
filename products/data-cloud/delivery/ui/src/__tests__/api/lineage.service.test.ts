...
describe('lineageService', () => {
  ...
  it('maps canonical lineage graph responses into the UI graph model', async () => {
    mockApiClient.get.mockResolvedValue({
      ...
      dag: {
        nodes: [
          ...
          { id: 'orders-raw', type: 'DATASET', name: 'orders_raw', role: 'upstream', metadata: {} },
        ],
        edges: [
    ...
    expect(graph).toEqual({
      nodes: [
        { id: 'orders', type: 'DATASET', name: 'orders', metadata: { role: 'root' } },
        { id: 'orders-raw', type: 'DATASET', name: 'orders_raw', metadata: { role: 'upstream' } },
      ],
      edges: [
        { source: 'orders-raw', target: 'orders', type: 'DERIVES_FROM' },
      ],
