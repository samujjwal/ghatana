...
describe("agentRegistryService", () => {
  ...
  it("fails closed before network calls when agent catalog gate is disabled", async () => {
    ...
    await expect(agentRegistryService.listAgents()).rejects.toThrow(
  ...
  it("maps the launcher agent catalog to AgentDefinition entries", async () => {
    ...
    expect(mockApiClient.get).toHaveBeenCalledWith("/agents/catalog", {
      params: {},
    });
  ...
  it("fails explicitly for unsupported registry mutations and live event streaming", async () => {
    ...
    await expect(
      agentRegistryService.deregisterAgent("agent-1"),
    ).rejects.toThrow(AGENT_REGISTRY_BOUNDARY_MESSAGE);

    expect(() =>
      agentRegistryService.streamRegistryEvents(undefined, () => undefined),
    ).toThrow(AGENT_REGISTRY_BOUNDARY_MESSAGE);
  });
});
