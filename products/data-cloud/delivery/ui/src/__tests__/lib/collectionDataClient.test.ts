...
describe("collection-data client", () => {
  ...
  it("parses create, get, and update record responses", async () => {
    ...
    const created = await collectionDataClient.createRecord(
      TEST_TENANT_ID,
      "orders",
      { data: { id: "1", total: 42 } },
    );
    const loaded = await collectionDataClient.getRecord(
      TEST_TENANT_ID,
      "orders",
      "record-1",
    );
    const updated = await collectionDataClient.updateRecord(
      TEST_TENANT_ID,
      "orders",
      "record-1",
      { data: { id: "1", total: 43 } },
  ...
  it("parses list and bulk responses", async () => {
    ...
    const listed = await collectionDataClient.listRecords(
      TEST_TENANT_ID,
      "orders",
      { limit: 20, offset: 0, search: "1" },
    );
    const bulkCreated = await collectionDataClient.bulkCreateRecords(
      TEST_TENANT_ID,
      "orders",
      { records: [{ id: "2" }] },
    );
    const deletedCount = await collectionDataClient.bulkDeleteRecords(
      TEST_TENANT_ID,
      "orders",
      ["record-1", "record-2"],
    );
