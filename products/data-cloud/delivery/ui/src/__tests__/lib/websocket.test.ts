...
import { validateWebSocketUrl, deriveDefaultWebSocketUrl } from '../../lib/websocket/client';
...
describe('validateWebSocketUrl', () => {
  it('accepts a valid ws: URL', () => {
    expect(validateWebSocketUrl('ws://localhost:8080/ws')).toBe('ws://localhost:8080/ws');
  });

  it('accepts a valid wss: URL', () => {
    expect(validateWebSocketUrl('wss://api.example.com/ws')).toBe('wss://api.example.com/ws');
  });

  it('throws on a non-URL string', () => {
    expect(() => validateWebSocketUrl('not-a-url')).toThrow(/could not be parsed/i);
  });

  it('throws on http: protocol', () => {
    expect(() => validateWebSocketUrl('http://example.com')).toThrow(/ws: or wss:/i);
  });

  it('throws on https: protocol', () => {
    expect(() => validateWebSocketUrl('https://example.com')).toThrow(/ws: or wss:/i);
  });
  ...
  it('throws on ws: in production mode', () => {
    ...
    expect(() => validateWebSocketUrl('ws://example.com/ws')).toThrow(/not allowed in production/i);
  });
...
describe('deriveDefaultWebSocketUrl', () => {
  ...
  it('uses custom path', () => {
    ...
    expect(deriveDefaultWebSocketUrl('/events')).toBe('ws://localhost:3000/events');
  });
});
