...
describe('capability schema loader endpoint sequencing', () => {
  ...
  it('falls back to /capabilities/schema when /surfaces/schema is unavailable', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url === '/api/v1/surfaces/schema') {
        return {
      ...
      if (url === '/api/v1/capabilities/schema') {
        return {
          ok: true,
          status: 200,
          statusText: 'OK',
    ...
    expect(schema.version).toBe('compat-version');
    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(fetchMock).toHaveBeenNthCalledWith(1, '/api/v1/surfaces/schema');
    expect(fetchMock).toHaveBeenNthCalledWith(2, '/api/v1/capabilities/schema');
    expect(warnSpy).toHaveBeenCalledWith(
      'Capability schema fetched from compatibility endpoint /api/v1/capabilities/schema; prefer /api/v1/surfaces/schema.',
    );
  });
});
