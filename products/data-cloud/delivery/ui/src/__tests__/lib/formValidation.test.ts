...
describe("nameSchema", () => {
  ...
  it("rejects name exceeding max length", () => {
    expect(
      parse(nameSchema, "a".repeat(FIELD_LIMITS.NAME_MAX + 1)).success,
    ).toBe(false);
  });
});
...
describe("descriptionSchema", () => {
  ...
  it("rejects description exceeding max length", () => {
    expect(
      parse(descriptionSchema, "x".repeat(FIELD_LIMITS.DESCRIPTION_MAX + 1))
        .success,
    ).toBe(false);
  });
});
...
describe("tagsSchema", () => {
  ...
  it("rejects arrays exceeding max count", () => {
    const tooMany = Array.from(
      { length: FIELD_LIMITS.TAG_MAX_COUNT + 1 },
      (_, i) => `tag-${i}`,
    );
...
describe("alertRuleSchema", () => {
  ...
  it("rejects invalid severity", () => {
    expect(
      parse(alertRuleSchema, { ...validAlert, severity: "ULTRA" }).success,
    ).toBe(false);
  });

  it("rejects empty notificationChannels", () => {
    expect(
      parse(alertRuleSchema, { ...validAlert, notificationChannels: [] })
        .success,
    ).toBe(false);
  });
});
...
describe("workflowSchema", () => {
  ...
  it("rejects negative timeout", () => {
    expect(
      parse(workflowSchema, { name: "W", timeoutSeconds: -1 }).success,
    ).toBe(false);
  });

  it("rejects timeout exceeding 24 hours", () => {
    expect(
      parse(workflowSchema, { name: "W", timeoutSeconds: 86_401 }).success,
    ).toBe(false);
  });
});
