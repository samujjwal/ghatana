  ...
  return (
    <div>
      <nav>
        ...
        <Link to="/data" data-testid="nav-data">
          Data
        </Link>
        <Link to="/entities" data-testid="nav-entities">
          Entities
        </Link>
      </nav>
      <Routes>
        <Route
          path="/"
          element={<div data-testid="page-home">Home Page</div>}
        />
        <Route
          path="/data"
          element={<div data-testid="page-data">Data Explorer</div>}
        />
        <Route
          path="/entities"
          element={<div data-testid="page-entities">Entities</div>}
        />
    ...
    it("resolves nested route parameters", () => {
      render(
        <MemoryRouter initialEntries={["/entities/123/details"]}>
          <Routes>
            <Route
              path="/entities/:entityId/details"
              element={<RouteTester />}
            />
          </Routes>
        </MemoryRouter>,
      ...
      expect(screen.getByTestId("entity-detail")).toHaveTextContent(
        "Entity 123",
      );
    });
  });
