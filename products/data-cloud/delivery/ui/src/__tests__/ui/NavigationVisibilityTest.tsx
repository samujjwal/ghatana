...
import { buildNavFromRegistry } from '../../layouts/DefaultLayout';
...
describe('UI Navigation Visibility Tests', () => {
  ...
  describe('Navigation Visibility by Role/Capability', () => {
    it('[NAV001]: Primary navigation shows core routes for viewer role', () => {
      ...
      const coreSection = navSections.find(s => s.title === 'layout.sectionCore');
    ...
    it('[NAV002]: Operations routes visible for operator role', () => {
      ...
      const manageSection = navSections.find(s => s.title === 'layout.sectionManage');
    ...
    it('[NAV003]: Admin-only routes hidden for non-admin roles', () => {
      ...
      const allPaths = navSections.flatMap(s => s.items.map(item => item.to));
  ...
  describe('Disabled-Surface UX', () => {
    it('[NAV008]: Disabled surface shows meaningful message', () => {
      const { container } = render(
        <QueryClientProvider client={queryClient}>
          <MemoryRouter initialEntries={['/disabled-surface']}>
            <Routes>
              <Route path="/disabled-surface" element={
                <div data-testid="disabled-surface">
                  <h1>Surface Disabled</h1>
                  <p>This surface is not available in your current configuration.</p>
                </div>
              } />
            </Routes>
          </MemoryRouter>
      ...
      const disabledSurface = container.querySelector('[data-testid="disabled-surface"]');
