...
const sampleGroups = [
  {
    ...
    aiConfidence: 0.91,
    suggestedAction:
      "Scale the orders consumer deployment and reprocess lagging partitions.",
...
const sampleSuggestions = [
  {
    ...
    alertId: "alert-1",
    suggestion:
      "Restart unhealthy consumers and replay the affected partition offsets.",
    confidence: 0.88,
    canAutoResolve: true,
...
describe("AlertsPage", () => {
  beforeEach(() => {
    ...
    mockAlertsService.getResolutionSuggestions.mockResolvedValue(
      sampleSuggestions,
    );
  ...
  it("renders the alerts shell with grouped-view triage controls", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /Review Rules/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Grouped by root cause/i }),
    ).toBeInTheDocument();
  ...
  it("renders AI grouped triage from canonical alert groups", async () => {
    ...
    expect(
      screen.getAllByText(/Grouped by root cause/i).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByText(/Orders pipeline degradation/i),
    ).toBeInTheDocument();
  ...
  it("toggles alert group expand/collapse to show correlated alerts", async () => {
    ...
    expect(screen.getAllByText("Kafka backlog spike").length).toBeGreaterThan(
      0,
    );
  });
  ...
  it("switches to list view and applies AI suggestions through the canonical service", async () => {
    ...
    expect(
      await screen.findByText(/AI Resolution Suggestions/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Restart unhealthy consumers/i),
    ).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: /apply/i }));

    await waitFor(() => {
      expect(mockAlertsService.applySuggestion).toHaveBeenCalledWith(
        "suggestion-1",
      );
    });
  });
  ...
  it("requires a reason before acknowledging an alert through the GuardedAction gate", async () => {
    ...
    const dialogConfirm = confirmButtons.find((btn) =>
    ...
    await waitFor(() => {
      expect(mockAlertsService.acknowledgeAlert).toHaveBeenCalledWith(
        "alert-1",
      );
    });
  });
  ...
  it("requires a reason before resolving an alert through the GuardedAction gate", async () => {
    ...
    const dialogConfirmButtons = screen.getAllByRole("button", {
      name: /^resolve$/i,
    });
    ...
    await user.type(
      reasonTextarea,
      "Root cause identified and remediation deployed.",
    );
  ...
  it("cancelling the GuardedAction dialog does not invoke the alert service", async () => {
    ...
    const textCancelButton = cancelButtons.find(
  ...
  it("surfaces an honest unsupported-state banner when the launcher does not expose alerts routes", async () => {
    mockAlertsService.getAlerts.mockRejectedValueOnce(
      new Error(ALERTS_UNSUPPORTED_MESSAGE),
    );
    mockAlertsService.getAlertGroups.mockRejectedValueOnce(
      new Error(ALERTS_UNSUPPORTED_MESSAGE),
    );
    mockAlertsService.getResolutionSuggestions.mockRejectedValueOnce(
      new Error(ALERTS_UNSUPPORTED_MESSAGE),
    );
    ...
    expect(
      await screen.findByText(
        /Operator-facing alert triage remains unavailable until the launcher exposes canonical alert routes/i,
      ),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText(/AI-Detected Correlations/i),
    ).not.toBeInTheDocument();
  });
});
