...
describe("TrustCenter", () => {
  beforeEach(() => {
    ...
    mockGovernanceService.getRecommendations.mockResolvedValue([
      {
        ...
        evidence: ["3 collections still require retention classification."],
      },
    ]);
    mockGovernanceService.getLifecycleSurfaces.mockResolvedValue([
      {
        ...
        status: "live-action",
        summary:
          "3 collections still need retention classification before coverage is complete.",
        evidence: [
      ...
      },
      {
        ...
        status: "unavailable",
        summary:
          "General policy create, update, toggle, and delete flows are still outside the current launcher-backed governance contract.",
        evidence: [
    ...
    mockGovernanceService.generateComplianceReport.mockResolvedValue({
      reportId: "compliance-1",
      status: "ready",
    });
  });
  ...
  it("renders the trust-center shell with lifecycle truth and action-center commands", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Trust Center" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Open Live Action/i }),
    ).toBeInTheDocument();
    expect(
  ...
  it("loads the trust-center sections through the canonical governance service hooks", async () => {
    ...
    expect(
      await screen.findByText(/Overall Compliance Score/i),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText((content) =>
        content.includes("Last updated by Auditor"),
      ),
    ).toBeInTheDocument();
    expect(
      screen.getAllByText(new RegExp(`governance:${TEST_TENANT_ID}`, "i"))
        .length,
    ).toBeGreaterThan(0);
    expect(mockGovernanceService.getPolicies).toHaveBeenCalledTimes(1);
    expect(mockGovernanceService.getAuditLogs).toHaveBeenCalledWith(
      undefined,
      undefined,
      10,
    );
    expect(mockGovernanceService.getComplianceReport).toHaveBeenCalledWith(
      "30d",
    );
  ...
  it("renders lifecycle truth cards with live, read-only, and unavailable governance posture", async () => {
    ...
    expect(
      await screen.findByTestId("trust-lifecycle-retention-operations"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("trust-lifecycle-access-review"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("trust-lifecycle-policy-lifecycle"),
    ).toHaveTextContent(
      /outside the current launcher-backed governance contract/i,
    );
  });
  ...
  it("renders governance recommendations and prefills the retention action from them", async () => {
    ...
    expect(
      await screen.findByTestId(
        "trust-recommendation-recommend-retention-classification",
      ),
    ).toBeInTheDocument();

    fireEvent.click(
      screen.getByTestId(
        "trust-recommendation-apply-recommend-retention-classification",
      ),
    );

    expect(
      await screen.findByTestId("trust-quick-action-dialog"),
    ).toBeInTheDocument();
    expect(screen.getByTestId("trust-retention-tier")).toHaveValue(
      "compliance",
    );
    expect(screen.getByTestId("trust-retention-reason")).toHaveValue(
      "Review 3 unclassified collections",
    );
    expect(screen.getByTestId("trust-retention-pii-fields")).toHaveValue(
      "email, ssn",
    );
  });
  ...
  it("refreshes compliance state through the launcher-backed summary action", async () => {
    ...
    fireEvent.click(
      await screen.findByTestId("trust-quick-action-refresh-compliance"),
    );

    await waitFor(() => {
      expect(
        mockGovernanceService.generateComplianceReport,
      ).toHaveBeenCalledWith("30d");
      expect(vi.mocked(toast.success)).toHaveBeenCalledWith(
        "Compliance summary refreshed: compliance-1",
      );
      expect(screen.getByTestId("trust-action-summary")).toHaveTextContent(
        /Compliance summary refreshed/i,
      );
    });
  });
  ...
  it("discloses access review as a read-only launcher boundary", async () => {
    ...
    fireEvent.click(
      await screen.findByTestId("trust-quick-action-access-review"),
    );
    ...
    expect(screen.getByTestId("trust-action-summary")).toHaveTextContent(
      /Access review remains read-only/i,
    );
    expect(screen.getByTestId("trust-action-summary")).toHaveTextContent(
      /planned for the Data Cloud governance backlog in Q3 2026/i,
    );
  });
  ...
  it("submits a real retention classification quick action", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Classify Retention/i }),
    );
    fireEvent.change(screen.getByLabelText("Collection"), {
      target: { value: "customers" },
    });
    fireEvent.change(screen.getByLabelText("PII Fields"), {
      target: { value: "email" },
    });
    ...
    await waitFor(() => {
      ...
      expect(vi.mocked(toast.success)).toHaveBeenCalledWith(
        "Retention tier applied to customers",
      );
      expect(screen.getByTestId("trust-action-summary")).toHaveTextContent(
  ...
  it("submits a real PII redaction quick action", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Entity ID"), {
      target: { value: "ent-123" },
    });
    fireEvent.change(screen.getByLabelText("Fields"), {
      target: { value: "email" },
    });
    fireEvent.change(screen.getByLabelText("Reason"), {
      target: { value: "Customer privacy request" },
    });
    fireEvent.click(screen.getByTestId("trust-quick-action-submit"));
  ...
  it("runs retention purge as a dry run before executing with the returned token", async () => {
    ...
    await waitFor(() => {
      expect(mockGovernanceService.getRetentionPolicy).toHaveBeenCalledWith(
        "customers",
      );
      expect(mockGovernanceService.purgeRetentionDryRun).toHaveBeenCalledWith({
        collection: "customers",
      });
      expect(vi.mocked(toast.success)).toHaveBeenCalledWith(
        "Dry run completed for customers",
      );
      expect(screen.getByTestId("trust-purge-preview")).toHaveTextContent(
        /Estimated rows: 24/i,
      );
    });
    ...
    await waitFor(() => {
      ...
      expect(vi.mocked(toast.success)).toHaveBeenCalledWith(
        "Retention purge completed for customers",
      );
      expect(screen.getByTestId("trust-action-summary")).toHaveTextContent(
