...
describe("PluginDetailsPage", () => {
  ...
  it("renders canonical plugin metadata, capabilities, and bundled delivery guidance", async () => {
    ...
    expect(
      screen.getAllByText(
        "Bundled connector for canonical plugin detail coverage",
      ),
    ).toHaveLength(2);
    ...
    expect(
      screen.getByText(
        new RegExp(
          PLUGIN_RUNTIME_TOGGLE_DOC_COMMENT.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&",
          ),
          "i",
    ...
    expect(
      screen.getByText(
        new RegExp(
          PLUGIN_BUNDLE_UPDATE_DOC_COMMENT.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&",
          ),
          "i",
    ...
    expect(
      screen.getByText(pluginDependencyBoundary.summary),
    ).toBeInTheDocument();
  });
  ...
  it("navigates back to the canonical plugins inventory", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: /back to plugins/i }),
    );
  ...
  it("shows the not-found state when the plugin lookup fails", async () => {
    vi.mocked(pluginService.getPlugin).mockRejectedValueOnce(
