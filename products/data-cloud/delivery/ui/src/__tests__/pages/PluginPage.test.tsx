...
describe("PluginPage — PluginsPage", () => {
  ...
  it("renders canonical bundled plugin inventory with live counts", async () => {
    ...
    expect(
      within(
        screen.getByText("Total").closest("div")?.parentElement
  ...
  it("shows honest catalog and delivery boundary guidance", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /How Plugin Changes Ship/i })
        .parentElement,
    ).toHaveTextContent(PLUGIN_DELIVERY_BOUNDARY_DETAIL);
    expect(
      screen.getByRole("heading", { name: /How Plugin Changes Ship/i })
        .parentElement,
    ).toHaveTextContent(PLUGIN_DELIVERY_BOUNDARY_CONTINUATION);
  });
  ...
  it("invokes the canonical runtime toggle actions for installed plugins", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /disable alpha connector/i }),
    );
