...
const mockGetConsolidationStatus = vi.mocked(
  memoryService.getConsolidationStatus,
);
...
describe("MemoryPage — MemoryPlaneViewerPage", () => {
  ...
  it("renders consolidation summary and canonical memory items", async () => {
    ...
    expect(
      screen.getByText("Failed to process order 12345"),
    ).toBeInTheDocument();
  ...
  it("filters client-side search results and shows an honest empty state when nothing matches", async () => {
    ...
    await screen.findByText("User asked about refund policy");
    fireEvent.change(screen.getByLabelText("Search memory items"), {
      target: { value: "zzznomatch" },
    });

    expect(
      screen.getByText(/No episodic memory items found/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Try clearing the search filter/i),
    ).toBeInTheDocument();
  ...
  it("toggles metadata visibility for a memory card", async () => {
    ...
    await screen.findByText("User asked about refund policy");
    fireEvent.click(
      screen.getAllByRole("button", { name: /show metadata/i })[0],
    );
