...
describe("GovernancePage — TrustCenter", () => {
  ...
  it("shows an honest empty-state message when no policies match the governance query", async () => {
    ...
    expect(
      await screen.findByText(
        /No derived policy coverage is available/i,
        undefined,
        { timeout: 15000 },
      ),
    ).toBeInTheDocument();
  ...
  it("renders canonical governance payloads into compliance score, policies, and audit log", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Governance Lifecycle Truth"),
      ).toBeInTheDocument();
      ...
      expect(screen.getAllByText("Applied GDPR policy").length).toBeGreaterThan(
        0,
      );
    ...
    await waitFor(() => {
      ...
      expect(mockGovernanceService.getLifecycleSurfaces).toHaveBeenCalledTimes(
        1,
      );
    });
  });
  ...
  it("filters policies through the Trust Center search input", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/search live safeguards/i), {
  ...
  it("refreshes the compliance summary through the operator quick action", async () => {
    ...
    fireEvent.click(
      screen.getByTestId("trust-quick-action-refresh-compliance"),
    );
