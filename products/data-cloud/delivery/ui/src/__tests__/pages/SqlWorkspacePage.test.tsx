...
describe("SqlWorkspacePage", () => {
      ...
      suggestions: [
        {
          ...
          explanation:
            "Summarize event volume by type for the past seven days.",
        },
      ],
      fallback: false,
      confidence: 0.86,
  ...
  it("renders the SQL workspace shell with editor and execution controls", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /SQL Workspace/i }),
    ).toBeInTheDocument();
    expect(
  ...
  it("routes query execution through the federated path when the toggle is enabled", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /federated/i }),
    ).toBeInTheDocument();
  ...
  it("disables federated queries when the runtime surface registry marks Trino unavailable", async () => {
    ...
    expect(
      screen.getByText(SQL_OPTIONAL_DEPENDENCIES_UNAVAILABLE_TITLE),
    ).toBeInTheDocument();
  });
  ...
  it("uses live analytics suggestions for AI query assist instead of inserting a fake generated SQL comment", async () => {
    ...
    expect(
      mockAnalytics.fetchAnalyticsQuerySuggestions.mock.calls[0][0],
    ).toContain("Collection context: orders");

    expect(
      await screen.findByText("Top event types this week"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText(
        /AI SQL generation is unavailable in this deployment/i,
      ),
    ...
    expect(
      screen.getByDisplayValue(/SELECT event_type, COUNT\(\*\) AS total/i),
    ).toBeInTheDocument();
  });
  ...
  it("explains the current query and renders plan guardrails", async () => {
    ...
    expect(screen.getByTestId("sql-query-plan-guardrails")).toHaveTextContent(
      /Optimizer hints available/i,
    );
  });
  ...
  it("surfaces a clarification prompt when the inferred scope is ambiguous", async () => {
    ...
    expect(
      await screen.findByTestId("sql-clarification-prompt"),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByTestId("sql-clarify-orders"));
    expect(
      screen.getByDisplayValue(/Compare this week for orders/i),
    ).toBeInTheDocument();
  });
  ...
  it("recommends the direct path for bounded queries and supports switching to the recommended engine", async () => {
    ...
    expect(
      await screen.findByText(/Recommended: direct analytics execution/i),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Use recommended path/i }),
    ).toBeInTheDocument();
  });
  ...
  describe("Degradation Indicators", () => {
    it("displays Analytics degradation indicator when Analytics is degraded", async () => {
          ...
          surfaces: [
            {
              ...
              summary: "DEGRADED",
              detail: "Analytics engine overloaded",
              rawValue: "DEGRADED",
            },
          ],
        },
      ...
      await waitFor(() => {
        ...
        expect(
          within(warning).getByText(/Analytics Engine:/i),
        ).toBeInTheDocument();
    ...
    it("displays Federated Query degradation indicator when Federated Query is unavailable", async () => {
        ...
        data: {
          ...
          surfaces: [
            ...
            },
            {
              ...
              summary: "UNAVAILABLE",
              detail: "Federated query not configured",
      ...
      await waitFor(() => {
        const warning = screen.getByTestId("sql-optional-dependencies-warning");
        expect(
          within(warning).getByText(/Federated Query:/i),
        ).toBeInTheDocument();
    ...
    it("does not display degradation indicators when all services are healthy", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByTestId("sql-optional-dependencies-warning"),
        ).not.toBeInTheDocument();
      });
    });
  });
