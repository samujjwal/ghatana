...
describe("WorkflowsPage", () => {
  const i18nLabel = (translated: string, key: string) =>
    new RegExp(`^(${translated}|${key})$`, "i");
  const i18nText = (translated: string, key: string) =>
    new RegExp(`(${translated}|${key})`, "i");
  ...
  it("shows an explicit unavailable state for AI hints when ai assist is not configured", async () => {
    ...
    fireEvent.click(
      screen.getByText(
        i18nText("Show pipeline details", "workflows.showPipelineDetails"),
      ),
    );

    expect(
      await screen.findByText(
        i18nText(WORKFLOW_HINTS_UNAVAILABLE_TITLE, "workflows.aiUnavailable"),
      ),
    ).toBeInTheDocument();
  ...
  it("renders the calmer outcome-first pipeline list with next-action messaging", async () => {
    ...
    expect(
      screen.getAllByText(
        i18nText("Check the latest outcome", "workflows.checkLatestOutcome"),
      ).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByRole("button", {
        name: i18nText("Review pipeline", "workflows.reviewPipeline"),
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: i18nText("Advanced editor", "workflows.advancedEditor"),
      }),
    ).toBeInTheDocument();
  }, 15000);
  ...
  it("keeps advanced pipeline details behind progressive disclosure in the review modal", async () => {
    ...
    expect(
      screen.getAllByText(i18nText("Flow size", "workflows.flowSize")).length,
    ).toBeGreaterThan(1);
    expect(
      screen.getAllByText(i18nText("Owner", "workflows.owner")).length,
    ).toBeGreaterThan(1);
  }, 15000);
  ...
  it("shows a degraded warning for AI hints when ai assist is degraded", async () => {
    ...
    expect(
      await screen.findByText(
        i18nText(WORKFLOW_HINTS_DEGRADED_TITLE, "workflows.aiDegraded"),
      ),
    ).toBeInTheDocument();
    expect(
      screen.getAllByText(i18nText("degraded", "workflows.aiDegradedDesc"))
        .length,
    ).toBeGreaterThan(0);
  }, 15000);
  ...
  it("paginates the workflow list when there are more than 20 items", async () => {
    ...
    const nextButton = screen.getByRole("button", {
      name: i18nLabel("Next", "workflows.next"),
    });
    const prevButton = screen.getByRole("button", {
      name: i18nLabel("Previous", "workflows.previous"),
    });
    ...
    expect(
      screen.getByRole("button", { name: i18nLabel("Next", "workflows.next") }),
    ).toBeDisabled();
