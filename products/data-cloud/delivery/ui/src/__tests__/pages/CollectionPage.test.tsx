...
describe("EditCollectionPage", () => {
  ...
  it("shows a loading indicator while fetching collection data", () => {
    ...
    const hasSpinner = document.querySelector(
      '[data-testid="spinner"], .animate-spin, [role="status"]',
    );
  ...
  it("loads the collection by the route ID from useParams - durable deep-link contract", async () => {
    ...
    expect(await screen.findByTestId("initial-name")).toHaveTextContent(
      "Deep-Linked Collection",
    );
  });
