...
describe("Contract-Backed Route Tests", () => {
  describe("Collections Page — contract compliance", () => {
    ...
    const _contractPaginatedResponse = PaginatedCollectionResponseSchema.parse({
    ...
    it("should render collection data that passes contract validation", async () => {
      // Override MSW with the first-class collection response shape consumed
      // by collectionsApi.list().
      server.use(
  ...
  describe("Analytics contracts", () => {
    ...
    it("canonical anomaly detection payloads pass contract validation", () => {
      ...
      const anomaly = DetectedAnomalySchema.parse({
        ...
        suggestedAction:
          "Inspect recent campaign launches and upstream retries.",
      });
    ...
    it("semantic similarity, RAG, anomaly query, and data product payloads pass contract validation", () => {
      ...
      const ragResponse = CollectionRagResponseSchema.parse({
        ...
        answer:
          "Grounded answer for the question based on the three closest matching orders.",
        context: [
          {
            id: "order-9",
            score: 0.89,
            ...
            excerpt:
              "Orders from the campaign channel retried after an upstream timeout.",
          },
        ],
        requestId: "rag-req-1",
      });
  ...
  describe("Canonical OpenAPI alignment", () => {
    ...
    it("keeps the canonical event contract aligned with the launcher response shape", () => {
      ...
      expect(canonicalOpenApi).toContain(
    ...
    it("keeps canonical pipeline paths in the Action Plane contract", () => {
      ...
      expect(actionPlaneOpenApi).toContain(
        "/api/v1/action/pipelines/{pipelineId}:",
      );
      expect(actionPlaneOpenApi).toContain(
        "/api/v1/action/pipelines/{pipelineId}/execute:",
    ...
    it("keeps the canonical brain contracts aligned with the launcher response shape", () => {
      ...
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/BrainRuntimeStats"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/BrainWorkspaceStatus"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/BrainAttentionThresholds"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/BrainPatternListResponse"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/BrainSalienceResponse"',
      );
    ...
    it("keeps the canonical analytics SQL and suggestion contracts aligned with the launcher response shape", () => {
      ...
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/AnalyticsSqlQueryRequest"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/AnalyticsSqlQueryResponse"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/AnalyticsSuggestResponse"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/LineageDagResponse"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/LineageImpactResponse"',
      );
      ...
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/AnomalyQueryResponse"',
      );
    });
    ...
    it("keeps the canonical context routes visible to contract coverage", () => {
      ...
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/CollectionContextResponse"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/McpToolRegistryResponse"',
      );
      expect(canonicalOpenApi).toContain(
      ...
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/CollectionRagRequest"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/CollectionRagResponse"',
      );
    });
    ...
    it("keeps the canonical semantic search and data product routes visible to contract coverage", () => {
      ...
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/PublishDataProductRequest"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/PublishDataProductResponse"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/DataProductListResponse"',
      );
      expect(canonicalOpenApi).toContain(
        '$ref: "#/components/schemas/DataProductSubscriptionResponse"',
      );
    });
  });
