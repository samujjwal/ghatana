...
describe("ContextExplorerPage", () => {
  ...
  it("renders collection context details from the unified API", async () => {
    ...
    await waitFor(
      () => {
        ...
        expect(
          screen.getByText("Contains customer communications"),
        ).toBeDefined();
    ...
    expect(vi.mocked(getCollectionContext)).toHaveBeenCalledWith("orders", {
      depth: 1,
    });
  });
  ...
  it("refetches context with the selected traversal depth", async () => {
    ...
    await waitFor(() => {
      expect(vi.mocked(getCollectionContext)).toHaveBeenLastCalledWith(
        "orders",
        { depth: 3 },
      );
      expect(vi.mocked(lineageService.getLineage)).toHaveBeenLastCalledWith(
        "orders",
        "BOTH",
        3,
      );
