...
vi.mock("../../lib/api/collections", () => ({
  collectionsApi: {
    list: vi
      .fn()
      .mockResolvedValue({ items: [{ id: "orders", name: "orders" }] }),
  },
}));
...
describe("InsightsPage", () => {
      ...
      data: {
        ...
        types: [
          ...
          },
          {
            ...
            reviewGuidance:
              "Review low-confidence drafts or any fallback-generated workflow before saving.",
          },
        ],
      },
    });
  ...
  it("renders live overview metrics from canonical services", async () => {
    ...
    await waitFor(() => {
      expect(
        within(
          screen.getByText("Records Processed").closest("div") as HTMLElement,
        ).getByText("1234"),
      ).toBeInTheDocument();
      expect(
        within(
          screen.getByText("Active Patterns").closest("div") as HTMLElement,
        ).getByText("7"),
      ).toBeInTheDocument();
      expect(
        within(
          screen.getByText("Active Pipelines").closest("div") as HTMLElement,
        ).getByText("9"),
      ).toBeInTheDocument();
      expect(
        within(
          screen.getByText("Est. Monthly Cost").closest("div") as HTMLElement,
        ).getByText("$2,450"),
      ).toBeInTheDocument();
    });
  }, 15000);
  ...
  it("renders AI suggestions and deep-links optimization actions", async () => {
    ...
    const matchingSuggestions = await screen.findAllByText(
      "Cache repeated analytics query",
    );
  ...
  it("renders the runtime surface truth panel", async () => {
    ...
    expect(
      screen.getByText(
        "Voice dependencies are optional in this launcher profile.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders operator diagnostics with tenant, auth, and runtime surface registry state", async () => {
    ...
    expect(
      screen.getByText("Authenticated session present"),
    ).toBeInTheDocument();
    expect(screen.getByText("0 unavailable / 1 degraded")).toBeInTheDocument();
    expect(screen.getByText("req-runtime")).toBeInTheDocument();
    expect(
      screen.getByText(INSIGHTS_CAPABILITY_SNAPSHOT_NOTE),
    ).toBeInTheDocument();
    expect(
      screen.getByText(INSIGHTS_REGISTRY_REQUEST_NOTE),
    ).toBeInTheDocument();
  });
  ...
  it("renders model telemetry for operator review", async () => {
    ...
    expect(
      within(
        screen.getByTestId("insights-ai-type-analytics_suggest"),
      ).getByText("3 requests"),
    ).toBeInTheDocument();
    expect(screen.getByText("Workflow draft generation")).toBeInTheDocument();
    expect(
      within(screen.getByTestId("insights-ai-type-pipeline_draft")).getByText(
        "50% fallback",
      ),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByText(
        "/api/v1/pipelines/draft • ai-envelope-and-draft-provenance",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        ...
        surfaces: [
          {
            ...
            detail:
              "Analytics connectors are not configured for this launcher profile.",
  ...
  it("shows a runtime surface loading state before runtime truth is available", async () => {
    ...
    expect(
      screen.getByText(
        "Checking which optional insights dependencies are active before rendering AI suggestions.",
      ),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(
        "Confirming analytics and federated query dependencies before enabling live analytics views.",
      ),
    ).toBeInTheDocument();
  });
