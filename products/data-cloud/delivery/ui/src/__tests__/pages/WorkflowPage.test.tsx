...
vi.mock("../../api/ai-operations.service", async (importOriginal) => {
  ...
  return {
    ...
    aiOperationsService: {
      ...
      getWorkflowAdvisories: vi.fn().mockResolvedValue({ advisories: [] }),
    },
  };
});
...
describe("WorkflowPage — WorkflowsPage", () => {
  ...
  it("renders the workflows shell with canonical search and filter controls", () => {
    ...
    expect(
      screen.getByPlaceholderText(
        /Search workflows by outcome, schedule, or owner/i,
      ),
    ).toBeInTheDocument();
  ...
  it("renders canonical pipeline payload details from the launcher route", async () => {
    ...
    expect(
      screen.getByText("Canonical launcher-backed workflow payload"),
    ).toBeInTheDocument();
  ...
  it("opens workflow details and shows canonical AI optimisation hints", async () => {
    ...
    fireEvent.click(
      workflowName.closest('[data-testid="workflow-item"]') ?? workflowName,
    );
    await user.click(
      await screen.findByRole("button", { name: /show pipeline details/i }),
    );

    expect(
      await screen.findByText("Inline AI Recommendations"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Reduce repeated enrichment lookups"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/cache repeated upstream calls/i),
    ).toBeInTheDocument();

    await waitFor(() => {
      expect(mockAi.getPipelineOptimisationHints).toHaveBeenCalledWith(
        "wf-contract-1",
      );
    });
  }, 15000);
