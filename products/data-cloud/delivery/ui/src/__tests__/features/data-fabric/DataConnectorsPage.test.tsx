...
function CollectionsProbe({ queryFn }: { queryFn: () => Promise<readonly string[]> }) {
  useQuery({
...
describe('DataConnectorsPage', () => {
  ...
  it('triggers connector sync and refreshes sync statistics after a successful run', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockToastSuccess).toHaveBeenCalledWith('Sync triggered successfully (Job ID: job-001)');
    });
  });
  ...
  it('surfaces sync failures and does not request sync statistics when trigger fails', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockToastError).toHaveBeenCalledWith('Failed to trigger sync: sync failed');
    });
  });
});
