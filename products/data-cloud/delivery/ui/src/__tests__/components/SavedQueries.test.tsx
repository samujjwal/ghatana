...
describe("SavedQueries", () => {
  it("exposes row favorite and actions controls with explicit accessible labels", async () => {
    render(
      <SavedQueries onSelect={vi.fn()} currentSql="SELECT * FROM events" />,
    );

    expect(
      await screen.findByRole("button", {
        name: /Toggle favorite for Revenue by Product/i,
      }),
    ).toBeInTheDocument();
  ...
  it("supports keyboard activation of row action menu and duplicate flow", async () => {
    ...
    expect(
      await screen.findByText("Active Users Last 7 Days (Copy)"),
    ).toBeInTheDocument();
  });
});
