...
import { TabWorkspace, type TabItem } from '../../components/common/TabWorkspace';
...
describe('TabWorkspace', () => {
  it('exposes close controls with explicit labels', () => {
    ...
    expect(screen.getByRole('button', { name: /Close tab Overview/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Close tab Metrics/i })).toBeInTheDocument();
  });
  ...
  it('supports keyboard close action on tab controls', async () => {
    ...
    const closeMetricsButton = screen.getByRole('button', { name: /Close tab Metrics/i });
