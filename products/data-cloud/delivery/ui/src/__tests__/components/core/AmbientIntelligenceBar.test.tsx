...
describe("AmbientIntelligenceBar OBS-001", () => {
  ...
  it("starts collapsed and does not show unsupported placeholder metrics", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByText(
          /Execution summary unavailable in the standalone launcher/i,
        ),
    ...
    expect(
      screen.queryByText(
        /System health summary unavailable in the standalone launcher/i,
      ),
    ).not.toBeInTheDocument();
  });
  ...
  it("hides completely when there are no actionable metrics", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByLabelText(/Collapse status bar/i),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("has explicit expand and collapse controls (integration contract)", () => {
    ...
    const { rerender: _rerender } = render(<AmbientIntelligenceBar />, {
      wrapper: TestWrapper,
    });
