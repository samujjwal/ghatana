...
describe("unsupportedSurfaceRegistry", () => {
  it("defines canonical boundary metadata for the primary unsupported surfaces", () => {
    ...
    expect(smartWorkflowGenerationBoundary.state).toBe(
    ...
    expect(dataFabricMetricsBoundary.summary).toContain(
      "preview topology only",
    );
    expect(smartWorkflowGenerationBoundary.summary).toContain(
