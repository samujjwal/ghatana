...
@DisplayName("DataLifecycleHandler purge tokens [GH-90000]")
class DataLifecycleHandlerPurgeTokenTest {

    @Test
    @DisplayName("local profile may use ephemeral purge token secret [GH-90000]")
    ...
    @Test
    @DisplayName("production profile requires configured durable purge token secret [GH-90000]")
    void productionProfileRequiresConfiguredDurablePurgeTokenSecret() { // GH-90000
        ...
        assertThat(requirement.message()).contains("DATACLOUD_PURGE_TOKEN_SECRET [GH-90000]");
        assertThat(requirement.profile()).isEqualTo("production [GH-90000]");
    }

    @Test
    @DisplayName("production profile validates tokens when durable secret is configured [GH-90000]")
    void productionProfileValidatesTokensWhenDurableSecretConfigured() { // GH-90000
        ...
        assertThat(invalid.reason()).contains("signature mismatch [GH-90000]");
    }
}