...
@DisplayName("EventLogAuditService [GH-90000]")
class EventLogAuditServiceTest extends EventloopTestBase {

    @Test
    @DisplayName("record persists audit events to the dedicated audit stream [GH-90000]")
    void recordPersistsAuditEvents() { // GH-90000
        ...
        runPromise(() -> auditService.record(AuditEvent.builder() // GH-90000
            ...
            .eventType("PII_REDACT [GH-90000]")
            .principal("svc-user [GH-90000]")
            .resourceType("GOVERNANCE [GH-90000]")
            .resourceId("users [GH-90000]")
            .success(true) // GH-90000
            .timestamp(Instant.parse("2026-04-14T12:00:00Z [GH-90000]"))
        ...
        assertThat(entryCaptor.getValue().eventType()).isEqualTo("PII_REDACT [GH-90000]");
        assertThat(entryCaptor.getValue().headers()).containsEntry("stream", "__audit"); // GH-90000
    }

    @Test
    @DisplayName("summarize aggregates persisted audit events by type [GH-90000]")
    void summarizeAggregatesPersistedAuditEvents() throws Exception { // GH-90000
        ...
        when(eventLogStore.readByTimeRange(any(TenantContext.class), any(Instant.class), any(Instant.class), anyInt())) // GH-90000
            .thenReturn(Promise.of(List.of( // GH-90000
                auditEntry(objectMapper, "AUTH_FAILURE", Instant.parse("2026-04-14T12:00:00Z [GH-90000]"), Map.of("requestId", "req-1")),
                auditEntry(objectMapper, "PII_REDACT", Instant.parse("2026-04-14T12:05:00Z [GH-90000]")),
                auditEntry(objectMapper, "RETENTION_PURGE", Instant.parse("2026-04-14T12:06:00Z [GH-90000]"))
            )));
        ...
        AuditSummaryProvider.AuditSummary summary = runPromise(() -> // GH-90000
            auditService.summarize("tenant-a", Instant.parse("2026-04-01T00:00:00Z [GH-90000]"), 100));

        assertThat(summary.lastAuditAt()).isEqualTo(Instant.parse("2026-04-14T12:06:00Z [GH-90000]"));
