...
describe('data fabric api service', () => {
  ...
  it('maps storage profile create requests to the shared contract', async () => {
    ...
    expect(mockApiClient.post).toHaveBeenCalledWith('/data-fabric/profiles', {
      name: 'Archive Storage',
      type: 'azure-blob',
      config: {
        container: 'archive',
  ...
  it('parses and maps connector responses and requests', async () => {
    ...
    expect(mockApiClient.post).toHaveBeenCalledWith('/data-fabric/connectors', {
      name: 'Webhook Feed',
      type: 'http-webhook',
      storageProfileId: 'profile-1',
      config: {
  ...
  it('validates connector test, sync trigger, and sync statistics helper responses', async () => {
    mockApiClient.post.mockResolvedValueOnce({ success: true, message: 'Connection verified' });
    ...
    expect(testResult).toEqual({ success: true, message: 'Connection verified' });
