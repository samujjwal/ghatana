...
describe('useCollectionData hook', () => {
  ...
  it('loads collection records through the canonical collectionDataClient', async () => {
    ...
    const { result } = renderHook(() => useCollectionData('orders', 'tenant-a', 10), {
      wrapper: TestWrapper,
    });

    await waitFor(() => expect(result.current.loading).toBe(false));

    expect(mockCollectionDataClient.listRecords).toHaveBeenCalledWith('tenant-a', 'orders', {
      offset: 0,
      limit: 10,
    });
  ...
  it('uses the session tenant when one is not passed explicitly', async () => {
    ...
    const { result } = renderHook(() => useCollectionData('orders', undefined, 5), {
      wrapper: TestWrapper,
    });

    await waitFor(() => expect(result.current.loading).toBe(false));

    expect(mockCollectionDataClient.listRecords).toHaveBeenCalledWith('tenant-session', 'orders', {
      offset: 0,
      limit: 5,
    });
  });
