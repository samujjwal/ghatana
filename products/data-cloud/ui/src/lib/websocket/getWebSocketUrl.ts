...
export function getWebSocketUrl(options: WebSocketUrlOptions): string {
    ...
    if (!urlStr.startsWith('ws://') && !urlStr.startsWith('wss://') && !urlStr.startsWith('http://') && !urlStr.startsWith('https://')) {
        const isSecure = secure ?? (typeof window !== 'undefined' && window.location.protocol === 'https:');
        urlStr = `${isSecure ? 'wss' : 'ws'}://${urlStr}`;
    } else {
        urlStr = urlStr.replace(/^http:\/\//, 'ws://').replace(/^https:\/\//, 'wss://');
