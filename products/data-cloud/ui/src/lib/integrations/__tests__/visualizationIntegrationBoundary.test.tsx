...
describe('visualization integration launcher boundaries', () => {
  it('returns boundary errors and empty data for visualization hooks', async () => {
    ...
    const { result: recentEventsResult } = renderHook(() => useRecentEvents({ limit: 10 }));

    expect(metricsResult.current.metrics).toEqual([]);
    expect(metricsResult.current.error?.message).toBe(VISUALIZATION_INTEGRATION_BOUNDARY_MESSAGE);
    await expect(metricsResult.current.refetch()).rejects.toThrow(
      VISUALIZATION_INTEGRATION_BOUNDARY_MESSAGE,
    );

    expect(dashboardsResult.current.dashboards).toEqual([]);
    expect(dashboardsResult.current.error?.message).toBe(VISUALIZATION_INTEGRATION_BOUNDARY_MESSAGE);
    await expect(
    ...
    expect(healthResult.current.error?.message).toBe(VISUALIZATION_INTEGRATION_BOUNDARY_MESSAGE);
