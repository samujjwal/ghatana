...
`products/data-cloud/planes/action/observability` contains AEP-specific tracing and orchestration instrumentation that does not belong in the generic platform observability module.
...
```bash
./gradlew :products:data-cloud:planes:action:observability:test
```