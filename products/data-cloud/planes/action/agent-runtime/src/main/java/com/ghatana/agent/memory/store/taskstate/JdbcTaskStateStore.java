...
public class JdbcTaskStateStore implements TaskStateStore {
     ...
     */
    public JdbcTaskStateStore(@NotNull TaskStateRepository repository, @Nullable EventLogStore eventLogStore) {
        this.repository = Objects.requireNonNull(repository, "repository");
        this.eventLogStore = eventLogStore;
        if (eventLogStore != null) {
            log.info("JdbcTaskStateStore initialized with EventLogStore event sourcing");
        } else {
            log.warn("JdbcTaskStateStore initialized WITHOUT EventLogStore — event sourcing disabled");
        }
    }
        ...
        try {
            ...
            return Promise.complete()
                    .whenComplete((ignored, error) -> {
                        if (error != null) {
                            log.warn("EventLogStore emission failed for {}/{}: {}",
                                    eventType, taskId, error.getMessage());
                        }
                    });
        } catch (Exception e) {
            log.warn("EventLogStore emission failed for {}/{}: {}", eventType, taskId, e.getMessage());
            return Promise.complete();
        }
    }
}
