...
```bash
./gradlew :products:data-cloud:planes:operations:config:test
```
...
- `products/data-cloud/planes/data/entity` — entity schemas consumed by compiled config.
- `products/data-cloud/delivery/api` — API layer that reads config at request time.
