...
public class RedisStateAdapter implements StateAdapter<String, String> {
    ...
    public Promise<Boolean> exists(String key) {
        ...
        try {
            try (Jedis jedis = jedisPool.getResource()) {
                return Promise.of(jedis.exists(fullKey(key)));
            }
        } catch (Exception e) {
            return Promise.ofException(e);
        }
    ...
    public Promise<Long> getSize() {
        ...
        try {
            try (Jedis jedis = jedisPool.getResource()) {
                ...
                for (String line : info.split("\n")) {
                    if (line.startsWith("used_memory:")) {
                        return Promise.of(Long.parseLong(line.split(":")[1].trim()));
                    }
                }
    ...
    public Promise<Long> getTtl(String key) {
        ...
        try {
            try (Jedis jedis = jedisPool.getResource()) {
                return Promise.of(jedis.pttl(fullKey(key)));
            }
        } catch (Exception e) {
            return Promise.ofException(e);
        }
    ...
    public Promise<Long> increment(String key, long delta) {
        ...
        try {
            try (Jedis jedis = jedisPool.getResource()) {
                return Promise.of(jedis.incrBy(fullKey(key), delta));
            }
        } catch (Exception e) {
            return Promise.ofException(e);
        }
