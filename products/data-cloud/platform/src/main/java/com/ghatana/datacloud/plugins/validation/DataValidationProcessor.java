 ...
 *
 * <p>This processor can optionally use AEP's advanced processing capabilities
 * when available via ServiceLoader. If AEP is not available, it falls back
 * to basic validation.
 *
 * @doc.type class
 * @doc.purpose Data validation using pattern detection with optional advanced strategy
 * @doc.layer plugin
...
public class DataValidationProcessor {
    ...
    public DataValidationProcessor() {
        ...
        if (aepStrategy != null) {
            log.info("DataValidationProcessor initialized with AEP-based validation");
