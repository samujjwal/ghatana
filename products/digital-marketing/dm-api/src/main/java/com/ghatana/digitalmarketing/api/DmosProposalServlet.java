...
/**
 * HTTP servlet for DMOS proposal generation, review, approval, and retrieval (F1-015).
 *
 * <p>Exposes four routes:
 * <ul>
 *   <li>POST /v1/workspaces/:workspaceId/proposal</li>
 *   <li>POST /v1/workspaces/:workspaceId/proposal/:proposalId/submit</li>
 *   <li>POST /v1/workspaces/:workspaceId/proposal/:proposalId/approve</li>
 *   <li>GET  /v1/workspaces/:workspaceId/proposal/:proposalId</li>
 * </ul>
 * </p>
 *
 * @doc.type class
 * @doc.purpose DMOS proposal API servlet for F1-015
 * @doc.layer product
 * @doc.pattern Controller, Adapter
 */
public final class DmosProposalServlet {

    private static final Logger LOG = LoggerFactory.getLogger(DmosProposalServlet.class);
    ...
    public DmosProposalServlet(ProposalService proposalService, Eventloop eventloop) {
        this.proposalService = Objects.requireNonNull(proposalService, "proposalService must not be null");
        this.eventloop = Objects.requireNonNull(eventloop, "eventloop must not be null");
    }

    /**
     * Returns the routing servlet for proposal endpoints.
     *
    ...
    public AsyncServlet getServlet() {
        ...
        RoutingServlet.builder(eventloop)
            ...
            .with(HttpMethod.POST, "/v1/workspaces/:workspaceId/proposal/:proposalId/approve",
    ...
    private Promise<HttpResponse> handleGenerateProposal(HttpRequest request) {
        return request.loadBody().then(__ -> {
            try {
                ...
                GenerateProposalRequest body = MAPPER.readValue(
                    request.getBody().getString(StandardCharsets.UTF_8),
                    GenerateProposalRequest.class
                );
                ProposalService.GenerateProposalCommand command =
                    new ProposalService.GenerateProposalCommand(
                        body.strategyId(),
                        body.templateId(),
                        body.templateVersion(),
                        body.assumptions() != null ? body.assumptions() : ""
                    );
                return proposalService.generateProposal(ctx, command)
                    .map(p -> jsonResponse(201, ProposalResponse.from(p)))
                    .then(r -> Promise.of(r), e -> mapServiceError("generate proposal", e, request));
            } catch (IllegalArgumentException e) {
                return Promise.of(DmosApiErrorResponses.error(400, e.getMessage(), request));
            } catch (Exception e) {
                LOG.error("[DMOS] Failed to generate proposal", e);
    ...
    private Promise<HttpResponse> handleSubmitForReview(HttpRequest request) {
        return request.loadBody().then(__ -> {
            try {
                ...
                DmOperationContext ctx = buildContext(request, workspaceId, true);
                return proposalService.submitForReview(ctx, proposalId)
                    .map(p -> jsonResponse(200, ProposalResponse.from(p)))
                    .then(r -> Promise.of(r), e -> mapServiceError("submit proposal for review", e, request));
            } catch (IllegalArgumentException e) {
                return Promise.of(DmosApiErrorResponses.error(400, e.getMessage(), request));
            } catch (Exception e) {
                LOG.error("[DMOS] Failed to submit proposal for review", e);
                return Promise.of(DmosApiErrorResponses.error(500, "Internal error", request));
            }
        });
    }

    private Promise<HttpResponse> handleApproveProposal(HttpRequest request) {
        return request.loadBody().then(__ -> {
            try {
                String workspaceId = request.getPathParameter("workspaceId");
                String proposalId = request.getPathParameter("proposalId");
                DmOperationContext ctx = buildContext(request, workspaceId, true);
                return proposalService.approveProposal(ctx, proposalId)
                    .map(p -> jsonResponse(200, ProposalResponse.from(p)))
                    .then(r -> Promise.of(r), e -> mapServiceError("approve proposal", e, request));
            } catch (IllegalArgumentException e) {
                return Promise.of(DmosApiErrorResponses.error(400, e.getMessage(), request));
            } catch (Exception e) {
                LOG.error("[DMOS] Failed to approve proposal", e);
                return Promise.of(DmosApiErrorResponses.error(500, "Internal error", request));
            }
        });
    }

    private Promise<HttpResponse> handleGetProposal(HttpRequest request) {
        try {
            String workspaceId = request.getPathParameter("workspaceId");
            String proposalId = request.getPathParameter("proposalId");
            DmOperationContext ctx = buildContext(request, workspaceId, false);
            return proposalService.getProposal(ctx)
                .map(p -> {
                    if (!p.getProposalId().equals(proposalId)) {
                        return DmosApiErrorResponses.error(404, "Proposal not found: " + proposalId, request);
                    }
                    return jsonResponse(200, ProposalResponse.from(p));
                })
                .then(r -> Promise.of(r), e -> mapServiceError("get proposal", e, request));
        } catch (IllegalArgumentException e) {
            return Promise.of(DmosApiErrorResponses.error(400, e.getMessage(), request));
        } catch (Exception e) {
            LOG.error("[DMOS] Failed to get proposal", e);
            return Promise.of(DmosApiErrorResponses.error(500, "Internal error", request));
        }
    }
    ...
    record GenerateProposalRequest(
        String strategyId,
        String templateId,
    ...
    record DeliverableDto(String deliverableType, String description, int timelineDays, String unit, int quantity) {
        static DeliverableDto from(ProposalDeliverable d) {
            return new DeliverableDto(d.deliverableType(), d.description(), d.timelineDays(), d.unit(), d.quantity());
        }
    }

    record PricingOptionDto(String pricingType, double amount, String currency, String description) {
        static PricingOptionDto from(PricingOption p) {
            return new PricingOptionDto(p.pricingType(), p.amount(), p.currency(), p.description());
        }
    }
    ...
    record ProposalResponse(
        ...
        List<PricingOptionDto> pricingOptions,
        String assumptions,
        String timeline,
        String rationale,
        String disclaimer,
        ...
        static ProposalResponse from(Proposal p) {
            return new ProposalResponse(
                p.getProposalId(),
                p.getWorkspaceId().getValue(),
                p.getStrategyId(),
                p.getTemplateId(),
                p.getTemplateVersion(),
                p.getStatus().name(),
                p.getDeliverables().stream().map(DeliverableDto::from).collect(Collectors.toList()),
                p.getPricingOptions().stream().map(PricingOptionDto::from).collect(Collectors.toList()),
                p.getAssumptions(),
                p.getTimeline(),
                p.getRationale(),
                p.getDisclaimer(),
                p.getExclusions(),
                p.getMeasurementPlan(),
                p.getModelVersion(),
                p.getGeneratedAt(),
                p.getGeneratedBy(),
                p.getReviewedAt(),
