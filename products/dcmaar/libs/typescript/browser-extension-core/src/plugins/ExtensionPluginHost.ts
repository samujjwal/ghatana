...
export class ExtensionPluginHost {
  ...
  constructor(options: ExtensionPluginHostOptions = {}) {
    ...
    this.connectorBridge =
      ...
      new ExtensionConnectorBridge({
        profiles: options.manifest?.connectors ?? [],
      });
  }
  ...
  async initializeFromManifest(
    ...
    const tasks = manifest.plugins
      .filter(
        (descriptor: ExtensionPluginDescriptor) => descriptor.enabled !== false,
      )
      .map(async (descriptor: ExtensionPluginDescriptor) => {
        ...
        console.log("[ExtensionPluginHost] Installing plugin:", {
          ...
          pluginKeys: Object.keys(plugin as Record<string, unknown>).slice(
            0,
            15,
          ),
  ...
  async shutdown(): Promise<void> {
    ...
    const uninstallTasks = plugins.map(
      async (plugin: Record<string, unknown>) => {
