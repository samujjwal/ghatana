...
impl UnixTransport {
    ...
    /// Establish a client connection to the configured Unix socket path.
    ...
    /// Bind a Unix listener and yield incoming connections as a stream.
...
impl UnixConnection {
    /// Encode and transmit a `Message` over the socket with a length prefix.
    ...
    /// Receive a framed `Message`, blocking until the entire payload arrives.
