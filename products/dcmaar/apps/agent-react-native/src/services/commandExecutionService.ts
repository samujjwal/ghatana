...
export const useCommandExecutionStore = {
  ...
  setState: (partial: Partial<CommandExecutionState>) => {
    if (partial.executionHistory !== undefined)
      commandExecStore.set(executionHistoryAtom, partial.executionHistory);
    if (partial.pendingCommands !== undefined)
      commandExecStore.set(pendingCommandsAtom, partial.pendingCommands);
    if (partial.isProcessing !== undefined)
      commandExecStore.set(isProcessingAtom, partial.isProcessing);
  },
};
...
export async function executeCommand(
  ...
  if (existing) {
    console.debug(
      `[CommandExecutionService] Command ${command.command_id} already executed`,
    );
    return existing;
  }
...
async function handleResult(result: CommandResult): Promise<void> {
  // Store in history
  const state = useCommandExecutionStore.getState();
  const newHistory = new Map(state.executionHistory);
  newHistory.set(result.command_id, result);
  useCommandExecutionStore.setState({ executionHistory: newHistory });
...
async function acknowledgeCommand(result: CommandResult): Promise<void> {
  ...
  if (!token) {
    console.warn(
      "[CommandExecutionService] No auth token, skipping acknowledgment",
    );
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.debug(
        `[CommandExecutionService] Acknowledged command ${result.command_id}`,
      );
    }
  } catch (error) {
