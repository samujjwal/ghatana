...
/**
 * G10-006: PHR Generation Mode Tests
 *
 * Tests for PHR generation mode configuration and task emission.
 *
 * @doc.type class
 * @doc.purpose Test PHR generation mode functionality
 * @doc.layer integration
 * @doc.pattern Unit Test
 */
@DisplayName("PHR Generation Mode Tests")
class PhrGenerationModeTest extends EventloopTestBase {

    private PhrGenerationMode generationMode;

    @BeforeEach
    void setUp() {
        generationMode = new PhrGenerationMode();
    }
    ...
    void standardMode_whenAddingDeprecationTask_keepsAsDeprecation() {
        generationMode.setMode(PhrGenerationMode.Mode.STANDARD);
    ...
    void cleanupMode_whenAddingDeprecationTask_convertsToDelete() {
        generationMode.setMode(PhrGenerationMode.Mode.CLEANUP);
        generationMode.addTask("deprecation", "/old/path", "Deprecated legacy path");
    ...
    void tasks_whenGeneratingReport_producesFormattedOutput() {
        ...
        assertThat(report).contains("PHR Generation Task Report");
