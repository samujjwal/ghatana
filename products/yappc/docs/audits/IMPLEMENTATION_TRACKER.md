...
| P1-019 | Implemented for mounted import review queue with backend persistence | The mounted import panel now presents a guided workflow with explicit Semantic model and Governed source paths, source type selection, source locator input, review-step copy, and no need for users to memorize `source:<type>:<locator>` command syntax. Imported round-trip fidelity loss points and residual islands create an explicit review queue with visible progress and apply/skip/promote decisions, while residual accept/reject decisions synchronize back into the queue. Import review queue decisions now persist through `POST /api/v1/yappc/artifacts/{artifactId}/import-review-decisions` before PageDesigner marks a queue item decided; the API records `YAPPC_IMPORT_REVIEW_DECISION_RECORDED` audit evidence and fails closed if audit persistence is unavailable. Legacy command parsing remains for compatibility. Richer side-by-side diff review remains open. | `pnpm --filter @ghatana/yappc-api-app exec vitest run src/routes/__tests__/import-review.test.ts src/routes/__tests__/registry-candidates.test.ts src/__tests__/openapi-contract.test.ts`; `pnpm --filter @ghatana/yappc-web-app exec vitest run src/services/canvas/commands/__tests__/ImportReviewDecisionService.test.ts src/services/canvas/commands/__tests__/ResidualIslandReviewService.test.ts src/components/canvas/page/__tests__/PageDesigner.test.tsx`; narrowed import/residual review typecheck greps |
