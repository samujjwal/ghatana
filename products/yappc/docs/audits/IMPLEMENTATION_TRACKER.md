...
Result: passed cleanly after the 2026-05-08 package-boundary and web-gate cleanup passes. Package-local `yappc-state type-check` is now also green. Package-local `yappc-ui type-check` remains the next direct-package cleanup slice: the first parse fixes landed for three story/demo files, but the direct package check still exposes older UI package API drift in AI, workspace, traceability, voice, utility, story, and rootDir surfaces that should be handled as the next 15-20 file batch.
