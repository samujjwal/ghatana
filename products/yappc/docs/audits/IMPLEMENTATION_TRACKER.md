...
| P1-012 | Partially implemented | Page builder nodes now expose keyboard-accessible movement shortcuts (`Alt+ArrowUp`, `Alt+ArrowDown`, `Alt+ArrowLeft`, `Alt+ArrowRight`) and invalid drop attempts surface an `aria-live` feedback message instead of silently returning. `Alt+ArrowRight` moves the focused component into the previous container's default slot when available, and announces a clear reason when no previous container target exists. Regression coverage now asserts the screen-reader contract directly: keyboard-movable builder nodes are focusable `treeitem`s with explicit movement instructions in their accessible name, and invalid keyboard moves announce through an `alert`/polite live region. Broader keyboard-only Playwright and aXe coverage remains open under P2 accessibility work. | `pnpm --filter @ghatana/yappc-web-app exec vitest run src/components/canvas/page/__tests__/PageDesigner.test.tsx src/components/canvas/nodes/__tests__/PageDesignerNode.test.tsx`; narrowed keyboard/drop/accessibility typecheck grep |
| P1-013 | Partially implemented | Extended the inspector governance controls so telemetry-emitting component contracts require explicit telemetry consent acknowledgement before changes can be applied, alongside required a11y, review-required prop, and privacy-classification enforcement. The inspector now edits responsive variants, state variants, data bindings, action bindings, and component-level privacy classification, routing all persisted updates through PageBuilderCommands. Binding controls now derive contract-aware data target and action event picker options from registry fields/default event props while preserving existing custom bindings. `update-props` stores node `metadata.dataClassification` with undo support, and PageDesigner passes selected-node/document classification into the inspector. Broader contract-specific rich editors for complex object/array/token fields remain open. | `pnpm --filter @ghatana/yappc-web-app exec vitest run src/components/canvas/page/__tests__/PropertyForm.test.tsx src/components/canvas/page/__tests__/PageDesigner.test.tsx src/services/canvas/commands/__tests__/PageBuilderCommands.test.ts`; narrowed inspector/privacy-classification typecheck grep |
