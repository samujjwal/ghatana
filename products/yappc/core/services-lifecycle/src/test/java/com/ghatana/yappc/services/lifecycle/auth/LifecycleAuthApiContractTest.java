 ...
 * {@code AuthProvider} (frontend/web/src/providers/AuthProvider.tsx):
 * <ul>
...
class LifecycleAuthApiContractTest extends EventloopTestBase {
    ...
    @BeforeEach
    void setup() {
        tokenProvider = mock(com.ghatana.platform.security.port.JwtTokenProvider.class);
        authController = new JwtAuthController(tokenProvider);

        byte[] salt = LifecycleLoginController.generateSalt();
        String hash = LifecycleLoginController.hashPassword("test-pass", salt);
        LifecycleLoginController.UserRecord user = new LifecycleLoginController.UserRecord(
                "user-42",
                ...
                List.of("admin"),
                "test-tenant");
        loginController = new LifecycleLoginController(tokenProvider, List.of(user));
    }
    ...
    class LoginContract {
        ...
        void loginSuccessResponseShape() {
            ...
            HttpRequest request = HttpRequest.post("http://localhost/api/auth/login")
                    .withBody("{\"email\":\"contract-test@yappc.io\",\"password\":\"test-pass\"}")
                    .build();

            HttpResponse response = runPromise(() -> loginController.login(request));
            ...
            assertThat(body).contains("\"tenantId\"");
            assertThat(body).contains("jwt.token.value");
        }

        @Test
        @DisplayName("failure response contains error and message fields")
        void loginFailureResponseShape() {
            HttpRequest request = HttpRequest.post("http://localhost/api/auth/login")
                    .withBody("{\"email\":\"contract-test@yappc.io\",\"password\":\"wrong\"}")
                    .build();
    ...
    class LogoutContract {
        ...
        void logoutResponseShape() {
            HttpRequest request = HttpRequest.post("http://localhost/api/auth/logout")
                    .build();

            HttpResponse response = runPromise(() -> loginController.logout(request));
    ...
    class MeContract {
        ...
        void meSuccessResponseShape() {
            ...
            HttpResponse response = runPromise(() -> authController.currentUser(request));
    ...
    class ValidateContract {
        ...
        void validateSuccessResponseShape() {
            when(tokenProvider.validateToken("valid-token")).thenReturn(true);
            when(tokenProvider.getUserIdFromToken("valid-token")).thenReturn(Optional.of("user-42"));
            when(tokenProvider.extractClaims("valid-token")).thenReturn(Optional.of(Map.of(
                    "tenantId", "test-tenant"
            ...
            HttpResponse response = runPromise(() -> authController.validate(request));
