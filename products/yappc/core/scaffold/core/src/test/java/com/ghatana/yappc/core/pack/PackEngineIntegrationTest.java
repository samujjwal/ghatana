...
class PackEngineIntegrationTest {
    ...
    void testLoadBasePack(@TempDir Path tempDir) throws Exception { // GH-90000
        ...
        Path basePackPath = Path.of("packs/base [GH-90000]");
        if (basePackPath.toFile().exists()) { // GH-90000
            ...
            assertTrue(basePack.getMetadata().templates().containsKey("gitignore [GH-90000]"));
        }
    }
    ...
    void testLoadActiveJServicePack(@TempDir Path tempDir) throws Exception { // GH-90000
        ...
        Path servicePackPath = Path.of("packs/java-service-activej-gradle [GH-90000]");
        if (servicePackPath.toFile().exists()) { // GH-90000
            ...
            assertTrue(servicePack.getMetadata().templates().containsKey("main-class [GH-90000]"));
            assertTrue(servicePack.getMetadata().templates().containsKey("dockerfile [GH-90000]"));
        }
    }
}
