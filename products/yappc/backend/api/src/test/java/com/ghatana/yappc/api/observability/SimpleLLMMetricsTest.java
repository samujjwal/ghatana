...
class SimpleLLMMetricsTest {
  ...
  void llmMetricsBuilderWithDefaults() {
    ...
    // When
    LLMMetrics metrics = LLMMetrics.builder().model(model).build();
  ...
  void llmMetricsWithNullModel_DoesNotThrowException() {
    ...
    assertThatNoException()
        .isThrownBy(
            () -> {
              LLMMetrics.builder().model(null).build();
            });
  }
  ...
  void llmMetricsWithError() {
    // Given
    LLMMetrics metrics = LLMMetrics.builder().model("gpt-4").errorCode("TIMEOUT").build();
  ...
  void llmMetricsWithEmptyErrorCode() {
    // Given
    LLMMetrics metrics = LLMMetrics.builder().model("gpt-4").errorCode("").build();
  ...
  void llmMetricsWithCustomTimestamp() {
    ...
    // When
    LLMMetrics metrics = LLMMetrics.builder().model("gpt-4").timestamp(customTimestamp).build();
  ...
  void llmMetricsWithMetadata() {
    ...
    // When
    LLMMetrics metrics = LLMMetrics.builder().model("gpt-4").metadata(metadata).build();
  ...
  void llmMetricsCalculateTotalTokens() {
    // Given
    LLMMetrics metrics =
        LLMMetrics.builder().model("gpt-4").promptTokens(100).completionTokens(50).build();
