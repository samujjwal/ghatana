...
class DlqIntegrationTest extends EventloopTestBase {
  ...
  @Mock private JdbcDlqRepository dlqRepository;

  @Captor private ArgumentCaptor<String> tenantCaptor;
  @Captor private ArgumentCaptor<String> pipelineCaptor;
  @Captor private ArgumentCaptor<String> nodeCaptor;
  @Captor private ArgumentCaptor<String> eventTypeCaptor;
  @Captor private ArgumentCaptor<Map<String, Object>> payloadCaptor;
  @Captor private ArgumentCaptor<String> failureReasonCaptor;
  @Captor private ArgumentCaptor<String> correlationCaptor;
  ...
  void setUp() {
    ...
    capturingPublisher =
        (tenantId, pipelineId, nodeId, eventType, eventPayload, failureReason, correlationId) -> {
  ...
  void retryStartsWorkflowAndResolvesEntry() {
    ...
    var execution =
        workflowMaterializer.startWorkflow(
  ...
  void retryUnknownTemplateAbandons() {
    UUID dlqId = UUID.randomUUID();
    DlqEntry pending =
        dlqEntry(dlqId, "PENDING", Map.of("workflow_template_id", "non-existent-template"));
    ...
    verify(dlqRepository)
        .updateStatus(eq(dlqId), eq(TENANT_ID), eq("ABANDONED"), eq(false), isNull());
  }
  ...
  void resolvedEntrySkipsRetry() {
    ...
    verify(dlqRepository, never())
        .updateStatus(eq(dlqId), eq(TENANT_ID), any(), anyBoolean(), any());
  }
  ...
  void listReturnsTenantEntries() {
    ...
    List<DlqEntry> entries =
        List.of(dlqEntry(id1, "PENDING", Map.of()), dlqEntry(id2, "PENDING", Map.of()));
