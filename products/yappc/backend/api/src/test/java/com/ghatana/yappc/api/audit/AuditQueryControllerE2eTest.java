...
class AuditQueryControllerE2eTest extends EventloopTestBase {
  ...
  class AuditQueryApiTests {
    ...
    void shouldQueryAllEventsForTenant() {
      ...
      List<AuditEvent> events = runPromise(() -> queryService.findByTenantId(tenantId));
    ...
    void shouldFilterByEventType() {
      ...
      // WHEN
      List<AuditEvent> started =
          runPromise(() -> queryService.findByEventType(tenantId, "AGENT_TURN_STARTED"));
    ...
    void shouldFilterByPrincipal() {
      ...
      queryService.recordEvent(
          createEventWithPrincipal(tenantId, "AGENT_TURN_STARTED", "agent-1", "alice"));
      queryService.recordEvent(
          createEventWithPrincipal(tenantId, "AGENT_TURN_STARTED", "agent-2", "bob"));
      queryService.recordEvent(
          createEventWithPrincipal(tenantId, "POLICY_APPLIED", "policy-1", "alice"));

      // WHEN
      List<AuditEvent> aliceEvents =
          runPromise(() -> queryService.findByPrincipal(tenantId, "alice"));
    ...
    void shouldFilterByResource() {
      ...
      // WHEN
      List<AuditEvent> agentEvents =
          runPromise(() -> queryService.findByResource(tenantId, "AGENT", "agent-1"));

      // THEN
      assertThat(agentEvents).hasSize(2);
      assertThat(agentEvents)
          .allMatch(e -> e.resourceType().equals("AGENT") && e.resourceId().equals("agent-1"));
    }
    ...
    void shouldFilterByTimeRange() {
      ...
      // WHEN
      List<AuditEvent> inRange =
          runPromise(() -> queryService.findByTimeRange(tenantId, before, after));
    ...
    void shouldFindEventById() {
      ...
      // WHEN
      Optional<AuditEvent> found = runPromise(() -> queryService.findById(tenantId, event.id()));
    ...
    void shouldReturnEmptyForNonExistentEvent() {
      // WHEN
      Optional<AuditEvent> found =
          runPromise(() -> queryService.findById("tenant-001", "non-existent-id"));
    ...
    void shouldCountTotalEvents() {
      ...
      // WHEN
      Long count = runPromise(() -> queryService.countByTenantId(tenantId));
  ...
  class FullWorkflowTests {
    ...
    void shouldRecordAndQueryAgentTurnEvents() {
      ...
      queryService.recordEvent(
          createEventWithDetails(
              tenantId, "AGENT_TURN_STARTED", agentId, Map.of("phase", "PERCEIVE", "turn", 1)));
      ...
      List<AuditEvent> turnEvents =
          runPromise(() -> queryService.findByTenantId(tenantId)).stream()
    ...
    void shouldSupportComplexFiltering() {
      ...
      String tenantId = "tenant-001";
      for (int i = 1; i <= 3; i++) {
        queryService.recordEvent(createEventWithResource(tenantId, "AGENT", "agent-" + i));
      }
      queryService.recordEvent(createEventWithResource(tenantId, "POLICY", "policy-1"));
  ...
  private void seedEvents(String tenantId, int count) {
    for (int i = 0; i < count; i++) {
      queryService.recordEvent(createEvent(tenantId, "EVENT_" + i, "resource-" + i));
    }
  }
  ...
  private AuditEvent createEventWithPrincipal(
      String tenantId, String eventType, String resourceId, String principal) {
    return AuditEvent.builder()
        .id(UUID.randomUUID().toString())
        .tenantId(tenantId)
        .eventType(eventType)
  ...
  private AuditEvent createEventWithResource(
  ...
  private AuditEvent createEventWithDetails(
      String tenantId, String eventType, String resourceId, Map<String, Object> details) {
    return AuditEvent.builder()
        .id(UUID.randomUUID().toString())
        .tenantId(tenantId)
        .eventType(eventType)
  ...
  /** In-memory test double for AuditQueryService. Thread-safe, tenant-isolated. */
  private static class InMemoryAuditQueryService implements AuditQueryService {
    ...
    void recordEvent(AuditEvent event) {
      store
          .computeIfAbsent(event.tenantId(), k -> Collections.synchronizedList(new ArrayList<>()))
          .add(event);
    }

    @Override
    public Promise<List<AuditEvent>> findByTenantId(String tenantId) {
      return Promise.of(new ArrayList<>(store.getOrDefault(tenantId, Collections.emptyList())));
    }
    ...
    public Promise<List<AuditEvent>> findByTenantId(String tenantId, int offset, int limit) {
      ...
      return Promise.of(all.stream().skip(offset).limit(limit).collect(Collectors.toList()));
    }

    @Override
    public Promise<List<AuditEvent>> findByResource(
        String tenantId, String resourceType, String resourceId) {
      return Promise.of(
          store.getOrDefault(tenantId, Collections.emptyList()).stream()
              .filter(
                  e -> e.resourceType().equals(resourceType) && e.resourceId().equals(resourceId))
              .collect(Collectors.toList()));
    }
    ...
    @Override
    public Promise<List<AuditEvent>> search(
        String tenantId, AuditQueryService.AuditSearchCriteria criteria) {
      return Promise.of(
          store.getOrDefault(tenantId, Collections.emptyList()).stream()
              .filter(
                  e ->
                      criteria.resourceType() == null
                          || criteria.resourceType().equals(e.resourceType()))
              .filter(
                  e ->
                      criteria.resourceId() == null || criteria.resourceId().equals(e.resourceId()))
              .filter(
                  e -> criteria.principal() == null || criteria.principal().equals(e.principal()))
              .filter(
                  e -> criteria.eventType() == null || criteria.eventType().equals(e.eventType()))
              .filter(
                  e -> criteria.fromDate() == null || !e.timestamp().isBefore(criteria.fromDate()))
