 ...
 *
 * <ul>
 *   <li>PostgreSQL test container
 *   <li>Database connection pool
 *   <li>Database cleanup between tests
 *   <li>Test data helpers
 * </ul>
 *
 * @doc.type class
...
public abstract class BaseIntegrationTest {
  ...
  protected String createTestUser(String userId, String tenantId, String email, String name)
      throws SQLException {
    try (Connection conn = dataSource.getConnection();
        Statement stmt = conn.createStatement()) {
      stmt.execute(
  ...
  protected String createTestWorkspace(
      String workspaceId, String tenantId, String ownerId, String name) throws SQLException {
    try (Connection conn = dataSource.getConnection();
        Statement stmt = conn.createStatement()) {
      stmt.execute(
          String.format(
  ...
  protected String createTestProject(
      String projectId, String tenantId, String workspaceId, String name) throws SQLException {
    try (Connection conn = dataSource.getConnection();
        Statement stmt = conn.createStatement()) {
      stmt.execute(
          String.format(
  ...
  protected void assertTenantIsolation(
      String tenantId, String tableName, String idColumn, String expectedId) throws SQLException {
    try (Connection conn = dataSource.getConnection();
        Statement stmt = conn.createStatement()) {
      var rs =
          stmt.executeQuery(
