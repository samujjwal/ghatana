 ...
 *
 * <p>Tests: - Schema loading and validation - Valid event validation - Invalid event rejection -
 * Schema existence checks - Error handling
 */
...
class EventSchemaValidatorTest {
  ...
  class SchemaLoading {
    ...
    void shouldLoadAllExpectedSchemas() {
      ...
      assertTrue(
          supportedTypes.contains("task.status.changed"),
          "Should support task.status.changed events");
      assertTrue(
          supportedTypes.contains("phase.transition"), "Should support phase.transition events");
    ...
    void shouldReportSchemaExistence() {
      ...
      assertTrue(
          validator.hasSchema("phase.transition"), "Should have schema for phase.transition");
      assertFalse(
          validator.hasSchema("unknown.event"), "Should not have schema for unknown events");
    }
  }
  ...
  class ErrorHandling {
    ...
    void shouldHandleNullEventPayload() {
      ...
      assertTrue(
          result.getErrors().get(0).contains("Validation error"), "Should return validation error");
    }
