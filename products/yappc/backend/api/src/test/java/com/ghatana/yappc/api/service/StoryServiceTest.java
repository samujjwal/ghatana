 ...
 *
 * <p>Covers story CRUD, lifecycle transitions, task management, acceptance criteria, blocker
 * management, and LLM-assisted generation.
 *
 * @doc.type class
...
class StoryServiceTest extends EventloopTestBase {
  ...
  void setUp() {
    ...
    when(repository.save(any(Story.class))).thenAnswer(inv -> Promise.of(inv.getArgument(0)));
  }
  ...
  class CreateStory {
    ...
    void shouldCreateStoryWithDefaults() {
      ...
      when(llmGateway.complete(any(CompletionRequest.class))).thenReturn(null);
    ...
    void shouldGenerateTasksFromLLM() {
      ...
      when(llmResult.getText())
          .thenReturn(
              "Design API|||Create REST endpoint design\n"
                  + "Implement handler|||Write HTTP handler code");
      when(llmGateway.complete(any(CompletionRequest.class))).thenReturn(Promise.of(llmResult));
  ...
  class GetStory {
    ...
    void shouldReturnStoryById() {
      ...
      when(repository.findById(TENANT, story.getId())).thenReturn(Promise.of(Optional.of(story)));
    ...
    void shouldReturnStoryByKey() {
      ...
      when(repository.findByKey(TENANT, "PROJ-42")).thenReturn(Promise.of(Optional.of(story)));
  ...
  class UpdateStory {
    ...
    void shouldUpdateTitleAndPriority() {
      ...
      UpdateStoryInput input = new UpdateStoryInput("Updated title", null, null, Priority.P1, null);
  ...
  class MoveStory {
    ...
    void shouldRejectInvalidTransition() {
      ...
      assertThatThrownBy(
              () ->
                  runPromise(
                      () -> service.moveStory(TENANT, story.getId(), StoryStatus.IN_PROGRESS)))
  ...
  class AssignStory {
    ...
    void shouldNotDuplicateAssignment() {
      ...
      assertThat(story.getAssignedTo().stream().filter("user-x"::equals).count()).isEqualTo(1);
    }
  ...
  class TaskManagement {
    ...
    void shouldUpdateTaskStatus() {
      ...
      runPromise(
          () -> service.updateTaskStatus(TENANT, story.getId(), task.getId().toString(), "DONE"));
