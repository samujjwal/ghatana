 ...
 *
 * <ol>
 *   <li>6.1.5 — Grafana dashboard configured with agent_execution_duration_ms, phase_advance_count,
 *       llm_call_latency
 *   <li>6.1.6 — Metrics endpoint (/metrics) exposes all collected metrics in Prometheus format
 * </ol>
 *
 * <p><b>Test Scenarios:</b>
 *
 * <ul>
 *   <li>Metrics collection active in MicrometerMetricsCollector
 *   <li>Agent execution duration histogram recorded
 *   <li>Phase advance counter incremented
 *   <li>LLM call latency measured
 *   <li>All metrics scraped via /metrics endpoint in Prometheus format
 *   <li>Metrics queryable via Prometheus queries (histogram percentiles, rates)
 *   <li>Correlation IDs included in metric tags
 * </ul>
 *
 * @doc.type class
...
class MetricsCollectionE2eTest extends EventloopTestBase {
  ...
  class PrometheusScraperTests {
    ...
    void shouldExposeMetricsInPrometheusFormat() {
      ...
      long histogramLines =
          prometheusText
              ...
              .filter(
                  line ->
                      line.contains("_bucket") || line.contains("_count") || line.contains("_sum"))
              .count();
      assertThat(histogramLines).isGreaterThan(0);

      // Find counter metric
      long counterLines = prometheusText.lines().filter(line -> line.contains("_total")).count();
    ...
    void shouldSupportHistogramQuantileQueries() {
      ...
      assertThat(prometheusText).contains("agent_execution_duration_ms_seconds_bucket");
      ...
      long bucketLines =
          prometheusText
              ...
              .filter(
                  line ->
                      line.contains("agent_execution_duration_ms_seconds_bucket")
                          && line.contains("le="))
              .count();
    ...
    void shouldSupportRateQueries() {
      ...
      assertThat(prometheusText).contains("phase_advance_count_total");
    ...
    void shouldLabelMetricsByPhase() {
      ...
      long perceiveLines =
          prometheusText.lines().filter(l -> l.contains("phase=\"PERCEIVE\"")).count();
    ...
    void shouldQueryIntentCapturePercentiles() {
      ...
      String countLine =
          prometheusText
              .lines()
              .filter(
                  l -> l.contains("intent_capture_duration_ms_seconds_count") && !l.startsWith("#"))
              .findFirst()
              .orElse("");
  ...
  /**
   * Helper class to record metrics into MicrometerMetricsCollector for testing. Simulates what the
   * actual services would do.
   */
  private static class MockMetricsRecorder {
    ...
    void recordAgentExecutionDurationWithCorrelationId(
        String agentId, long durationMs, String correlationId) {
      Timer.builder("agent_execution_duration_ms")
