 ...
 *
 * <p>Covers plan task 9.1.6 — agent turn episode stored in memory plane, survives logical restart
 * (i.e., plane is reconstructed and the item is still retrievable from the same underlying store).
 *
 * @doc.type class
...
class PersistentMemoryPlaneTest extends EventloopTestBase {
  ...
  void episodeSurvivesLogicalRestart() {
    ...
    PersistentMemoryPlane restoredPlane =
        new PersistentMemoryPlane(
            itemRepository,
