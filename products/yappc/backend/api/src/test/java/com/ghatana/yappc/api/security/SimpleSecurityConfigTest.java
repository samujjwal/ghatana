...
class SimpleSecurityConfigTest {
  ...
  void developerHasProjectAccess() {
    ...
    // Then
    assertThat(developerPermissions)
        .anyMatch(p -> p.getPathPattern().equals("/api/v1/projects/**"));
  ...
  void viewerHasLimitedAccess() {
    ...
    // Then
    assertThat(viewerPermissions)
        .anyMatch(p -> p.getPathPattern().equals("/api/v1/projects/{projectId}"));
  ...
  void addRolePermissions() {
    ...
    List<Permission> newPermissions =
        List.of(Permission.readOnly("/api/v1/new/**"), Permission.crud("/api/v1/other/**"));
  ...
  void addRolePermissionToExistingRole() {
    // Given
    securityConfig.addRolePermissions(
        "existing_role", List.of(Permission.readOnly("/api/v1/existing/**")));
  ...
  void customConstructor() {
    // Given
    Map<String, List<Permission>> customPermissions =
        Map.of("custom_role", List.of(Permission.readOnly("/api/v1/custom/**")));
