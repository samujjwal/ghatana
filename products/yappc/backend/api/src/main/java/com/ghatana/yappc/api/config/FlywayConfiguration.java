 ...
 *
 * <p>Manages schema versioning and migrations using Flyway. Migrations are located in {@code
 * src/main/resources/db/migration/} and follow the naming convention:
 *
 * <ul>
 *   <li>{@code V1__Initial_schema.sql} — versioned migrations
 *   <li>{@code R__Repeatable_migration.sql} — repeatable migrations
 * </ul>
 *
 * <p>Migration execution happens automatically on application startup via {@link
...
public class FlywayConfiguration {
   ...
   *
   * <p>This method should be called during application initialization, before any database
  ...
  public static String getMigrationInfo(DataSource dataSource) {
    try {
      ...
      return String.format(
          ...
          current != null ? current.getVersion() : "none", pending.length, info.applied().length);
    } catch (Exception e) {
   ...
   *
   * <p>This @Provides method is automatically invoked by the DI container during startup, ensuring
   * all database migrations are applied before any business logic executes.
   *
