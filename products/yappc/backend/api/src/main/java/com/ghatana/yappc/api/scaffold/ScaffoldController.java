...
public class ScaffoldController extends AbstractModule {
            ...
            templateOpt ->
                templateOpt
                    ...
                    .orElseGet(
                        () ->
                            ApiResponse.error(
                                404, "NOT_FOUND", "Template not found", request.getPath())));
  }
            ...
            scaffoldRequest -> {
              if (!scaffoldRequest.isValid()) {
                return ApiResponse.error(
                        400, "INVALID_REQUEST", "Invalid scaffold request", request.getPath())
                    .toPromise();
              }
              return scaffoldService
                  .scaffoldProject(scaffoldRequest)
            ...
            statusOpt ->
                statusOpt
                    ...
                    .orElseGet(
                        () ->
                            ApiResponse.error(
                                404, "NOT_FOUND", "Job not found", request.getPath())));
  }
                    ...
                    .map(
                        project ->
                            HttpResponse.ofCode(200)
                                ...
                                .withHeader(
                                    CONTENT_DISPOSITION,
                                    HttpHeaderValue.of("attachment; filename=\"project.zip\""))
                                .withBody(project)
                                .build())
                    .orElseGet(
                        () ->
                            ApiResponse.error(
                                404,
                                "NOT_FOUND",
                                "Project not found or not ready",
                                request.getPath())));
  }
        ...
        .map(
            success ->
                success
                    ...
                    : ApiResponse.error(
                        404, "NOT_FOUND", "Job not found or already completed", request.getPath()));
  }

  private Promise<HttpResponse> listFeaturePacks(HttpRequest request) {
    return scaffoldService.listFeaturePacks().map(packs -> ApiResponse.ok(packs));
  }
        ...
        .then(
            featurePackRequest -> {
              if (!featurePackRequest.isValid()) {
                return ApiResponse.error(
                        400, "INVALID_REQUEST", "Invalid request", request.getPath())
                    .toPromise();
              }
              return scaffoldService
                  .addFeaturePack(featurePackRequest.projectId(), featurePackRequest)
  ...
  private Promise<HttpResponse> getConflicts(HttpRequest request) {
    String jobId = request.getPathParameter("jobId");
    return scaffoldService.getConflicts(jobId).map(conflicts -> ApiResponse.ok(conflicts));
  }
  ...
  private Promise<HttpResponse> resolveConflicts(HttpRequest request) {
    ...
    return JsonUtils.parseBody(request, ConflictResolution.class)
        .then(
            resolution ->
                scaffoldService
                    .resolveConflicts(jobId, resolution)
                    .map(
                        success ->
                            success
