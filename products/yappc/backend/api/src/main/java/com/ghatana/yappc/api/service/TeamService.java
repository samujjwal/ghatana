...
public class TeamService {
  ...
  public Promise<Team> updateMemberRole(
      String tenantId, UUID teamId, String userId, MemberRole newRole) {
    return repository
        .findById(tenantId, teamId)
        .then(
            opt -> {
              ...
              if (member.getRole() == MemberRole.OWNER && newRole != MemberRole.OWNER) {
                long ownerCount =
                    team.getMembers().stream().filter(m -> m.getRole() == MemberRole.OWNER).count();
                if (ownerCount <= 1) {
                  return Promise.ofException(
                      new IllegalStateException("Cannot remove the last owner"));
                }
              }
  ...
  public record AddMemberInput(String userId, String email, String displayName, MemberRole role) {}

  public record TeamStatistics(int totalMembers, int owners, int admins, int members, int guests) {}
}
