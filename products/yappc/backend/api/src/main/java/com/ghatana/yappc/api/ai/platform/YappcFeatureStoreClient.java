 ...
 *
 * <p>Provides non-blocking access to the two-tier feature store (Redis hot path + PostgreSQL cold
 * path) for use in YAPPC's event-loop-based async pipeline. All blocking JDBC/Redis calls are
 * dispatched to the configured {@link Executor} via {@link Promise#ofBlocking}.
 *
 * <h3>YAPPC use cases</h3>
 *
 * <ul>
 *   <li>Agent feature ingestion — write agent action embeddings, complexity scores, and progress
 *       indicators as features per workspace entity.
 *   <li>LLM prompt enrichment — retrieve historical feature vectors ({@code getFeatures}) before
 *       calling the LLM gateway so the prompt contains relevant context signals.
 *   <li>Requirement analysis — store and retrieve semantic similarity scores computed during AI
...
public final class YappcFeatureStoreClient {
   ...
   * @param blockingExecutor executor for blocking JDBC/Redis calls; must not be the ActiveJ
   *     event-loop thread
   */
   ...
   * @param entityId entity identifier — typically a workspace ID, requirement ID, or agent run ID
  ...
  public Promise<Void> ingest(String tenantId, Feature feature) {
    ...
    return Promise.ofBlocking(
        ...
        () -> {
          ...
          log.debug(
              "[FeatureStore] Ingested '{}' for entity '{}' tenant='{}'",
              feature.getName(),
              feature.getEntityId(),
              tenantId);
          return null;
   ...
   *
   * <p>Features that fail individually are logged at WARN level and the remaining features continue
   * to be processed. This prevents a single bad feature from blocking an entire agent snapshot.
   *
   * @param tenantId tenant identifier
        ...
        () -> {
          ...
          for (Feature f : features) {
            try {
              ...
              log.warn(
                  "[FeatureStore] Failed to ingest '{}' tenant='{}': {}",
                  f.getName(),
                  tenantId,
                  ex.getMessage());
              err++;
          ...
          log.debug("[FeatureStore] Batch ingest — ok={} errors={} tenant='{}'", ok, err, tenantId);
          return null;
        });
  }
   ...
   *
   * <p>Probes Redis first (cache hit ≈ 1 ms P99); loads from PostgreSQL on misses and backfills
   * Redis automatically.
   *
   * @param tenantId tenant identifier
   * @param entityId entity identifier
   * @param featureNames ordered list of feature names to fetch
   * @return promise resolving to a map of featureName → value; absent features are omitted from the
   *     map (never null values)
   */
  public Promise<Map<String, Double>> getFeatures(
    ...
    return Promise.ofBlocking(
        ...
        () -> {
          ...
          log.debug(
              "[FeatureStore] Retrieved {}/{} features for entity '{}' tenant='{}'",
              result.size(),
              featureNames.size(),
              entityId,
              tenantId);
  ...
  /**
   * Convenience single-feature retrieval; returns {@code null} when the feature is absent.
   *
   * @param tenantId tenant identifier
   * @param entityId entity identifier
  ...
  public Promise<Double> getFeature(String tenantId, String entityId, String featureName) {
    return getFeatures(tenantId, entityId, List.of(featureName)).map(m -> m.get(featureName));
  }
