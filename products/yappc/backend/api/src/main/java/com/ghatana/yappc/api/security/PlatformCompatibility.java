 ...
 *
 * <p>This class bridges the security middleware and the underlying ActiveJ HTTP framework,
 * providing factory methods for common HTTP response patterns and request-context extraction.
 *
 * @doc.type class
...
public final class PlatformCompatibility {
  ...
  public static HttpResponse createResponse(int statusCode, @NotNull String body) {
    return HttpResponse.ofCode(statusCode).withBody(body.getBytes(StandardCharsets.UTF_8)).build();
  }
   ...
   *
   * <p>Objects are attached to requests via {@link HttpRequest#attach(Object)} during the
   * authentication phase and subsequently retrieved in the authorization phase.
   *
   ...
   * @return the attached object, or {@code null} if nothing is attached or the request is not an
   *     {@link HttpRequest}
   */
