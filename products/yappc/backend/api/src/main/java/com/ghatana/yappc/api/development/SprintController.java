...
public class SprintController {
                        ...
                        body -> {
                          try {
                            CreateSprintRequest req =
                                mapper.readValue(body.getArray(), CreateSprintRequest.class);
                            CreateSprintInput input =
                                new CreateSprintInput(
                                    req.projectId(),
                                    req.name(),
                                    req.goals(),
                                    req.startDate(),
                                    ...
                                    ctx.userId());
                            return service
                                .createSprint(ctx.tenantId(), input)
  ...
  public Promise<HttpResponse> getSprint(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getSprint(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> getCurrentSprint(HttpRequest request, String projectId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getCurrentSprint(ctx.tenantId(), UUID.fromString(projectId))
                    .map(
                        opt ->
                            opt.map(ApiResponse::ok)
  ...
  public Promise<HttpResponse> listProjectSprints(HttpRequest request, String projectId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .listProjectSprints(ctx.tenantId(), UUID.fromString(projectId))
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> startSprint(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.startSprint(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
                    ...
                    .then(
                        body -> {
                          try {
                            ...
                            return service
                                .completeSprint(ctx.tenantId(), UUID.fromString(id), retro)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            CancelSprintRequest req =
                                mapper.readValue(body.getArray(), CancelSprintRequest.class);
                            return service
                                .cancelSprint(ctx.tenantId(), UUID.fromString(id), req.reason())
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            UpdateGoalsRequest req =
                                mapper.readValue(body.getArray(), UpdateGoalsRequest.class);
                            return service
                                .updateSprintGoals(ctx.tenantId(), UUID.fromString(id), req.goals())
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            UpdateDatesRequest req =
                                mapper.readValue(body.getArray(), UpdateDatesRequest.class);
                            return service
                                .updateSprintDates(
                                    ctx.tenantId(),
                                    UUID.fromString(id),
  ...
  public Promise<HttpResponse> deleteSprint(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .deleteSprint(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        deleted ->
                            deleted
                                ? ApiResponse.noContent()
                                : ApiResponse.notFound("Sprint not found")))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> {
              ...
              return service
                  .getAverageVelocity(ctx.tenantId(), UUID.fromString(projectId), count)
