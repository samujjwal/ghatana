...
public class MetricController {
                        ...
                        body -> {
                          try {
                            ...
                            return metricService
                                .createMetric(ctx.tenantId(), input)
  ...
  public Promise<HttpResponse> getMetric(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                metricService
                    .getMetric(ctx.tenantId(), UUID.fromString(metricId))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> listProjectMetrics(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> metricService.listProjectMetrics(ctx.tenantId(), projectId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> searchMetrics(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> metricService.searchMetrics(ctx.tenantId(), pattern).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
        ...
        .then(
            ctx -> {
              try {
                ...
                return metricService.listMetricsByType(ctx.tenantId(), type).map(ApiResponse::ok);
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            List<DataPointInput> inputs =
                                objectMapper.readValue(
                                    body.getString(StandardCharsets.UTF_8),
                                    objectMapper
                                        .getTypeFactory()
                                        .constructCollectionType(List.class, DataPointInput.class));
                            return metricService
                                .recordDataPoints(ctx.tenantId(), UUID.fromString(metricId), inputs)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                            return Promise.of(ApiResponse.badRequest(e.getMessage()));
                          }
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> {
              ...
              return metricService
                  .queryDataPoints(ctx.tenantId(), UUID.fromString(metricId), start, end)
                  .map(ApiResponse::ok);
            })
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getMetricSummary(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                metricService
                    .getMetricSummary(ctx.tenantId(), UUID.fromString(metricId))
                    .map(
                        summary ->
                            summary != null
  ...
  public Promise<HttpResponse> getProjectStatistics(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                metricService.getProjectStatistics(ctx.tenantId(), projectId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> deleteMetric(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                metricService
                    .deleteMetric(ctx.tenantId(), UUID.fromString(metricId))
