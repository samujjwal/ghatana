...
public class AuditQueryController {
  ...
  public Promise<HttpResponse> queryAuditEvents(
    ...
    if (tenantId == null || tenantId.isEmpty()) {
      ...
      return Promise.of(
          HttpResponse.ofCode(400).withJson("{\"error\":\"tenantId required\"}").build());
    }
    ...
    logger.info(
        "[AUDIT] Query: tenant={} eventType={} principal={} limit={} offset={}",
        tenantId,
        eventType,
        principal,
        normalizedLimit,
        ...
        .map(
            events -> {
              Map<String, Object> response =
                  Map.of(
                      "events",
                      events.stream().map(this::eventToMap).collect(Collectors.toList()),
                      "limit",
                      normalizedLimit,
                      "offset",
                      normalizedOffset,
              ...
              return HttpResponse.ok200().withJson(toJson(response)).build();
            })
        .mapException(
            ex -> {
  ...
  public Promise<HttpResponse> countTotalEvents(String tenantId) {
    ...
    return auditQueryService
        ...
        .map(
            count -> {
              Map<String, Object> response = Map.of("total", count);
              return HttpResponse.ok200().withJson(toJson(response)).build();
            })
        .mapException(
            ex -> {
