        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return alertService.createAlert(workspaceId, input).map(ApiResponse::created);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return alertService
                      .getAlert(workspaceId, id)
                      .map(
                          opt ->
                              opt.map(ApiResponse::ok)
                                  .orElse(ApiResponse.notFound("Alert not found")));
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return alertService
                      .listProjectAlerts(workspaceId, projectId)
                      .map(ApiResponse::ok);
                }));
  }
    ...
    return ApiResponse.wrap(
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  UUID workspaceId = UUID.fromString(ctx.tenantId());
                  return alertService.listOpenAlerts(workspaceId).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return alertService
                      .listAlertsBySeverity(workspaceId, severity)
                      .map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return alertService
                      .acknowledgeAlert(workspaceId, id, userId)
                      .map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  UUID id = UUID.fromString(request.getPathParameter("id"));
                  UUID userId = UUID.fromString(ctx.userId());
                  return alertService.resolveAlert(workspaceId, id, userId).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  UUID projectId = UUID.fromString(request.getPathParameter("projectId"));
                  return alertService
                      .getAlertStatistics(workspaceId, projectId)
                      .map(ApiResponse::ok);
                }));
  }
