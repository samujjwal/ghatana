 ...
 *
 * <p>All async operations use ActiveJ {@link Promise} — no {@code CompletableFuture} or blocking
 * {@code .get()} calls are used anywhere in the request path.
 *
 * <p>Features:
 *
 * <ul>
...
public class SecurityMiddleware implements AsyncServlet {
  ...
  private boolean requiresAuthentication(@NotNull HttpRequest request) {
    ...
    List<String> publicPaths =
        Arrays.asList("/health", "/metrics", "/api/auth/login", "/api/auth/register", "/api/docs");
   ...
   *
   * <p>Uses ActiveJ {@link Promise} returned by {@link JwtTokenProvider#validateToken} without
   ...
   *
   * <p>Returns {@link Promise} (not {@code CompletableFuture}) for consistency with the ActiveJ
   * async model.
   */
  private Promise<Boolean> hasRequiredPermission(
      @NotNull UserContext user, @NotNull String path, @NotNull String method) {
    if (user.getRoles().contains("admin")) {
  ...
  private HttpResponse addSecurityHeaders(@NotNull HttpResponse response) {
    ...
    SECURITY_HEADERS.forEach((key, value) -> builder.withHeader(HttpHeaders.of(key), value));
  ...
  public static SecurityMiddleware create(
      @NotNull JwtTokenProvider jwtTokenProvider, @NotNull AsyncServlet delegate) {
