...
public class CanvasCollaborationHandler implements WebSocketMessageHandler {
  ...
  @Inject
  public CanvasCollaborationHandler(
      ConnectionManager connectionManager, PresenceManager presenceManager) {
  ...
  private void handleJoin(WebSocketConnection connection, Map<String, Object> message) {
    ...
    presenceManager.userConnected(
        connection.getTenantId(), connection.getUserId(), sessionId, metadata);
    ...
    Map<String, Object> response =
        Map.of(
            ...
            sessionId,
            "onlineUsers",
            presenceManager.getOnlineUsersInSession(connection.getTenantId(), sessionId),
            "timestamp",
  ...
  private void handleLeave(WebSocketConnection connection, Map<String, Object> message) {
    ...
    presenceManager.userDisconnected(connection.getTenantId(), connection.getUserId(), sessionId);
  ...
  private void handleUpdate(WebSocketConnection connection, Map<String, Object> message) {
    ...
    logger.debug(
        "Canvas update from user {} in session {}: {}",
  ...
  private void handleCursor(WebSocketConnection connection, Map<String, Object> message) {
    ...
    presenceManager.updateCursor(
        connection.getTenantId(), connection.getUserId(), sessionId, cursor);
  }
  ...
  private void sendError(WebSocketConnection connection, String error) {
    Map<String, Object> errorMessage =
        Map.of("type", "error", "error", error, "timestamp", System.currentTimeMillis());
