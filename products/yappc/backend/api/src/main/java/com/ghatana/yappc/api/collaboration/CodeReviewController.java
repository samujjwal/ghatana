...
public class CodeReviewController {
                        ...
                        body -> {
                          try {
                            CreateReviewRequest req =
                                mapper.readValue(body.getArray(), CreateReviewRequest.class);
                            CreateReviewInput input =
                                new CreateReviewInput(
                                    req.projectId(),
                                    req.storyId(),
                                    req.pullRequestUrl(),
                                    ...
                                    req.fileChanges());
                            return service
                                .createReview(ctx.tenantId(), input)
  ...
  public Promise<HttpResponse> getReview(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getReview(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> listProjectReviews(HttpRequest request, String projectId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.listProjectReviews(ctx.tenantId(), projectId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listStoryReviews(HttpRequest request, String storyId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.listStoryReviews(ctx.tenantId(), storyId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listMyPendingReviews(HttpRequest request) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.listPendingReviews(ctx.tenantId(), ctx.userId()).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listMyAuthoredReviews(HttpRequest request) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.listAuthoredReviews(ctx.tenantId(), ctx.userId()).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
                        ...
                        body -> {
                          try {
                            SubmitReviewRequest req =
                                mapper.readValue(body.getArray(), SubmitReviewRequest.class);
                            SubmitReviewInput input =
                                new SubmitReviewInput(ctx.userId(), req.decision(), req.comment());
                            return service
                                .submitReview(ctx.tenantId(), UUID.fromString(id), input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                ...
                request
                    ...
                    .then(
                        body -> {
                          try {
                            CommentRequest req =
                                mapper.readValue(body.getArray(), CommentRequest.class);
                            return service
                                .approveReview(
                                    ctx.tenantId(),
                                    UUID.fromString(id),
                        ...
                        body -> {
                          try {
                            AddCommentRequest req =
                                mapper.readValue(body.getArray(), AddCommentRequest.class);
                            AddCommentInput input =
                                new AddCommentInput(
                                    ...
                                    req.suggestionCode());
                            return service
                                .addComment(ctx.tenantId(), UUID.fromString(id), input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
  ...
  public Promise<HttpResponse> resolveComment(HttpRequest request, String id, String commentId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .resolveComment(ctx.tenantId(), UUID.fromString(id), commentId, ctx.userId())
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> mergeReview(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.mergeReview(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> closeReview(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.closeReview(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getProjectStatistics(HttpRequest request, String projectId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.getProjectStatistics(ctx.tenantId(), projectId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public record SubmitReviewRequest(ReviewDecision decision, String comment) {}
