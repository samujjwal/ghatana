            ...
            ctx ->
                JsonUtils.parseBody(request, CreateWorkflowRequest.class)
                    .map(
                        req -> {
                          logger.info(
                              "Creating approval workflow for tenant={} user={}",
                              ctx.tenantId(),
                              ctx.userId());
                          return ApiResponse.created(
                              approvalService.create(ctx.tenantId(), ctx.userId(), req));
                        }))
  ...
  public Promise<HttpResponse> getPendingApprovals(HttpRequest request) {
    return TenantContextExtractor.requireAuthenticated(request)
        .map(
            ctx -> {
              logger.debug(
                  "Fetching pending approvals for user={} tenant={}", ctx.userId(), ctx.tenantId());
            ...
            ctx ->
                JsonUtils.parseBody(request, SubmitDecisionRequest.class)
                    .map(
                        req -> {
                          logger.info(
                              "Decision submitted for workflow={} by user={} decision={}",
                              id,
                              ctx.userId(),
                              req.decision());
                          return ApiResponse.ok(
                              approvalService.submitDecision(
                                  ctx.tenantId(), ctx.userId(), id, req));
                        }))
        .then(Promise::of, e -> Promise.of(ApiResponse.fromException(e)));
  }
