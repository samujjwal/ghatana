...
public class StoryController {
                        ...
                        body -> {
                          try {
                            CreateStoryRequest req =
                                mapper.readValue(body.getArray(), CreateStoryRequest.class);
                            CreateStoryInput input =
                                new CreateStoryInput(
                                    ...
                                    req.generateAcceptanceCriteria() != null
                                        ? req.generateAcceptanceCriteria()
                                        : true);
                            return service
                                .createStory(ctx.tenantId(), input)
  ...
  public Promise<HttpResponse> getStory(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getStory(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        opt ->
                            opt.map(ApiResponse::ok)
  ...
  public Promise<HttpResponse> getStoryByKey(HttpRequest request, String key) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getStoryByKey(ctx.tenantId(), key)
                    .map(
                        opt ->
                            opt.map(ApiResponse::ok)
                                .orElse(ApiResponse.notFound("Story not found"))))
  ...
  public Promise<HttpResponse> listSprintStories(HttpRequest request, String sprintId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .listSprintStories(ctx.tenantId(), UUID.fromString(sprintId))
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listBacklog(HttpRequest request, String projectId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .listBacklog(ctx.tenantId(), UUID.fromString(projectId))
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listAssignedStories(HttpRequest request, String userId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.listAssignedStories(ctx.tenantId(), userId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listBlockedStories(HttpRequest request, String projectId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .listBlockedStories(ctx.tenantId(), UUID.fromString(projectId))
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            UpdateStoryRequest req =
                                mapper.readValue(body.getArray(), UpdateStoryRequest.class);
                            UpdateStoryInput input =
                                new UpdateStoryInput(
                                    req.title(),
                                    req.description(),
                                    req.type(),
                                    req.priority(),
                                    req.storyPoints());
                            return service
                                .updateStory(ctx.tenantId(), UUID.fromString(id), input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            MoveStoryRequest req =
                                mapper.readValue(body.getArray(), MoveStoryRequest.class);
                            return service
                                .moveStory(ctx.tenantId(), UUID.fromString(id), req.status())
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            AssignStoryRequest req =
                                mapper.readValue(body.getArray(), AssignStoryRequest.class);
                            return service
                                .assignStory(ctx.tenantId(), UUID.fromString(id), req.userId())
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
  ...
  public Promise<HttpResponse> unassignStory(HttpRequest request, String id, String userId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .unassignStory(ctx.tenantId(), UUID.fromString(id), userId)
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            MoveToSprintRequest req =
                                mapper.readValue(body.getArray(), MoveToSprintRequest.class);
                            UUID sprintId =
                                req.sprintId() != null ? UUID.fromString(req.sprintId()) : null;
                            return service
                                .moveToSprint(ctx.tenantId(), UUID.fromString(id), sprintId)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                    ...
                    .then(
                        body -> {
                          try {
                            ...
                            return service
                                .addTask(ctx.tenantId(), UUID.fromString(id), task)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                        ...
                        body -> {
                          try {
                            UpdateTaskRequest req =
                                mapper.readValue(body.getArray(), UpdateTaskRequest.class);
                            return service
                                .updateTaskStatus(
                                    ctx.tenantId(), UUID.fromString(id), taskId, req.status())
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                        ...
                        body -> {
                          try {
                            AcceptanceCriterion criterion =
                                mapper.readValue(body.getArray(), AcceptanceCriterion.class);
                            return service
                                .addAcceptanceCriterion(
                                    ctx.tenantId(), UUID.fromString(id), criterion)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
  ...
  public Promise<HttpResponse> updateAcceptanceCriterion(
      HttpRequest request, String id, String criterionId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            UpdateCriterionRequest req =
                                mapper.readValue(body.getArray(), UpdateCriterionRequest.class);
                            return service
                    ...
                    .then(
                        body -> {
                          try {
                            ...
                            return service
                                .linkPullRequest(ctx.tenantId(), UUID.fromString(id), pr)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            AddBlockerRequest req =
                                mapper.readValue(body.getArray(), AddBlockerRequest.class);
                            return service
                                .addBlocker(
                                    ctx.tenantId(), UUID.fromString(id), req.blockingStoryId())
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
  ...
  public Promise<HttpResponse> removeBlocker(HttpRequest request, String id, String blockerId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .removeBlocker(ctx.tenantId(), UUID.fromString(id), blockerId)
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> deleteStory(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .deleteStory(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        deleted ->
                            deleted
                                ? ApiResponse.noContent()
                                : ApiResponse.notFound("Story not found")))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public record UpdateStoryRequest(
      String title, String description, StoryType type, Priority priority, Integer storyPoints) {}
