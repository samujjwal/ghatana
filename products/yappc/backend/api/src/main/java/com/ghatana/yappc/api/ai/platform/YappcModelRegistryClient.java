 ...
 *
 * <p>Exposes the model lifecycle capabilities needed by YAPPC agents and AI services — registering
 * new model versions, querying active models, and transitioning between deployment stages — without
 * blocking the ActiveJ event-loop thread. All JDBC-backed operations are dispatched to the provided
 * {@link Executor} via {@link Promise#ofBlocking}.
 *
 * <h3>YAPPC use cases</h3>
 *
 * <ul>
 *   <li>{@link com.ghatana.products.yappc.domain.agent.PredictionAgent} — fetch the current {@code
 *       PRODUCTION} model for phase prediction.
 *   <li>AI suggestion pipeline — register fine-tuned code suggestion models and promote them after
 *       offline evaluation.
 *   <li>Canary evaluation — query {@code CANARY} models for shadow-mode inference before full
 *       promotion.
 * </ul>
 *
 * <h3>Example</h3>
...
public final class YappcModelRegistryClient {
   ...
   * @param blockingExecutor executor for blocking JDBC calls; must not be the ActiveJ event-loop
   *     thread
   */
  ...
  /**
   * Returns the single {@code PRODUCTION} or {@code ACTIVE} model for the given name; empty when no
   * active model exists.
   *
  ...
  public Promise<Optional<ModelMetadata>> findActiveModel(String tenantId, String modelName) {
    ...
    return Promise.ofBlocking(
        blockingExecutor,
        () ->
            delegate.findByStatus(tenantId, DeploymentStatus.ACTIVE).stream()
                .filter(m -> m.getName().equals(modelName))
                .findFirst()
                .or(
                    () ->
                        delegate.findByStatus(tenantId, DeploymentStatus.PRODUCTION).stream()
                            .filter(m -> m.getName().equals(modelName))
                            .findFirst()));
  }
  ...
  public Promise<Optional<ModelMetadata>> findByVersion(
    ...
    return Promise.ofBlocking(
        blockingExecutor, () -> delegate.findByName(tenantId, modelName, version));
  }
  ...
  public Promise<List<ModelMetadata>> listVersions(String tenantId, String modelName) {
    Objects.requireNonNull(tenantId, "tenantId");
    Objects.requireNonNull(modelName, "modelName");
    return Promise.ofBlocking(blockingExecutor, () -> delegate.listVersions(tenantId, modelName));
  }
  ...
  public Promise<List<ModelMetadata>> listByStatus(String tenantId, DeploymentStatus status) {
    ...
    return Promise.ofBlocking(blockingExecutor, () -> delegate.findByStatus(tenantId, status));
  }
  ...
  public Promise<Void> registerStaged(
    ...
    return Promise.ofBlocking(
        ...
        () -> {
          ...
          log.info(
              "[ModelRegistry] Registered STAGED model '{}/{}' tenant='{}'",
              modelName,
              version,
              tenantId);
          return null;
