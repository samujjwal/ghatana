 ...
 *
 * <ul>
 *   <li>Project-specific connection management
 *   <li>User authentication and tenant isolation
 *   <li>Automatic cleanup of disconnected clients
 *   <li>Heartbeat/ping-pong for connection health
 *   <li>Event broadcasting to connected clients
 * </ul>
 *
 * @doc.type class
...
public class LifecycleWebSocketHandler {
  ...
  private final Map<String, CopyOnWriteArrayList<WebSocketConnection>> projectConnections =
      new ConcurrentHashMap<>();
  private volatile boolean shuttingDown = false;

  // Configuration
  private static final long heartbeatIntervalMs = 30_000;
  private static final long connectionTimeoutMs = 60_000;
  ...
  public Promise<WebSocket> handleConnection(
      @NotNull WebSocket webSocket, @Nullable WebSocketUserContext user) {
    try {
      ...
      if (!canAccessProject(user, projectId)) {
        LOG.warn(
            "WebSocket connection rejected: user {} cannot access project {}",
            user.getUserId(),
            projectId);
        webSocket.close();
        return Promise.of(null);
      ...
      WebSocketConnection connection =
          new WebSocketConnection(connectionId, webSocket, user, projectId);
      ...
      LOG.info(
          "WebSocket connection established: {} for user {} in project {}",
          connectionId,
          user.getUserId(),
          projectId);
  ...
  public void broadcastLifecycleUpdate(
      @NotNull String projectId, @NotNull Map<String, Object> update) {
    ...
    LOG.debug(
        "Broadcasted lifecycle update to project {}: {} successful, {} failed",
        projectId,
        successCount,
        failureCount);
  }
  ...
  public Map<String, Object> getStatistics() {
    ...
    projectConnections.forEach((projectId, conns) -> projectCounts.put(projectId, conns.size()));
  ...
  private void cleanupProjectConnection(@NotNull WebSocketConnection connection) {
    CopyOnWriteArrayList<WebSocketConnection> projectConns =
        projectConnections.get(connection.getProjectId());
   ...
   *
   * <p>Tracks connection state and delegates to the underlying ActiveJ {@link WebSocket}. Message
   * reading is started automatically on construction via a non-blocking read loop.
   *
   * @doc.type class
  ...
  private static class WebSocketConnection {
    ...
    public WebSocketConnection(
        String id, WebSocket webSocket, WebSocketUserContext user, String projectId) {
      this.id = id;
    ...
    public WebSocketUserContext getUser() {
      return user;
    }
    ...
    public void sendMessage(@NotNull String message) {
      if (closed) return;
      webSocket
          .writeMessage(WebSocket.Message.text(message))
          .whenException(
              e -> {
