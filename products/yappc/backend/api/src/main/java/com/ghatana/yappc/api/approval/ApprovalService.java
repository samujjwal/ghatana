 ...
 *
 * <ul>
 *   <li>Multi-stage sequential approval chains
 *   <li>Per-stage approver lists and required approval counts
 *   <li>Approve / reject decisions with comments
 *   <li>Automatic stage advancement on quorum
 *   <li>Workflow cancellation by initiator
 *   <li>Tenant-isolated storage
 * </ul>
 *
 * <p><b>Note:</b> This implementation uses in-memory storage. Replace with a persistent store
 * (PostgreSQL via platform:java:database) before production use.
 *
 * @doc.type class
...
public class ApprovalService {
  ...
  public WorkflowResponse create(String tenantId, String userId, CreateWorkflowRequest req) {
    ...
    logger.info(
        "[tenant={}] Created approval workflow {} (type={}, stages={})",
        tenantId,
        workflowId,
        req.workflowType(),
        stages.size());
  ...
  public WorkflowResponse submitDecision(
    ...
    if (wf.status == Status.APPROVED || wf.status == Status.REJECTED) {
      throw new ApiException(
          409, "WORKFLOW_CLOSED", "Workflow is already " + wf.status.name().toLowerCase());
    }
    ...
    if (!currentStage.approvers.isEmpty() && !currentStage.approvers.contains(userId)) {
      throw new ApiException(
          403, "NOT_APPROVER", "User is not an approver for stage: " + currentStage.name);
    }

    // Check for duplicate decision
    boolean alreadyDecided = currentStage.records.stream().anyMatch(r -> r.userId.equals(userId));
    if (alreadyDecided) {
      throw new ApiException(
          409, "ALREADY_DECIDED", "User has already submitted a decision for this stage");
    }
    ...
    if (decision == Decision.REJECT) {
      ...
      logger.info(
          "[tenant={}] Workflow {} REJECTED at stage '{}' by {}",
          tenantId,
          workflowId,
          currentStage.name,
          userId);
      ...
      if (wf.currentStageIndex < wf.stages.size() - 1) {
        ...
        logger.info(
            "[tenant={}] Workflow {} advanced to stage '{}' (index={})",
            tenantId,
            workflowId,
        ...
        logger.info("[tenant={}] Workflow {} APPROVED (all stages complete)", tenantId, workflowId);
      }
    } else {
      wf.status = Status.IN_PROGRESS;
    }
  ...
  public WorkflowResponse cancel(String tenantId, String userId, String workflowId) {
    ...
    if (!wf.initiator.equals(userId)) {
      throw new ApiException(403, "NOT_INITIATOR", "Only the workflow initiator can cancel");
    }
    if (wf.status == Status.APPROVED || wf.status == Status.REJECTED) {
      throw new ApiException(
          409,
  ...
  private Decision parseDecision(String raw) {
    try {
      ...
      throw new ApiException(
          400, "INVALID_DECISION", "Decision must be APPROVE or REJECT, got: " + raw);
    }
  }
  ...
  private WorkflowResponse toResponse(Workflow wf) {
    ...
    for (int i = 0; i < wf.stages.size(); i++) {
      ...
      stageResponses.add(
          new StageResponse(s.name, s.approvers, List.of(), s.requiredApprovals, s.parallel));
    ...
    String currentStageName = wf.stages.isEmpty() ? "" : wf.stages.get(wf.currentStageIndex).name;
    ...
    return new WorkflowResponse(
        ...
        wf.currentStageIndex,
        currentStageName,
        wf.createdAt.toString(),
        wf.updatedAt.toString());
  }
}
