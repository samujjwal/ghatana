 ...
 *
 * <p>All workflow runs are persisted to PostgreSQL so they survive server restarts. {@link
 * WorkflowRun} state is serialised as follows:
 *
 * <ul>
 *   <li>{@code step_statuses} — JSONB array of {@link StepStatus} names
 *   <li>{@code status} — TEXT column holding the {@link RunStatus} name
 *   <li>{@code failure_reason} — nullable TEXT
 * </ul>
 *
...
public class JdbcWorkflowStateStore implements WorkflowStateStore {
  ...
  public void save(@NotNull WorkflowRun run) {
    try (Connection conn = dataSource.getConnection();
      ...
      List<String> stepNames = java.util.Arrays.stream(steps).map(Enum::name).toList();
  ...
  public Optional<WorkflowRun> load(@NotNull String workflowId) {
    try (Connection conn = dataSource.getConnection();
      ...
      StepStatus[] stepStatuses =
          stepNames.stream().map(StepStatus::valueOf).toArray(StepStatus[]::new);

      WorkflowRun run = WorkflowRun.restore(workflowId, stepStatuses, status, failure);
      log.debug(
          "Loaded workflow run: id={} status={} steps={}", workflowId, status, stepNames.size());
