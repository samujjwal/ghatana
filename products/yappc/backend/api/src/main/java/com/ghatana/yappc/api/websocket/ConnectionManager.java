...
public class ConnectionManager {
  ...
  private final Map<String, Map<String, WebSocketConnection>> connections =
  ...
  public void add(WebSocketConnection connection) {
    connections
        .computeIfAbsent(connection.getTenantId(), k -> new ConcurrentHashMap<>())
  ...
  public void remove(WebSocketConnection connection) {
    ...
    logger.info(
        "User {} disconnected from tenant {}", connection.getUserId(), connection.getTenantId());
  }
