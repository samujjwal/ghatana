 ...
 *
 * <ul>
 *   <li>POST /api/testing/generate - Generate tests from source code
 *   <li>POST /api/testing/coverage - Analyze test coverage gaps
 *   <li>GET /api/testing/templates - List test templates
 * </ul>
 *
 * @doc.type class
...
public class TestGenerationController extends AbstractModule {
        ...
        .then(
            request -> {
              log.info(
                  ...
                  request.framework(),
                  request.testType());
              return testGenerationService.generateTests(request).map(ApiResponse::ok);
            });
  }
    ...
    return JsonUtils.parseBody(httpRequest, CoverageAnalysisRequest.class)
        .then(
            request -> {
              ...
              return testGenerationService.analyzeCoverage(request).map(ApiResponse::ok);
            });
  }
  ...
  public Promise<HttpResponse> listTemplates(HttpRequest httpRequest) {
    ...
    return testGenerationService.listTemplates(framework).map(ApiResponse::ok);
  }
}
