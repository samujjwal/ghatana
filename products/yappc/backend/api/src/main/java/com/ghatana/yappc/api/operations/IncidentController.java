            ...
            .then(
                ctx -> {
                  ...
                  CreateIncidentInput input =
                      objectMapper.readValue(body, CreateIncidentInput.class);
                  return incidentService
                      .createIncident(workspaceId, input)
                      ...
                      .map(
                          opt ->
                              opt.map(ApiResponse::ok)
                                  .orElse(ApiResponse.notFound("Incident not found")));
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return incidentService
                      .listProjectIncidents(workspaceId, projectId)
                      .map(ApiResponse::ok);
                }));
  }
    ...
    return ApiResponse.wrap(
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  UUID workspaceId = UUID.fromString(ctx.tenantId());
                  return incidentService.listOpenIncidents(workspaceId).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return incidentService
                      .listIncidentsBySeverity(workspaceId, severity)
                      .map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  UUID workspaceId = UUID.fromString(ctx.tenantId());
                  UUID id = UUID.fromString(request.getPathParameter("id"));
                  return incidentService.startInvestigation(workspaceId, id).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return incidentService
                      .resolveIncident(workspaceId, id, input)
                      .map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  UUID workspaceId = UUID.fromString(ctx.tenantId());
                  UUID id = UUID.fromString(request.getPathParameter("id"));
                  return incidentService.closeIncident(workspaceId, id).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return incidentService
                      .assignIncident(workspaceId, id, assigneeId)
                      .map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  UUID projectId = UUID.fromString(request.getPathParameter("projectId"));
                  return incidentService
                      .getIncidentStatistics(workspaceId, projectId)
                      .map(ApiResponse::ok);
                }));
  }
