 ...
 *
 * <p><b>Purpose</b><br>
 * Implements business logic for story management, including creation, lifecycle transitions, task
 * management, and AI-assisted features.
 *
 * @doc.type class
...
public class StoryService {
  ...
  @Inject
  public StoryService(
      StoryRepository repository, AuditService auditService, LLMGateway llmGateway) {
  ...
  public Promise<Story> updateTaskStatus(
      String tenantId, UUID storyId, String taskId, String status) {
    return repository
        .findById(tenantId, storyId)
        .then(
            opt -> {
  ...
  public Promise<Story> addAcceptanceCriterion(
      String tenantId, UUID storyId, AcceptanceCriterion criterion) {
    return repository
        .findById(tenantId, storyId)
        .then(
            opt -> {
  ...
  public Promise<Story> updateAcceptanceCriterion(
      String tenantId, UUID storyId, String criterionId, boolean completed) {
    return repository
        .findById(tenantId, storyId)
        .then(
            opt -> {
  ...
  private Promise<List<Task>> generateTasksFromDescription(String title, String description) {
    CompletionRequest request =
        CompletionRequest.builder()
            .prompt(
                "You are a software project manager. Generate implementation tasks for the following user story.\n\n"
                    + "Title: "
                    + title
                    + "\nDescription: "
                    + description
    ...
    if (completionPromise == null) {
      logger.warn(
          "LLM gateway returned null completion promise for task generation, using fallback");
      completionPromise =
          Promise.ofException(
  ...
  private Promise<List<AcceptanceCriterion>> generateAcceptanceCriteria(
      String title, String description) {
    CompletionRequest request =
        CompletionRequest.builder()
            .prompt(
                "You are a QA engineer. Generate acceptance criteria for the following user story.\n\n"
                    + "Title: "
                    + title
                    + "\nDescription: "
                    + description
    ...
    if (completionPromise == null) {
      logger.warn(
          "LLM gateway returned null completion promise for criteria generation, using fallback");
      completionPromise =
          Promise.ofException(
              new IllegalStateException("LLM gateway returned null completion promise"));
    }
