 ...
 *
 * <ul>
 *   <li>Executing agent tasks on work items
 *   <li>Batch execution of multiple agent requests
 *   <li>Cancelling pending executions
 *   <li>Querying execution status and agent health
 * </ul>
 *
 * <p><b>API Endpoints:</b>
 *
...
public class WorkflowAgentController {
  ...
  public WorkflowAgentController(
      WorkflowAgentService agentService, WorkflowAgentRegistry agentRegistry) {
            ...
            body -> {
              try {
                ...
                Map<String, Object> requestBody =
                    parseJsonBody(body.getString(StandardCharsets.UTF_8));
                ...
                Map<String, Object> input =
                    (Map<String, Object>) requestBody.getOrDefault("input", Map.of());
                String priorityStr = (String) requestBody.getOrDefault("priority", "medium");

                if (agentId == null || roleCode == null || itemId == null) {
                  return Promise.of(
                      ApiResponse.badRequest("Missing required fields: agentId, role, itemId"));
                }
            ...
            body -> {
              try {
                ...
                List<Map<String, Object>> requests =
                    (List<Map<String, Object>>) requestBody.get("requests");
                ...
                for (Map<String, Object> req : requests) {
                  ...
                  @SuppressWarnings("unchecked")
                  Map<String, Object> input =
                      (Map<String, Object>) req.getOrDefault("input", Map.of());
                ...
                return agentService
                    .executeBatch(agentRequests)
                    .map(
                        results -> {
                          List<Map<String, Object>> resultMaps =
                              results.stream().map(this::toResponseMap).toList();
  ...
  public Promise<HttpResponse> cancelExecution(HttpRequest request) {
    ...
    return agentService
        ...
        .map(
            cancelled -> {
              if (cancelled) {
                return ApiResponse.ok(Map.of("cancelled", true, "requestId", requestId));
              } else {
                return ApiResponse.notFound(
                    "Execution not found or already completed: " + requestId);
              }
            });
  }
  ...
  public Promise<HttpResponse> getExecutionStatus(HttpRequest request) {
    ...
    return agentService
        .getStatus(requestId)
        .map(status -> ApiResponse.ok(Map.of("requestId", requestId, "status", status.name())));
  }
  ...
  public Promise<HttpResponse> listAgents(HttpRequest request) {
    ...
    for (WorkflowAgentRole role : WorkflowAgentRole.values()) {
      Promise<List<Map<String, Object>>> roleAgents =
          agentRegistry
              ...
              .then(
                  agentIds -> {
                    ...
                    for (String agentId : agentIds) {
                      Promise<Map<String, Object>> agentPromise =
                          agentRegistry
                  ...
                  .map(
                      agentMaps -> {
                        List<Map<String, Object>> agents =
                            agentMaps.stream().filter(map -> !map.isEmpty()).toList();
  ...
  private Map<String, Object> toAgentMap(
      String agentId, WorkflowAgentRegistry.AgentMetadata metadata) {
    return Map.of(
        "id", agentId,
        "name", metadata.name(),
