 ...
 *
 * <ul>
 *   <li>Cost tracking per tenant/user/feature
 *   <li>Latency monitoring (p50, p95, p99)
 *   <li>Token usage tracking
 *   <li>Error rate monitoring
 *   <li>Cache hit rate
 * </ul>
 *
 * <p>Metrics are logged and can be exported to Prometheus, DataDog, or other monitoring systems.
 *
 * @doc.type class
...
public class LLMObservabilityTracker {
  ...
  public void track(LLMMetrics metrics) {
    ...
    if (metrics.getTenantId() != null) {
      tenantMetrics
          .computeIfAbsent(metrics.getTenantId(), k -> new TenantMetrics())
          .update(metrics);
    }
    ...
    if (metrics.getFeature() != null) {
      featureMetrics
          .computeIfAbsent(metrics.getFeature(), k -> new FeatureMetrics())
          .update(metrics);
    }
    ...
    // NOTE: Metrics are collected in-memory; production wiring to Prometheus/OTel is via platform
    // MetricsCollector.
  }
  ...
  private void logMetrics(LLMMetrics metrics) {
    ...
    if (metrics.getEstimatedCost() > 0.10) {
      logger.warn(
          "High-cost LLM request: ${} for feature '{}' (tenant: {})",
    ...
    if (metrics.getLatencyMs() > 5000) {
      logger.warn(
          "Slow LLM request: {}ms for model '{}' (feature: {})",
