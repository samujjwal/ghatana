...
public class RailController {
  ...
  public Promise<HttpResponse> getComponents(HttpRequest request) {
    ...
    return railService.getComponents(tenantId, category, query, mode).map(ApiResponse::ok);
  }

  public Promise<HttpResponse> getInfrastructure(HttpRequest request) {
    String tenantId = resolveTenantId(request);
    String mode = request.getQueryParameter("mode");
    return railService.getInfrastructure(tenantId, mode).map(ApiResponse::ok);
  }

  public Promise<HttpResponse> getFiles(HttpRequest request) {
    String path = request.getQueryParameter("path");
    return railService.getFiles(path != null ? path : "/").map(ApiResponse::ok);
  }

  public Promise<HttpResponse> getDataSources(HttpRequest request) {
    return railService.getDataSources().map(ApiResponse::ok);
  }

  public Promise<HttpResponse> getHistory(HttpRequest request) {
    String tenantId = resolveTenantId(request);
    return railService.getHistory(tenantId).map(ApiResponse::ok);
  }

  public Promise<HttpResponse> getFavorites(HttpRequest request) {
    String tenantId = resolveTenantId(request);
    return railService.getFavorites(tenantId).map(ApiResponse::ok);
  }
        ...
        .then(
            body -> {
              ...
              try {
                ...
                logger.warn(
                    "Invalid rail suggestion request body, using empty context: {}",
                    e.getMessage());
              }
