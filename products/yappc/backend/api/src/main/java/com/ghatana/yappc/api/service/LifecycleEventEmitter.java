 ...
 *
 * <p><b>Purpose</b><br>
 * Provides a typed API for emitting domain events from YAPPC domain services. Each method
 * corresponds to a specific lifecycle transition (e.g., requirement created, sprint started, build
 * completed). Events are published to AEP in a fire-and-forget manner — emission failures are
 * logged but never propagated to callers so that the main domain flow is never blocked.
 *
 ...
 * This class is a thin adapter (not a domain object) and lives in the application service layer. It
 * depends on {@link AepService} (infrastructure) to publish.
 *
 * @doc.type class
...
public class LifecycleEventEmitter {
  ...
  /**
   * Creates a no-op {@link LifecycleEventEmitter} that logs events without publishing. Suitable for
  ...
  public static LifecycleEventEmitter noop(ObjectMapper objectMapper) {
    return new LifecycleEventEmitter(null, objectMapper) {
      ...
      protected void emit(String eventType, String tenantId, Map<String, Object> payload) {
        logger.debug(
            "[NOOP] Would emit event type={} tenantId={} payload={}", eventType, tenantId, payload);
      }
    };
  }
  ...
  /**
   * Builds the event envelope and publishes it fire-and-forget via AepService. Errors are caught
   * and logged — never propagated to callers.
   *
  ...
  protected void emit(String eventType, String tenantId, Map<String, Object> payload) {
    try {
      ...
      logger.warn(
          "Failed to publish lifecycle event type={} tenantId={}: {}",
          eventType,
          tenantId,
          e.getMessage());
    } catch (Exception e) {
      logger.warn(
          "Unexpected error emitting lifecycle event type={} tenantId={}: {}",
          eventType,
          tenantId,
          e.getMessage());
    }
