...
public class PluginAuditController {
   ...
   *
   * <p>Returns the audit trail for the given plugin id filtered to the authenticated tenant.
   *
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  log.debug(
                      "Plugin audit requested for pluginId={} tenantId={}",
                      pluginId,
                      ctx.tenantId());
                  return auditStore.getRecords(pluginId, ctx.tenantId()).map(ApiResponse::ok);
                }));
  }
}
