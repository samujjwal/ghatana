...
public class AlertService {
  ...
  public Promise<SecurityAlert> createAlert(UUID workspaceId, CreateAlertInput input) {
    ...
    SecurityAlert alert =
        SecurityAlert.of(workspaceId, input.alertType(), input.severity(), input.title());
  ...
  public Promise<Optional<SecurityAlert>> getAlert(UUID workspaceId, UUID alertId) {
    return repository.findById(workspaceId, alertId).map(Optional::ofNullable);
  }
    ...
    return repository
        ...
        .map(
            alerts -> {
              ...
              stats.criticalCount = alerts.stream().filter(SecurityAlert::isCritical).count();
              stats.acknowledgedCount =
                  alerts.stream()
                      .filter(a -> "ACKNOWLEDGED".equalsIgnoreCase(a.getStatus()))
                      .count();
              stats.resolvedCount =
                  alerts.stream().filter(a -> "RESOLVED".equalsIgnoreCase(a.getStatus())).count();
              return stats;
            });
  }
