...
public class TraceService {
  ...
  public Promise<Trace> addSpan(String tenantId, String traceId, AddSpanInput input) {
    return repository
        .findByTraceId(tenantId, traceId)
        .then(
            trace -> {
              if (trace == null) {
  ...
  /** Completes a span. */
  public Promise<Trace> completeSpan(
      String tenantId, String traceId, String spanId, CompleteSpanInput input) {
    return repository
        .findByTraceId(tenantId, traceId)
        .then(
            trace -> {
  ...
  public Promise<Optional<Trace>> getTrace(String tenantId, UUID traceId) {
    return repository.findById(tenantId, traceId).map(t -> Optional.ofNullable(t));
  }

  /** Gets a trace by trace ID string. */
  public Promise<Optional<Trace>> getTraceByTraceId(String tenantId, String traceId) {
    return repository.findByTraceId(tenantId, traceId).map(t -> Optional.ofNullable(t));
  }
  ...
  public Promise<List<Trace>> listOperationTraces(
      String tenantId, String service, String operation, Integer limit) {
    int effectiveLimit = normalizeLimit(limit);
  ...
  public Promise<List<Trace>> listSlowTraces(
      String tenantId, String projectId, long minDurationMs, Integer limit) {
    int effectiveLimit = normalizeLimit(limit);
  ...
  public Promise<List<Trace>> getTracesByTimeRange(
      String tenantId, String projectId, Instant start, Instant end, Integer limit) {
    int effectiveLimit = normalizeLimit(limit);
  ...
  public Promise<TraceStats> getTraceStatistics(String tenantId, String projectId, String service) {
    return repository
        .findByProject(tenantId, projectId, 1000)
        .then(
            traces ->
                repository
                    .getAverageDuration(tenantId, projectId, service)
                    .map(
                        avgDuration -> {
  ...
  public record CompleteSpanInput(Boolean error, String errorMessage, String stackTrace) {}
