...
public class ProjectController {
                        ...
                        body -> {
                          try {
                            CreateProjectRequest req =
                                mapper.readValue(body.getArray(), CreateProjectRequest.class);
                            ...
                            return service
                                .createProject(ctx.tenantId(), input)
  ...
  public Promise<HttpResponse> getProject(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getProject(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> getProjectByKey(
      HttpRequest request, String workspaceId, String key) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getProjectByKey(ctx.tenantId(), workspaceId, key)
                    .map(
                        opt ->
                            opt.map(ApiResponse::ok)
                                .orElse(ApiResponse.notFound("Project not found"))))
  ...
  public Promise<HttpResponse> listWorkspaceProjects(HttpRequest request, String workspaceId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .listWorkspaceProjects(ctx.tenantId(), UUID.fromString(workspaceId))
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
                        ...
                        body -> {
                          try {
                            UpdateProjectRequest req =
                                mapper.readValue(body.getArray(), UpdateProjectRequest.class);
                            UpdateProjectInput input =
                                new UpdateProjectInput(
                                    req.name(), req.description(), null, null, null);
                            return service
                                .updateProject(ctx.tenantId(), UUID.fromString(id), input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
  ...
  public Promise<HttpResponse> startProject(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.startProject(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> pauseProject(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.pauseProject(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> resumeProject(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> service.resumeProject(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> completeProject(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service.completeProject(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> archiveProject(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> service.archiveProject(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> deleteProject(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .deleteProject(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        deleted ->
                            deleted
  ...
  public Promise<HttpResponse> getProjectStatistics(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getProjectStatistics(ctx.tenantId(), UUID.fromString(id))
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
