 ...
 *
 * <p><b>Purpose</b><br>
 * Implements business logic for project management with clean API alignment to the domain model.
 *
 * @doc.type class
...
public class ProjectService {
  ...
  public Promise<Project> updateProject(String tenantId, UUID projectId, UpdateProjectInput input) {
    return repository
        .findById(UUID.fromString(tenantId), projectId)
        .then(
            opt -> {
              if (opt.isEmpty()) {
  ...
  public Promise<Boolean> deleteProject(String tenantId, UUID projectId) {
    ...
    return repository
        .delete(UUID.fromString(tenantId), projectId)
        .whenResult(
            deleted -> {
              if (deleted) {
                AuditEvent event =
   ...
   */
  public Promise<Optional<Project>> getProjectByKey(
      String tenantId, String workspaceId, String key) {
