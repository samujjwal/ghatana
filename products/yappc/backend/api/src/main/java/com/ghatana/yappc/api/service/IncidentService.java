...
public class IncidentService {
  ...
  public Promise<Optional<Incident>> getIncident(UUID workspaceId, UUID incidentId) {
    return repository.findById(workspaceId, incidentId).map(Optional::ofNullable);
  }
  ...
  public Promise<Incident> startInvestigation(UUID workspaceId, UUID incidentId) {
    ...
    return repository
        .findById(workspaceId, incidentId)
        .then(
            incident -> {
              if (incident == null) {
            ...
            incidents -> {
              ...
              stats.criticalCount =
                  incidents.stream()
                      ...
                      .count();
              stats.highCount =
                  incidents.stream().filter(i -> "HIGH".equalsIgnoreCase(i.getSeverity())).count();
              stats.mttrMs =
                  incidents.stream()
  ...
  public record ResolveInput(String rootCause, String resolution) {}
