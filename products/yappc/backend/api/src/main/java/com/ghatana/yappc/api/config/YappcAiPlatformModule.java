 ...
 *
 * <dl>
 *   <dt>{@link FeatureStoreService}
 *   <dd>Two-tier (Redis hot + PostgreSQL cold) feature store; uses the tenant-isolated YAPPC {@link
 *       DataSource}.
 *   <dt>{@link ModelRegistryService}
 *   <dd>JDBC-backed model registry for registering, querying, and lifecycle-managing AI model
 *       versions across YAPPC tenants.
 *   <dt>{@link YappcFeatureStoreClient}
 *   <dd>ActiveJ {@code Promise}-based async façade over {@link FeatureStoreService} suitable for
 *       direct use on the event-loop.
 *   <dt>{@link YappcModelRegistryClient}
 *   <dd>ActiveJ {@code Promise}-based async façade over {@link ModelRegistryService}.
 * </dl>
 *
 * <h3>Installation</h3>
 *
 ...
 * <h3>Dependencies</h3>
 *
 * The module requires the following bindings to already be present in the injector (supplied by
 * {@link DataSourceModule} and {@link ProductionModule}):
 *
 * <ul>
 *   <li>{@link DataSource} — from {@link DataSourceModule}
 *   <li>{@link MetricsCollector} — from {@link ProductionModule#metricsCollector()}
 * </ul>
 *
 * <h3>Thread pool</h3>
 *
 * An internal bounded thread pool ({@code yappc-ai-platform-N} daemon threads) is used to dispatch
 * blocking JDBC and Redis calls off the ActiveJ event-loop. The pool is sized to the number of
 * available processors with a minimum of 2.
 *
 * @doc.type class
...
public final class YappcAiPlatformModule extends AbstractModule {
  ...
  /**
   * Shared blocking executor used by all AI platform client façades. Daemon threads so they do not
  ...
  /**
   * Provides the platform {@link FeatureStoreService} backed by the YAPPC {@link DataSource} and
   * shared {@link MetricsCollector}.
   *
  ...
  FeatureStoreService featureStoreService(
      DataSource dataSource, MetricsCollector metricsCollector) {
    log.info("Initialising FeatureStoreService for YAPPC");
    return new FeatureStoreService(dataSource, metricsCollector);
  }

  /**
   * Provides the platform {@link ModelRegistryService} backed by the YAPPC {@link DataSource} and
   * shared {@link MetricsCollector}.
   *
   * @param dataSource YAPPC JDBC connection pool
   * @param metricsCollector platform observability collector
