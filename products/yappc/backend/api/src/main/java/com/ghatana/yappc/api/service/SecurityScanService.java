...
public class SecurityScanService {
  ...
  public Promise<Optional<ScanJob>> getScan(UUID workspaceId, UUID scanId) {
    return repository.findById(workspaceId, scanId).map(Optional::ofNullable);
  }
        ...
        .map(
            scans -> {
              ...
              stats.completedCount =
                  scans.stream().filter(s -> s.getStatus() == ScanStatus.COMPLETED).count();
              stats.runningCount =
                  scans.stream().filter(s -> s.getStatus() == ScanStatus.RUNNING).count();
              stats.failedCount =
                  scans.stream().filter(s -> s.getStatus() == ScanStatus.FAILED).count();
              stats.totalFindings = scans.stream().mapToInt(ScanJob::getFindingsCount).sum();
              stats.totalCritical = scans.stream().mapToInt(ScanJob::getCriticalCount).sum();
              return stats;
            });
  }
