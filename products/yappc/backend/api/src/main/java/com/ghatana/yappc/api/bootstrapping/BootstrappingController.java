...
public class BootstrappingController {
                        ...
                        body -> {
                          try {
                            StartSessionRequest req =
                                mapper.readValue(body.getArray(), StartSessionRequest.class);
                            StartSessionInput input =
                                new StartSessionInput(
                                    req.workspaceId(), req.initialPrompt(), ctx.userId());
                            return service
                                .startSession(ctx.tenantId(), input)
  ...
  public Promise<HttpResponse> getSession(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getSession(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> listWorkspaceSessions(HttpRequest request, String workspaceId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> service.listWorkspaceSessions(ctx.tenantId(), workspaceId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            ConversationTurnRequest req =
                                mapper.readValue(body.getArray(), ConversationTurnRequest.class);
                            return service
                                .addConversationTurn(
                                    ctx.tenantId(),
                                    UUID.fromString(id),
                        ...
                        body -> {
                          try {
                            ProjectDefinition definition =
                                mapper.readValue(body.getArray(), ProjectDefinition.class);
                            return service
                                .setProjectDefinition(
                                    ctx.tenantId(), UUID.fromString(id), definition)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            RefineProjectRequest req =
                                mapper.readValue(body.getArray(), RefineProjectRequest.class);
                            return service
                                .refineProject(
                                    ctx.tenantId(), UUID.fromString(id), req.refinementPrompt())
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
  ...
  public Promise<HttpResponse> generateProjectGraph(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .generateProjectGraph(ctx.tenantId(), UUID.fromString(id))
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
        ...
        .then(
            ctx ->
                service
                    ...
                    .map(
                        opt ->
                            opt.map(s -> ApiResponse.ok(s.getProjectGraph()))
                                .orElse(ApiResponse.notFound("Session not found"))))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> validateProject(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service.validateProject(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
                        ...
                        body -> {
                          try {
                            ApproveProjectRequest req =
                                mapper.readValue(body.getArray(), ApproveProjectRequest.class);
                            ApproveProjectInput input =
                                new ApproveProjectInput(
                                    req.projectName(),
                                    req.projectDescription(),
                                    req.sprintDuration(),
                                    req.teamSize());
                            return service
                                .approveProject(ctx.tenantId(), UUID.fromString(id), input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
  ...
  public Promise<HttpResponse> abandonSession(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .abandonSession(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        deleted ->
                            deleted
  ...
  public record StartSessionRequest(String workspaceId, String initialPrompt) {}
