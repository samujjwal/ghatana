...
public class NotificationController {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> {
              ...
              return service
                  .listUserNotifications(ctx.tenantId(), ctx.userId(), limit, offset)
                  .map(ApiResponse::ok);
            })
  ...
  public Promise<HttpResponse> listUnreadNotifications(HttpRequest request) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service.listUnreadNotifications(ctx.tenantId(), ctx.userId()).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getUnreadCount(HttpRequest request) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getUnreadCount(ctx.tenantId(), ctx.userId())
        ...
        .then(
            ctx -> {
              try {
                ...
                return service
                    .listByCategory(ctx.tenantId(), ctx.userId(), cat)
                    .map(ApiResponse::ok);
  ...
  public Promise<HttpResponse> getNotification(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getNotification(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> markAsRead(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .markAsRead(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        success ->
                            success
                                ? ApiResponse.ok(Map.of("marked", true))
  ...
  public Promise<HttpResponse> markAllAsRead(HttpRequest request) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .markAllAsRead(ctx.tenantId(), ctx.userId())
  ...
  public Promise<HttpResponse> deleteNotification(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .deleteNotification(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        deleted ->
                            deleted
