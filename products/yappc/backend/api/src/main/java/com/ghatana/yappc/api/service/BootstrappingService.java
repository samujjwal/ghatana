...
public class BootstrappingService {
  ...
  public Promise<List<BootstrappingSession>> listWorkspaceSessions(
      String tenantId, String workspaceId) {
    return sessionRepository
        .findAllByTenant(tenantId)
        .map(
            sessions ->
                sessions.stream().filter(s -> workspaceId.equals(s.getWorkspaceId())).toList());
  }
        ...
        .then(
            opt -> {
              ...
              return generateRefinementResponse(session, refinementPrompt)
                  ...
                  .then(
                      updatedSession ->
                          refineProjectDefinition(updatedSession, refinementPrompt)
                              .map($ -> updatedSession))
                  .then(sessionRepository::save);
            });
  }
    ...
    return sessionRepository
        ...
        .then(
            opt -> {
              ...
              return sessionRepository.save(session).map(BootstrappingSession::getProjectGraph);
            });
  }
        ...
        .then(
            opt -> {
              ...
              if (session.getProjectDefinition() == null) {
                return Promise.ofException(
                    new IllegalStateException("Cannot validate - project definition not set"));
              }
              ...
              return sessionRepository.save(session).map(BootstrappingSession::getValidationReport);
            });
  }

  /**
   * Approve the project. This marks the session as approved and prepares transition data. The
   * actual project creation is delegated to the caller.
   */
        ...
        .then(
            opt -> {
              ...
              if (report != null && !report.getBlockers().isEmpty()) {
                return Promise.ofException(
                    new IllegalStateException("Cannot approve - validation has blockers"));
              }
    ...
    return sessionRepository
        ...
        .then(
            opt -> {
              ...
              return sessionRepository.save(session).map(s -> true);
            });
  }
  ...
  private Promise<String> generateInitialResponse(String prompt) {
    CompletionRequest request =
        CompletionRequest.builder()
            .prompt(
                "You are a project bootstrapping assistant. A user has described their project idea. "
    ...
    return completionPromise
        ...
        .then(
            ...
            e -> {
              ...
              return Promise.of(
                  "I'd be happy to help you with your project idea! "
  ...
  private Promise<String> generateConversationResponse(BootstrappingSession session) {
    ...
    for (var turn : session.getConversationHistory()) {
      ChatMessage.Role role =
          "user".equals(turn.getRole()) ? ChatMessage.Role.USER : ChatMessage.Role.ASSISTANT;
      messages.add(ChatMessage.of(role, turn.getContent()));
    }

    CompletionRequest request =
        CompletionRequest.builder().messages(messages).maxTokens(512).temperature(0.7).build();
    ...
    return completionPromise
        ...
        .then(
            ...
            e -> {
              ...
              if (turnCount < 6) {
                return Promise.of(
                    "Great, that helps! Can you tell me more about the target users for this project?");
              } else if (turnCount < 10) {
                return Promise.of(
                    "Excellent! I think I have a good understanding now. Would you like me to create a project definition?");
              } else {
                return Promise.of(
                    "I've gathered enough information. Let me create a project definition for you.");
              }
            });
  }
  ...
  private Promise<String> generateRefinementResponse(
      BootstrappingSession session, String refinementPrompt) {
    String context =
        session.getProjectDefinition() != null
            ? "Current project: " + session.getProjectDefinition().getName()
            : "No project definition yet";
    CompletionRequest request =
        CompletionRequest.builder()
            .prompt(
                "You are a project bootstrapping assistant. The user wants to refine their project.\n\n"
                    + context
    ...
    return completionPromise
        ...
        .then(
            ...
            e -> {
              ...
              return Promise.of(
                  "I've updated the project definition based on your feedback. "
                      + "Please review the changes and let me know if anything else needs adjustment.");
            });
  }

  private Promise<Void> refineProjectDefinition(
      BootstrappingSession session, String refinementPrompt) {
    ProjectDefinition definition = session.getProjectDefinition();
    if (definition == null) {
    ...
    String currentDef =
        targetDefinition.getName() != null
            ? "Name: "
                + targetDefinition.getName()
                + ", Description: "
                + targetDefinition.getDescription()
            : "Empty project definition";
    CompletionRequest request =
        CompletionRequest.builder()
            .prompt(
                "You are a project definition assistant. Update the project definition based on feedback.\n\n"
                    + "Current definition: "
                    + currentDef
                    + "\n\nRefinement: "
                    + refinementPrompt
  ...
  private ProjectGraph buildProjectGraph(
      ProjectDefinition definition, BootstrappingSession session) {
      ...
      for (int i = 0; i < features.size(); i++) {
        ...
        if (feature.getSubFeatures() != null) {
          for (int j = 0; j < feature.getSubFeatures().size(); j++) {
            ...
            subNode.setPhase(
                subFeature.getPhase() != null ? subFeature.getPhase() : feature.getPhase());
  ...
  public record StartSessionInput(String workspaceId, String initialPrompt, String userId) {}

  public record ApproveProjectInput(
      String projectName, String projectDescription, Integer sprintDuration, Integer teamSize) {}
}
