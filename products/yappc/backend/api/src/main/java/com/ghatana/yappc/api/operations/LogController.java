...
public class LogController {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            RecordLogInput input =
                                objectMapper.readValue(
                                    body.getString(StandardCharsets.UTF_8), RecordLogInput.class);
                            return logService
                                .recordLog(ctx.tenantId(), input)
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            List<RecordLogInput> inputs =
                                objectMapper.readValue(
                                    body.getString(StandardCharsets.UTF_8),
                                    objectMapper
                                        .getTypeFactory()
                                        .constructCollectionType(List.class, RecordLogInput.class));
                            return logService
                                .recordLogs(ctx.tenantId(), inputs)
                                .map(ApiResponse::created);
                          } catch (Exception e) {
                            return Promise.of(ApiResponse.badRequest(e.getMessage()));
                          }
  ...
  public Promise<HttpResponse> getLogEntry(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                logService
                    .getLogEntry(ctx.tenantId(), UUID.fromString(logId))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> listProjectLogs(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                logService.listProjectLogs(ctx.tenantId(), projectId, limit).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listServiceLogs(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> logService.listServiceLogs(ctx.tenantId(), service, limit).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
        ...
        .then(
            ctx -> {
              try {
                ...
                return logService
                    .listLogsByLevel(ctx.tenantId(), level, limit)
                    .map(ApiResponse::ok);
  ...
  public Promise<HttpResponse> listErrorLogs(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> logService.listErrorLogs(ctx.tenantId(), projectId, limit).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getLogsByTrace(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> logService.getLogsByTrace(ctx.tenantId(), traceId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getLogsByRequest(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> logService.getLogsByRequest(ctx.tenantId(), requestId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getLogsByUser(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> logService.getLogsByUser(ctx.tenantId(), userId, limit).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> searchLogs(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> logService.searchLogs(ctx.tenantId(), query, limit).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> {
              ...
              return logService
                  .getLogsByTimeRange(ctx.tenantId(), projectId, start, end, limit)
                  .map(ApiResponse::ok);
            })
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getLogLevelStats(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> logService.getLogLevelStats(ctx.tenantId(), projectId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> deleteLogEntry(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                logService
                    .deleteLogEntry(ctx.tenantId(), UUID.fromString(logId))
