...
public class TeamController {
                        ...
                        body -> {
                          try {
                            CreateTeamRequest req =
                                mapper.readValue(body.getArray(), CreateTeamRequest.class);
  ...
  public Promise<HttpResponse> getTeam(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .getTeam(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> listOrganizationTeams(HttpRequest request, String orgId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.listOrganizationTeams(ctx.tenantId(), orgId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listUserTeams(HttpRequest request, String userId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.listUserTeams(ctx.tenantId(), userId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listMyTeams(HttpRequest request) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> service.listUserTeams(ctx.tenantId(), ctx.userId()).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
                        ...
                        body -> {
                          try {
                            UpdateTeamRequest req =
                                mapper.readValue(body.getArray(), UpdateTeamRequest.class);
                            UpdateTeamInput input =
                                new UpdateTeamInput(
                                    ...
                                    req.timezone());
                            return service
                                .updateTeam(ctx.tenantId(), UUID.fromString(id), input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                        ...
                        body -> {
                          try {
                            AddMemberRequest req =
                                mapper.readValue(body.getArray(), AddMemberRequest.class);
                            AddMemberInput input =
                                new AddMemberInput(
                                    req.userId(), req.email(), req.displayName(), req.role());
                            return service
                                .addMember(ctx.tenantId(), UUID.fromString(id), input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                        ...
                        body -> {
                          try {
                            UpdateRoleRequest req =
                                mapper.readValue(body.getArray(), UpdateRoleRequest.class);
                            return service
                                .updateMemberRole(
                                    ctx.tenantId(), UUID.fromString(id), memberId, req.role())
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
  ...
  public Promise<HttpResponse> removeMember(HttpRequest request, String id, String memberId) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .removeMember(ctx.tenantId(), UUID.fromString(id), memberId)
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getTeamStatistics(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service.getTeamStatistics(ctx.tenantId(), UUID.fromString(id)).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> deleteTeam(HttpRequest request, String id) {
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                service
                    .deleteTeam(ctx.tenantId(), UUID.fromString(id))
                    .map(
                        deleted ->
                            deleted
  ...
  public record UpdateTeamRequest(
      String name, String description, TeamVisibility visibility, String timezone) {}

  public record AddMemberRequest(
      String userId, String email, String displayName, MemberRole role) {}
