 ...
 *
 * <p>Returns mock responses for all LLM requests. Used as a fallback when no LLM API keys
 * (OPENAI_API_KEY, ANTHROPIC_API_KEY) are configured.
 *
 * @deprecated Move to test fixtures or a dedicated dev-tools module. Production code should fail
 *     fast when LLM integration is required but not configured, rather than silently returning mock
 *     responses.
 * @doc.type class
...
public class NoOpLLMGateway implements LLMGateway {
  ...
  public Promise<CompletionResult> complete(@NotNull CompletionRequest request) {
    ...
    String mockText =
        "This is a mock response from NoOpLLMGateway. "
  ...
  public Promise<CompletionResult> completeWithTools(
      @NotNull CompletionRequest request, @NotNull List<ToolDefinition> tools) {
  ...
  public Promise<CompletionResult> continueWithToolResults(
      @NotNull CompletionRequest request, @NotNull List<ToolCallResult> toolResults) {
  ...
  public Promise<EmbeddingResult> embed(@NotNull String text) {
    ...
    return Promise.of(new EmbeddingResult(text, new float[0], "no-op"));
  }
  ...
  public Promise<List<EmbeddingResult>> embedBatch(@NotNull List<String> texts) {
    ...
    List<EmbeddingResult> results =
        texts.stream().map(text -> new EmbeddingResult(text, new float[0], "no-op")).toList();
