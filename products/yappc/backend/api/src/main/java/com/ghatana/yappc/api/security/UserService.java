 ...
 *
 * <p>All operations on the in-memory store are non-blocking {@link ConcurrentHashMap} lookups and
 * therefore return synchronous {@link Promise#of} values. No blocking executor is required because
 * no IO is involved.
 *
 * <p>Features:
 *
 * <ul>
 *   <li>User authentication with password verification
 *   <li>User creation and management
 *   <li>Role and permission management
 *   <li>Tenant-based user isolation
 *   <li>Password hashing and validation
 * </ul>
 *
 * @doc.type class
...
public class UserService {
   ...
   */
  @NotNull
  public Promise<@Nullable UserContext> authenticate(
   ...
   */
  @NotNull
  public Promise<@Nullable UserContext> createUser(
    ...
    try {
      ...
      UserData userData =
          new UserData(
              userId, email, userName, passwordHash, tenantId, roles, getDefaultPermissions(roles));
  ...
  private List<Permission> getRoleDefaultPermissions(@NotNull String role) {
    return switch (role) {
      case "admin" -> List.of(Permission.crud("/api/v1/**"), Permission.crud("/admin/**"));
      case "tenant_admin" ->
          List.of(
              Permission.crud("/api/v1/tenants/{tenantId}/**"),
              Permission.crud("/api/v1/projects/**"));
      case "developer" ->
          List.of(Permission.crud("/api/v1/projects/**"), Permission.readOnly("/api/v1/agents/**"));
      case "analyst" ->
          List.of(
              Permission.readOnly("/api/v1/projects/**"), Permission.readOnly("/api/v1/agents/**"));
      case "viewer" -> List.of(Permission.readOnly("/api/v1/projects/{projectId}"));
      default -> List.of();
    };
  }
   ...
   * Hashes a password.
   *
   * <p><b>WARNING: dev-only implementation.</b> Replace with BCrypt/Argon2 before production
   * deployment. See {@code platform:java:security} for the platform {@code PasswordHasher}.
   *
   * @param password plaintext password
   * @return hashed password (dev-only: {@code String.hashCode()}-based)
   */
  // NOTE: Dev-only hash — production deployments MUST replace with platform PasswordHasher
  // (BCrypt/Argon2).
