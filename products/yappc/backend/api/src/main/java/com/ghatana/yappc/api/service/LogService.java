...
public class LogService {
  ...
  public Promise<Optional<LogEntry>> getLogEntry(String tenantId, UUID logId) {
    return repository.findById(tenantId, logId).map(l -> Optional.ofNullable(l));
  }
  ...
  public Promise<LogLevelStats> getLogLevelStats(String tenantId, String projectId) {
    return repository
        .countByLevel(tenantId, projectId, LogLevel.ERROR)
        .then(
            errorCount ->
                repository
                    .countByLevel(tenantId, projectId, LogLevel.WARN)
                    .then(
                        warnCount ->
                            repository
                                .countByLevel(tenantId, projectId, LogLevel.INFO)
                                .map(
                                    infoCount -> {
