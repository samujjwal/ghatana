 ...
 *
 * <p>Validates {@code X-API-Key} header (or {@code apiKey} query parameter as a browser fallback)
 * during the HTTP upgrade handshake. Connections with missing or invalid keys are rejected with
 * HTTP 401 before the WebSocket protocol is established.
 *
 * <p>Accepted connections have their tenant ID resolved from the {@code tenantId} query parameter
 * and propagated to all message handlers.
 *
 * @doc.type class
...
public class WebSocketController {
  ...
  /**
   * Creates a {@link WebSocketServlet} that authenticates the upgrade request via {@code X-API-Key}
  ...
  public WebSocketServlet createServlet(Reactor reactor) {
    ...
    return new WebSocketServlet(reactor) {
      ...
      /**
       * Validates API key at the HTTP-upgrade level — before the WebSocket is created. Returns 401
       * immediately if the key is absent or not in the allowed set.
       */
      @Override
      protected Promise<HttpResponse> onRequest(HttpRequest request) {
        ...
        if (apiKey == null || !keys.contains(apiKey.trim())) {
          logger.warn(
              "WebSocket upgrade rejected: missing or invalid X-API-Key ({})",
              request.getRemoteAddress());
          return Promise.of(
              HttpResponse.ofCode(401)
                  .withPlainText(
                      "Unauthorized: provide a valid X-API-Key header or apiKey query parameter")
                  .build());
        }
      ...
      protected void onWebSocket(IWebSocket webSocket) {
        ...
        logger.info(
            "WebSocket connection accepted: connId={} tenant={} user={}", connId, tenantId, userId);
  ...
  /**
   * HTTP fallback for non-WebSocket GET requests to {@code /ws}. Real WebSocket clients use the
   * {@code createServlet()} result instead.
   */
    ...
    webSocket
        ...
        .whenResult(
            msg -> {
              if (msg != null) {
                logger.debug(
                    "Received message from user={}: {}", connection.getUserId(), msg.getText());
                readLoop(webSocket, connection);
              } else {
