 ...
 *
 * <p>Endpoints: - POST /api/auth/login - User authentication - POST /api/auth/refresh - Token
 * refresh - POST /api/auth/logout - User logout - GET /api/auth/profile - Current user info - POST
...
public class AuthController {
          ...
          .then(
              refreshRequest -> {
                ...
                if (tokenBlacklist.containsKey(refreshToken)) {
                  return Promise.of(
                      PlatformCompatibility.createJsonResponse(
                          401,
  ...
  public Promise<HttpResponse> logout(@NotNull HttpRequest request) {
    try {
      ...
      return Promise.of(
          ok200(
              Map.of(
                  "message", "Logged out successfully", "timestamp", System.currentTimeMillis())));
  ...
  public void cleanupExpiredTokens() {
    ...
    tokenBlacklist
        .entrySet()
        .removeIf(
            entry -> entry.getValue().plusSeconds(3600).isBefore(now) // Remove after 1 hour
            );
  }
