 ...
 *
 * <p><b>Purpose</b><br>
 * Routes AEP events to the appropriate TypedAgent based on event routing configuration. Wraps
 * agents with backpressure handling and provides unified event dispatch.
 *
 * <p><b>Architecture</b><br>
 *
 * <ul>
 *   <li>Maintains a registry of agent ID → AgentEventOperator mappings
 *   <li>Wraps operators with BackpressureOperator for load handling
 *   <li>Reads routing configuration from event-routing.yaml
 *   <li>Integrates with TypedAgent instances from agent-framework
 * </ul>
 *
 * <p><b>Event Flow</b><br>
 *
 ...
 * </pre>
 *
 * <p>All async operations use ActiveJ {@link Promise} — no {@code CompletableFuture} is used in the
...
public class YappcAgentEventRouter {
  ...
  public YappcAgentEventRouter registerAgent(
      @NotNull String topic, @NotNull TypedAgent<Map<String, Object>, Map<String, Object>> agent) {
   ...
   *
   * <p>Returns a {@link Promise} that completes with the processed result, or an error event map if
   * routing or processing fails.
   *
   * @param topic the event topic
  ...
  public Promise<Map<String, Object>> routeEvent(
      @NotNull String topic, @NotNull Map<String, Object> event) {
      ...
      return agent
          ...
          .then(
              result -> {
                ...
                return Promise.of(
                    createErrorEvent(topic, "AGENT_PROCESSING_FAILED", result.getExplanation()));
              });
    } catch (Exception e) {
   ...
   */
  private Map<String, Object> createSuccessEvent(
      @NotNull String topic, @NotNull Map<String, Object> data) {
   ...
   */
  private Map<String, Object> createErrorEvent(
      @NotNull String topic, @NotNull String errorCode, @NotNull String errorMessage) {
    Map<String, Object> event = new HashMap<>();
