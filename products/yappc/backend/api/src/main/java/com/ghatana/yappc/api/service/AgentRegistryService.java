 ...
 *
 * <p>Replaces the previous in-memory approach. All state is durable in PostgreSQL, enabling agents
 * to survive process restarts and enabling distributed deployments to share a single source of
 * truth.
 *
 * <p>Usage example:
 *
...
public class AgentRegistryService {
        ...
        .whenResult(
            saved ->
                logger.info(
                    ...
                    saved.getTenantId()))
        .whenException(
            e -> logger.error("Failed to register agent id={}: {}", entry.getId(), e.getMessage()));
  }
  ...
  public Promise<Void> activate(String tenantId, String agentId) {
    return repository
        .updateStatus(tenantId, agentId, AgentStatus.ACTIVE)
  ...
  public Promise<Void> suspend(String tenantId, String agentId) {
    return repository
        .updateStatus(tenantId, agentId, AgentStatus.SUSPENDED)
  ...
  public Promise<Void> terminate(String tenantId, String agentId) {
    return repository
        .updateStatus(tenantId, agentId, AgentStatus.TERMINATED)
  ...
  public Promise<Void> heartbeat(String tenantId, String agentId) {
    return repository
        .updateHeartbeat(tenantId, agentId)
        .whenException(
            e ->
                logger.warn(
  ...
  public Promise<Void> recordMetric(
      String tenantId, String agentId, String metricName, double value) {
