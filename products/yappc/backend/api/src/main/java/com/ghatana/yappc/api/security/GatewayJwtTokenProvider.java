 ...
 *
 * <p>Calls the shared {@code auth-gateway} service's {@code /auth/validate} and {@code /auth/login}
 * endpoints over HTTP instead of performing local JJWT parsing. This is the recommended production
 * configuration for centralized authentication.
 *
 * <p>For development and testing, the local {@link JwtTokenProvider} can still be used. Switch
 * implementations via configuration.
 *
 * <p>All remote calls use ActiveJ {@link HttpClient} and return non-blocking {@link Promise}
 * results (bridged to {@link Promise} where the existing API requires it).
 *
 * @doc.type class
...
public class GatewayJwtTokenProvider {
   ...
   */
  public GatewayJwtTokenProvider(@NotNull HttpClient httpClient, @NotNull String gatewayBaseUrl) {
  ...
  public Promise<Boolean> validateTokenPromise(@NotNull String token) {
    HttpRequest request =
        HttpRequest.get(gatewayBaseUrl + "/auth/validate")
   ...
   *
   * <p>Returns a Promise of UserContext for non-blocking usage. Legacy synchronous callers should
   * migrate to this method with proper async chaining.
   *
   * @param token the raw JWT token
  ...
  public Promise<@Nullable UserContext> getUserFromToken(@NotNull String token) {
    return getUserFromTokenPromise(token)
        .whenException(e -> LOG.error("Failed to get user from token via auth-gateway", e));
  }
