...
public class WorkspaceService {
  ...
  public Promise<WorkspaceResponse> createWorkspace(
      String tenantId, String ownerId, CreateWorkspaceRequest request) {
    ...
    WorkspaceMember owner =
        WorkspaceMember.create(
            ...
            "unknown@email.com",
            "Owner",
            MemberRole.OWNER); // Email/Name would come from User Service ideally
    workspace.addMember(owner);

    return repository.save(workspace).map(v -> mapToResponse(workspace));
  }
        ...
        .map(
            list ->
                list.stream()
                    .filter(
                        w ->
                            isMember(w, userId) || w.getOwnerId().equals(userId)) // Basic filtering
  ...
  public Promise<WorkspaceResponse> updateWorkspace(
      String tenantId, String id, UpdateWorkspaceRequest request) {
    return repository
        .findById(tenantId, UUID.fromString(id))
        .then(
            opt -> {
  ...
  public Promise<List<WorkspaceMemberResponse>> listMembers(String tenantId, String workspaceId) {
    return repository
        .findById(tenantId, UUID.fromString(workspaceId))
        .map(
            opt ->
                opt.map(
                        workspace ->
                            workspace.getMembers().stream().map(this::mapMemberToResponse).toList())
                    .orElse(List.of()));
  }
            ...
            opt -> {
              ...
              if (workspace.isOwner(userId)) {
                return Promise.ofException(
                    new IllegalStateException("Cannot remove workspace owner"));
              }
  ...
  private WorkspaceSettingsResponse mapSettingsToResponse(Workspace workspace) {
    WorkspaceSettings settings =
        workspace.getSettings() != null ? workspace.getSettings() : new WorkspaceSettings();
