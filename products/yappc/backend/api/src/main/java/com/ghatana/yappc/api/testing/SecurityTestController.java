 ...
 *
 * <ul>
 *   <li>POST /api/testing/security/scan - Run security scan
 *   <li>POST /api/testing/security/dependencies - Scan dependencies for vulnerabilities
 *   <li>POST /api/testing/security/sast - Run SAST analysis
 * </ul>
 *
 * @doc.type class
...
public class SecurityTestController extends AbstractModule {
    ...
    return JsonUtils.parseBody(httpRequest, SecurityScanRequest.class)
        .then(
            request -> {
              ...
              return securityTestService.runSecurityScan(request).map(ApiResponse::ok);
            });
  }
    ...
    return JsonUtils.parseBody(httpRequest, DependencyScanRequest.class)
        .then(
            request -> {
              ...
              return securityTestService.scanDependencies(request).map(ApiResponse::ok);
            });
  }
  ...
  private Promise<HttpResponse> runSAST(HttpRequest httpRequest) {
    return JsonUtils.parseBody(httpRequest, SASTRequest.class)
        .then(
            request -> {
              log.info("Running SAST: path={}, rules={}", request.projectPath(), request.ruleSet());
              return securityTestService.runSAST(request).map(ApiResponse::ok);
            });
  }
}
