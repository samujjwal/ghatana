...
public class MetricService {
  ...
  public Promise<Optional<Metric>> getMetric(String tenantId, UUID metricId) {
    return repository.findById(tenantId, metricId).map(m -> Optional.ofNullable(m));
  }
  ...
  public Promise<Metric> recordDataPoints(
      String tenantId, UUID metricId, List<DataPointInput> inputs) {
  ...
  public Promise<MetricSummary> getMetricSummary(String tenantId, UUID metricId) {
    return repository.findById(tenantId, metricId).map(m -> m != null ? m.getSummary() : null);
  }
    ...
    return repository
        ...
        .map(
            metrics -> {
              ...
              long counters =
                  metrics.stream().filter(m -> m.getType() == MetricType.COUNTER).count();
              long gauges = metrics.stream().filter(m -> m.getType() == MetricType.GAUGE).count();
              long histograms =
                  metrics.stream().filter(m -> m.getType() == MetricType.HISTOGRAM).count();
  ...
  public record DataPointInput(Instant timestamp, double value, Map<String, String> tags) {}
  ...
  public static class ProjectMetricsStats {
    ...
    public void setTotalMetrics(int totalMetrics) {
    ...
    public long getGaugeCount() {
      return gaugeCount;
    }

    public void setGaugeCount(long gaugeCount) {
