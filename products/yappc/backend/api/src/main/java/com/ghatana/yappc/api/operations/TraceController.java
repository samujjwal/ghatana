...
public class TraceController {
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            StartTraceInput input =
                                objectMapper.readValue(
                                    body.getString(StandardCharsets.UTF_8), StartTraceInput.class);
                            return traceService
                                .startTrace(ctx.tenantId(), input)
                    ...
                    .then(
                        body -> {
                          try {
                            AddSpanInput input =
                                objectMapper.readValue(
                                    body.getString(StandardCharsets.UTF_8), AddSpanInput.class);
                            return traceService
                                .addSpan(ctx.tenantId(), traceId, input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                            return Promise.of(ApiResponse.badRequest(e.getMessage()));
                          }
            ...
            ctx ->
                request
                    ...
                    .then(
                        body -> {
                          try {
                            CompleteSpanInput input =
                                objectMapper.readValue(
                                    body.getString(StandardCharsets.UTF_8),
                                    CompleteSpanInput.class);
                            return traceService
                                .completeSpan(ctx.tenantId(), traceId, spanId, input)
                                .map(ApiResponse::ok);
                          } catch (Exception e) {
                            return Promise.of(ApiResponse.badRequest(e.getMessage()));
                          }
  ...
  public Promise<HttpResponse> completeTrace(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(ctx -> traceService.completeTrace(ctx.tenantId(), traceId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getTrace(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                traceService
                    .getTrace(ctx.tenantId(), UUID.fromString(traceId))
                    .map(
                        opt ->
  ...
  public Promise<HttpResponse> getTraceByTraceId(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                traceService
                    .getTraceByTraceId(ctx.tenantId(), traceId)
                    .map(
                        opt ->
                            opt.map(ApiResponse::ok)
                                .orElse(ApiResponse.notFound("Trace not found"))))
  ...
  public Promise<HttpResponse> listProjectTraces(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                traceService
                    .listProjectTraces(ctx.tenantId(), projectId, limit)
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listServiceTraces(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                traceService.listServiceTraces(ctx.tenantId(), service, limit).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listOperationTraces(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                traceService
                    .listOperationTraces(ctx.tenantId(), service, operation, limit)
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listErrorTraces(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                traceService.listErrorTraces(ctx.tenantId(), projectId, limit).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> {
              ...
              return traceService
                  .listSlowTraces(ctx.tenantId(), projectId, minDurationMs, limit)
                  .map(ApiResponse::ok);
            })
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> listUserTraces(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> traceService.listUserTraces(ctx.tenantId(), userId, limit).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getTracesByRequest(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> traceService.getTracesByRequest(ctx.tenantId(), requestId).map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getTracesByTimeRange(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx -> {
              Instant start =
                  startStr != null ? Instant.parse(startStr) : Instant.now().minusSeconds(3600);
              Instant end = endStr != null ? Instant.parse(endStr) : Instant.now();

              return traceService
                  .getTracesByTimeRange(ctx.tenantId(), projectId, start, end, limit)
                  .map(ApiResponse::ok);
            })
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> getTraceStatistics(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                traceService
                    .getTraceStatistics(ctx.tenantId(), projectId, service)
                    .map(ApiResponse::ok))
        .then(r -> Promise.of(r), e -> Promise.of(ApiResponse.fromException(e)));
  }
  ...
  public Promise<HttpResponse> deleteTrace(HttpRequest request) {
    ...
    return TenantContextExtractor.requireAuthenticated(request)
        .then(
            ctx ->
                traceService
                    .deleteTrace(ctx.tenantId(), UUID.fromString(traceId))
