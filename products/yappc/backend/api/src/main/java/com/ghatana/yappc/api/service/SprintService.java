 ...
 *
 * <p><b>Purpose</b><br>
 * Implements business logic for sprint management, including creation, lifecycle transitions, and
 * velocity tracking.
 *
 * @doc.type class
...
public class SprintService {
  ...
  public Promise<Sprint> createSprint(String tenantId, CreateSprintInput input) {
    ...
    return sprintRepository
        .findByProject(tenantId, input.projectId())
        .then(
            existingSprints -> {
        ...
        .then(
            opt -> {
              ...
              if (sprint.getStatus() != SprintStatus.PLANNING) {
                return Promise.ofException(
                    new IllegalStateException("Can only start sprints in PLANNING status"));
              }

              // Calculate committed points from stories
              return storyRepository
                  .sumStoryPoints(tenantId, sprintId.toString())
                  .then(
                      committedPoints -> {
  ...
  /** Completes a sprint. */
  public Promise<Sprint> completeSprint(
      String tenantId, UUID sprintId, SprintRetrospective retrospective) {
    return sprintRepository
        .findById(tenantId, sprintId)
        .then(
            opt -> {
              ...
              if (sprint.getStatus() != SprintStatus.ACTIVE) {
                return Promise.ofException(
                    new IllegalStateException("Can only complete ACTIVE sprints"));
              }

              return storyRepository
                  .sumCompletedStoryPoints(tenantId, sprintId.toString())
                  .then(
                      completedPoints -> {
  ...
  public Promise<Sprint> updateSprintDates(
      String tenantId, UUID sprintId, LocalDate startDate, LocalDate endDate) {
    return sprintRepository
        .findById(tenantId, sprintId)
        .then(
            opt -> {
    ...
    return sprintRepository
        ...
        .then(
            opt -> {
              ...
              if (sprint.getStatus() != SprintStatus.PLANNING) {
                return Promise.ofException(
                    new IllegalStateException("Can only delete sprints in PLANNING status"));
              }
