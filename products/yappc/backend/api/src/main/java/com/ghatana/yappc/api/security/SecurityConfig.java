 ...
 *
 * <p>Provides: - Role-based permission mappings - Security policy settings - Authentication
 * configuration - Tenant isolation rules
 *
 * @doc.type class
...
public class SecurityConfig {
  ...
  public SecurityConfig(
      @NotNull Map<String, List<Permission>> rolePermissions,
          ...
          if (existing == null) {
            ...
            return List.copyOf(
                java.util.stream.Stream.concat(
                        existing.stream(), java.util.stream.Stream.of(permission))
                    .toList());
          }
        });
  }
  ...
  public static SecurityConfig createDevelopment() {
    ...
    config.addRolePermissions(
        "admin", List.of(Permission.crud("/api/v1/**"), Permission.crud("/dev/**")));
    return config;
  }
