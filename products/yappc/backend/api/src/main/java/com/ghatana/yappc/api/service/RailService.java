...
public class RailService {
  ...
  public record ComponentItem(
      String id, String name, String category, String description, List<String> tags, int usage) {}

  public record InfraResource(
      String id, String name, String type, String provider, String status, double cost) {}

  public record FileItem(
      String id, String name, String type, long size, Instant modified, String path) {}
  ...
  public record FavoriteItem(
      String id, String name, String type, Instant dateAdded, int usageCount) {}

  public record AISuggestion(
      String id, String title, String description, String type, double confidence, String action) {}

  public Promise<List<ComponentItem>> getComponents(
      String tenantId, String category, String query, String mode) {
        ...
        .map(
            workspaces -> {
              ...
              if (normalizedCategory != null && !normalizedCategory.isBlank()) {
                stream =
                    stream.filter(
                        item -> item.category().toLowerCase().contains(normalizedCategory));
              }
  ...
  public Promise<List<AISuggestion>> getSuggestions(Map<String, Object> context) {
    ...
    CompletionRequest request =
        CompletionRequest.builder()
            .prompt(
                "You are a YAPPC left-rail assistant for mode: "
                    + mode
                    + ".\n"
                    + "Tenant: "
                    + tenantId
    ...
    return llmGateway
        ...
        .map(
            result -> {
              ...
              String title =
                  lines.length > 0 && !lines[0].isBlank() ? lines[0] : "AI Recommendation";
