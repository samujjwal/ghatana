 ...
 *
 * <p>This is an intentionally minimal representation carrying only the data needed by the WebSocket
 * layer (identity, tenant, admin flags). It avoids a circular dependency between the {@code
 * websocket} module and the {@code api} module. The API layer is responsible for constructing this
 * from its own {@code UserContext} before passing it to {@link
 * LifecycleWebSocketHandler#handleConnection}.
 *
