 ...
 * </pre>
 *
 * <p>The reload endpoint requires the caller to hold the {@code ADMIN} role. Access is validated
 * via {@link TenantContextExtractor}.
 *
 * @doc.type class
...
public class PluginManagementController {
   ...
   *
   * <p>Triggers a hot-reload of the specified plugin without a service restart. Only users with the
   * {@code ADMIN} persona may call this endpoint.
   *
            ...
            .then(
                ctx -> {
                  ...
                  if (!"ADMIN".equalsIgnoreCase(ctx.persona())) {
                    return Promise.of(
                        ApiResponse.forbidden("Admin role required to reload plugins."));
                  }
                          ...
                          () -> {
                            try {
                              ...
                              throw new RuntimeException(
                                  "Reload failed for plugin: " + pluginId, e);
                            }
                          })
                      .map(
                          ignored ->
                              ApiResponse.ok("Plugin '" + pluginId + "' reloaded successfully."));
                }));
  }
   ...
   * GET /api/v1/plugins
   *
   * <p>Returns a catalog of all registered plugins with id, version, and compatibility status
   * relative to the current platform version.
   *
   * @param request HTTP request carrying the authenticated principal
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  log.debug(
                      "Plugin catalog requested by tenant={}, {} plugin(s) registered.",
                      ctx.tenantId(),
                      registeredCount);
                  return Promise.of(
                      ApiResponse.ok(
                          "Plugin registry contains " + registeredCount + " plugin(s)."));
                }));
  }
}
