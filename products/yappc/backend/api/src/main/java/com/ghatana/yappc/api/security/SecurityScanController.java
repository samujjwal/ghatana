        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return scanService.startScan(workspaceId, input).map(ApiResponse::created);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return scanService
                      .getScan(workspaceId, id)
                      .map(
                          opt ->
                              opt.map(ApiResponse::ok)
                                  .orElse(ApiResponse.notFound("Scan not found")));
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return scanService.listProjectScans(workspaceId, projectId).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return scanService.listScansByType(workspaceId, type).map(ApiResponse::ok);
                }));
  }
    ...
    return ApiResponse.wrap(
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  UUID workspaceId = UUID.fromString(ctx.tenantId());
                  return scanService.listRunningScans(workspaceId).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return scanService.completeScan(workspaceId, id, input).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  ...
                  return scanService.failScan(workspaceId, id, reason).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  UUID workspaceId = UUID.fromString(ctx.tenantId());
                  UUID projectId = UUID.fromString(request.getPathParameter("projectId"));
                  return scanService.getScanStatistics(workspaceId, projectId).map(ApiResponse::ok);
                }));
  }
        ...
        TenantContextExtractor.requireAuthenticated(request)
            .then(
                ctx -> {
                  UUID workspaceId = UUID.fromString(ctx.tenantId());
                  UUID id = UUID.fromString(request.getPathParameter("id"));
                  return scanService.deleteScan(workspaceId, id).map(v -> ApiResponse.noContent());
                }));
  }
}
