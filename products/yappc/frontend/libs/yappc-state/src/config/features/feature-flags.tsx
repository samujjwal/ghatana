...
const DEFAULT_FLAGS: Record<FeatureFlagName, boolean> = {
  ...
  // Development Features - Environment-dependent (safely check for env)
  [FEATURE_FLAGS.DEV_DEBUG_MODE]: false,
  [FEATURE_FLAGS.DEV_MOCK_SERVICES]: false,
...
/**
 * Get feature flag value from environment variable (safe for non-Vite environments)
 */
function getEnvFlag(name: string): boolean | undefined {
  // Safe access to import.meta.env if available
  const env =
    typeof import.meta !== 'undefined' && (import.meta as any).env
      ? (import.meta as any).env
      : {};
  const envValue = env[`VITE_${name.replace(/\./g, '_').toUpperCase()}`];
 ...
 */
export const FeatureFlagsContext = createContext<UseFeatureFlagsReturn | null>(
  null
);
