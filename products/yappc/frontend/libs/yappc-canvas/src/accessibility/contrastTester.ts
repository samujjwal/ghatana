...
export function runBatchTests(
  ...
  if (failed > 0) {
    recommendations.push(
      `${failed} color pairs fail WCAG ${finalConfig.wcagLevel} compliance`
    );
    recommendations.push('Review remediation suggestions for each failure');
  }
  if (warnings > 0) {
    recommendations.push(
      `${warnings} color pairs meet AA but not AAA standards`
    );
  }
...
export function exportMarkdownReport(report: ContrastTestReport): string {
  ...
  results.forEach((result) => {
    ...
    const statusEmoji =
      severity === 'pass' ? '✅' : severity === 'warning' ? '⚠️' : '❌';
...
function generateRemediationSuggestion(
  ...
  if (shouldDarkenFg) {
    type = 'darken';
    suggestedForeground = adjustColor(
      pair.foreground,
      targetRatio,
      bgLum,
      'darken'
...
function adjustColor(
  ...
  const adjusted = rgb.map((c) =>
    Math.round(Math.max(0, Math.min(255, c * factor)))
  );
