...
export function useSemanticZoom(
  ...
  const getVisibility = useCallback(
    (node: UniversalNode, contract?: ArtifactContract): VisibilityState => {
      ...
      if (contract?.renderContract.zoomBehavior) {
        ...
        if (behavior.hideAtZoom !== undefined && zoom < behavior.hideAtZoom) {
    ...
    [zoom, detailLevel, minVisibleSize, collapseSmallNodes, visibilityResolver]
  );
  ...
  const getRepresentation = useCallback(
    (node: UniversalNode, contract?: ArtifactContract): NodeRepresentation => {
      const screenWidth = node.transform.width * zoom;
      const screenHeight = node.transform.height * zoom;
      const minDimension = Math.min(screenWidth, screenHeight);
