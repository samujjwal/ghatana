...
export function useViewport(
  ...
  // Fit content in view
  const fitContent = useCallback((padding = 50, animate = true) => {
...
export const InfiniteViewport: React.FC<InfiniteViewportProps> = ({
  ...
  const [startTransform, setStartTransform] =
    useState<ViewportTransform | null>(null);

  // Handle transform change
  const setTransform = useCallback(
    (newTransform: Partial<ViewportTransform>) => {
      const updated = { ...transform, ...newTransform };
      updated.zoom = Math.max(
        config.minZoom,
        Math.min(config.maxZoom, updated.zoom)
      );
