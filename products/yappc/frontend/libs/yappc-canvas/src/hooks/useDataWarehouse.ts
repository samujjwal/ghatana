...
export function useDataWarehouse(
  options: UseDataWarehouseOptions = {}
  ...
  const updateTable = useCallback(
    (tableId: string, updates: Partial<DWTable>) => {
      setTables((prev) =>
        prev.map((table) =>
  ...
  const deleteTable = useCallback((tableId: string) => {
    ...
    setETLMappings((prev) =>
      prev.filter((mapping) => mapping.targetTableId !== tableId)
    );
  }, []);

  const getTable = useCallback(
    (tableId: string): DWTable | undefined => {
  ...
  const addColumn = useCallback(
    (tableId: string, column: Omit<Column, 'id'>) => {
  ...
  const addRelationship = useCallback(
    (relationship: Omit<ForeignKeyRelationship, 'id'>) => {
  ...
  const updateETLMapping = useCallback(
    (mappingId: string, updates: Partial<ETLMapping>) => {
      setETLMappings((prev) =>
        prev.map((mapping) =>
  ...
  const getConformedDimensions = useCallback((): DWTable[] => {
    return tables.filter(
      (table) =>
        table.type === 'dimension' && table.dimensionType === 'conformed'
    );
  }, [tables]);
  ...
  const validateSchema = useCallback((): {
    ...
    for (const rel of relationships) {
      ...
      if (!fromTable) {
        errors.push(
          `Relationship references non-existent from table: ${rel.fromTableId}`
        );
      }
      if (!toTable) {
        errors.push(
          `Relationship references non-existent to table: ${rel.toTableId}`
        );
      }
    }
    ...
    for (const dim of dimTables) {
      if (!dim.dimensionType) {
        warnings.push(
          `Dimension "${dim.name}" should specify a dimension type`
        );
      }
