...
export const useZeroTrustArchitecture = (): UseZeroTrustArchitectureReturn => {
  ...
  // Security zone operations
  const addSecurityZone = useCallback(
    (zone: Omit<SecurityZone, 'id'>): string => {
  ...
  const updateSecurityZone = useCallback(
    (id: string, updates: Partial<SecurityZone>) => {
  ...
  const deleteSecurityZone = useCallback((id: string) => {
    ...
    setPolicyRules((prev) =>
      prev.filter(
        (rule) => rule.sourceZone !== id && rule.destinationZone !== id
      )
    );
  }, []);
  ...
  // Identity provider operations
  const addIdentityProvider = useCallback(
    (idp: Omit<IdentityProvider, 'id'>): string => {
  ...
  const updateIdentityProvider = useCallback(
    (id: string, updates: Partial<IdentityProvider>) => {
  ...
  const updatePolicyRule = useCallback(
    (id: string, updates: Partial<PolicyRule>) => {
      setPolicyRules((prev) =>
  ...
  const analyzeCoverage = useCallback((): CoverageAnalysis => {
    ...
    const policyPercentage =
      totalZones > 0
        ? Math.round((zonesWithPolicies.size / totalZones) * 100)
        : 0;
  ...
  const validateArchitecture = useCallback((): string[] => {
    ...
    if (securityZones.length > 0) {
      ...
      const untrustedZones = securityZones.filter(
        (z) => z.trustLevel === 'untrusted'
      );
      if (untrustedZones.length === 0) {
        issues.push(
          'No untrusted zones defined (consider Internet-facing resources)'
        );
      }
      ...
      if (zoneTypes.size === 1 && securityZones.length > 1) {
        issues.push(
          'All zones are the same type - consider diversifying zone types'
        );
      }
  ...
  const generateRecommendations = useCallback((): string[] => {
    ...
    if (identityProviders.length === 0) {
      recommendations.push(
        'Add an identity provider (OIDC/SAML) for centralized authentication'
      );
    } else if (identityProviders.length === 1) {
      recommendations.push(
        'Consider adding a backup identity provider for redundancy'
      );
    }
    ...
    if (securityZones.length === 0) {
      ...
      recommendations.push(
        'Consider adding more zones: DMZ, Private, and Restricted'
      );
    }
    ...
    if (policyRules.length === 0 && securityZones.length > 0) {
      recommendations.push(
        'Define policy rules to control traffic between zones'
      );
    }
    ...
    if (allowRules > 0 && denyRules === 0) {
      recommendations.push(
        'Add explicit deny rules to implement deny-by-default'
      );
    }
    ...
    // MFA recommendations
    const idpsWithoutMfa = identityProviders.filter(
      (idp) => !idp.mfaEnabled
    ).length;
    if (idpsWithoutMfa > 0) {
  ...
  const exportToTerraform = useCallback((): string => {
    ...
    securityZones.forEach((zone) => {
      ...
      lines.push(
        `  description = "${zone.description || `Security zone: ${zone.type}`}"`
      );
    ...
    policyRules.forEach((rule) => {
      ...
      if (rule.action === 'allow' && sourceZone && destZone) {
        ...
        lines.push(
          `  source_security_group_id = aws_security_group.${sourceZone.id}.id`
        );
        lines.push(
          `  security_group_id        = aws_security_group.${destZone.id}.id`
        );
  ...
  const exportToDiagram = useCallback((): string => {
    ...
    policyRules.forEach((rule) => {
      ...
      if (sourceZone && destZone) {
        ...
        lines.push(
          `"${sourceZone.name}" ${arrow} "${destZone.name}" : ${label}`
        );
      }
    });
