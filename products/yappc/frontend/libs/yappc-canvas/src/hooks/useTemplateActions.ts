      ...
      try {
        ...
        for (const node of nodes) {
          ...
          switch (nodeData.type) {
            case 'service':
              if (aiCodeGen) {
                const serviceResult = await aiCodeGen.generateService(
                  nodeData as unknown,
                  {
              ...
              if (options?.createRunbooks) {
                const deploymentRunbook =
                  await devSecOps.generateDeploymentRunbook(
                    nodeData as unknown,
                    {
            ...
            case 'database':
              if (aiCodeGen) {
                const schemaResult = await aiCodeGen.generateDatabaseSchema(
                  nodeData as unknown
                );
                if (schemaResult.success && schemaResult.files) {
                  files.push(
                    ...schemaResult.files.map((f) => ({
                      path: f.path,
                      content: f.content,
                      language: f.language,
                    }))
              ...
              if (options?.createRunbooks) {
                const infraRunbook =
                  await devSecOps.generateInfrastructureRunbook(
                    nodeData as unknown
                  );
                runbooks.push({
            ...
            case 'apiEndpoint':
              if (aiCodeGen) {
                const routeResult = await aiCodeGen.generateAPIRoute(
                  nodeData as unknown
                );
                if (routeResult.success && routeResult.files) {
                  files.push(
                    ...routeResult.files.map((f) => ({
                      path: f.path,
                      content: f.content,
                      language: f.language,
                    }))
            ...
            case 'testSuite':
              if (options?.createRunbooks) {
                const testRunbook = await devSecOps.generateTestRunbook(
                  nodeData as unknown
                );
                runbooks.push({
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to export code';
        setError(errorMessage);
        return {
          success: false,
          files: [],
