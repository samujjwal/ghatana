...
export function useFullStackMode(
  ...
  const { frontendPartition, backendPartition } = useMemo(() => {
    ...
    const calculateBounds = (nodeList: Node[]) => {
      ...
      const maxX = Math.max(
        ...xs.map((x, i) => x + (nodeList[i].width || 200))
      );
      const maxY = Math.max(
        ...ys.map((y, i) => y + (nodeList[i].height || 100))
      );
  ...
  const validate = useCallback((): CrossCanvasValidation => {
    ...
    const frontendApiCalls = frontendPartition.nodes.filter(
      (n) =>
        n.type === 'apiCall' ||
        !!(n.data as Record<string, unknown> | undefined)?.isApiCall
    );
    ...
    frontendApiCalls.forEach((node) => {
      const hasBackendEndpoint = dataFlowEdges.some(
        (edge) =>
          edge.source === node.id &&
          backendPartition.nodes.some(
            (bn) => bn.id === edge.target && bn.type === 'api'
          )
      );
    ...
    backendApis.forEach((node) => {
      const hasFrontendConsumer = dataFlowEdges.some(
        (edge) =>
          edge.target === node.id &&
          frontendPartition.nodes.some((fn) => fn.id === edge.source)
      );
    ...
    (sourceId: string, targetId: string, dataType: string) => {
      const newEdge: DataFlowEdge = {
        ...
        data: {
          dataType,
          validated: false,
          flowDirection: frontendPartition.nodes.some((n) => n.id === sourceId)
            ? 'frontend-to-backend'
            : 'backend-to-frontend',
        },
      };
