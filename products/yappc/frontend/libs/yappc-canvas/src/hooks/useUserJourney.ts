...
export interface UseUserJourneyResult {
  ...
  updateStage: (
    stageId: string,
    updates: Partial<Omit<JourneyStage, 'id'>>
  ) => void;
  ...
  addTouchpoint: (
    stageId: string,
    touchpoint: Omit<JourneyTouchpoint, 'id'>
  ) => void;
  ...
  addPainPoint: (
    stageId: string,
    painPoint: Omit<JourneyPainPoint, 'id'>
  ) => void;
...
export const useUserJourney = (
  options: UseUserJourneyOptions = {}
): UseUserJourneyResult => {
  const {
  ...
  const updateStage = useCallback(
    (stageId: string, updates: Partial<Omit<JourneyStage, 'id'>>) => {
      setStages((prev) =>
        prev.map((stage) =>
  ...
  const getStage = useCallback(
    (stageId: string): JourneyStage | undefined => {
  ...
  const addTouchpoint = useCallback(
    (stageId: string, touchpoint: Omit<JourneyTouchpoint, 'id'>) => {
      setStages((prev) =>
        prev.map((stage) => {
          if (stage.id === stageId) {
  ...
  const deleteTouchpoint = useCallback(
    (stageId: string, touchpointId: string) => {
      setStages((prev) =>
        prev.map((stage) => {
          if (stage.id === stageId) {
            return {
              ...stage,
              touchpoints: (stage.touchpoints || []).filter(
  ...
  const addPainPoint = useCallback(
    (stageId: string, painPoint: Omit<JourneyPainPoint, 'id'>) => {
      setStages((prev) =>
        prev.map((stage) => {
          if (stage.id === stageId) {
  ...
  const deletePainPoint = useCallback(
    (stageId: string, painPointId: string) => {
      setStages((prev) =>
        prev.map((stage) => {
          if (stage.id === stageId) {
            return {
              ...stage,
              painPoints: (stage.painPoints || []).filter(
  ...
  const addEmotion = useCallback(
    (stageId: string, emotion: EmotionType) => {
      setStages((prev) =>
        prev.map((stage) => {
          if (stage.id === stageId) {
  ...
  // Add user quote
  const addUserQuote = useCallback(
    (stageId: string, quote: Omit<UserQuote, 'id'>) => {
      setStages((prev) =>
        prev.map((stage) => {
          if (stage.id === stageId) {
    ...
    setStages((prev) =>
      prev.map((stage) => {
        if (stage.id === stageId) {
          return {
            ...stage,
            userQuotes: (stage.userQuotes || []).filter(
              (q) => q.id !== quoteId
            ),
          };
        }
  ...
  const analyzeTranscript = useCallback(
    async (transcript: string): Promise<void> => {
      ...
      lines.forEach((line) => {
        ...
        if (
          painKeywords.some((keyword) => lowerLine.includes(keyword)) &&
          stages.length > 0
        ) {
      ...
      lines.forEach((line) => {
        const lowerLine = line.toLowerCase();
        if (
          positiveKeywords.some((keyword) => lowerLine.includes(keyword)) &&
          stages.length > 0
        ) {
  ...
  // Generate heatmap
  const generateHeatmap = useCallback((): Array<{
    stageId: string;
    intensity: number;
  }> => {
  ...
  const getTouchpointCount = useCallback((): number => {
    return stages.reduce(
      (count, stage) => count + (stage.touchpoints?.length || 0),
      0
    );
  }, [stages]);
  ...
  const getPainPointCount = useCallback((): number => {
    return stages.reduce(
      (count, stage) => count + (stage.painPoints?.length || 0),
      0
    );
  }, [stages]);
  ...
  const getQuoteCount = useCallback((): number => {
    return stages.reduce(
      (count, stage) => count + (stage.userQuotes?.length || 0),
      0
    );
  }, [stages]);
