...
export const useReviewMode = (
  ...
  const [analysisResults, setAnalysisResults] = useState<
    Map<string, StaticAnalysisResult>
  >(new Map());
  ...
  const addFile = useCallback(
    (file: Omit<ReviewFile, 'id' | 'annotations'>) => {
  ...
  const updateFile = useCallback(
    (fileId: string, updates: Partial<ReviewFile>) => {
      setFiles((prev) =>
        prev.map((file) =>
      ...
      setFiles((prev) =>
        prev.map((file) =>
          file.id === annotation.fileId
            ? {
                ...file,
                annotations: file.annotations.filter(
                  (a) => a.id !== annotationId
                ),
              }
            : file
  ...
  const runStaticAnalysis = useCallback(
    async (fileId: string): Promise<StaticAnalysisResult> => {
      ...
      const cyclomaticComplexity =
        1 + ifCount + forCount + whileCount + caseCount;
      ...
      const result: StaticAnalysisResult = {
        ...
        metrics: {
          linesOfCode,
          commentDensity:
            linesOfCode > 0 ? (commentLines / linesOfCode) * 100 : 0,
   ...
   */
  const getAnalysisForFile = useCallback(
    (fileId: string) => {
  ...
  const importReview = useCallback((data: string) => {
    try {
      ...
      if (parsed.annotations) {
        setAnnotations(
          parsed.annotations.map((a: unknown) => ({
            ...a,
