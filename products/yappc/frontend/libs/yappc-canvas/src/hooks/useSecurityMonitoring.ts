...
async function mockScanNode(node: Node): Promise<Vulnerability[]> {
  ...
  if (
    dependencies.includes('log4j') ||
    node.data.label?.toLowerCase().includes('log')
  ) {
  ...
  if (node.type === 'apiEndpoint' && metadata.method === 'POST') {
    vulnerabilities.push({
      ...
      description:
        'The application may be vulnerable to SQL injection attacks due to unsanitized user input.',
      severity: 'high',
  ...
  if (node.type === 'apiEndpoint' && metadata.authentication === 'none') {
    vulnerabilities.push({
      ...
      description:
        'The API endpoint does not require authentication, potentially exposing sensitive data.',
      severity: 'high',
      package: node.data.label || 'Unknown',
      currentVersion: '1.0.0',
      cweId: 'CWE-306',
      ...
      for (const node of nodes) {
        ...
        if (filteredVulns.length > 0) {
          statuses.push({
            nodeId: node.id,
            nodeLabel:
              (node.data as { label?: string }).label || 'Unnamed Node',
            vulnerabilities: filteredVulns,
            totalCount: filteredVulns.length,
            criticalCount: filteredVulns.filter(
              (v) => v.severity === 'critical'
            ).length,
            highCount: filteredVulns.filter((v) => v.severity === 'high')
              .length,
  ...
  const generateFix = useCallback(
    async (vulnerabilityId: string, nodeId: string): Promise<FixSuggestion> => {
      ...
      try {
        ...
        const vulnerability = nodeStatus?.vulnerabilities.find(
...
3. Testing recommendations`;
   ...
   * Dismiss vulnerability
   */
  const dismissVulnerability = useCallback(
    (nodeId: string, vulnerabilityId: string) => {
