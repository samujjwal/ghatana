...
export interface UseComplianceReturn {
  ...
  addEvidence: (
    controlId: string,
    evidence: Omit<Evidence, 'id' | 'controlId'>
  ) => string;
...
const calculateComplianceScore = (controls: ComplianceControl[]): number => {
  ...
  const totalWeight = controls.reduce(
    (sum, control) => sum + weights[control.status],
    0
  );
...
const analyzeGaps = (
  ...
  const notImplementedCount = controls.filter(
  ...
  const noEvidenceCount = controls.filter(
    (c) =>
      ...
      evidence.filter((e) => e.controlId === c.id).length === 0
  ).length;
  ...
  if (controls.length < 10) {
    recommendations.push(
      'Add more controls to ensure comprehensive coverage of compliance requirements'
    );
  }

  const estimatedDaysToClose = gaps.reduce(
...
const calculateRisk = (
  ...
  const implementationRate =
    controls.length > 0 ? implementedCount / controls.length : 0;
  ...
  const evidenceCoverage =
    controls.length > 0 ? controlsWithEvidence / controls.length : 0;
  ...
  const reviewRate =
    controls.length > 0 ? needsReviewCount / controls.length : 0;
...
export const useCompliance = (): UseComplianceReturn => {
  ...
  const addControl = useCallback(
    (control: Omit<ComplianceControl, 'id'>): string => {
      const id = generateId();
  ...
  const updateControl = useCallback(
    (id: string, updates: Partial<ComplianceControl>) => {
      setControls((prev) =>
        prev.map((control) =>
          control.id === id
            ? {
  ...
  const addEvidence = useCallback(
    (controlId: string, ev: Omit<Evidence, 'id' | 'controlId'>): string => {
      const id = generateId();
  ...
  const updateEvidence = useCallback(
    (id: string, updates: Partial<Evidence>) => {
      setEvidence((prev) =>
  ...
  const validateEvidence = useCallback(
    (controlId: string): boolean => {
      const controlEvidence = evidence.filter(
        (ev) => ev.controlId === controlId
      );
      return (
        controlEvidence.length > 0 &&
        controlEvidence.every((ev) => ev.verified !== false)
      );
    },
    [evidence]
  );
