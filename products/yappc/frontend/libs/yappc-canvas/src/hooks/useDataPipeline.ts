...
export interface UseDataPipelineResult {
  ...
  updateNodeConfig: (
    id: string,
    config: Partial<PipelineNode['config']>
  ) => void;
...
export function useDataPipeline(
  ...
  const updateNodeConfig = useCallback(
    (id: string, config: Partial<PipelineNode['config']>) => {
      setNodes((prev) =>
        prev.map((n) =>
   ...
   * Set node schema
   */
  const setNodeSchema = useCallback(
    (nodeId: string, schema: ColumnSchema[]) => {
   ...
   * Add schema mapping
   */
  const addSchemaMapping = useCallback(
    (nodeId: string, mapping: SchemaMapping) => {
      setNodes((prev) =>
        prev.map((n) => {
          if (n.id === nodeId) {
...
select * from transformed`;
  ...
  const getNode = useCallback(
    (id: string): PipelineNode | undefined => {
  ...
  const getNodesByType = useCallback(
    (type: PipelineNodeType): PipelineNode[] => {
