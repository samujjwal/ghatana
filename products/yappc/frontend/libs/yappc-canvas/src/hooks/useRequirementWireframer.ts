...
export function useRequirementWireframer(
  ...
  const parseStory = useCallback(
    (story: string): ParsedUserStory | null => {
      // Validate
  ...
  const updateElement = useCallback(
    (id: string, updates: Partial<WireframeElement>) => {
      setElements((prev) =>
  ...
  const updateRule = useCallback(
    (id: string, updates: Partial<BusinessRule>) => {
      setRules((prev) =>
   ...
   */
  const unlinkRuleFromElement = useCallback(
    (ruleId: string, elementId: string) => {
      setRules((prev) =>
        prev.map((rule) =>
          rule.id === ruleId
            ? {
                ...rule,
                appliesTo: rule.appliesTo.filter((id) => id !== elementId),
              }
            : rule
        )
      );
