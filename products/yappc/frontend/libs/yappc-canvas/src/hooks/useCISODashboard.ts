...
const organizationAtom = atomWithStorage<string>(
  'ciso-organization',
  'My Organization'
);
...
export const useCISODashboard = (): UseCISODashboardReturn => {
  ...
  const [selectedIncidentStatus, setSelectedIncidentStatus] = useAtom(
    selectedIncidentStatusAtom
  );
  const [selectedTrendPeriod, setSelectedTrendPeriod] = useAtom(
    selectedTrendPeriodAtom
  );
  ...
  const getSecurityKPIs = (): SecurityKPIs => {
    ...
    const activeIncidentsCount = incidents.filter((i) =>
      ['critical', 'active', 'investigating', 'contained'].includes(i.status)
    ).length;
    ...
    const complianceScore =
      totalVulns > 0 ? Math.round((patchedVulns / totalVulns) * 100) : 100;
  ...
  const calculateSystemRisk = (systemName: string): number => {
    const systemCVEs = cves.filter((c) =>
      c.affectedSystems.includes(systemName)
    );
    const systemIncidents = incidents.filter((i) =>
      i.affectedSystems.includes(systemName)
    );
    ...
    // Vulnerability risk
    riskScore +=
      systemCVEs.filter((c) => c.severity === 'critical').length * 40;
    ...
    // Incident risk
    riskScore +=
      systemIncidents.filter((i) => i.status === 'critical').length * 30;
    riskScore +=
      systemIncidents.filter((i) =>
        ['active', 'investigating'].includes(i.status)
      ).length * 15;
    ...
    cve.affectedSystems.forEach((systemName) => {
      ...
      if (system) {
        ...
        const updatedSystem: SystemRisk = {
          ...
          criticalVulnerabilities: updatedVulns.filter(
            (v) => v.severity === 'critical'
          ).length,
          highVulnerabilities: updatedVulns.filter((v) => v.severity === 'high')
            .length,
          riskScore,
          riskLevel,
        ...
        setSystems(
          systems.map((s) => (s.systemName === systemName ? updatedSystem : s))
        );
      }
    });
  ...
  const updateCVE = (id: string, updates: Partial<CVE>): void => {
    ...
    if (updates.affectedSystems) {
      ...
      if (cve) {
        const allAffectedSystems = new Set([
          ...cve.affectedSystems,
          ...updates.affectedSystems,
        ]);
          ...
          setSystems(
            systems.map((s) => {
              if (s.systemName === systemName) {
                const updatedVulns = cves.filter((c) =>
                  c.affectedSystems.includes(systemName)
                );
                return {
                  ...s,
                  ...
                  highVulnerabilities: updatedVulns.filter(
                    (v) => v.severity === 'high'
                  ).length,
                  riskScore,
                  riskLevel,
  ...
  const updateIncident = (
    ...
    setIncidents(
      incidents.map((i) => (i.id === id ? { ...i, ...updates } : i))
    );
  ...
  const getSecurityTrends = (period: TrendPeriod): SecurityTrends => {
    ...
    const vulnerabilityChange =
      cves.length > 0 ? Math.round((Math.random() - 0.5) * 20) : 0;
    const incidentChange =
      incidents.length > 0 ? Math.round((Math.random() - 0.5) * 30) : 0;
  ...
  const getVulnerabilityTrends = (
    ...
    for (let i = dataPoints - 1; i >= 0; i--) {
      ...
      trends.push({
        ...
        count: Math.max(
          0,
          cves.length + Math.round((Math.random() - 0.5) * 10)
        ),
      });
    }
  ...
  const getIncidentTrends = (
    ...
    for (let i = dataPoints - 1; i >= 0; i--) {
      ...
      trends.push({
        ...
        count: Math.max(
          0,
          incidents.length + Math.round((Math.random() - 0.5) * 5)
        ),
      });
    }
  ...
  const generateBoardReport = (): string => {
    ...
    if (kpis.criticalVulnerabilities > 0) {
      recommendations.push(
        `• IMMEDIATE: Address ${kpis.criticalVulnerabilities} critical vulnerabilities`
      );
    }
    if (kpis.criticalIncidents > 0) {
      recommendations.push(
        `• IMMEDIATE: Resolve ${kpis.criticalIncidents} critical security incidents`
      );
    }
    if (kpis.complianceScore < 80) {
      recommendations.push(
        `• HIGH: Improve compliance score to 80% minimum (current: ${kpis.complianceScore}%)`
      );
    }
    if (kpis.highVulnerabilities > 5) {
      recommendations.push(
        `• MEDIUM: Reduce high-severity vulnerabilities (current: ${kpis.highVulnerabilities})`
      );
    }
    if (kpis.meanTimeToResolve > 48) {
      recommendations.push(
        `• MEDIUM: Improve incident response time (current MTTR: ${kpis.meanTimeToResolve}h)`
      );
    }
    if (kpis.patchCompliance < 90) {
      recommendations.push(
        `• MEDIUM: Increase patch compliance rate (current: ${kpis.patchCompliance}%)`
      );
    }
