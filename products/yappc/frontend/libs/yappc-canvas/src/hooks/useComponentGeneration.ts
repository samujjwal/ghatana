...
export function useComponentGeneration(
  ...
  const [lastGenerated, setLastGenerated] = useState<GeneratedComponent | null>(
    null
  );
...
`;
      ...
      try {
        ...
        const testCode = generateTemplateTestFile(
          componentName,
          componentCode,
          props
        );
  ...
  const generateComponent = useCallback(
    async (
      ...
      try {
        ...
        let componentCode = '';
        if (framework === 'react') {
          componentCode = await generateReactComponent(
            componentName,
            genOptions
          );
        } else {
   ...
   */
  const generateStorybook = useCallback(
    async (node: Node, variants?: StoryVariant[]): Promise<string> => {
      const componentName = node.data.label || node.data.name || 'Component';
...
function generateTemplateComponent(
  ...
  const tailwindClasses =
    styling === 'tailwind' ? ' className="p-4 bg-white rounded-lg shadow"' : '';
  const a11yAttrs = accessible
    ? ' role="main" aria-label="' + componentName + '"'
    : '';
  const responsiveClass =
    responsive && styling === 'tailwind' ? ' sm:p-6 md:p-8' : '';
