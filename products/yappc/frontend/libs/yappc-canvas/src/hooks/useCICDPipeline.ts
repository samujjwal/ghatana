...
export interface UseCICDPipelineResult {
  ...
  addStage: (
    stage: Omit<
      PipelineStage,
      'id' | 'status' | 'steps' | 'environmentVariables'
    >
  ) => string;
...
export const useCICDPipeline = (
  options: UseCICDPipelineOptions = {}
): UseCICDPipelineResult => {
  const { initialPipelineName = 'CI/CD Pipeline', initialRepository = '' } =
    options;
  ...
  const updateStage = useCallback(
    (stageId: string, updates: Partial<Omit<PipelineStage, 'id'>>) => {
      setStages((prev) =>
        prev.map((stage) =>
  ...
  const addStep = useCallback(
    (stageId: string, step: Omit<PipelineStep, 'id'>) => {
  ...
  const addEnvironmentVariable = useCallback(
    (stageId: string, key: string, value: string) => {
      setStages((prev) =>
        prev.map((stage) =>
          stage.id === stageId
            ? {
    ...
    stages.forEach((stage, index) => {
      ...
      stage.steps?.forEach((step, stepIndex) => {
        if (!step.command.trim()) {
          issues.push(
            `Step ${stepIndex + 1} in stage "${stage.name}" has no command`
          );
        }
      });
    });
  ...
  const exportToGitHubActions = useCallback((): string => {
    ...
    stages.forEach((stage) => {
      ...
      if (
        stage.environmentVariables &&
        Object.keys(stage.environmentVariables).length > 0
      ) {
        yaml.push('    env:');
  ...
  const getStepCount = useCallback((): number => {
    return stages.reduce(
      (count, stage) => count + (stage.steps?.length || 0),
      0
    );
  }, [stages]);
