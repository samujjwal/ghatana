...
export const usePerformanceAnalysis = (): UsePerformanceAnalysisReturn => {
  ...
  const updateService = useCallback(
    (id: string, updates: Partial<Service>): void => {
      setServices((prev) =>
        prev.map((service) =>
  ...
  const addMetric = useCallback(
    (metric: Omit<PerformanceMetric, 'id' | 'timestamp'>): string => {
  ...
  const updateSLO = useCallback(
    (serviceId: string, slo: Partial<SLO>): void => {
      setSLOs((prev) => {
        const newMap = new Map(prev);
  ...
  const isSLOViolated = useCallback(
    (serviceId: string): boolean => {
      ...
      const errorRateMetric = latestMetrics.find(
        (m) => m.type === 'error_rate'
      );
  ...
  const getSLOCompliance = useCallback(
    (serviceId: string, timeRange: TimeRange): number => {
      ...
      const compliantMetrics = p95Metrics.filter(
        (m) => m.value <= slo.p95Target
      ).length;
  ...
  const calculatePercentile = useCallback(
    (serviceId: string, timeRange: TimeRange, percentile: number): number => {
      const rangeMetrics = getServiceMetrics(serviceId, timeRange);
      const latencyMetrics = rangeMetrics.filter((m) =>
        m.type.startsWith('latency_')
      );
    ...
    services.forEach((service) => {
      ...
      if (errorRateMetric && errorRateMetric.value > slo.errorRateTarget * 2) {
        detected.push({
          ...
          severity:
            errorRateMetric.value > slo.errorRateTarget * 5
              ? 'critical'
              : 'high',
  ...
  const addQuery = useCallback(
    (query: Omit<QueryPerformance, 'id' | 'timestamp'>): string => {
  ...
  const getQueryStatistics = useCallback(
    (queryId: string) => {
      ...
      const executionTimes = similarQueries
  ...
  const generateOptimizationRecommendations =
    useCallback((): OptimizationRecommendation[] => {
      ...
      services.forEach((service) => {
        ...
        if (p95Metric && p95Metric.value > slo.p95Target * 1.2) {
          if (service.type === 'database') {
            recommendations.push({
              ...
              priority:
                p95Metric.value > slo.p95Target * 2 ? 'critical' : 'high',
        ...
        if (
          errorRateMetric &&
          errorRateMetric.value > slo.errorRateTarget * 1.5
        ) {
          recommendations.push({
            id: `rec-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            serviceId: service.id,
            ...
            priority:
              errorRateMetric.value > slo.errorRateTarget * 3
                ? 'critical'
                : 'high',
        ...
        if (service.type === 'api' || service.type === 'database') {
          const throughput =
            latestMetrics.find((m) => m.type === 'throughput')?.value || 0;
          if (throughput > 100) {
            recommendations.push({
              id: `rec-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
              serviceId: service.id,
  ...
  const compareSnapshots = useCallback(
    (snapshot1Id: string, snapshot2Id: string) => {
      ...
      const added = snapshot2.services.filter(
        (s) => !snapshot1ServiceIds.has(s.id)
      );
      const removed = snapshot1.services.filter(
        (s) => !snapshot2ServiceIds.has(s.id)
      );
      const modified = snapshot2.services.filter((s) =>
        snapshot1ServiceIds.has(s.id)
      );

      // Compare latency
      const latencyChanges: Record<
        string,
        { before: number; after: number; change: number }
      > = {};
      ...
      modified.forEach((service) => {
        ...
        if (before.length > 0 && after.length > 0) {
          const avgBefore =
            before.reduce((sum, v) => sum + v, 0) / before.length;
  ...
  const exportReport = useCallback(
    (format: 'json' | 'csv'): string => {
      ...
      const rows = services.map((service) => {
        const latestMetrics = getLatestMetrics(service.id);
        const p50 =
          latestMetrics.find((m) => m.type === 'latency_p50')?.value || 0;
        const p95 =
          latestMetrics.find((m) => m.type === 'latency_p95')?.value || 0;
        const p99 =
          latestMetrics.find((m) => m.type === 'latency_p99')?.value || 0;
        const errorRate =
          latestMetrics.find((m) => m.type === 'error_rate')?.value || 0;
