...
describe('useComponentGeneration', () => {
  ...
  describe('Component Generation', () => {
    ...
    it('should update lastGenerated after generation', async () => {
      ...
      expect(result.current.lastGenerated?.componentFile.filename).toBe(
        'ProfileCard.tsx'
      );
    });
  ...
  describe('Storybook Generation', () => {
    ...
    it('should generate stories with custom variants', async () => {
      ...
      const variants = [
        {
          name: 'Default',
          args: { user: { id: 1, name: 'John' }, editable: false },
        },
        {
          name: 'Editable',
          args: { user: { id: 1, name: 'John' }, editable: true },
        },
      ];

      let storyContent = '';
      await act(async () => {
        storyContent = await result.current.generateStorybook(
          mockNode,
          variants
        );
      });
  ...
  describe('Preview Management', () => {
    it('should set preview code', () => {
      ...
      expect(result.current.previewCode).toBe(
        'const Component = () => <div />;'
      );
    });
