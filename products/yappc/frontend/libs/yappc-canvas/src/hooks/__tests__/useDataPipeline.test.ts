...
describe('useDataPipeline', () => {
  ...
  describe('Schema Management', () => {
    ...
    it('should remove schema mapping', () => {
      ...
      expect(result.current.nodes[0].config.mappings).toHaveLength(1);
      expect(result.current.nodes[0].config.mappings![0].sourceColumn).toBe(
        'col2'
      );
    });
  });
  ...
  describe('Validation', () => {
    ...
    it('should validate unconfigured nodes', () => {
      ...
      expect(
        errors.some((e) => e.message.includes('Source type not configured'))
      ).toBe(true);
      expect(
        errors.some((e) => e.message.includes('Sink type not configured'))
      ).toBe(true);
    });
    ...
    it('should validate disconnected nodes', () => {
      ...
      expect(errors.some((e) => e.message.includes('not connected'))).toBe(
        true
      );
    });
