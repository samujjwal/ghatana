...
describe('useServiceBlueprint', () => {
  describe('Initialization', () => {
    ...
    it('should initialize with service description', () => {
      ...
      expect(result.current.serviceDescription).toBe(
        'Complete checkout flow for online orders'
      );
    });
  ...
  describe('Configuration Management', () => {
    ...
    it('should update service description', () => {
      ...
      act(() => {
        result.current.setServiceDescription(
          'Guest check-in and room assignment process'
        );
      });
  ...
  describe('Node Management', () => {
    it('should add node to customer lane', () => {
      ...
      const customerLane = result.current.lanes.find(
        (l) => l.type === 'customer'
      );
    ...
    it('should add node with duration', () => {
      ...
      const frontstageLane = result.current.lanes.find(
        (l) => l.type === 'frontstage'
      );
  ...
  describe('Touchpoint Management', () => {
    it('should add touchpoint to node', () => {
      ...
      let nodeId: string;
      act(() => {
        nodeId = result.current.addNode('frontstage', {
          name: 'Greet Customer',
        });
  ...
  describe('Validation', () => {
    it('should report empty lanes', () => {
      ...
      expect(issues).toContain(
        'Empty lanes: customer, frontstage, backstage, support'
      );
    });
    ...
    it('should report customer nodes without touchpoints', () => {
      ...
      expect(
        issues.some((i) => i.includes('customer nodes without touchpoints'))
      ).toBe(true);
    });
    ...
    it('should report frontstage nodes without touchpoints', () => {
      ...
      expect(
        issues.some((i) => i.includes('frontstage nodes without touchpoints'))
      ).toBe(true);
    });
    ...
    it('should not report backstage nodes without touchpoints', () => {
      ...
      expect(
        issues.some((i) => i.includes('backstage nodes without touchpoints'))
      ).toBe(false);
    });
  ...
  describe('Export', () => {
    ...
    it('should include nodes in export', () => {
      ...
      const customerLane = parsed.lanes.find(
        (l: unknown) => l.type === 'customer'
      );
      expect(customerLane.nodes).toHaveLength(1);
  ...
  describe('Complex Scenarios', () => {
    it('should handle complete e-commerce checkout blueprint', () => {
      ...
      act(() => {
        // Customer journey
        const browse = result.current.addNode('customer', {
          name: 'Browse Products',
          duration: '5 min',
        });
        const cart = result.current.addNode('customer', {
          name: 'Add to Cart',
          duration: '30 sec',
        });
        const checkout = result.current.addNode('customer', {
          name: 'Checkout',
          duration: '2 min',
        });

        // Frontstage
        const showProducts = result.current.addNode('frontstage', {
          name: 'Show Products',
          duration: '1 sec',
        });
        const processPayment = result.current.addNode('frontstage', {
          name: 'Process Payment',
          duration: '10 sec',
        });

        // Backstage
        const inventoryCheck = result.current.addNode('backstage', {
          name: 'Check Inventory',
        });
        const orderProcessing = result.current.addNode('backstage', {
          name: 'Process Order',
        });

        // Support
        const database = result.current.addNode('support', {
          name: 'Product Database',
        });
        const paymentGateway = result.current.addNode('support', {
          name: 'Payment Gateway',
        });

        // Touchpoints
        result.current.addTouchpoint(browse, {
          name: 'Web',
          channel: 'Website',
          metrics: '95% satisfaction',
        });
        result.current.addTouchpoint(cart, {
          name: 'Mobile App',
          channel: 'Mobile',
        });
        result.current.addTouchpoint(checkout, {
          name: 'Checkout Form',
          channel: 'Web',
          metrics: '85% conversion',
        });
        result.current.addTouchpoint(showProducts, {
          name: 'Product Page',
          channel: 'Web',
        });
        result.current.addTouchpoint(processPayment, {
          name: 'Payment Form',
          channel: 'Web',
          metrics: '99% success',
        });
