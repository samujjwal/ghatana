...
describe('useZeroTrustArchitecture', () => {
  ...
  describe('Security Zone Management', () => {
    ...
    it('should add multiple security zones', () => {
      ...
      expect(result.current.securityZones.map((z) => z.name)).toEqual([
        'DMZ',
        'Database',
      ]);
    });
  ...
  describe('Architecture Validation', () => {
    ...
    it('should detect missing untrusted zones', () => {
      ...
      expect(issues.some((issue) => issue.includes('No untrusted zones'))).toBe(
        true
      );
    });
    ...
    it('should detect policies without conditions', () => {
      ...
      expect(issues.some((issue) => issue.includes('have no conditions'))).toBe(
        true
      );
    });
    ...
    it('should detect high-trust public zones', () => {
      ...
      expect(
        issues.some((issue) =>
          issue.includes('public zone(s) have high trust level')
        )
      ).toBe(true);
    });
  ...
  describe('Recommendation Engine', () => {
    ...
    it('should recommend adding DMZ zone', () => {
      ...
      expect(
        recommendations.some((rec) => rec.includes('Add a DMZ zone'))
      ).toBe(true);
    });
    ...
    it('should recommend enabling MFA', () => {
      ...
      expect(recommendations.some((rec) => rec.includes('Enable MFA'))).toBe(
        true
      );
    });
    ...
    it('should recommend adding policy conditions', () => {
      ...
      expect(
        recommendations.some((rec) =>
          rec.includes('Add conditions to policies')
        )
      ).toBe(true);
    });
    ...
    describe('Terraform Export', () => {
      it('should export empty architecture to Terraform', () => {
        ...
        expect(terraform).toContain(
          '# Generated from: Zero-Trust Architecture'
        );
      });
  ...
  describe('Complex Scenario: Complete Zero-Trust Architecture', () => {
    it('should handle comprehensive enterprise architecture', () => {
      ...
      act(() => {
        ...
        result.current.setArchitectureName(
      ...
      const terraform = result.current.exportToTerraform();
      expect(terraform).toContain('resource "aws_security_group"');
      expect(terraform.split('resource "aws_security_group"').length - 1).toBe(
        6
      );
