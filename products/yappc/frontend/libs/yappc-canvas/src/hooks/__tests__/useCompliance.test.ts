...
describe('useCompliance', () => {
  ...
  describe('Control Filtering', () => {
    it('should filter controls by status', () => {
      ...
      const implementedControls =
        result.current.getControlsByStatus('implemented');
      expect(implementedControls).toHaveLength(2);

      const notImplementedControls =
        result.current.getControlsByStatus('not-implemented');
  ...
  describe('Gap Analysis', () => {
    ...
    it('should not flag controls with evidence', () => {
      ...
      const missingEvidenceGaps = gapAnalysis.gaps.filter(
        (g) => g.controlId === 'CC3.1'
      );
    ...
    it('should generate recommendations', () => {
      ...
      expect(
        gapAnalysis.recommendations.some((r) => r.includes('not-implemented'))
      ).toBe(true);
      expect(
        gapAnalysis.recommendations.some((r) => r.includes('failed'))
      ).toBe(true);
    });
  });
  ...
  describe('Export to CSV', () => {
    it('should export to CSV format', () => {
      ...
      expect(csv).toContain(
        'Framework,Control ID,Title,Status,Category,Evidence Count'
      );
  ...
  describe('Complex Scenario: Multi-Framework Compliance Program', () => {
    it('should handle comprehensive compliance program', () => {
      ...
      expect(result.current.getControlsByFramework('iso-27001')).toHaveLength(
        2
      );
      ...
      expect(gapAnalysis.gaps.some((g) => g.severity === 'critical')).toBe(
        true
      ); // Failed control
