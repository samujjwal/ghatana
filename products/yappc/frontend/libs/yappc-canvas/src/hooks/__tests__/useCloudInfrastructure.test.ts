...
describe('useCloudInfrastructure', () => {
  describe('Initialization', () => {
    ...
    it('should allow setting infrastructure name', () => {
      ...
      expect(result.current.infrastructureName).toBe(
  ...
  describe('Resource CRUD Operations', () => {
    ...
    it('should get resources by category', () => {
      ...
      expect(computeResources.every((r) => r.category === 'compute')).toBe(
        true
      );

      const databaseResources =
        result.current.getResourcesByCategory('database');
  ...
  describe('Infrastructure Validation', () => {
    ...
    it('should detect single region deployment', () => {
      ...
      expect(issues.some((issue) => issue.includes('single region'))).toBe(
        true
      );
    });
  ...
  describe('Compliance Checking', () => {
    it('should check encryption at rest', () => {
      ...
      const encryptionCheck = compliance.checks.find((c) =>
        c.name.includes('encryption')
      );
    ...
    it('should check network segmentation', () => {
      ...
      const segmentationCheck = compliance.checks.find((c) =>
        c.name.includes('segmentation')
      );
    ...
    it('should check firewall protection', () => {
      ...
      const firewallCheck = compliance.checks.find((c) =>
        c.name.includes('Firewall')
      );
    ...
    it('should check database backups', () => {
      ...
      const backupCheck = compliance.checks.find((c) =>
  ...
  describe('High Availability Analysis', () => {
    ...
    it('should identify single points of failure', () => {
      ...
      expect(
        ha.singlePointsOfFailure.some((spof) => spof.includes('Single compute'))
      ).toBe(true);
      expect(
        ha.singlePointsOfFailure.some((spof) => spof.includes('database'))
      ).toBe(true);
    });
  });
  ...
  describe('Recommendation Engine', () => {
    it('should recommend adding resources for empty infrastructure', () => {
      ...
      expect(
        recommendations.some((rec) => rec.includes('Add cloud resources'))
      ).toBe(true);
    });
    ...
    it('should recommend adding firewall', () => {
      ...
      expect(recommendations.some((rec) => rec.includes('firewall'))).toBe(
        true
      );
    });
    ...
    it('should recommend load balancer for multiple instances', () => {
      ...
      expect(recommendations.some((rec) => rec.includes('load balancer'))).toBe(
        true
      );
    });
  ...
  describe('Complex Scenario: Enterprise Multi-Cloud Infrastructure', () => {
    it('should handle comprehensive enterprise infrastructure', () => {
      ...
      act(() => {
        result.current.setInfrastructureName(
  ...
  describe('Utility Functions', () => {
    it('should get resource types for AWS compute category', () => {
      ...
      const types = result.current.getResourceTypesForCategory(
        'aws',
        'compute'
      );
    ...
    it('should get different resource types for each provider', () => {
      ...
      const awsTypes = result.current.getResourceTypesForCategory(
        'aws',
        'compute'
      );
      const gcpTypes = result.current.getResourceTypesForCategory(
        'gcp',
        'compute'
      );
      const azureTypes = result.current.getResourceTypesForCategory(
        'azure',
        'compute'
      );
