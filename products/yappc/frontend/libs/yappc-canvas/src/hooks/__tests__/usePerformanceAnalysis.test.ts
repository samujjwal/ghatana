...
describe('usePerformanceAnalysis', () => {
  ...
  describe('Metric Collection', () => {
    ...
    it('should filter metrics by service', () => {
      ...
      const service1Metrics = result.current.getServiceMetrics(
        service1Id,
        '1h'
      );
      const service2Metrics = result.current.getServiceMetrics(
        service2Id,
        '1h'
      );
  ...
  describe('Latency Analysis', () => {
    ...
    it('should get latency distribution', () => {
      ...
      const distribution = result.current.getLatencyDistribution(
        serviceId,
        '1h'
      );
  ...
  describe('Bottleneck Detection', () => {
    ...
    it('should get bottlenecks by service', () => {
      ...
      const service1Bottlenecks =
        result.current.getBottlenecksByService(service1Id);
      const service2Bottlenecks =
        result.current.getBottlenecksByService(service2Id);
  ...
  describe('Query Analysis', () => {
    ...
    it('should filter slow queries by service', () => {
      ...
      const service1Queries = allSlowQueries.filter(
        (q) => q.serviceId === service1Id
      );
      const service2Queries = allSlowQueries.filter(
        (q) => q.serviceId === service2Id
      );
  ...
  describe('Optimization', () => {
    it('should generate optimization recommendations for high latency database', () => {
      ...
      const dbRecommendation = recommendations.find(
    ...
    it('should generate optimization recommendations for high latency API', () => {
      ...
      const recommendations =
        result.current.generateOptimizationRecommendations();
      const apiRecommendation = recommendations.find(
        (r) => r.serviceId === serviceId
      );
    ...
    it('should filter recommendations by priority', () => {
      ...
      const criticalRecommendations =
        result.current.getRecommendationsByPriority('critical');
      const highRecommendations =
        result.current.getRecommendationsByPriority('high');

      expect(
        criticalRecommendations.length + highRecommendations.length
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe('Comparison', () => {
    ...
    it('should compare two snapshots', () => {
      ...
      const comparison = result.current.compareSnapshots(
        snapshot1Id,
        snapshot2Id
      );
  ...
  describe('Complex Performance Scenario', () => {
    it('should handle complex multi-service performance analysis', () => {
      ...
      const databaseBottlenecks =
        result.current.getBottlenecksByService(databaseId);
      expect(databaseBottlenecks.length).toBeGreaterThan(0);
      expect(
        databaseBottlenecks.some(
      ...
      const databaseRecommendations = recommendations.filter(
        (r) => r.serviceId === databaseId
      );
      expect(databaseRecommendations.length).toBeGreaterThan(0);
      expect(
        databaseRecommendations.some((r) => r.recommendation.includes('index'))
      ).toBe(true);

      const apiRecommendations = recommendations.filter(
        (r) => r.serviceId === apiId
      );
      ...
      const dbDistribution = result.current.getLatencyDistribution(
        databaseId,
        '1h'
      );
