...
describe('useAIBrainstorming', () => {
  ...
  describe('generateIdeas', () => {
    it('should generate ideas without AI service (mock mode)', async () => {
      ...
      expect(result.current.response?.prompt).toBe(
    ...
    it('should set loading state during generation', async () => {
      const { result } = renderHook(() =>
        useAIBrainstorming({ autoSpawn: false })
      );
    ...
    it('should handle AI service errors', async () => {
      const mockAIService = createMockAIService();
      (mockAIService.complete as unknown).mockRejectedValue(
    ...
    it('should parse profile-related prompts correctly', async () => {
      ...
      await act(async () => {
        await result.current.generateIdeas(
          'Create a user profile with edit functionality'
        );
      });
      ...
      const hasProfileView = suggestions.some((s) =>
        s.label.toLowerCase().includes('profile')
      );
      const hasEditMode = suggestions.some((s) =>
        s.label.toLowerCase().includes('edit')
      );
  ...
  describe('spawnNodes', () => {
    it('should spawn nodes without source node', () => {
      const onNodesSpawned = vi.fn();
      const { result } = renderHook(() =>
        useAIBrainstorming({ onNodesSpawned })
      );
