...
describe('useFullStackMode', () => {
  ...
  describe('Data Flow Edges', () => {
    ...
    it('should set flow direction for data flow edges', () => {
      ...
      expect(frontendToBackend?.data?.flowDirection).toBe(
        'frontend-to-backend'
      );
      ...
      expect(backendToFrontend?.data?.flowDirection).toBe(
        'backend-to-frontend'
      );
    });
  ...
  describe('Validation', () => {
    ...
    it('should warn about unused backend APIs', () => {
      ...
      expect(result.current.validation.warnings[0].message).toContain(
    ...
    it('should detect type mismatches', () => {
      ...
      expect(
        result.current.validation.errors.some((e) => e.type === 'type-mismatch')
      ).toBe(true);
    });
  });
