...
describe('useServiceHealth', () => {
  ...
  describe('Alert Management', () => {
    ...
    it('should acknowledge alerts', async () => {
      ...
      const acknowledgedAlert = result.current.alerts.find(
        (a) => a.id === firstAlert.id
      );
  ...
  describe('Circuit Breaker Management', () => {
    it('should enable circuit breaker', () => {
      ...
      expect(result.current.circuitBreakers.get('service-1')?.enabled).toBe(
        true
      );
      expect(result.current.circuitBreakers.get('service-1')?.threshold).toBe(
        5
      );
    });
