...
describe('useThreatModeling', () => {
      ...
      const { result } = renderHook(() =>
        useThreatModeling({
          initialSystemDescription:
            'Web application handling payment processing',
        })
      );
  ...
  describe('Configuration Management', () => {
    ...
    it('should update system description', () => {
      ...
      act(() => {
        result.current.setSystemDescription(
          'Microservices architecture with OAuth2'
        );
      });
  ...
  describe('Threat Management', () => {
    ...
    it('should add threat with attack vector', () => {
      ...
      expect(result.current.threats[0].attackVector).toBe(
  ...
  describe('Mitigation Management', () => {
    it('should add mitigation to threat', () => {
      ...
      expect(threat?.mitigations?.[0].strategy).toBe(
    ...
    it('should update mitigation status', () => {
      ...
      act(() => {
        result.current.updateMitigationStatus(
          threatId!,
          mitigationId,
          'in-progress'
        );
    ...
    it('should cycle through mitigation statuses', () => {
      ...
      expect(result.current.getThreat(threatId!)?.mitigations?.[0].status).toBe(
        'in-progress'
      );
