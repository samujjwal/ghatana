...
describe('useCICDPipeline', () => {
  ...
  describe('Step Management', () => {
    ...
    it('should count steps across all stages', () => {
      ...
      act(() => {
        const stage1 = result.current.addStage({
          name: 'Build',
          type: 'build',
        });
  ...
  describe('Environment Variables', () => {
    it('should add environment variable to stage', () => {
      ...
      act(() => {
        stageId = result.current.addStage({ name: 'Build', type: 'build' });
        result.current.addEnvironmentVariable(
          stageId,
          'NODE_ENV',
          'production'
        );
  ...
  describe('Validation', () => {
    ...
    it('should require pipeline name', () => {
      ...
      expect(issues.some((i) => i.includes('Pipeline name is required'))).toBe(
        true
      );
    });
    ...
    it('should require repository', () => {
      ...
      expect(issues.some((i) => i.includes('Repository URL is required'))).toBe(
        true
      );
    });
    ...
    it('should require build stage', () => {
      ...
      act(() => {
        const testStage = result.current.addStage({
          name: 'Test',
          type: 'test',
        });
        result.current.addStep(testStage, {
          name: 'Run Tests',
          command: 'npm test',
        });
      ...
      expect(
        issues.some((i) => i.includes('should have at least one build stage'))
      ).toBe(true);
    });
    ...
    it('should pass validation for complete pipeline', () => {
      ...
      act(() => {
        ...
        result.current.setRepository('github.com/org/repo');
        const buildStage = result.current.addStage({
          name: 'Build',
          type: 'build',
        });
        result.current.addStep(buildStage, {
          name: 'Build',
          command: 'npm run build',
        });
      });
  ...
  describe('Complex Scenarios', () => {
    it('should handle complete CI/CD pipeline', () => {
      ...
      act(() => {
        ...
        // Security
        const security = result.current.addStage({
          name: 'Security',
          type: 'security',
        });
        result.current.addStep(security, {
          name: 'SAST',
        ...
        // Deploy
        const deploy = result.current.addStage({
          name: 'Deploy',
          type: 'deploy',
        });
        result.current.addStep(deploy, {
          name: 'Deploy to Prod',
