...
export interface UseThreatModelingResult {
   ...
   */
  addThreat: (
    threat: Omit<Threat, 'id' | 'riskLevel' | 'mitigations'>
  ) => string;
  ...
  updateThreat: (
    threatId: string,
    updates: Partial<Omit<Threat, 'id'>>
  ) => void;
  ...
  addMitigation: (
    threatId: string,
    mitigation: Omit<Mitigation, 'id' | 'status'>
  ) => void;
  ...
  analyzeSTRIDE: () => {
    categoryBreakdown: Record<
      ThreatCategory,
      { count: number; mitigatedCount: number }
    >;
 ...
 */
export const useThreatModeling = (
  options: UseThreatModelingOptions = {}
): UseThreatModelingResult => {
  const { initialModelName = 'Threat Model', initialSystemDescription = '' } =
    options;
  ...
  const calculateRiskLevel = useCallback(
    (impact?: number, likelihood?: number): RiskLevel => {
  ...
  const addThreat = useCallback(
    (threat: Omit<Threat, 'id' | 'riskLevel' | 'mitigations'>): string => {
  ...
  const updateThreat = useCallback(
    (threatId: string, updates: Partial<Omit<Threat, 'id'>>) => {
      setThreats((prev) =>
        prev.map((threat) => {
  ...
  const getThreat = useCallback(
    (threatId: string): Threat | undefined => {
  ...
  const addMitigation = useCallback(
    (threatId: string, mitigation: Omit<Mitigation, 'id' | 'status'>) => {
      ...
      setThreats((prev) =>
        prev.map((threat) =>
          threat.id === threatId
            ? {
                ...threat,
                mitigations: [...(threat.mitigations || []), newMitigation],
              }
            : threat
        )
      );
  ...
  // Update mitigation status
  const updateMitigationStatus = useCallback(
    (threatId: string, mitigationId: string, status: MitigationStatus) => {
      setThreats((prev) =>
        prev.map((threat) =>
          threat.id === threatId
            ? {
  ...
  const deleteMitigation = useCallback(
    (threatId: string, mitigationId: string) => {
      setThreats((prev) =>
        prev.map((threat) =>
          threat.id === threatId
            ? {
                ...threat,
                mitigations: (threat.mitigations || []).filter(
                  (m) => m.id !== mitigationId
                ),
              }
            : threat
  ...
  const calculateRiskScore = useCallback(
    (threatId: string): number => {
      ...
      const implementedCount = (threat.mitigations || []).filter(
  ...
  const analyzeSTRIDE = useCallback(() => {
    ...
    const unmitigatedThreats = threats.filter(
      (t) =>
        !t.mitigations ||
    ...
    const totalScore = threats.reduce(
      (sum, threat) => sum + calculateRiskScore(threat.id),
      0
    );
    const averageRiskScore =
      threats.length > 0 ? totalScore / threats.length : 0;
  ...
  const getMitigationCount = useCallback((): number => {
    return threats.reduce(
      (count, threat) => count + (threat.mitigations?.length || 0),
      0
    );
  }, [threats]);
