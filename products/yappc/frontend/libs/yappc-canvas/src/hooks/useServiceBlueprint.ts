...
export interface UseServiceBlueprintResult {
  ...
  addNode: (
    laneType: LaneType,
    node: Omit<ProcessNode, 'id' | 'touchpoints'>
  ) => string;
  ...
  updateNode: (
    nodeId: string,
    updates: Partial<Omit<ProcessNode, 'id'>>
  ) => void;
...
const DEFAULT_LANE_TYPES: LaneType[] = [
  'customer',
  'frontstage',
  'backstage',
  'support',
 ...
 */
export const useServiceBlueprint = (
  options: UseServiceBlueprintOptions = {}
): UseServiceBlueprintResult => {
  const {
  ...
  // Initialize lanes
  const initialLanes: BlueprintLane[] = DEFAULT_LANE_TYPES.map(
    (type, index) => ({
      id: `lane-${type}`,
      type,
      nodes: [],
  ...
  const updateNode = useCallback(
    (nodeId: string, updates: Partial<Omit<ProcessNode, 'id'>>) => {
      setLanes((prev) =>
        prev.map((lane) => ({
          ...lane,
  ...
  const getNode = useCallback(
    (nodeId: string): ProcessNode | undefined => {
  ...
  const addTouchpoint = useCallback(
    (nodeId: string, touchpoint: Omit<Touchpoint, 'id'>) => {
        ...
        prev.map((lane) => ({
          ...
          nodes: lane.nodes.map((node) =>
            node.id === nodeId
              ? {
                  ...node,
                  touchpoints: [...(node.touchpoints || []), newTouchpoint],
                }
              : node
          ),
        }))
  ...
  const deleteTouchpoint = useCallback(
    (nodeId: string, touchpointId: string) => {
      setLanes((prev) =>
        prev.map((lane) => ({
          ...lane,
          nodes: lane.nodes.map((node) =>
            node.id === nodeId
              ? {
                  ...node,
                  touchpoints: (node.touchpoints || []).filter(
  ...
  const validateBlueprint = useCallback((): string[] => {
    ...
    lanes.forEach((lane) => {
      if (lane.type === 'customer' || lane.type === 'frontstage') {
        const noTouchpoints = lane.nodes.filter(
          (n) => !n.touchpoints || n.touchpoints.length === 0
        );
        if (noTouchpoints.length > 0) {
          issues.push(
            `${lane.type} nodes without touchpoints: ${noTouchpoints.map((n) => n.name).join(', ')}`
          );
        }
      }
    });
    ...
    if (orphaned.length > 0) {
      issues.push(
        `Orphaned nodes (no connections): ${orphaned.map((n) => n.name).join(', ')}`
      );
    }
  ...
  const analyzeFlow = useCallback(() => {
    ...
    const crossLaneConnections = connections.filter((conn) => {
      ...
      const fromLane = lanes.find((l) =>
        l.nodes.some((n) => n.id === fromNode.id)
      );
  ...
  const getTouchpointCount = useCallback((): number => {
    return lanes.reduce(
      (count, lane) =>
        count +
        lane.nodes.reduce(
          (nodeCount, node) => nodeCount + (node.touchpoints?.length || 0),
          0
        ),
      0
    );
