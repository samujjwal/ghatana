...
const RESOURCE_TYPES: Record<
  CloudProvider,
  Record<ResourceCategory, ResourceType[]>
> = {
  aws: {
...
export const useCloudInfrastructure = (): UseCloudInfrastructureReturn => {
  ...
  const addResource = useCallback(
    (resource: Omit<CloudResource, 'id'>): string => {
  ...
  const validateInfrastructure = useCallback((): string[] => {
    ...
    if (!hasVPC && resources.length > 2) {
      issues.push(
        'No VPC/VNet defined - resources should be in a virtual network'
      );
    }
    ...
    if (regions.size === 1 && resources.length > 5) {
      issues.push(
        'All resources in single region - consider multi-region for HA'
      );
    }
    ...
    if (cost.totalMonthlyCost > 1000) {
      ...
      if (vmCount > 5) {
        issues.push(
          'Consider using auto-scaling groups or containers to optimize VM costs'
        );
      }
    }
  ...
  const checkCompliance = useCallback((): ComplianceReport => {
    ...
    const storageResources = getResourcesByCategory('storage').concat(
      getResourcesByCategory('database')
    );
    ...
    checks.push({
      ...
      passed:
        encryptedStorage.length === storageResources.length ||
    ...
    const databaseResources = getResourcesByCategory('database');
    const databasesWithBackup = databaseResources.filter(
      (r) => r.config.backup === true
    );
    checks.push({
      name: 'Database backups enabled',
      passed:
        databasesWithBackup.length === databaseResources.length ||
  ...
  const analyzeHighAvailability = useCallback((): HighAvailabilityAnalysis => {
    ...
    if (
      ...
      !databaseResources[0].config.replication
    ) {
  ...
  const generateRecommendations = useCallback((): string[] => {
    ...
    if (resources.length === 0) {
      recommendations.push(
        'Add cloud resources to start building your infrastructure'
      );
    ...
    if (!hasFirewall && computeCount > 0) {
      recommendations.push(
        'Add firewall/security groups to protect compute resources'
      );
    }
    ...
    if (!hasWAF && computeCount > 0) {
      recommendations.push(
        'Consider adding WAF for web application protection'
      );
    }

    const hasKMS = resources.some((r) => r.type === 'kms');
    const storageCount =
      getResourcesByCategory('storage').length +
      getResourcesByCategory('database').length;
    ...
    if (
      !haAnalysis.hasBackup &&
      getResourcesByCategory('database').length > 0
    ) {
    ...
    if (cost.totalMonthlyCost > 500) {
      ...
      if (vmCount > 3) {
        recommendations.push(
          'Consider using auto-scaling or spot instances to reduce costs'
        );
      }
    }
    ...
    if (hasStorage && !hasCDN) {
      recommendations.push(
        'Add CDN for faster content delivery and reduced bandwidth costs'
      );
    }
    ...
    const hasDatabase = resources.some(
      (r) => r.type === 'rds' || r.type === 'nosql'
    );
    if (hasDatabase && !hasCaching) {
      recommendations.push(
        'Add caching layer (Redis/Memcached) to reduce database load'
      );
    }
  ...
  const exportToPulumi = useCallback((): string => {
    ...
    resources.forEach((resource) => {
      ...
      lines.push(
        `const ${resource.id} = new ${pulumiType}("${resource.name}", {`
      );
