...
describe('ThreatAnalyzer', () => {
      ...
      const nodes = [
        {
          id: '1',
          type: 'database',
          data: { label: 'User Query', parameterized: false },
        },
      ];
      ...
      const nodes = [
        {
          id: '1',
          type: 'database',
          data: { label: 'User Query', inputValidation: false },
        },
      ];
      ...
      const nodes = [
        {
          id: '1',
          type: 'api',
          data: { label: 'API Endpoint', rateLimited: false },
        },
      ];
      ...
      const nodes = [
        {
          id: '1',
          type: 'api',
          data: { label: 'Protected API', rateLimited: true },
        },
      ];
      ...
      const nodes = [
        {
          id: '1',
          type: 'payment',
          data: { label: 'Payment', encrypted: false },
        },
      ];

      const findings = analyzer.validatePCIDSS(nodes);

      const encryptionFinding = findings.find((f) =>
        f.title.includes('encryption')
      );
      ...
      const nodes = [
        {
          id: '1',
          type: 'payment',
          data: { label: 'Payment', tokenized: false },
        },
      ];

      const findings = analyzer.validatePCIDSS(nodes);

      const tokenFinding = findings.find((f) =>
        f.title.includes('tokenization')
      );
      ...
      const nodes = [
        {
          id: '1',
          type: 'payment',
          data: { label: 'Payment', auditLogged: false },
        },
      ];
    ...
    it('detects all STRIDE threats', () => {
      const nodes = [
        { id: '1', type: 'api', data: { label: 'API', authenticated: false } },
        {
          id: '2',
          type: 'data',
          data: { label: 'Data', integrityCheck: false },
        },
        {
          id: '3',
          type: 'transaction',
          data: { label: 'Trans', auditLogged: false },
        },
        {
          id: '4',
          type: 'database',
          data: { label: 'DB', encryptedAtRest: false },
        },
      ...
      const infoDisclosure = findings.find(
        (f) => f.type === 'Information Disclosure'
      );
      const dos = findings.find((f) => f.type === 'Denial of Service');
      const privilege = findings.find(
        (f) => f.type === 'Elevation of Privilege'
      );
  ...
  describe('analyze', () => {
    it('combines all analysis results', () => {
      const nodes = [
        {
          id: '1',
          type: 'api',
          data: { label: 'API', authenticated: false, rateLimited: false },
        },
        {
          id: '2',
          type: 'database',
          data: { label: 'DB', parameterized: false },
        },
      ...
      const strideFindings = findings.filter((f) =>
        ['Spoofing', 'Denial of Service'].includes(f.type)
      );
