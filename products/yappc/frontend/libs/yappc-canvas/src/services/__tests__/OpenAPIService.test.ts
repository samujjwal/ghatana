...
describe('OpenAPIService', () => {
  describe('generateSpec', () => {
    ...
    it('should infer path parameters', () => {
      ...
      const spec = OpenAPIService.generateSpec(nodes);
      const endpoint = spec.paths['/users/{userId}/posts/{postId}']
        .get as unknown;
    ...
    it('should add request body for POST/PUT/PATCH', () => {
      ...
      expect(endpoint.requestBody.content['application/json'].schema.type).toBe(
        'object'
      );
    });
    ...
    it('should add security schemes for authenticated endpoints', () => {
      ...
      expect((spec.components?.securitySchemes?.jwt as unknown).type).toBe(
        'http'
      );
      expect((spec.components?.securitySchemes?.apiKey as unknown).type).toBe(
        'apiKey'
      );
    });
  ...
  describe('validate', () => {
    ...
    it('should detect invalid path format', () => {
      ...
      expect(result.valid).toBe(false);
      expect(
        result.errors.some((e) => e.includes('Path must start with /'))
      ).toBe(true);
    });
    ...
    it('should detect invalid HTTP method', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.includes('Invalid HTTP method'))).toBe(
        true
      );
    });
  });
