...
describe('UserStoryParser', () => {
  describe('parseUserStory', () => {
    ...
    it('should extract UI elements from story', () => {
      const story =
        'As a user, I want to view the product list and filter by price using a dropdown';
      const result = UserStoryParser.parseUserStory(story);

      expect(result.elements.length).toBeGreaterThan(0);
      const labels = result.elements.map((e) => e.label.toLowerCase());
      expect(
        labels.some((l) => l.includes('list') || l.includes('product'))
      ).toBe(true);
    });

    it('should extract business rules with conditions', () => {
      const story =
        'As a user, I want to search products. If no results found then show a message.';
      const result = UserStoryParser.parseUserStory(story);
  ...
  describe('generateAcceptanceCriteria', () => {
    ...
    it('should include criteria from elements', () => {
      const story =
        'As a user, I want to view a list of products and filter by price';
      const parsed = UserStoryParser.parseUserStory(story);
      const criteria = UserStoryParser.generateAcceptanceCriteria(parsed);
    ...
    it('should include criteria from rules', () => {
      const story =
        'As a user, I want to search products. If no results then show a message.';
      const parsed = UserStoryParser.parseUserStory(story);
      const criteria = UserStoryParser.generateAcceptanceCriteria(parsed);
  ...
  describe('estimateComplexity', () => {
    ...
    it('should estimate medium complexity for moderate stories', () => {
      const story =
        'As a user, I want to view products, filter by price, and sort by name. If out of stock then hide items.';
      const parsed = UserStoryParser.parseUserStory(story);
      const result = UserStoryParser.estimateComplexity(parsed);
    ...
    it('should factor in number of elements', () => {
      const story =
        'As a user, I want to see a search bar, dropdown, list, filter button, and sort menu';
      const parsed = UserStoryParser.parseUserStory(story);
      const result = UserStoryParser.estimateComplexity(parsed);
    ...
    it('should factor in business rules', () => {
      const story =
        'As a user, I want to search. If no results then show message. When clicked then navigate. Unless logged in cannot proceed.';
      const parsed = UserStoryParser.parseUserStory(story);
      const result = UserStoryParser.estimateComplexity(parsed);

      expect(result.factors.some((f) => f.includes('business rules'))).toBe(
        true
      );
    });
    ...
    it('should factor in flow steps', () => {
      const story =
        'As a user, I want to view a list, select item, view details, add to cart, checkout, and confirm order';
      const parsed = UserStoryParser.parseUserStory(story);
      const result = UserStoryParser.estimateComplexity(parsed);
  ...
  describe('Element Extraction Patterns', () => {
    it('should extract search component', () => {
      ...
      const searchElement = result.elements.find((e) =>
    ...
    it('should extract filter component', () => {
      ...
      const filterElement = result.elements.find((e) =>
    ...
    it('should extract list component', () => {
      ...
      const listElement = result.elements.find(
        (e) => e.label.toLowerCase().includes('list') || e.type === 'component'
      );
    ...
    it('should extract button component', () => {
      ...
      const buttonElement = result.elements.find((e) =>
        e.label.toLowerCase().includes('button')
      );
    ...
    it('should extract form components', () => {
      const story =
        'As a user, I want to fill out a form with text fields and checkboxes';
      const result = UserStoryParser.parseUserStory(story);
    ...
    it('should avoid duplicate elements', () => {
      ...
      const searchElements = result.elements.filter((e) =>
        e.label.toLowerCase().includes('search')
      );
  ...
  describe('Business Rule Extraction', () => {
    it('should extract if-then rules', () => {
      const story =
        'As a user, I want to search. If no results found then show error message.';
      const result = UserStoryParser.parseUserStory(story);

      expect(result.rules.length).toBeGreaterThan(0);
      const rule = result.rules.find((r) =>
        r.description.toLowerCase().includes('if')
      );
      expect(rule).toBeDefined();
    });
    ...
    it('should extract when-then rules', () => {
      ...
      const rule = result.rules.find((r) =>
        r.description.toLowerCase().includes('when')
      );
      expect(rule).toBeDefined();
    });
    ...
    it('should extract unless rules', () => {
      ...
      const rule = result.rules.find((r) =>
        r.description.toLowerCase().includes('unless')
      );
      expect(rule).toBeDefined();
    });
    ...
    it('should extract must/should constraints', () => {
      const story =
        'As a user, I must provide email. Password must be 8 characters.';
      const result = UserStoryParser.parseUserStory(story);

      const rules = result.rules.filter((r) =>
        r.description.toLowerCase().includes('must')
      );
  ...
  describe('Complex Scenarios', () => {
    ...
    it('should handle form submission story', () => {
      ...
      expect(
        result.elements.some((e) => e.label.toLowerCase().includes('form'))
      ).toBe(true);
      expect(result.rules.length).toBeGreaterThan(2);
