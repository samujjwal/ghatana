...
export class UserStoryParser {
  ...
  private static extractElements(story: string): WireframeElement[] {
    ...
    const patterns: Array<{
      ...
      {
        regex:
          /\b(view|screen|page|form|modal|dialog)\s+(?:for\s+)?([a-z\s]+)/gi,
        type: 'screen',
      ...
      {
        regex:
          /\b(button|input|field|dropdown|checkbox|radio|slider|toggle)\s*(?:for\s+)?([a-z\s]*)/gi,
        type: 'component',
  ...
  static validateUserStory(story: string): {
    ...
    if (story.length < 20) {
      warnings.push(
        'User story seems too short. Consider adding more details.'
      );
    }

    if (story.length > 500) {
      warnings.push(
        'User story is quite long. Consider breaking it into multiple stories.'
      );
    }
