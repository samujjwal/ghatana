...
export class ThreatAnalyzer {
      ...
      if (node.type === 'logging' || node.type === 'analytics') {
        ...
        if (piiFields.length > 0) {
          findings.push({
            ...
            nodeId: node.id,
            recommendation:
              'Remove PII from logs or implement data masking/encryption',
          });
        }
      }
    });
  ...
  private validatePCIDSS(nodes: AnalysisNode[]): SecurityFinding[] {
    ...
    const paymentNodes = nodes.filter(
      (n) => n.type === 'payment' || n.label.toLowerCase().includes('payment')
    );
    ...
    paymentNodes.forEach((node) => {
      ...
      if (!hasEncryption) {
        findings.push({
          ...
          nodeId: node.id,
          recommendation:
            'Implement TLS 1.2+ encryption for all payment data transmission',
        });
      }
      ...
      if (!hasTokenization) {
        findings.push({
          ...
          nodeId: node.id,
          recommendation:
            'Implement tokenization to avoid storing sensitive card data',
        });
      }
  ...
  private checkTampering(nodes: AnalysisNode[]): SecurityFinding[] {
    return nodes
      .filter(
        (n) => (n.type === 'data' || n.type === 'database') && !n.data.integrity
      )
      .map((n) => ({
  ...
  private checkRepudiation(nodes: AnalysisNode[]): SecurityFinding[] {
    return nodes
      .filter(
        (n) =>
          (n.type === 'transaction' || n.type === 'payment') &&
          !n.data.auditLogged
      )
      .map((n) => ({
