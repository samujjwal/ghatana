...
export class StorybookService {
  ...
  private static generateDefaultVariants(
    ...
    props.forEach((prop) => {
      if (prop.defaultValue !== undefined) {
        defaultArgs[prop.name] = this.parseDefaultValue(
          prop.defaultValue,
          prop.type
        );
      } else {
    ...
    props.forEach((prop) => {
      withAllPropsArgs[prop.name] = this.getTypeExampleValue(
        prop.type,
        prop.name
      );
    });
  ...
  private static generateStorybookContent(
    ...
    const argTypes = this.generateArgTypes(
      props,
      includeActions,
      includeControls
    );
    const stories = variants
      .map((v) => this.generateStoryVariant(componentName, v))
      .join('\n\n');
...
${propsStr}
   ...
   */
  private static generateStoryVariant(
    componentName: string,
  ...
  private static isActionType(type: string): boolean {
    return (
      type.includes('=>') || type.includes('function') || type.startsWith('on')
    );
  }
