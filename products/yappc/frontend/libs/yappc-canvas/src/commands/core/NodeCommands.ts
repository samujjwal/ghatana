...
export interface IDocumentStore {
  ...
  /** Update node content */
  updateNodeContent(id: UniqueId, content: UniversalNode['content']): void;
  /** Reparent a node */
  reparentNode(
    id: UniqueId,
    newParentId: UniqueId | null,
    index?: number
  ): void;
  /** Reorder children */
...
function getStore(): IDocumentStore {
  if (!documentStore) {
    throw new Error(
      'Document store not initialized. Call initializeDocumentStore first.'
    );
  }
 ...
 */
abstract class BaseNodeCommand<TPayload, TResult = void> implements Command<
  TPayload,
  TResult
> {
  ...
  canMerge(other: Command): boolean {
    ...
    return (
      ...
      this.getTargetNodeId() ===
        (other as BaseNodeCommand<unknown>).getTargetNodeId()
    );
  }
 ...
 */
export class InsertNodeCommand extends BaseNodeCommand<
  InsertNodePayload,
  UniversalNode
> {
    ...
    try {
      ...
      const newNode: UniversalNode = {
        ...
        content: this.payload.content
          ? {
              type: this.payload.content
                .type as UniversalNode['content']['type'],
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to undo transform update',
  ...
  async execute(): Promise<CommandResult> {
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Failed to reparent node',
  ...
  async undo(): Promise<CommandResult> {
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Failed to undo reparent',
  ...
  async execute(): Promise<CommandResult> {
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Failed to reorder children',
  ...
  async undo(): Promise<CommandResult> {
    ...
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Failed to undo reorder',
