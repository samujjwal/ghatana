 ...
 */
export function createIDGeneratorState(
...
export function validateID(
  ...
  if (invalidChars.test(id)) {
    errors.push(
      'ID contains invalid characters (<>:"/\\|?* or control characters)'
    );
  }
...
export function normalizeID(
  ...
  const validation = validateID(normalized, {
    ...options,
    maxLength: undefined,
  });
  if (!validation.valid) {
    throw new Error(
      `Invalid ID after normalization: ${validation.errors.join(', ')}`
    );
  }
...
export function resetIDGenerator(
  ...
  return createIDGeneratorState({
    ...
    namespace:
      options.namespace !== undefined ? options.namespace : state.namespace,
