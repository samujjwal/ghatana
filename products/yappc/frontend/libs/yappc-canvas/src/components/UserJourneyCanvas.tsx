...
import {
  ...
  Share2 as ShareIcon,
  TrendingDown as PainIcon,
  TrendingUp as DelightIcon,
  Annoyed as SentimentVeryDissatisfied,
  Frown as SentimentDissatisfied,
  Meh as SentimentNeutral,
  Smile as SentimentSatisfied,
  Laugh as SentimentVerySatisfied,
  Pointer as TouchpointIcon,
  Brain as InsightIcon,
  Quote as QuoteIcon,
...
export const UserJourneyCanvas: React.FC<UserJourneyCanvasProps> = ({
  ...
  const calculateEmotionAverage = (stage: JourneyStage): number => {
    ...
    const sum = stage.emotions.reduce(
      (acc, e) => acc + emotionValues[e.type],
      0
    );
  ...
  return (
    <Box className="flex h-screen bg-gray-50 dark:bg-gray-950">
          ...
          <List className="p-2">
            ...
            <ListItem disablePadding>
              <ListItemButton onClick={() => setTouchpointDialogOpen(true)}>
                ...
                <ListItemText
                  primary="Add Touchpoint"
                  secondary="Interaction point"
                />
              </ListItemButton>
            </ListItem>
            ...
            <ListItem disablePadding>
              <ListItemButton onClick={() => setPainPointDialogOpen(true)}>
                ...
                <ListItemText
                  primary="Add Pain Point"
                  secondary="User frustration"
                />
              </ListItemButton>
            </ListItem>
      ...
      <Box className="flex-1 flex flex-col">
        ...
        <Paper
          variant="raised"
          className="p-4 rounded-none border-gray-200 dark:border-gray-700 border-b"
        >
          <Box className="flex items-center gap-4 flex-wrap">
            ...
            <Box className="flex items-center">
              <Chip
                label={`${journey.stages.length} Stages`}
                size="sm"
                className="mr-2"
              />
              <Chip
                label={`${journey.getTouchpointCount()} Touchpoints`}
                size="sm"
                className="mr-2"
              />
              <Chip
                label={`${journey.getPainPointCount()} Pain Points`}
                size="sm"
                tone="danger"
              />
            </Box>
          ...
          {journey.stages.length === 0 ? (
            <Alert severity="info" className="mt-8 max-w-[600px] mx-auto">
              ...
              <Typography as="p" className="text-sm">
                Click "Add Stage" in the left sidebar to create journey phases
                like Awareness, Consideration, Purchase, etc.
              </Typography>
            </Alert>
          ) : (
            <Box>
                        ...
                        >
                          {stage.touchpoints.map((tp) => (
                            <Chip
                              ...
                              variant="outlined"
                              onDelete={() =>
                                journey.deleteTouchpoint(stage.id, tp.id)
                              }
                            />
                          ))}
                        </Stack>
                            ...
                            <ListItem
                              ...
                              secondaryAction={
                                <IconButton
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    journey.deletePainPoint(stage.id, pp.id)
                                  }
                                >
                                  <DeleteIcon size={16} />
                                </IconButton>
                          ...
                          {stage.emotions.map((emotion) => {
                            ...
                            return (
                              <Chip
                                ...
                                label={config.label}
                                size="sm"
                                className="text-white [&_.MuiChip-icon]:text-white"
                                style={{ backgroundColor: config.color }}
                                onDelete={() =>
                                  journey.deleteEmotion(stage.id, emotion.id)
                                }
                              />
                            );
                          })}
                          ...
                          {stage.userQuotes.map((quote) => (
                            <ListItem
                              ...
                              secondaryAction={
                                <IconButton
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    journey.deleteUserQuote(stage.id, quote.id)
                                  }
                                >
                                  <DeleteIcon size={16} />
                                </IconButton>
                            ...
                            >
                              <ListItemText
                                primary={`"${quote.text}"`}
                                secondary={
                                  quote.source ? `— ${quote.source}` : undefined
                                }
                              />
                            </ListItem>
                          ))}
      ...
      <Dialog
        open={painPointDialogOpen}
        onClose={() => setPainPointDialogOpen(false)}
        size="sm"
        fullWidth
      >
      ...
      >
        ...
        <DialogContent>
          <Alert severity="info" className="mb-4">
            Paste user interview transcript. AI will extract pain points,
            emotions, and quotes automatically.
          </Alert>
          <TextField
            margin="dense"
            label="Transcript"
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography
            ...
            gutterBottom
          >
            Export includes all stages, touchpoints, pain points, emotions, and
            user quotes.
          </Typography>
          <Box className="mt-4">
            ...
            />
            <Chip
              label={`${journey.getQuoteCount()} Quotes`}
              size="sm"
              tone="success"
            />
          </Box>
