...
export const NodeGroup = memo<NodeProps<NodeGroupData>>(
  ({ id, data, selected }) => {
    ...
    return (
      <>
        <Paper
          ...
          className="w-full h-full min-w-[300px] min-h-[150px] border-[3px] p-4 relative cursor-move transition-all duration-200 hover:shadow-lg border-dashed"
          style={{
          ...
          >
            ...
            <Box display="flex" alignItems="center" gap={0.5}>
              <Tooltip title="Edit Label">
                <IconButton size="sm" onClick={() => setIsEditingLabel(true)}>
                  <EditIcon size={16} />
                </IconButton>
              </Tooltip>
              ...
              <Tooltip title="More Options">
                <IconButton size="sm" onClick={handleContextMenuOpen}>
                  <MoreIcon size={16} />
                </IconButton>
              </Tooltip>
            </Box>
            ...
            <Box>
              ...
              {data.children && (
                <Typography
                  ...
                  mt={0.5}
                >
                  Contains {data.children.length} node
              ...
              {data.tags && data.tags.length > 0 && (
                <Box display="flex" gap={0.5} mt={1} flexWrap="wrap">
                  {data.tags.map((tag, idx) => (
                    <Chip key={idx} label={tag} size="sm" variant="outlined" />
                  ))}
                </Box>
              )}
            </Box>
          ...
          {Object.entries(STATUS_CONFIG).map(([status, config]) => (
            ...
            >
              <Box display="flex" alignItems="center" gap={1}>
                <Box component="span" className="inline-flex text-base">
                  {config.icon}
                </Box>
