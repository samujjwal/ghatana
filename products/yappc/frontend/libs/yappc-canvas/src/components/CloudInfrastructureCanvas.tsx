...
export interface CloudInfrastructureCanvasProps {
  ...
  onExport?: (
    config: string,
    format: 'terraform' | 'cloudformation' | 'pulumi' | 'arm'
  ) => void;
 ...
 * Cloud Infrastructure Canvas Component
 */
export const CloudInfrastructureCanvas: React.FC<
  CloudInfrastructureCanvasProps
> = ({ onExport, onValidate }) => {
  const {
    // State
    infrastructureName,
  ...
  const [newResourceCategory, setNewResourceCategory] =
    useState<ResourceCategory>('compute');
  ...
  const [exportFormat, setExportFormat] = useState<
    'terraform' | 'cloudformation' | 'pulumi' | 'arm'
  >('terraform');
  ...
  const handleAddResource = () => {
    ...
    const config: Record<string, unknown> = newResourceConfig
      ? JSON.parse(newResourceConfig)
      : {};
          ...
          <div className="mb-4">
            ...
            <div className="space-y-4">
              {Object.entries(CATEGORIES).map(([category, config]) => {
                const categoryResources = getResourcesByCategory(
                  category as ResourceCategory
                );
                const resourceTypes = getResourceTypesForCategory(
                  selectedProvider,
                  category as ResourceCategory
                );
                  ...
                  <div key={category}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-gray-500">
                        {categoryResources.length} / {resourceTypes.length}{' '}
                        types
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      {resourceTypes.slice(0, 3).map((type) => (
                        <button
                          ...
                          className={`w-full px-3 py-2 text-left text-sm rounded-lg border-2 transition-colors ${
                            config.color
                          } hover:shadow-md`}
                        >
                          {type}
              ...
              <div className="flex justify-between text-sm">
                ...
                <span
                  className={`font-medium ${
                    complianceReport.compliantPercentage >= 80
                      ? 'text-green-600'
                      : complianceReport.compliantPercentage >= 60
                        ? 'text-yellow-600'
                        : 'text-red-600'
                  }`}
                >
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex items-center gap-3">
                                ...
                                <span
                                  className={`px-2 py-0.5 text-xs rounded ${providerConfig.color}`}
                                >
            ...
            {resources.length === 0 && (
              <div className="p-12 bg-blue-50 border border-blue-200 rounded-lg text-center">
                ...
                <div className="text-sm text-gray-500 mb-4">
                  Start building your cloud infrastructure by adding resources
                  from the palette
                </div>
                <button
                  onClick={() => setAddResourceDialogOpen(true)}
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                ...
                <span
                  className={`font-medium ${haAnalysis.hasMultiAZ ? 'text-green-600' : 'text-red-600'}`}
                >
              ...
              <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                ...
                <span className="font-medium">
                  {haAnalysis.redundancyScore}/10
                </span>
              </div>
              <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                <span className="text-sm text-gray-600">Backup</span>
                <span
                  className={`font-medium ${haAnalysis.hasBackup ? 'text-green-600' : 'text-red-600'}`}
                >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewResourceCategory(e.target.value as ResourceCategory)
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
                  {Object.entries(CATEGORIES).map(([category, config]) => (
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-gray-50 rounded-lg">
                <div className="text-sm text-gray-700">
                  ...
                  <div>
                    • {complianceReport.compliantPercentage}% compliance
                  </div>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(costEstimate.breakdown)
                    ...
                    .map(([category, cost]) => {
                      const percentage =
                        (cost / costEstimate.totalMonthlyCost) * 100;
                      return (
                        <div key={category}>
