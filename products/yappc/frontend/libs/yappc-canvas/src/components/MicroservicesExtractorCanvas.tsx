...
import {
  ...
  Cloud as CloudIcon,
  Building2 as ArchIcon,
  Brain as AIIcon,
  Download as DownloadIcon,
  Share2 as ShareIcon,
  ZoomIn as ZoomInIcon,
  CompareArrows as CouplingIcon,
  Layers as CohesionIcon,
} from 'lucide-react';
...
const EXTRACTION_STRATEGIES = [
  ...
  },
  {
    ...
    description: 'Central orchestrator coordinates services',
  },
  {
    value: 'big_bang',
    label: 'Big Bang Migration',
    description: 'Complete rewrite (high risk)',
  },
] as const;
 ...
 * Microservices Extractor Canvas Component
 */
export const MicroservicesExtractorCanvas: React.FC<
  MicroservicesExtractorCanvasProps
> = ({ onExport, onShare }) => {
  const {
    // State
    monolithName,
  ...
  const [createBoundaryDialogOpen, setCreateBoundaryDialogOpen] =
    useState(false);
  ...
  const [newBoundaryStrategy, setNewBoundaryStrategy] =
    useState<ExtractionStrategy>('strangler_fig');
  ...
  return (
    <Box className="h-full flex flex-col bg-[#f5f5f5]">
                    ...
                    {entitiesOfType.map((entity) => (
                      ...
                      >
                        ...
                        <ListItemText
                          primary={entity.name}
                          secondary={
                            entity.contextId ? 'Assigned' : 'Unassigned'
                          }
          ...
          <Stack spacing={2}>
            ...
            >
              {contexts.map((context) => {
                const contextEntities = entities.filter(
                  (e) => e.contextId === context.id
                );
                ...
                return (
                  <Box key={context.id}>
                    <Card
                      className="h-full border-blue-600"
                      style={{
                        border: selectedContextId === context.id ? 2 : 0,
                      }}
                    >
                      <CardContent>
                        <Stack spacing={1}>
                          ...
                          >
                            <CouplingIcon
                              className="text-base"
                              style={{ color: couplingConfig.color }}
                            />
                            <Typography
                              as="span"
            ...
            {contexts.length === 0 && (
              <Paper className="p-8">
                <Alert severity="info">
                  Add bounded contexts or use AI analysis to identify them
                  automatically
                </Alert>
              </Paper>
            )}
          ...
          <Stack spacing={3}>
                  ...
                  <Chip
                    label={
                      EXTRACTION_STRATEGIES.find(
                        (s) => s.value === recommendedStrategy.strategy
                      )?.label
                    }
                    tone="primary"
                    size="sm"
            ...
            <Box>
              <Box
                ...
                mb={2}
              >
                <Typography as="h6">Service Boundaries</Typography>
                <IconButton
                  size="sm"
                  tone="primary"
                    ...
                    <Card key={boundary.id} variant="outlined">
                      <CardContent className="pb-2">
                        <Stack spacing={1}>
                          ...
                          >
                            Contexts:{' '}
                            {boundaryContexts.map((c) => c.name).join(', ')}
                          </Typography>
                        </Stack>
                      </CardContent>
                    </Card>
          ...
          <Stack spacing={2} className="mt-2">
            ...
            <FormControl fullWidth>
              ...
              <Select
                value={newEntityType}
                onChange={(e) =>
                  setNewEntityType(e.target.value as typeof newEntityType)
                }
                label="Entity Type"
              >
      ...
      <Dialog
        open={createBoundaryDialogOpen}
        onClose={() => setCreateBoundaryDialogOpen(false)}
        size="sm"
        fullWidth
      >
          ...
          <Stack spacing={2} className="mt-2">
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) =>
                  setNewBoundaryStrategy(e.target.value as ExtractionStrategy)
                }
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) =>
                  setNewBoundaryContextIds(e.target.value as string[])
                }
        ...
        <DialogActions>
          <Button onClick={() => setCreateBoundaryDialogOpen(false)}>
            Cancel
          </Button>
          <Button
            ...
            variant="solid"
            disabled={
              !newBoundaryName.trim() || newBoundaryContextIds.length === 0
            }
          >
            Create Service
          </Button>
      ...
      <Dialog
        ...
        onClose={() => !aiAnalyzing && setAiAnalysisDialogOpen(false)}
        size="sm"
        fullWidth
      >
        ...
        <DialogContent>
          <Stack spacing={2} className="mt-2">
            <Alert severity="info">
              AI will analyze your monolith entities to identify natural bounded
              contexts based on:
              <ul>
        ...
        <DialogActions>
          <Button
            onClick={() => setAiAnalysisDialogOpen(false)}
