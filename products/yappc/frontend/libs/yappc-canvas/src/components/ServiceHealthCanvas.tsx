 ...
 */
const HealthStatusIcon: React.FC<{
  status: HealthStatus;
...
const MetricDisplay: React.FC<{ metric: Metric }> = ({ metric }) => {
  ...
  return (
    <Box className="mb-2">
      ...
      <Box className="w-full rounded-lg mt-1 overflow-hidden h-[4px] bg-[#e0e0e0]">
        <Box
          style={{
    ...
    <Box className="mb-4">
      <Box className="flex justify-between items-center mb-1">
        ...
        <Typography
          ...
          style={{ color: isBreached ? '#f44336' : '#4caf50' }}
        >
          {percentage.toFixed(2)}%
        </Typography>
      </Box>
      <Box className="flex gap-2 items-center">
        <Box className="flex-1 overflow-hidden h-[8px] bg-[#e0e0e0] rounded-2xl">
          <Box
            style={{
              width: `${percentage}%`,
              height: '100%',
...
export const ServiceHealthCanvas: React.FC<ServiceHealthCanvasProps> = ({
  ...
  const [circuitBreakerDialogOpen, setCircuitBreakerDialogOpen] =
    React.useState(false);
  const [incidentReportDialogOpen, setIncidentReportDialogOpen] =
    React.useState(false);
  ...
  return (
    <>
            ...
            <Collapse in={metricsPanelOpen}>
              <Box className="p-4 overflow-auto max-h-[300px]">
                {Array.from(healthData.values()).map((health) => (
                  <Box
                    key={health.nodeId}
                    className="mb-4 pb-4 border-gray-200 dark:border-gray-700 border-b"
                  >
                      ...
                      <Box className="flex gap-1">
                        <Tooltip title="Circuit Breaker">
                          <IconButton
                            size="sm"
                            color={
                              circuitBreakers.has(health.nodeId)
                                ? 'error'
                                : 'default'
                            }
                            onClick={() => {
                        ...
                        <Tooltip title="Incident Report">
                          <IconButton
                            size="sm"
                            onClick={() =>
                              handleCreateIncidentReport(health.nodeId)
                            }
                          >
                            <ReportIcon size={16} />
                          </IconButton>
      ...
      >
              ...
              {circuitBreakers.has(selectedNodeId) && (
                <Box className="mt-4">
                  <TextField
                    ...
                    type="number"
                    defaultValue={
                      circuitBreakers.get(selectedNodeId)?.threshold
                    }
                    className="mb-4"
                  />
                  <TextField
                  ...
                  <TextField
                    ...
                    defaultValue={
                      circuitBreakers.get(selectedNodeId)?.fallbackStrategy
                    }
                  >
        ...
        <DialogActions>
          <Button onClick={() => setCircuitBreakerDialogOpen(false)}>
            Close
          </Button>
          <Button
            variant="solid"
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setIncidentReportDialogOpen(false)}>
            Close
          </Button>
          <Button
            variant="solid"
