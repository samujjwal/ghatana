 ...
 * Zero-Trust Architecture Canvas Component
 */
export const ZeroTrustArchitectureCanvas: React.FC<
  ZeroTrustArchitectureCanvasProps
> = ({ onExport, onValidate }) => {
  const {
    // State
    architectureName,
  ...
  const [newZoneTrustLevel, setNewZoneTrustLevel] =
    useState<TrustLevel>('medium');

  const [newIdpName, setNewIdpName] = useState('');
  const [newIdpType, setNewIdpType] = useState<'saml' | 'oauth' | 'oidc'>(
    'oidc'
  );
  ...
  const [newPolicyAction, setNewPolicyAction] = useState<'allow' | 'deny'>(
    'deny'
  );

  const [exportFormat, setExportFormat] = useState<
    'terraform' | 'k8s' | 'diagram'
  >('terraform');
  ...
  const handleAddPolicy = () => {
    if (
      !newPolicyName.trim() ||
      !newPolicySource.trim() ||
      !newPolicyDestination.trim()
    )
      return;
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2">
              {Object.entries(ZONE_TYPES).map(([type, config]) => {
                const zonesOfType = securityZones.filter(
                  (z) => z.type === type
                );
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            <div
                              className={`text-xs mt-1 font-medium ${TRUST_LEVELS[zone.trustLevel].color}`}
                            >
            ...
            {policyRules.length === 0 && (
              <div className="p-8 bg-purple-50 border border-purple-200 rounded-lg text-center">
                ...
                <div className="text-sm text-gray-500 mt-1">
                  Add policies to control access between zones
                </div>
              </div>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                ...
                <span className="font-medium">
                  {coverage.zonesWithPolicies}/{coverage.totalZones}
                </span>
              </div>
              <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                <span className="text-sm text-gray-600">IdP Integration</span>
                <span
                  className={`font-medium ${coverage.hasIdentityProvider ? 'text-green-600' : 'text-red-600'}`}
                >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={newZoneType}
                  onChange={(e) =>
                    setNewZoneType(e.target.value as SecurityZoneType)
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
                  {Object.entries(ZONE_TYPES).map(([type, config]) => (
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewZoneTrustLevel(e.target.value as TrustLevel)
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={newIdpType}
                  onChange={(e) =>
                    setNewIdpType(e.target.value as 'saml' | 'oauth' | 'oidc')
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                >
