...
import {
  Bot as AIIcon,
  FileText as RequirementIcon,
  GitBranch as FlowIcon,
  Users as StakeholderIcon,
...
export function AIPromptNode({
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <Box className="px-4 py-2 flex items-center gap-2">
        <AIIcon size={16} />
...
export function DatabaseNode({
  ...
  const handleSaveSchema = useCallback(
    (tableName: string, fields: SchemaField[]) => {
...
export function ServiceNode({
  ...
  id,
  selected,
}: NodeProps<UIServiceNodeData>) {
  const theme = useTheme();
  const [showConfig, setShowConfig] = useState(false);
  const [config, setConfig] = useState<DeploymentConfig | undefined>(
    data.config
  );
  const borderColor = selected
...
export function APIEndpointNode({
  data,
  id,
  selected,
}: NodeProps<APIEndpointNodeData>) {
  const theme = useTheme();
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <Box
        className="text-[#fff] px-4 py-2 flex items-center gap-2"
        style={{
          backgroundColor:
            methodColors[apiConfig?.method || data.method] ||
...
export function UIScreenNode({
  ...
  return (
    ...
    >
      ...
      {/* Mini Preview */}
      <Box className="p-4 flex flex-col gap-1 bg-gray-100 dark:bg-gray-800 min-h-[80px]">
