...
import {
  HardDrive as SourceIcon,
  GitBranch as TransformIcon,
  CloudUpload as SinkIcon,
  Plus as AddIcon,
  X as CloseIcon,
...
const TRANSFORMATION_TYPES = [
  ...
  },
  {
    ...
    label: 'Aggregate',
    icon: '📊',
    description: 'Group by and aggregate (sum, avg, count)',
  },
  {
    value: 'join',
    label: 'Join',
...
export const DataPipelineCanvas: React.FC<DataPipelineCanvasProps> = ({
  ...
  const updateNodeConfig = useCallback(
    (nodeId: string, config: Partial<PipelineNode['config']>) => {
      setNodes((prev) =>
        prev.map((n) =>
...
load_${taskId}_task = PythonOperator(
    ...
    connections.forEach((conn) => {
      ...
      dependencies.push(
        `extract_${fromTask}_task >> transform_${toTask}_task` ||
          `transform_${fromTask}_task >> load_${toTask}_task`
      );
    });
...
if __name__ == "__main__":
  ...
  return (
    <Box className="flex h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <Paper
        elevation={3}
        className="transition-all duration-300 flex flex-col rounded-none"
        style={{ width: libraryExpanded ? 280 : 48 }}
      >
        <Box className="p-4 flex items-center justify-between">
          ...
          >
            <ExpandMoreIcon
              style={{
                transform: libraryExpanded ? 'rotate(180deg)' : 'rotate(90deg)',
              }}
            />
          </IconButton>
        </Box>
      ...
      <Box className="flex-1 flex flex-col p-4">
        ...
        <Paper variant="raised" className="p-4 mb-4 flex items-center gap-4">
          ...
          <TextField
            ...
            InputProps={{
              startAdornment: (
                <ScheduleIcon className="mr-2 text-gray-500 dark:text-gray-400" />
              ),
            }}
          />
          ...
          {nodes.length === 0 ? (
            <Box className="flex flex-col items-center justify-center h-full text-center">
              ...
              <Typography as="p" className="text-sm" color="text.secondary">
                Add sources, transformations, and sinks from the sidebar to
                build your pipeline
              </Typography>
            </Box>
          ) : (
            <Box className="flex flex-col gap-4">
              {nodes.map((pipelineNode, index) => (
                <Paper
                  ...
                  className="p-4 cursor-pointer border-blue-600 hover:bg-gray-100 hover:dark:bg-gray-800"
                  style={{ border: selectedNode === pipelineNode.id ? 2 : 0 }}
                >
                  <Box className="flex items-center gap-4">
                    {pipelineNode.type === 'source' && (
                      <SourceIcon tone="primary" />
                    )}
                    {pipelineNode.type === 'transform' && (
                      <TransformIcon tone="secondary" />
                    )}
                    {pipelineNode.type === 'sink' && (
                      <SinkIcon tone="success" />
                    )}
                    ...
                    <Box className="flex-1">
                      ...
                      <Typography
                        ...
                        color="text.secondary"
                      >
                        {pipelineNode.type.charAt(0).toUpperCase() +
                          pipelineNode.type.slice(1)}
                        {pipelineNode.config.sourceType &&
                          ` • ${pipelineNode.config.sourceType}`}
                        {pipelineNode.config.transformationType &&
                          ` • ${pipelineNode.config.transformationType}`}
                        {pipelineNode.config.sinkType &&
                          ` • ${pipelineNode.config.sinkType}`}
                      </Typography>
                    </Box>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          updateNodeConfig(currentNode.id, {
                            sourceType: e.target.value as DataSourceType,
                          })
                        }
                        label="Source Type"
                      >
                  ...
                  <Box>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          updateNodeConfig(currentNode.id, {
                            sinkType: e.target.value as SinkType,
                          })
                        }
                        label="Sink Type"
                      >
