...
export const InnovationLabCanvas: React.FC<InnovationLabCanvasProps> = ({
  ...
  const calculateROI = (proto: Prototype) => {
    ...
    const roi =
      ((proto.roi.expectedReturn - proto.roi.investment) /
        proto.roi.investment) *
      100;
    ...
    <Card key={prototype.id} className="mb-4">
        ...
        {prototype.abTestResults && (
          <Box className="mb-4 p-2 rounded bg-gray-100 dark:bg-gray-800">
            ...
            <Box className="flex gap-4 mt-1">
              <Chip
                label={`Variant A: ${prototype.abTestResults.variantA}%`}
                size="sm"
              />
              <Chip
                label={`Variant B: ${prototype.abTestResults.variantB}%`}
                size="sm"
              />
        ...
        {prototype.roi && (
          <Box className="mb-4 p-2 rounded bg-gray-100 dark:bg-gray-800">
            ...
            <Box className="flex gap-4 mt-1 flex-wrap">
              <Chip
                label={`Investment: $${prototype.roi.investment.toLocaleString()}`}
                size="sm"
              />
              <Chip
                label={`Return: $${prototype.roi.expectedReturn.toLocaleString()}`}
                size="sm"
              />
              <Chip
                label={`ROI: ${calculateROI(prototype)}%`}
                tone="success"
                size="sm"
              />
      ...
      <CardActions>
        <Button
          size="sm"
          startIcon={<Science />}
          onClick={() => onRunABTest?.(prototype.id)}
        >
          Run A/B Test
        </Button>
        ...
        <IconButton
          size="sm"
          onClick={() => onDeletePrototype?.(prototype.id)}
      ...
      >
        ...
        <DialogContent>
          <Box className="flex flex-col gap-4 mt-2">
            <TextField
              ...
              onChange={(e) =>
                setNewPrototype({ ...newPrototype, name: e.target.value })
              }
              fullWidth
              autoFocus
            />
        ...
        <DialogActions>
          ...
          <Button
            ...
            variant="solid"
            disabled={!newPrototype.name.trim()}
          >
            Create
          </Button>
        </DialogActions>
