...
const STRIDE_CATEGORIES = [
  {
    ...
    icon: <AccountIcon />,
    color: '#f44336',
    description: 'Illegitimate access to identity',
  },
  {
    id: 'tampering',
    label: 'Tampering',
    icon: <EditIcon />,
    color: '#ff9800',
    description: 'Malicious data modification',
  },
  {
    id: 'repudiation',
    label: 'Repudiation',
  ...
  },
  {
    ...
    icon: <LockIcon />,
    color: '#9c27b0',
    description: 'Unauthorized information access',
  },
  {
    id: 'denialOfService',
...
export const ThreatModelingCanvas: React.FC<ThreatModelingCanvasProps> = ({
  ...
  const [strideResults, setStrideResults] = useState<ReturnType<
    typeof analyzeSTRIDE
  > | null>(null);
  ...
  const getCategoryConfig = (category: ThreatCategory) => {
    return (
      STRIDE_CATEGORIES.find((c) => c.id === category) || STRIDE_CATEGORIES[0]
    );
  };
  ...
  return (
    <Box className="h-full flex flex-col bg-[#fafafa]">
          ...
          <Box className="min-w-0" style={{ flex: '1 1 0' }}>
            ...
            <Card className="mb-6">
              <CardContent>
                ...
                {threats.length === 0 ? (
                  <Alert severity="info">
                    No threats identified yet. Click "Add Threat" to start your
                    threat model.
                  </Alert>
                ) : (
                  <Stack spacing={2}>
                  ...
                  <Box className="flex flex-wrap gap-4">
                    {assets.map((asset) => {
                      ...
                      return (
                        ...
                        >
                          <Paper elevation={2} className="p-4 relative">
                            <IconButton
                              size="sm"
      ...
      <Dialog
        ...
        onClose={() => setAddThreatDialogOpen(false)}
        size="sm"
        fullWidth
      >
        ...
        <DialogContent>
          <Stack spacing={2} className="mt-2">
            ...
            <TextField
              ...
              onChange={(e) =>
                setNewThreat({ ...newThreat, affectedAsset: e.target.value })
              }
              fullWidth
            />
            <TextField
              ...
              onChange={(e) =>
                setNewThreat({ ...newThreat, attackVector: e.target.value })
              }
              fullWidth
          ...
          <Stack spacing={2} className="mt-2">
            ...
            <FormControl fullWidth>
              ...
              <Select
                value={newAsset.type}
                onChange={(e) =>
                  setNewAsset({ ...newAsset, type: e.target.value as unknown })
                }
                label="Asset Type"
              >
      ...
      <Dialog
        open={addMitigationDialogOpen}
        onClose={() => setAddMitigationDialogOpen(false)}
        size="sm"
        fullWidth
      >
        ...
        <DialogContent>
          <Stack spacing={2} className="mt-2">
            <TextField
              ...
              onChange={(e) =>
                setNewMitigation({ ...newMitigation, strategy: e.target.value })
              }
              fullWidth
              required
            ...
            <TextField
              ...
              onChange={(e) =>
                setNewMitigation({ ...newMitigation, owner: e.target.value })
              }
              fullWidth
        ...
        <DialogActions>
          <Button onClick={() => setAddMitigationDialogOpen(false)}>
            Cancel
          </Button>
          <Button
            onClick={handleAddMitigation}
            variant="solid"
            disabled={!newMitigation.strategy}
          >
            Add Mitigation
          </Button>
        </DialogActions>
      ...
      <Dialog
        open={strideAnalysisOpen}
        onClose={() => setStrideAnalysisOpen(false)}
        size="md"
        fullWidth
      >
        ...
        <DialogContent>
          {strideResults && (
            <Stack spacing={2}>
              <Alert
                severity={
                  strideResults.overallRisk === 'critical'
                    ? 'error'
                    : strideResults.overallRisk === 'high'
                      ? 'warning'
                      : 'info'
                }
              >
                <Typography as="p" className="text-lg font-medium">
                  Overall Risk Level:{' '}
                  <strong>{strideResults.overallRisk.toUpperCase()}</strong>
                </Typography>
                <Typography as="p" className="text-sm">
                  Average Risk Score:{' '}
                  {strideResults.averageRiskScore.toFixed(1)}/10
                </Typography>
              </Alert>

              <Typography as="h6">Category Breakdown</Typography>
              {Object.entries(strideResults.categoryBreakdown).map(
                ([category, data]) => {
                      ...
                      >
                        ...
                        <Stack direction="row" spacing={2}>
                          ...
                          <Chip
                            ...
                            size="sm"
                            color={
                              data.mitigatedCount === data.count
                                ? 'success'
                                : 'warning'
                            }
                          />
              ...
              {strideResults.unmitigatedThreats.length > 0 && (
                <>
                  <Typography as="h6" tone="danger">
                    Unmitigated Threats (
                    {strideResults.unmitigatedThreats.length})
                  </Typography>
                  <List>
