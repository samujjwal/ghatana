...
import {
  InnovationLabCanvas,
  Prototype,
} from '../components/InnovationLabCanvas';
...
describe('InnovationLabCanvas', () => {
  ...
  it('calls onPromoteToRoadmap when button is clicked', () => {
    ...
    render(
      <InnovationLabCanvas
        prototypes={mockPrototypes}
        onPromoteToRoadmap={onPromoteToRoadmap}
      />
    );
  ...
  it('calls onAddPrototype when creating a new prototype', async () => {
    ...
    render(
      <InnovationLabCanvas prototypes={[]} onAddPrototype={onAddPrototype} />
    );
    ...
    await waitFor(() => {
      ...
      expect(onAddPrototype).toHaveBeenCalledWith(
  ...
  it('displays empty state when no prototypes', () => {
    ...
    expect(
      screen.getByText('Create prototypes to test new ideas and innovations')
    ).toBeInTheDocument();
  });
