...
describe('SecurityAlerts', () => {
  ...
  describe('Vulnerability Display', () => {
    ...
    it('should expand/collapse node details on click', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText('Test Vulnerability')
  ...
  describe('Fix Dialog', () => {
    ...
    it('should show generating indicator', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Generating AI-powered fix/i)
        ).toBeInTheDocument();
      });
    });
    ...
    it('should call createPR on Create PR click', async () => {
      ...
      const createPRButton = await screen.findByRole('button', {
        name: /Create PR/i,
      });
