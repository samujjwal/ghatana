...
export const ReviewMode: React.FC<ReviewModeProps> = ({
  ...
  const handleSubmitDecision = useCallback(() => {
    ...
    if (onReviewSubmit) {
      onReviewSubmit(
        selectedDecision === 'approve' ? 'approved' : 'changes-requested',
        decisionComment
      );
    }
  ...
  const analysisResult = selectedFile
    ? getAnalysisForFile(selectedFile.id)
    : undefined;
      ...
      <Paper className="flex-1 p-4 overflow-y-auto">
        {selectedFile ? (
          <>
                      ...
                      <Stack spacing={1}>
                        <Box>
                          ...
                          <LinearProgress
                            variant="determinate"
                            value={Math.min(
                              (analysisResult.complexity.cyclomatic / 20) * 100,
                              100
                            )}
                            color={
                              analysisResult.complexity.cyclomatic > 15
                        ...
                        <Box>
                          <Typography as="p" className="text-sm">
                            Maintainability:{' '}
                            {analysisResult.complexity.maintainabilityIndex}/100
                          </Typography>
                          <LinearProgress
                            variant="determinate"
                            value={
                              analysisResult.complexity.maintainabilityIndex
                            }
                            color={
                              analysisResult.complexity.maintainabilityIndex <
                              50
            ...
            <Paper
              variant="outlined"
              className="mt-4 p-4 overflow-x-auto bg-[#f5f5f5]"
            >
              <pre
        ...
        <Stack spacing={2}>
          ...
          {unresolvedAnnotations.length > 0 && (
            <Alert severity="warning">
              <Typography as="p" className="text-sm font-medium">
                {unresolvedAnnotations.length} unresolved annotation
        ...
        <DialogContent>
          <Stack spacing={2} className="mt-2">
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) =>
                  setNewAnnotation({
                    ...newAnnotation,
                    severity: e.target.value as AnnotationSeverity,
                  })
                }
              >
            ...
            <TextField
              ...
              onChange={(e) =>
                setNewAnnotation({ ...newAnnotation, message: e.target.value })
              }
              fullWidth
              multiline
              rows={4}
          ...
          {selectedAnnotation && (
            <Box className="mb-4">
              <Alert severity={SEVERITY_COLORS[selectedAnnotation.severity]}>
                <Typography as="p" className="text-sm">
                  <strong>{selectedAnnotation.author}:</strong>{' '}
