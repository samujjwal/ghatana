...
export const MobileCanvas: React.FC<MobileCanvasProps> = ({
  ...
  const deviceFrame = useMemo(() => {
    if (
      platform === 'android' &&
      (device.startsWith('iphone') || device === 'iphone-14-pro')
    ) {
  ...
  const addComponent = useCallback(
    (componentType: MobileComponentType) => {
      const componentDef = MOBILE_COMPONENTS.find(
  ...
  return (
    <Box className="flex h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <Box className="flex-1 flex flex-col p-4">
        ...
        {/* Device Frame(s) */}
        <Box className="flex-1 flex justify-center items-center gap-8 overflow-auto">
          {/* iOS Frame */}
          {(platform === 'ios' || platform === 'both') && (
            ...
            >
              {/* Status Bar */}
              <Box className="flex items-center justify-center bg-[#f5f5f5] border-b border-solid border-b-[#e0e0e0] h-[deviceFrame.statusBarHeight]">
                <Typography
                  as="span"
              ...
              <Box className="overflow-auto bg-[#fafafa] h-[deviceFrame.height - deviceFrame.statusBarHeight]">
                {components.map((comp) => (
                  <Box
                    ...
                    className="relative p-2 border-b border-solid border-b-[#e0e0e0] cursor-pointer hover:bg-gray-100 hover:dark:bg-gray-800"
                    style={{
                      backgroundColor:
          ...
          {(platform === 'android' || platform === 'both') && (
            <Paper
              elevation={8}
              className="overflow-hidden relative rounded-[24px] bg-[#fff]"
              style={{
                width: DEVICE_FRAMES['pixel-7'].width,
                height: DEVICE_FRAMES['pixel-7'].height,
              }}
            >
              {/* Status Bar */}
              <Box
                className="flex items-center justify-center bg-[#212121]"
                style={{ height: DEVICE_FRAMES['pixel-7'].statusBarHeight }}
              >
                <Typography
                  as="span"
                  className="text-xs text-gray-500"
              ...
              {/* Screen Content */}
              <Box
                className="overflow-auto bg-[#fafafa]"
                style={{
                  height:
                    DEVICE_FRAMES['pixel-7'].height -
                    DEVICE_FRAMES['pixel-7'].statusBarHeight,
                }}
              >
                {components.map((comp) => (
                  <Box
        ...
        {components.length === 0 && (
          <Box className="absolute text-center top-[50%] left-[50%]">
            <Typography as="h6" color="text.secondary" gutterBottom>
              No Components Added
            </Typography>
            <Typography as="p" className="text-sm" color="text.secondary">
              Click on components in the sidebar to add them to your mobile
              screen
            </Typography>
          </Box>
        )}
