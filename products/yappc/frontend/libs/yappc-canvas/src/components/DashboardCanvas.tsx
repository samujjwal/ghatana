...
export const DashboardCanvas: React.FC<DashboardCanvasProps> = ({
  ...
  const [selectedNodeType, setSelectedNodeType] =
    useState<DashboardNodeType>('chart');
  ...
  const handleAddNode = useCallback(() => {
    ...
    onAddNode?.({
      ...
      data: {
        query:
          selectedNodeType === 'datasource' ? newNodeData.query : undefined,
        chartType:
          selectedNodeType === 'chart' ? newNodeData.chartType : undefined,
        filterType:
          selectedNodeType === 'filter' ? newNodeData.filterType : undefined,
      },
    });
  ...
  const renderNode = (node: DashboardNode) => (
    <Paper
      ...
      className="p-4 mb-2"
      style={{
        borderLeft: `4px solid ${node.type === 'datasource' ? '#3b82f6' : node.type === 'chart' ? '#10b981' : '#f59e0b'}`,
      }}
    >
      <Box className="flex items-start gap-2">
        ...
        <Box className="flex-1">
          ...
          <Box className="mt-2 flex gap-1 flex-wrap">
            ...
            {node.data.chartType && (
              <Chip
                icon={getChartIcon(node.data.chartType)}
                label={node.data.chartType}
                size="sm"
              />
            )}
            {node.data.filterType && (
              <Chip label={node.data.filterType} size="sm" />
            )}
          </Box>
        </Box>
        <IconButton
          ...
          <Box className="flex flex-col gap-4 mt-2">
            ...
            {selectedNodeType === 'datasource' && (
              <TextField
                ...
                onChange={(e) =>
                  setNewNodeData({ ...newNodeData, query: e.target.value })
                }
                fullWidth
                multiline
                rows={4}
