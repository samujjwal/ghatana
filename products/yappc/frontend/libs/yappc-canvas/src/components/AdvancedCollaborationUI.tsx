...
import { OperationalTransform } from '../hooks/useAdvancedCollaboration';
import type {
  ConflictResolution,
  VersionHistory,
  CollaborativeUser,
...
> = ({ open, onClose, conflicts, onResolveConflict, participants }) => {
    ...
    >
      ...
      <DialogContent>
        <Box className="mb-4 border-gray-200 dark:border-gray-700 border-b">
          ...
          >
            <Tab
              label={`Active Conflicts (${conflicts.filter((c) => c.resolution === 'manual').length})`}
            />
            <Tab
              label={`Resolved (${conflicts.filter((c) => c.resolution !== 'manual').length})`}
            />
                          ...
                          secondary={
                            <Box mt={1}>
                              <Typography
                                ...
                                gutterBottom
                              >
                                {conflict.operations.length} conflicting
                                operations:
                              </Typography>
                              {conflict.operations.map((op, index) => (
                                <Paper
                                  key={op.id}
                                  className="p-2 mb-2 bg-gray-50 dark:bg-gray-800"
                                >
                                  <Box
                                    display="flex"
                                    alignItems="center"
                                  ...
                                  >
                                    ...
                                    <Typography variant="body2">
                                      <strong>
                                        {getParticipantName(op.author)}
                                      </strong>{' '}
                                      {op.operation}d {op.target}
                                    </Typography>
                                    <Chip
                              ...
                              <Box display="flex" gap={1} mt={2}>
                                <Button
                                  ...
                                  startIcon={<MergeIcon />}
                                  onClick={() =>
                                    handleResolveConflict(conflict.conflictId)
                                  }
                                >
                                  Auto Merge
                                </Button>
            ...
            <Paper className="p-4">
              <FormControl fullWidth className="mb-4">
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setResolutionStrategy(e.target.value as unknown)
                  }
                >
              ...
              <Alert severity="info">
                This would apply the {resolutionStrategy} resolution to the
        ...
        <Box>
          {versions.map((version, index) => (
            <Box key={version.id} display="flex" mb={2}>
              ...
              <Box flex={1}>
                <Card
                  className="mb-4 border-blue-600 cursor-pointer"
                  style={{
                    border: selectedVersions.includes(version.id) ? 2 : 0,
                  }}
                ...
                >
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Version {version.version} • {version.changes.length}{' '}
                      operations
                    </Typography>
...
export const CollaborationMetricsDialog: React.FC<
  CollaborationMetricsDialogProps
> = ({
  open,
  onClose,
  metrics,
                    ...
                    <ListItemText
                      ...
                      secondary={
                        <Box display="flex" alignItems="center" gap={1}>
                          ...
                          <Typography variant="caption">
                            • Last seen{' '}
                            {formatDistanceToNow(participant.lastSeen)} ago
                          </Typography>
                        </Box>
                      }
                    />
