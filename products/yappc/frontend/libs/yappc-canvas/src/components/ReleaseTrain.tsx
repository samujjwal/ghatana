    ...
    <div className="w-full h-full p-6 space-y-6 bg-gray-50 overflow-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="border-l-4 border-orange-500 bg-orange-50">
          <CardContent className="p-4">
            ...
            <Typography
              variant="body1"
              className="font-bold mt-2 text-orange-700"
            >
        ...
        <Card
          className={`border-l-4 ${releaseRisk === 'critical' || releaseRisk === 'high' ? 'border-red-500 bg-red-50' : 'border-green-500 bg-green-50'}`}
        >
          <CardContent className="p-4">
            <Typography
              variant="caption"
            ...
            <Typography
              variant="h4"
              className={`font-bold mt-2 uppercase ${getRiskColor(releaseRisk)}`}
            >
              {releaseRisk}
            </Typography>
          </CardContent>
      ...
      <div className="space-y-6">
        {teams.map((team) => {
          ...
          const blockedFeatures = teamFeatures.filter(
            (f) => f.status === 'blocked'
          ).length;
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <Typography variant="caption" className="text-gray-600">
                      {teamFeatures.length} features • Velocity: {velocity}{' '}
                      points/week
                    </Typography>
                  </div>
                  {blockedFeatures > 0 && (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <span
                              className={`px-2 py-0.5 rounded text-xs font-semibold ml-2 ${getRiskColor(risk)}`}
                            >
                              {risk}
                            </span>
                          </div>
                          ...
                          <span
                            className={`inline-block px-2 py-1 rounded text-xs font-semibold border mb-2 ${getStatusColor(feature.status)}`}
                          >
                            {feature.status}
                          </span>
                          ...
                          <div className="space-y-1 text-xs text-gray-600">
                            <div>
                              Est: {feature.estimatedDays}d • Actual:{' '}
                              {feature.actualDays}d
                            </div>
                            ...
                            {readiness.filter((r) => r.passed).length > 0 && (
                              <div className="text-green-600">
                                ✓ {readiness.filter((r) => r.passed).length}/
                                {readiness.length} checks
                              </div>
                            )}
                          </div>
                        </div>
            ...
            {(
              ...
              return (
                <div key={status} className="text-center">
                  ...
                  <span
                    className={`inline-block px-3 py-1 rounded text-sm font-semibold border mt-2 ${getStatusColor(status)}`}
                  >
                    {status}
                  </span>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="mt-1">
                    <span
                      className={`inline-block px-3 py-1 rounded text-sm font-semibold border ${getStatusColor(featureDetails.status)}`}
                    >
                ...
                <div>
                  ...
                  <Typography
                    variant="body1"
                    className={`font-bold uppercase ${getRiskColor(calculateFeatureRisk(featureDetails.id))}`}
                  >
              ...
              {getFeatureBlockers(featureDetails.id).length > 0 && (
                <div>
                  <Typography
                    variant="body2"
                    className="font-semibold mb-2 text-red-600"
                  >
              ...
              {getFeatureDependencies(featureDetails.id).length > 0 && (
                <div>
                  <Typography variant="body2" className="font-semibold mb-2">
                    Dependencies (
                    {getFeatureDependencies(featureDetails.id).length})
                  </Typography>
                  <div className="space-y-2">
                    {getFeatureDependencies(featureDetails.id).map((dep) => {
                      const depFeature = getFeatures().find(
                        (f) => f.id === dep.dependsOnFeatureId
                      );
                      return (
                        <div
                          key={dep.id}
        ...
        <DialogContent>
          <div className="space-y-4">
            <div className="p-4 bg-orange-50 rounded border border-orange-200">
              ...
              <Typography variant="body2" className="text-gray-600 mt-1">
                This is the longest chain of dependent features that determines
                the minimum release timeline.
              </Typography>
            </div>
                  ...
                  <div key={featureId} className="relative">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-white rounded border-2 border-orange-300">
                      ...
                      <div className="flex-1">
                        ...
                        <Typography variant="caption" className="text-gray-600">
                          Team:{' '}
                          {teams.find((t) => t.id === feature.teamId)?.name} •
                          Status: {feature.status} • {feature.estimatedDays}{' '}
                          days
                        </Typography>
                      </div>
                      <span
                        className={`px-2 py-1 rounded text-xs font-semibold border ${getStatusColor(feature.status)}`}
                      >
                        {feature.status}
                      </span>
                    </div>
      ...
      <Dialog
        open={showReleaseReadiness}
        onClose={() => setShowReleaseReadiness(false)}
        maxWidth="lg"
      >
          ...
          <div className="space-y-4">
            <div
              className={`p-4 rounded border-2 ${
                releaseRisk === 'low' || releaseRisk === 'medium'
                ...
                {(
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`inline-block px-2 py-1 rounded text-xs font-semibold border mt-1 ${getStatusColor(status)}`}
                      >
                        {status}
                      </span>
                    </div>
                ...
                <div className="space-y-2">
                  {getFeaturesByStatus('blocked').map((feature) => (
                    ...
                    >
                      ...
                      <Typography variant="caption" className="text-gray-600">
                        Team: {teams.find((t) => t.id === feature.teamId)?.name}{' '}
                        • {getFeatureBlockers(feature.id).length} blockers
                      </Typography>
                    </div>
                  ))}
                </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {teams.map((team) => {
                  const teamFeatures = getFeaturesByTeam(team.id);
                  const completedFeatures = teamFeatures.filter(
                    (f) => f.status === 'done'
                  ).length;
