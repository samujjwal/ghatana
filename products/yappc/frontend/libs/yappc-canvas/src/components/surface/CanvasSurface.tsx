      ...
      (elementId: string, event: React.MouseEvent) => {
        ...
        if (isMultiSelect) {
          if (isSelected) {
            ...
            newSelectedIds = selection.selectedIds.filter(
              (id) => id !== elementId
            );
          } else {
            // Add to selection
      ...
      (element: CanvasElement) => {
        ...
        const commonProps = {
          ...
          style: {
            ...
            border: isSelected
              ? `2px solid ${surfaceTheme.selection}`
              : '1px solid #ddd',
          },
        };
        ...
        switch (element.type) {
          case 'node':
            ...
            return (
              ...
              >
                <div className="node-content">{nodeLabel}</div>
                <div
                  id={`element-${element.id}-description`}
                  className="sr-only"
                >
          ...
          case 'edge':
            const edgeSourceId = isCanvasEdge(element)
              ? element.sourceId
              : 'unknown';
            const edgeTargetId = isCanvasEdge(element)
              ? element.targetId
              : 'unknown';
            const sourceEl = isCanvasEdge(element)
              ? elementMap.get(element.sourceId)
              : undefined;
            const targetEl = isCanvasEdge(element)
              ? elementMap.get(element.targetId)
              : undefined;
            ...
            const errorRate =
              typeof edgeCustom?.errorRate === 'number'
                ? edgeCustom.errorRate
                : 0;
            const throughput =
              typeof edgeCustom?.throughput === 'number'
                ? edgeCustom.throughput
                : 0;
          ...
          case 'group':
            ...
            const metadataLabel =
              typeof customMetadata?.label === 'string'
                ? customMetadata.label
                : undefined;
            const groupLabel =
              metadataLabel && metadataLabel.trim().length > 0
                ? metadataLabel
                : 'Group';
            const childCount = isCanvasGroup(element)
