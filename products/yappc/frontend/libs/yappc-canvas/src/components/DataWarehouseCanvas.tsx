...
import {
  ...
  BarChart3 as FactIcon,
  Tag as DimensionIcon,
  Code as CodeIcon,
  Check as CheckIcon,
...
export const DataWarehouseCanvas: React.FC<DataWarehouseCanvasProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'tables' | 'relationships' | 'etl' | 'validation'
  >('tables');
  ...
  const [exportFormat, setExportFormat] = useState<
    'postgres' | 'mysql' | 'snowflake' | 'bigquery' | 'mermaid'
  >('postgres');
    ...
    <Box className="p-6">
      ...
      {activeTab === 'tables' && (
        <Paper className="p-4">
              ...
              <Paper key={table.id} variant="outlined" className="p-4">
                ...
                >
                  <Stack direction="row" spacing={1} alignItems="center">
                    {table.type === 'fact' ? (
                      <FactIcon tone="primary" />
                    ) : (
                      <DimensionIcon tone="secondary" />
                    )}
                    <Typography
                      as="p"
          ...
          {tables.length === 0 && (
            <Alert severity="info">
              No tables defined yet. Click "Add Table" to create your first
              table.
            </Alert>
          )}
        </Paper>
        ...
        <Paper className="p-4">
          ...
          <List>
            {relationships.map((rel, idx) => {
              ...
              const toCol = toTable?.columns.find(
                (c) => c.id === rel.toColumnId
              );
          ...
          {relationships.length === 0 && (
            <Alert severity="info">
              No relationships defined yet. Click "Add Relationship" to create
      ...
      {activeTab === 'etl' && (
        <Paper className="p-4">
                ...
                <React.Fragment key={mapping.id}>
                  <ListItem
                    secondaryAction={
                      <IconButton
                        edge="end"
                        onClick={() => deleteETLMapping(mapping.id)}
                      >
                        <DeleteIcon />
                      </IconButton>
                    }
          ...
          {etlMappings.length === 0 && (
            <Alert severity="info">
              No ETL mappings defined yet. Click "Add ETL Mapping" to configure
              data sources.
            </Alert>
          )}
        </Paper>
          ...
          <Stack spacing={2} className="mt-2">
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setNewTableForm({
                      ...newTableForm,
                      scdType: e.target.value as SCDType,
                    })
                  }
                >
                  {SCD_TYPES.map((type) => (
            ...
            {newTableForm.type === 'fact' && (
              <TextField
                ...
                onChange={(e) =>
                  setNewTableForm({ ...newTableForm, grain: e.target.value })
                }
                fullWidth
                multiline
                rows={2}
        ...
        <DialogContent>
          <Stack spacing={2} className="mt-2">
            ...
            <TextField
              ...
              onChange={(e) =>
                setNewETLForm({ ...newETLForm, sourceName: e.target.value })
              }
              fullWidth
            />
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) =>
                  setNewETLForm({
                    ...newETLForm,
                    loadStrategy: e.target.value as
                      | 'full'
                      | 'incremental'
                      | 'upsert',
                  })
