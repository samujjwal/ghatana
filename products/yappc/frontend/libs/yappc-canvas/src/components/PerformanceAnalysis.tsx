    ...
    <div className="w-full h-full p-6 space-y-6 bg-gray-50 overflow-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {services.map((service) => {
          ...
          const errorRate =
            metrics.find((m) => m.type === 'error_rate')?.value || 0;
          const throughput =
            metrics.find((m) => m.type === 'throughput')?.value || 0;
              ...
              <CardContent className="p-4 space-y-3">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-1">
                    <div
                      className={`p-2 rounded border text-center ${getLatencyColor(p50, slo.p50Target)}`}
                    >
                      <div className="text-xs font-semibold">P50</div>
                      <div className="text-sm font-bold">{p50.toFixed(0)}</div>
                    </div>
                    <div
                      className={`p-2 rounded border text-center ${getLatencyColor(p95, slo.p95Target)}`}
                    >
                      <div className="text-xs font-semibold">P95</div>
                      <div className="text-sm font-bold">{p95.toFixed(0)}</div>
                    </div>
                    <div
                      className={`p-2 rounded border text-center ${getLatencyColor(p99, slo.p99Target)}`}
                    >
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <Typography
                      variant="caption"
                      className={
                        compliance >= 99 ? 'text-green-600' : 'text-red-600'
                      }
                    >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <span
                      className={`ml-1 font-semibold ${errorRate > 1 ? 'text-red-600' : 'text-green-600'}`}
                    >
                      {errorRate.toFixed(2)}%
                    </span>
                  </div>
          ...
          <CardContent>
            <div className="flex items-center gap-3 overflow-x-auto pb-2">
              {criticalPath.map((serviceId, index) => {
                ...
                const latency =
                  metrics.find((m) => m.type === 'latency_p95')?.value || 0;
              ...
              <Typography variant="caption" className="text-orange-800">
                ...
                {criticalPath
                  .reduce((sum, serviceId) => {
                    ...
                    return (
                      sum +
                      (metrics.find((m) => m.type === 'latency_p95')?.value ||
                        0)
                    );
                  }, 0)
                  .toFixed(0)}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-5 gap-2">
                  {Object.entries(
                    getLatencyDistribution(serviceDetails.id, selectedTimeRange)
              ...
              <div>
                ...
                <div className="space-y-2">
                  {(['p50', 'p95', 'p99'] as const).map((percentile) => {
                    ...
                    const actual = calculatePercentile(
                      serviceDetails.id,
                      selectedTimeRange,
                      percentile === 'p50' ? 50 : percentile === 'p95' ? 95 : 99
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <span
                            className={`text-sm font-bold ${ratio > 1 ? 'text-red-600' : 'text-green-600'}`}
                          >
      ...
      <Dialog
        open={showBottleneckAnalysis}
        onClose={() => setShowBottleneckAnalysis(false)}
        maxWidth="lg"
      >
              ...
              <div className="space-y-3">
                {bottlenecks.map((bottleneck) => {
                  const service = services.find(
                    (s) => s.id === bottleneck.serviceId
                  );
                  return (
                    <div
                      key={bottleneck.id}
                      className="p-4 bg-red-50 rounded border-2 border-red-200"
                    >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                            <div>
                              ...
                              <span
                                className={`ml-1 font-bold ${getPriorityColor(bottleneck.severity)}`}
                              >
        ...
        <DialogActions>
          <Button onClick={() => setShowBottleneckAnalysis(false)}>
            Close
          </Button>
        </DialogActions>
      </Dialog>
              ...
              <div className="space-y-3">
                {recommendations.map((rec) => {
                  ...
                  return (
                    <div
                      key={rec.id}
                      className="p-4 bg-white rounded border-2 border-gray-200"
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span
                          className={`text-xs font-bold uppercase ${getPriorityColor(rec.priority)}`}
                        >
                          {rec.priority}
                        </span>
                      </div>
      ...
      <Dialog
        open={showQueryAnalysis}
        onClose={() => setShowQueryAnalysis(false)}
        maxWidth="lg"
      >
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <span className="ml-1 font-semibold">
                            {stats.p95ExecutionTime.toFixed(0)}ms
                          </span>
                        </div>
                        <div>
