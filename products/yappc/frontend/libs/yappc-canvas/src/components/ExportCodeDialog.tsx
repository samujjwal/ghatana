...
export const ExportCodeDialog: React.FC<ExportCodeDialogProps> = ({
  ...
  const [exportResult, setExportResult] = useState<CodeExportResult | null>(
    null
  );
      ...
      <DialogContent>
        <Box className="mt-4 flex flex-col gap-4">
          <Typography variant="body2" color="text.secondary">
            Export code and configuration from {nodeCount} canvas node
            {nodeCount !== 1 ? 's' : ''}.
          </Typography>
          ...
          <Box>
            ...
            <Typography
              variant="caption"
              display="block"
              className="ml-8 text-gray-500 dark:text-gray-400"
            >
          ...
          {exportResult && exportResult.success && (
            <Alert severity="success" data-testid="export-success">
              Successfully exported {exportResult.files.length} file
              {exportResult.files.length !== 1 ? 's' : ''}
              {exportResult.runbooks && exportResult.runbooks.length > 0 && (
                <>
                  {' '}
                  and {exportResult.runbooks.length} runbook
                  {exportResult.runbooks.length !== 1 ? 's' : ''}
                </>
              )}
            </Alert>
          )}
