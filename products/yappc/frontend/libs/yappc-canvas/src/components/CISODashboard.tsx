...
export const CISODashboard: React.FC<CISODashboardProps> = ({
  ...
  const cves = selectedSeverity
    ? getCVEsBySeverity(selectedSeverity)
    : getCVEs();
  const incidents = selectedIncidentStatus
    ? getIncidentsBystatus(selectedIncidentStatus)
    : getIncidents();
  ...
  const systemDetails = selectedSystem
    ? getSystemRiskDetails(selectedSystem)
    : null;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className={`border-l-4 ${getRiskColor(kpis.overallRisk)}`}>
          <CardContent className="p-6">
            ...
            <Typography
              variant="h3"
              className={`font-bold mt-2 ${getRiskTextColor(kpis.overallRisk)}`}
            >
            ...
            <div className="mt-2 space-y-1">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">
                  {kpis.criticalVulnerabilities}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-orange-600">High:</span>
                <span className="font-semibold">
                  {kpis.highVulnerabilities}
                </span>
              </div>
            </div>
          </CardContent>
        ...
        <Card className={`border-l-4 ${getRiskColor(kpis.overallRisk)}`}>
          <CardContent className="p-6">
            ...
            <Typography
              variant="h3"
              className={`font-bold mt-2 uppercase ${getRiskTextColor(kpis.overallRisk)}`}
            >
              {kpis.overallRisk}
            </Typography>
            <div className="mt-2">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-gray-50 rounded">
              ...
              <Typography
                variant="h4"
                className={`font-bold mt-2 ${trends.vulnerabilityChange >= 0 ? 'text-red-600' : 'text-green-600'}`}
              >
                {trends.vulnerabilityChange >= 0 ? '+' : ''}
                {trends.vulnerabilityChange}%
              </Typography>
            ...
            <div className="p-4 bg-gray-50 rounded">
              ...
              <Typography
                variant="h4"
                className={`font-bold mt-2 ${trends.incidentChange >= 0 ? 'text-red-600' : 'text-green-600'}`}
              >
                {trends.incidentChange >= 0 ? '+' : ''}
                {trends.incidentChange}%
              </Typography>
              <Typography variant="caption" className="text-gray-500">
                vs previous {selectedTrendPeriod}
              </Typography>
            ...
            <div className="p-4 bg-gray-50 rounded">
              ...
              <Typography
                variant="h4"
                className={`font-bold mt-2 ${trends.complianceChange >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
                {trends.complianceChange >= 0 ? '+' : ''}
                {trends.complianceChange}%
              </Typography>
              <Typography variant="caption" className="text-gray-500">
                vs previous {selectedTrendPeriod}
              </Typography>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <Typography
                        variant="caption"
                        className={`uppercase font-bold ${getRiskTextColor(system.riskLevel)}`}
                      >
                        {system.riskLevel}
                      </Typography>
                      <Typography
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-semibold ${getSeverityColor(cve.severity)}`}
                        >
                          {cve.severity}
                        </span>
                      </div>
                      ...
                      <Typography variant="caption" className="text-gray-500">
                        CVSS: {cve.cvssScore} | Affected:{' '}
                        {cve.affectedSystems.length} systems
                      </Typography>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex gap-2">
              {(
                ...
                <Button
                  ...
                  onClick={() =>
                    setSelectedIncidentStatus(
                      selectedIncidentStatus === status ? null : status
                    )
                  }
                  className={
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`px-2 py-1 rounded text-xs font-semibold border ${getIncidentStatusColor(incident.status)}`}
                      >
                        {incident.status}
                      </span>
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Typography
                    variant="body1"
                    className={`font-bold ${getRiskTextColor(systemDetails.riskLevel)}`}
                  >
                  ...
                  {systemDetails.vulnerabilities.map((vuln) => (
                    <div key={vuln.cveId} className="p-2 bg-gray-50 rounded">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-semibold ${getSeverityColor(vuln.severity)}`}
                        >
                          {vuln.severity}
                        </span>
                      </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded text-xs font-semibold ${getSeverityColor(cveDetails.severity)}`}
                  >
                    {cveDetails.severity}
                  </span>
                </div>
              ...
              {cveDetails.mitigationSteps &&
                cveDetails.mitigationSteps.length > 0 && (
                  <div>
                    <Typography variant="body2" className="font-semibold mb-2">
                      Mitigation Steps
                    </Typography>
