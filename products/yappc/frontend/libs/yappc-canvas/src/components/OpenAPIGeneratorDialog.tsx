 ...
 * OpenAPIGeneratorDialog component
 */
export function OpenAPIGeneratorDialog({
  open,
  onClose,
  apiNodes,
}: OpenAPIGeneratorDialogProps) {
  // State
  const [title, setTitle] = useState('My API');
  const [version, setVersion] = useState('1.0.0');
  const [description, setDescription] = useState(
    'API documentation generated from YAPPC canvas'
  );
          ...
          <Box display="flex" flexDirection="column" gap={2}>
            ...
            <Paper variant="outlined" className="p-4">
              ...
              <Box display="flex" gap={1} flexWrap="wrap" mt={1}>
                {tags.length > 0 ? (
                  tags.map((tag) => <Chip key={tag} label={tag} size="sm" />)
                ) : (
                  <Typography
                    as="span"
                    className="text-xs text-gray-500"
            ...
            {spec.components?.securitySchemes &&
              Object.keys(spec.components.securitySchemes).length > 0 && (
                <Paper variant="outlined" className="p-4">
                  ...
                  <Box display="flex" gap={1} flexWrap="wrap" mt={1}>
                    {Object.keys(spec.components.securitySchemes).map(
                      (scheme) => (
                        <Chip
                          key={scheme}
                          label={scheme}
