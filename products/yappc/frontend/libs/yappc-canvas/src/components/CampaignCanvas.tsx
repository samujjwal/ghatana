...
export const CampaignCanvas: React.FC<CampaignCanvasProps> = ({
  ...
  const renderNode = (node: CampaignNode) => (
    <Paper
      ...
      className="p-4 mb-4"
      style={{
        borderLeft: `4px solid ${node.channel === 'email' ? '#3b82f6' : node.channel === 'social' ? '#10b981' : '#f59e0b'}`,
      }}
    >
      <Box className="flex items-start gap-2">
        ...
        <Box className="flex-1">
          ...
          <Box className="mt-2 flex gap-2 flex-wrap">
            ...
            {node.kpis.impressions && (
              <Chip label={`${node.kpis.impressions} impressions`} size="sm" />
            )}
            {node.kpis.clicks && (
              <Chip label={`${node.kpis.clicks} clicks`} size="sm" />
            )}
            {node.kpis.conversions && (
              <Chip
                label={`${node.kpis.conversions} conversions`}
                size="sm"
                tone="success"
              />
            )}
