...
import {
  ...
  ArrowUpDown as InteractionIcon,
...
export const ServiceBlueprintCanvas: React.FC<ServiceBlueprintCanvasProps> = ({
  ...
  const [connectionType, setConnectionType] = useState<'flow' | 'support'>(
    'flow'
  );
          ...
          <List className="p-2">
            <ListItem disablePadding>
              <ListItemButton onClick={() => setNodeDialogOpen(true)}>
                ...
                <ListItemText
                  primary="Add Process Node"
                  secondary="Activity or step"
                />
              </ListItemButton>
            </ListItem>
            ...
            <ListItem disablePadding>
              <ListItemButton onClick={() => setTouchpointDialogOpen(true)}>
                ...
                <ListItemText
                  ...
                  secondary="Customer interaction"
                />
              </ListItemButton>
            </ListItem>
              ...
              <Box className="w-full">
                ...
                {LANE_CONFIGS.map((config) => (
                  <Chip
                    ...
                    label={config.label}
                    size="sm"
                    className="m-1 text-white [&_.MuiChip-icon]:text-white"
      ...
      <Box className="flex-1 flex flex-col">
        ...
        >
          <Box className="flex gap-4 items-center flex-wrap">
            ...
            <Box>
              <Chip
                label={`${blueprint.getNodeCount()} Nodes`}
                size="sm"
                className="mr-2"
              />
              <Chip
                label={`${blueprint.getConnectionCount()} Connections`}
                size="sm"
                className="mr-2"
              />
              <Chip
                label={`${blueprint.getTouchpointCount()} Touchpoints`}
                size="sm"
                tone="success"
              />
            </Box>
          ...
          {blueprint.lanes.length === 0 ? (
            <Alert severity="info" className="mt-8 max-w-[600px] mx-auto">
              ...
              <Typography as="p" className="text-sm">
                Service blueprints map the entire service ecosystem across 4
                layers: Customer Actions, Frontstage (visible), Backstage
                (internal), and Support Processes.
              </Typography>
            </Alert>
          ) : (
            <Box>
              ...
              {blueprint.lanes.map((lane, laneIndex) => {
                ...
                return (
                  <Card key={lane.id} elevation={2} className="mb-4">
                    <CardContent>
                      {/* Lane Header */}
                      <Box className="flex items-center mb-4">
                        <Box
                          className="flex items-center px-4 py-2 rounded mr-4 text-white"
                          style={{
                      ...
                      {lane.nodes.length === 0 ? (
                        <Alert severity="info" variant="outlined">
                          No processes in this layer. Click "Add Process Node"
                          to start.
                        </Alert>
                      ) : (
                        <Box className="grid gap-4">
                          {lane.nodes.map((node) => (
                            <Box key={node.id}>
                              <Paper
                                variant="raised"
                                className="p-4 border-[2px] relative hover:shadow-lg"
                                style={{
                                  borderColor: config.color,
                                  gridTemplateColumns: {
                                    xs: '1fr',
                  ...
                  <CardContent>
                    ...
                    <List dense>
                      {blueprint.connections.map((conn) => {
                        const fromNode = getAllNodes().find(
                          (n) => n.id === conn.from
                        );
                        const toNode = getAllNodes().find(
                          (n) => n.id === conn.to
                        );
                        return (
                          <ListItem
                            ...
                            secondaryAction={
                              <IconButton
                                ...
                                size="sm"
                                onClick={() =>
                                  blueprint.deleteConnection(conn.id)
                                }
                              >
                                <DeleteIcon size={16} />
                              </IconButton>
      ...
      >
        ...
        <DialogContent>
          ...
          <Box className="mt-4">
            ...
            <Chip
              label={`${blueprint.lanes.length} Lanes`}
              size="sm"
              className="mr-2"
            />
            <Chip
