...
export class ArtifactRegistry {
  ...
  private registry: Map<ArtifactKind, Map<SemanticVersion, RegistryEntry>> =
    new Map();
  ...
  register(
    ...
    if (this.sealed && source === 'core') {
      console.warn(
        `Registry is sealed, cannot register core artifact: ${kind}`
      );
      return false;
    }
  ...
  query(options: QueryOptions = {}): ArtifactContract[] {
    ...
    for (const kind of candidates) {
      const contract = options.latestOnly ? this.get(kind) : this.get(kind); // NOTE: handle multiple versions
      ...
      if (options.tags && options.tags.length > 0) {
        const hasTag = options.tags.some((t) =>
          contract.identity.tags.includes(t)
        );
        if (!hasTag) continue;
      }
      ...
      if (options.search) {
        ...
        const nameMatch = contract.identity.name
          .toLowerCase()
