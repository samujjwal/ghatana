...
import { CreateShareLinkRequest } from '../schemas/export-schemas';
import type {
  ExportOptions,
  ExportResult,
  ExportFormat,
    ...
    async (
      ...
      try {
        ...
        setExportHistory((prev) =>
          [...successfulResults, ...prev].slice(0, 10)
        );
...
export const useShareLinks = ({
  ...
  const updateShareLink = useCallback(
    async (
      linkId: string,
      configUpdates: Partial<ShareLinkConfig>
    ): Promise<ShareLink> => {
  ...
  const validateShareLink = useCallback(
    async (token: string): Promise<ShareLink | null> => {
      ...
      if (
        ...
        new Date() > new Date(link.config.expiresAt)
      ) {
            ...
            const suspiciousPatterns = [
              {
                pattern: /<script/i,
                type: 'high' as const,
                message: 'Script tag detected in node label',
              },
              {
                pattern: /javascript:/i,
                type: 'high' as const,
                message: 'JavaScript URL detected in node',
              },
              {
                pattern: /on\w+\s*=/i,
                type: 'medium' as const,
                message: 'Event handler detected in node',
              },
            ];
      ...
      try {
        ...
        const patterns = [
          ...
          },
          {
            ...
            type: 'high' as const,
            message: 'JavaScript URL found',
          },
          {
            pattern: /vbscript:/gi,
            type: 'high' as const,
            message: 'VBScript URL found',
          },
          {
            pattern: /on\w+\s*=/gi,
            type: 'medium' as const,
            message: 'Event handler attribute found',
          },
          {
            pattern: /eval\s*\(/gi,
            type: 'high' as const,
            message: 'eval() function found',
          },
          {
            pattern: /innerHTML/gi,
            type: 'medium' as const,
            message: 'innerHTML usage found',
          },
        ];
