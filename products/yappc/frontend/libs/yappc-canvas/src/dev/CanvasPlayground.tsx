...
export const CanvasPlayground: React.FC<CanvasPlaygroundProps> = ({
  ...
  const insertNode = useCallback(
    (kind: string, _overrides?: Partial<UniversalNode>) => {
      if (onInsertNode) {
        onInsertNode(kind);
        return;
      }
      ...
      alert(
        `Dev mode: Would insert ${kind} node. Attach playground inside a CanvasProvider to enable.`
      );
    },
    [onInsertNode]
  );
  ...
  const insertSketch = useCallback(
    () => insertNode('drawing:sketch'),
    [insertNode]
  );
  const insertDrawing = useCallback(
    () => insertNode('drawing:freehand'),
    [insertNode]
  );
  const insertDiagram = useCallback(
    () => insertNode('diagram:flowchart'),
    [insertNode]
  );
  ...
  const clearCanvas = useCallback(() => {
    ...
    alert(
      'Dev mode: Would clear canvas. Attach playground inside a CanvasProvider to enable.'
    );
  }, [onClearCanvas]);
  ...
  const applyPredictive = useCallback(() => {
    ...
    alert(
      'Dev mode: Would apply predictive layout. Attach playground inside a CanvasProvider to enable.'
    );
  }, [onRunPredictiveLayout]);
  ...
  const runCodegen = useCallback(async () => {
    ...
    alert(
      'Dev mode: Would run codegen. Attach playground inside a CanvasProvider to enable.'
    );
  }, [onRunCodegen]);
    ...
    <Box className="p-2 flex flex-col gap-2 rounded overflow-y-auto max-w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow max-h-[calc(100vh - 120px)]">
      ...
      <Stack direction="row" spacing={0.5} flexWrap="wrap" alignItems="center">
        <Typography
          ...
          className="w-full mb-1 text-gray-500 dark:text-gray-400"
        >
          Insert:
        </Typography>
        <Button
      ...
      <Stack direction="row" spacing={0.5} flexWrap="wrap" alignItems="center">
        ...
        <Button
          ...
          aria-label="Apply Predictive Layout"
        >
          Layout
        </Button>
        <Button
        ...
        <Button
          ...
          onClick={zoomInSemantic}
          aria-label="Semantic Zoom In"
        >
          Zoom+
        </Button>
        <Button
          ...
          onClick={zoomOutSemantic}
          aria-label="Semantic Zoom Out"
        >
          Zoom-
        </Button>
      </Stack>
      ...
      <Stack
        ...
        alignItems="center"
        className="pt-1 border-gray-200 dark:border-gray-700 border-t"
      >
        <Button
          size="sm"
          variant="outlined"
        ...
        <Button
          ...
          onClick={() => {
            ...
            else alert('Dev mode: Resync not available');
          }}
          aria-label="Resync Canvas"
        >
          Resync
        </Button>
      </Stack>
