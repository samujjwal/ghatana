...
describe('LifecycleStateMachine', () => {
  ...
  describe('Transition Validation', () => {
    ...
    it('should not allow undefined transitions', () => {
      ...
      expect(
        validation.errors.some((e) => e.code === 'TRANSITION_NOT_DEFINED')
      ).toBe(true);
    });
    ...
    it('should validate exit criteria when enforced', () => {
      ...
      expect(validation.valid).toBe(false);
      expect(
        validation.errors.some((e) => e.code === 'EXIT_CRITERION_NOT_MET')
      ).toBe(true);
    });
  ...
  describe('Transitions', () => {
    ...
    it('should add warning for backward transitions', async () => {
      ...
      expect(
        result.warnings?.some((w) => w.code === 'BACKWARD_TRANSITION')
      ).toBe(true);
    });
  ...
  describe('Default Transitions', () => {
    it('should generate forward transitions for all stages', () => {
      ...
      const forwardTransitions = defaultTransitions.filter(
        (t) => t.type === 'forward'
      );
    ...
    it('should generate backward transitions for all stages', () => {
      ...
      const backwardTransitions = defaultTransitions.filter(
        (t) => t.type === 'backward'
      );
    ...
    it('should include skip transitions', () => {
      ...
      const skipTransitions = defaultTransitions.filter(
        (t) => t.type === 'skip'
      );

      expect(skipTransitions.length).toBeGreaterThan(0);
      expect(skipTransitions.every((t) => t.requiresApproval === true)).toBe(
        true
      );
    });
  });
