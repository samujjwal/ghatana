    ...
    it('should load task domain YAML files', async () => {
      ...
      const mockDomainConfig = {
        ...
        tasks: [
          createMockTask({
            id: 'create-schema',
            domain: 'data-modeling' as TaskDomainId,
          }),
          createMockTask({
            id: 'define-relations',
            domain: 'data-modeling' as TaskDomainId,
...
describe('Task Execution', () => {
  ...
  describe('Agent Selection', () => {
    it('should select appropriate agent based on task domain', () => {
      ...
      const selectedAgent =
        domainAgentMapping[task.domain as keyof typeof domainAgentMapping];
...
describe('State Management', () => {
  describe('Workflow State', () => {
    ...
    it('should persist workflow state', () => {
      const state = {
        ...
        workflows: new Map([
          ['workflow-123', { id: 'workflow-123', status: 'running' }],
        ]),
      };
  ...
  describe('Lifecycle State Machine', () => {
    ...
    it('should validate stage transitions', () => {
      ...
      const canTransition =
        allowedTransitions[currentStage].includes(targetStage);
      expect(canTransition).toBe(true);

      const invalidTarget: LifecycleStage = 'discovery';
      const invalidTransition =
        allowedTransitions[currentStage].includes(invalidTarget);
...
describe('Full Integration Flow', () => {
  it('should complete end-to-end workflow execution', async () => {
    ...
    const completedTasks = executions.filter(
