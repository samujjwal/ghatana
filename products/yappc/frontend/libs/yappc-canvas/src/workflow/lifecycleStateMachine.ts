...
export class LifecycleStateMachine {
  ...
  constructor(
    ...
    initialStage: LifecycleStage = 'intent'
  ) {
    this.config = config;
    this.state = {
  ...
  canTransitionTo(targetStage: LifecycleStage): TransitionValidation {
    ...
    if (this.config.enforceEntryCriteria) {
      const targetStageConfig = this.config.stages.find(
        (s) => s.id === targetStage
      );
    ...
    if (transitionConfig.type === 'backward') {
      warnings.push({
        ...
        message:
          'Moving to a previous stage. Progress in current stage may be affected.',
      });
    }
  ...
  async transitionTo(
    ...
    try {
      ...
      const currentHistoryEntry =
        this.state.stageHistory[this.state.stageHistory.length - 1];
