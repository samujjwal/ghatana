...
const { SPACING, Z_INDEX, COLORS, TYPOGRAPHY, CANVAS, RADIUS, SHADOWS } =
  CANVAS_TOKENS;
    ...
    >
      ...
      <Tooltip title="Zoom out (-)">
        <IconButton
          ...
          style={{ fontSize: TYPOGRAPHY.SM, color: COLORS.TEXT_PRIMARY }}
        >
          −
        </IconButton>
      </Tooltip>
      ...
      <Tooltip title="Click to reset to 100%">
        <Box
          onClick={handleReset}
          className="font-medium cursor-pointer select-none min-w-[45px] text-center"
          style={{
      ...
      <Tooltip title={minimapVisible ? 'Hide minimap' : 'Show minimap'}>
        <IconButton
          ...
          className="w-[28px] h-[28px] text-[16px] mx-1"
          style={{
      ...
      {showCoords && (
        <>
          ...
          />
          <Box className="whitespace-nowrap font-mono text-xs">
