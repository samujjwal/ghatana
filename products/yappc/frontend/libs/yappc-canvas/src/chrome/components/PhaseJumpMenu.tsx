  ...
  return (
    ...
    >
      ...
      <Box className="flex items-center flex-1 gap-2">
        <Box
          className="text-[24px] flex items-center justify-center w-[40px] h-[40px]"
          style={{
      ...
      <Box className="flex flex-col items-end gap-1">
        ...
        {stats.frameCount > 0 && (
          <Box className="flex items-center gap-1">
            ...
            <Typography className="text-right min-w-[35px] text-xs font-semibold">
        ...
        {stats.hasRecentActivity && (
          <Chip
            label="Active"
            size="small"
            className="h-[18px] text-white [&_.MuiChip-label]:px-1"
            style={{ fontSize: TYPOGRAPHY.XS, backgroundColor: COLORS.SUCCESS }}
          />
        )}
      </Box>
    </MenuItem>
  ...
  return (
    ...
    >
      ...
      <Box style={{ paddingTop: SPACING.XS, paddingBottom: SPACING.XS }}>
        {(
          Object.keys(LIFECYCLE_PHASES) as Array<keyof typeof LIFECYCLE_PHASES>
        ).map((phase) => {
          const stats = phaseStatsMap.get(phase) || {
            phase,
            frameCount: 0,
