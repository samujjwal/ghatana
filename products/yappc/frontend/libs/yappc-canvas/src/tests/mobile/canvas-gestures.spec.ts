...
test.describe('Mobile Canvas - Touch Gestures', () => {
  ...
  test('should select node on single tap @mobile @visual', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="selection-toolbar"]')
    ).toBeVisible();
  });
  ...
  test('should deselect node on background tap @mobile', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="node-1"]')).toHaveClass(
      /selected/
    );
    ...
    await expect(page.locator('[data-testid="node-1"]')).not.toHaveClass(
      /selected/
    );
  });
  ...
  test('should open node editor on double tap @mobile', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="node-editor-modal"]')
    ).toBeVisible();
    ...
    if (isMobile) {
      await expect(
        page.locator('input[data-testid="node-label"]')
      ).toBeFocused();
    }
  });
  ...
  test('should respond to touch within 100ms @mobile @performance', async ({
    page,
  }) => {
    const node = page.locator('[data-testid="node-1"]');
  ...
  test('should handle simultaneous touches gracefully @mobile', async ({
    page,
  }) => {
    const canvas = page.locator('[data-testid="canvas-container"]');
  ...
  test('should handle touch during zoom animation @mobile', async ({
    page,
  }) => {
...
test.describe('Mobile Canvas - Responsive Layout', () => {
  ...
  for (const viewport of viewports) {
    test(`should render correctly on ${viewport.name} @mobile @visual`, async ({
      page,
    }) => {
...
test.describe('Mobile Canvas - Accessibility', () => {
  test('should have touch targets at least 44x44px @mobile @a11y', async ({
    page,
  }) => {
    await page.goto('/canvas/test-canvas');
  ...
  test('should announce touch interactions to screen readers @mobile @a11y', async ({
    page,
  }) => {
    await page.goto('/canvas/test-canvas');
