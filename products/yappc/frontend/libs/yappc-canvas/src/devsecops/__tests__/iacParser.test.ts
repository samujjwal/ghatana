...
describe.skip('IaC Parser - Terraform Plan Parsing', () => {
  ...
  it('should parse resource changes', () => {
    ...
    const webInstance = plan.resources.find(
      (r) => r.address === 'aws_instance.web'
    );
    expect(webInstance?.change?.actions).toContain('create');

    const s3Bucket = plan.resources.find(
      (r) => r.address === 'aws_s3_bucket.assets'
    );
...
describe.skip('IaC Parser - Drift Detection', () => {
  ...
  it('should detect drifted resources', () => {
    ...
    const driftedResource = drifts.find(
      (d) => d.address === 'aws_s3_bucket.assets' && d.status === 'drifted'
    );
  ...
  it('should detect extra resources', () => {
    ...
    const extraDrift = drifts.find(
      (d) => d.address === 'aws_instance.database'
    );
...
describe.skip('IaC Parser - Canvas Conversion', () => {
  ...
  it('should apply drift highlighting', () => {
    ...
    const driftedNode = graph.nodes.find(
      (n) => n.id === 'aws_s3_bucket.assets'
    );
