...
import {
  BuildPipelineCanvas,
  BuildStep,
} from '../devsecops/BuildPipelineCanvas';
...
describe('BuildPipelineCanvas', () => {
  ...
  it('calls onDeleteStep when deleting a step', () => {
    ...
    render(
      <BuildPipelineCanvas steps={mockSteps} onDeleteStep={onDeleteStep} />
    );
  ...
  it('displays empty state when no steps', () => {
    ...
    expect(
      screen.getByText('Add build steps to optimize your pipeline')
    ).toBeInTheDocument();
  });
