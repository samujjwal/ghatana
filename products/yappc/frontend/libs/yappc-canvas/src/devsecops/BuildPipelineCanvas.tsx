...
export const BuildPipelineCanvas: React.FC<BuildPipelineCanvasProps> = ({
  ...
  const renderStep = (step: BuildStep) => (
    <Paper
      ...
      className="p-4 mb-4"
      style={{
        borderLeft: `4px solid ${step.parallel ? '#10b981' : '#3b82f6'}`,
      }}
    >
      <Box className="flex items-start gap-2">
        ...
        <Box className="flex-1">
          ...
          <Box className="mt-2 flex gap-1 flex-wrap">
            ...
            />
            {step.parallel && (
              <Chip label="Parallel" tone="success" size="sm" />
            )}
            {step.cacheEnabled && (
              <Chip
                icon={<Storage />}
                label={step.cacheStrategy}
                tone="primary"
                size="sm"
              />
            )}
    ...
    <Box className="h-full flex flex-col">
      <Paper className="p-4 mb-4">
        <Box className="flex items-center gap-4 flex-wrap">
          ...
          />
          {parallelizable && (
            <Chip icon={<PlayArrow />} label="Parallelizable" tone="success" />
          )}
        </Box>
      </Paper>
