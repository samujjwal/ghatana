...
export function detectDrift(
  ...
  const plannedResources = new Map(plan.resources.map((r) => [r.address, r]));
  ...
  priorResources.forEach((priorResource, address) => {
    ...
    if (!actual) {
      ...
      if (
        ...
        planned?.change?.actions.includes('update')
      ) {
        drifts.push({
          address,
          status: 'drifted',
...
export function convertTerraformToCanvas(
  ...
  const changedResources = plan.resources.filter(
    (r) => r.change && !r.change.actions.includes('no-op')
  ).length;

  const driftedResources =
    drifts?.filter((d) => d.status === 'drifted').length || 0;
...
export function generateRemediationSuggestions(
  ...
  switch (drift.status) {
    case 'missing':
      suggestions.push(
        `Run 'terraform apply' to create the missing resource: ${drift.address}`
      );
    ...
    case 'drifted':
      suggestions.push(
        `Run 'terraform apply' to update the drifted resource: ${drift.address}`
      );
    ...
    case 'extra':
      suggestions.push(
        `Resource ${drift.address} exists but is not managed by Terraform`
      );
  ...
  if (
    ...
    Object.keys(expected).forEach((key) => {
      ...
      if (!(key in actual)) {
        ...
        differences.push(
          ...findDifferences(expected[key], actual[key], newPath)
        );
 ...
 */
function generateRemediationUrl(
 ...
 */
export function createIaCParserConfig(
  overrides?: Partial<IaCParserConfig>
): IaCParserConfig {
  return {
