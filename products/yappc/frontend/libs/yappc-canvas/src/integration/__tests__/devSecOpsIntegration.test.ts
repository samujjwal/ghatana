...
describe('DevSecOpsCanvasIntegration', () => {
  ...
  describe('generateDeploymentRunbook', () => {
    it('should generate rolling deployment runbook', async () => {
      ...
      const runbook = await integration.generateDeploymentRunbook(
        serviceNode,
        config
      );
    ...
    it('should generate blue-green deployment runbook', async () => {
      ...
      expect(
        runbook.steps.some((s) => s.name.includes('Green Environment'))
      ).toBe(true);
      expect(runbook.steps.some((s) => s.name.includes('Switch Traffic'))).toBe(
        true
      );
    });
