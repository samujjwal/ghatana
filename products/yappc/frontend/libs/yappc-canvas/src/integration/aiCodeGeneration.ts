...
export class AICodeGenerationService {
  ...
  async generateService(
    ...
    try {
      const promptData =
        'type' in data && data.type === 'service'
          ? {
              ...data,
              persona:
                (data as unknown as { persona?: string }).persona ??
                'developer',
              status:
                (data as unknown as { status?: string }).status ?? 'draft',
            }
          : data;
      ...
      const systemPrompt = options.persona
        ? PERSONA_SYSTEM_PROMPTS[options.persona]
        : PERSONA_SYSTEM_PROMPTS.developer;
  ...
  async extractRequirements(
    userInput: string,
    options: AICodeGenerationOptions = {}
  ): Promise<RequirementNodeData | null> {
    try {
  ...
  private extractCode(content: string): string {
    ...
    const codeBlockMatch = content.match(
      /```(?:typescript|ts|javascript|js|prisma)?\n([\s\S]*?)```/
    );
    if (codeBlockMatch) {
 ...
 */
export function createAICodeGenerationService(
