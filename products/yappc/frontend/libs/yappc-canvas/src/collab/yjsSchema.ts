...
export class YjsSchemaValidator {
  ...
  validateDocument(doc: Y.Doc): ValidationResult {
    ...
    if (
      attachmentsMap &&
      attachmentsMap instanceof Y.Map &&
      attachmentsMap.size > 0
    ) {
  ...
  validateNode(node: unknown): ValidationError[] {
    ...
    if (this.containsBinary(node)) {
      errors.push({
        ...
        message:
          'Node contains embedded binary data. Use attachment references instead.',
      });
    }
  ...
  private createDefaultConfig(
    overrides?: Partial<YjsSchemaConfig>
  ): YjsSchemaConfig {
    return {
      allowedPaths: new Set<YjsPath>([
        'nodes',
        'edges',
        'viewport',
        'selection',
 ...
 */
export function createYjsValidator(
  config?: Partial<YjsSchemaConfig>
): YjsSchemaValidator {
