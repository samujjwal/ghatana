...
export const assertOpSequence = (events: unknown[], start = 1) => {
  ...
  const seq = events
    .map((e) => {
      ...
      if (v == null)
        throw new Error(`missing opSeq on event ${JSON.stringify(e)}`);
      return v;
    })
    .sort((a, b) => a - b);
  ...
  for (const v of seq) {
    if (v !== expected)
      throw new Error(`expected opSeq ${expected} but got ${v}`);
    expected += 1;
  }
  return true;
};

export const findEventsByCollection = (
  events: unknown[],
  collectionName: string
) => {
  return (events || []).filter(
    (e) => (e?.metadata?.collection ?? null) === collectionName
  );
};

export const assertDeltaContains = (
  ev: unknown,
  expected: Record<string, unknown>
) => {
  if (!ev?.delta) throw new Error('event has no delta');
  for (const k of Object.keys(expected)) {
    if (ev.delta[k] !== expected[k])
      throw new Error(
        `delta.${k} expected ${expected[k]} but found ${ev.delta[k]}`
      );
  }
  return true;
};

export const assertEventsForCollection = (
  events: unknown[],
  collectionName: string,
  ...
  if (expectedCount != null && found.length !== expectedCount) {
    throw new Error(
      `expected ${expectedCount} events for collection "${collectionName}" but found ${found.length}`
    );
  }
  return found;
};

export const assertNoEventsForCollection = (
  events: unknown[],
  collectionName: string
) => {
  const found = findEventsByCollection(events, collectionName);
  if (found.length > 0)
    throw new Error(
      `expected no events for collection "${collectionName}" but found ${found.length}`
    );
  return true;
};

export const assertEventsContainDelta = (
  events: unknown[],
  ...
  if (!matches)
    throw new Error(
      `no event contains expected delta: ${JSON.stringify(expectedDelta)}`
    );
  return true;
};

export const assertHistoryIncludes = (
  history: unknown[],
  expected: Record<string, unknown>
) => {
  ...
  if (!matches)
    throw new Error(
      `history does not include expected event: ${JSON.stringify(expected)}`
    );
  return true;
};
