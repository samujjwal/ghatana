...
const DiffView: React.FC<DiffViewProps> = ({
  ...
  const [selectedResolution, setSelectedResolution] =
    useState<ConflictResolution>('postpone');
...
const ConflictItem: React.FC<ConflictItemProps> = ({ conflict, onResolve }) => {
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      >
        <div className="text-lg">{getConflictIcon()}</div>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
            {getConflictDescription()}
          </div>
          <div className="text-xs text-gray-500 dark:text-gray-400">
            Line {conflict.position.line + 1}, Column{' '}
            {conflict.position.column + 1} •{conflict.users.length} users
            involved •{new Date(conflict.timestamp).toLocaleTimeString()}
          </div>
        </div>
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            value={sortBy}
            onChange={(e) =>
              setSortBy(e.target.value as 'time' | 'type' | 'users')
            }
            className="px-3 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300"
          >
