...
export const AIAssistant: React.FC<AIAssistantProps> = ({
  ...
  const generateSuggestion = useCallback(
    async (query?: string): Promise<AIMessage> => {
      try {
        ...
        const suggestions = [
          {
            ...
            content:
              'Consider using the React.useMemo hook to optimize performance',
  ...
  const generateCode = useCallback(
    async (description: string, language: string): Promise<AIMessage> => {
      try {
        ...
        {messages.length === 0 && (
          <div className="text-center text-gray-500 dark:text-gray-400 py-8">
            ...
            <p className="text-sm mt-2">
              Ask me anything about your code or request suggestions,
              explanations, or refactoring help.
            </p>
          </div>
        )}
