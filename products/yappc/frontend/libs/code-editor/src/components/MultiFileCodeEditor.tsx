...
export const MultiFileCodeEditor: React.FC<{
  ...
  const [collaborativeCursors, setCollaborativeCursors] = useState<
    Map<string, CollaborativeCursor[]>
  >(new Map());
  ...
  useEffect(() => {
    if (config.enableCollaborativeCursors && activeFileId) {
      ...
      setCollaborativeCursors((prev) =>
        new Map(prev).set(activeFileId, cursors)
      );
    }
  }, [
    collaborativeEditor.cursors,
    activeFileId,
    config.enableCollaborativeCursors,
  ]);
      ...
      <div className="tab-bar flex items-center border-b border-gray-700 bg-gray-800 overflow-x-auto">
        {files.map((file) => (
          <div
            ...
            className={`tab flex items-center px-4 py-2 cursor-pointer border-r border-gray-700 whitespace-nowrap transition-colors ${
      ...
      <div className="editor-container flex-1 overflow-hidden">
        {activeFile && activeFileId && (
          <EnhancedCodeEditor
            ...
            config={{
              language:
                (activeFile.language as unknown as CodeLanguage) ||
                'typescript',
              theme: 'vs-dark',
            }}
      ...
      <div className="status-bar flex items-center justify-between px-4 py-2 bg-gray-800 border-t border-gray-700 text-sm text-gray-400">
        ...
        <div className="flex items-center gap-2">
          {collaborativeEditor.isReady && (
            <>
              <span>
                {collaborativeEditor.activeUsers.length} collaborators
              </span>
            </>
          )}
        </div>
