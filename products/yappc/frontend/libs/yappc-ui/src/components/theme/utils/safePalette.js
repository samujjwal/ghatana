 ...
 */
export function resolveThemeColor(theme, key, fallback = 'primary') {
    const p = (theme && theme.palette) || {};
    if (key && p[key] && typeof p[key].main === 'string')
        return key;
 ...
 */
export function resolveMuiColor(theme, key, fallback = 'primary') {
 ...
 */
export function getPaletteMain(theme, key, fallback = 'primary') {
    ...
    return p[paletteKey]?.main ?? theme.palette.primary.main;
}
