...
export const SentimentIndicator: React.FC<SentimentIndicatorProps> = ({
  ...
  const renderMinimal = () => (
    <Tooltip
      title={`${config.label} (${Math.round(result.confidence * 100)}% confidence)`}
    >
      <Chip
        icon={config.icon}
        label={config.emoji}
  ...
  const renderDetailed = () => (
    <Paper elevation={2} className={`p-4 min-w-[280px] ${className || ''}`}>
      <Stack spacing={2}>
        ...
        <Box className="flex items-center gap-2">
          ...
          {showHistory && history.length > 0 && (
            <IconButton size="sm" onClick={() => setIsExpanded(!isExpanded)}>
        ...
        {result.explanation && (
          <Box className="p-2 rounded bg-gray-100 dark:bg-gray-800">
            <Typography
              as="span"
              className="text-xs text-gray-500"
              color="text.secondary"
