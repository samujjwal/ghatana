  ...
  (props, _ref) => {
    ...
    const handleFiles = (fileList: FileList | null) => {
      ...
      if (onUpload && validFiles.length > 0) {
        const validUploadedFiles = newUploadedFiles.filter(
          (f) => f.status === 'pending'
        );
      ...
      <div className={cn('w-full', className)} {...rest}>
        ...
        <div
          ...
          className={cn(
            ...
            !isDragging &&
              !disabled &&
              'border-grey-300 hover:border-primary-400 hover:bg-grey-50',
            disabled &&
              'border-grey-200 bg-grey-50 cursor-not-allowed opacity-50'
          )}
        >
          <input
            ref={inputRef}
