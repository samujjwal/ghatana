...
const SimpleLineChart = React.memo<{
  ...
  if (data.length === 0) {
    return (
      <Box className="flex items-center justify-center text-gray-500 dark:text-gray-400">
        No data available
      </Box>
    );
  }
  ...
  const minValue = Math.min(
    ...values,
    baseline || 0,
    alertThresholds?.warning || 0
  );
  const maxValue = Math.max(
    ...values,
    baseline || 0,
    alertThresholds?.critical || 0
  );
            ...
            point.isRegression
              ...
              : point.severity === 'critical'
                ...
                : point.severity === 'warning'
                  ? 'var(--color-warning-main, #ff9800)'
                  : 'var(--color-primary-dark, #1976d2)'
          }
          stroke={
            point.isRegression ? 'var(--color-common-white, #fff)' : 'none'
          }
...
const TrendIndicator = React.memo<TrendIndicatorProps>(
  ({ trend, compact = false }) => {
    if (!trend) {
      return (
        <Chip
...
export const PerformanceTrendingChart: React.FC<TrendingChartProps> = ({
  ...
  const chartData: ChartDataPoint[] = useMemo(() => {
    return metricData.map((dataPoint) => ({
      ...
      isRegression: regressionPoints.some(
        (regPoint) =>
          Math.abs(regPoint.getTime() - dataPoint.timestamp.getTime()) < 60000
      ),
      severity: alertThresholds
        ? dataPoint.value >= alertThresholds.critical
          ? 'critical'
          : dataPoint.value >= alertThresholds.warning
            ? 'warning'
            : 'normal'
        : 'normal',
    }));
  ...
  const criticalCount = chartData.filter(
    (point) => point.severity === 'critical'
  ).length;
  const warningCount = chartData.filter(
  ...
  return (
    <Card className={className} className="h-full">
      <CardContent>
              ...
              <Alert severity="error" className="mb-2">
                <Box className="flex items-center gap-2">
                  ...
                  <Typography as="p" className="text-sm">
                    {criticalCount} critical threshold breach
                    {criticalCount !== 1 ? 'es' : ''} detected
                  </Typography>
                </Box>
              </Alert>
            )}
            {warningCount > 0 && (
              <Alert severity="warning">
                <Box className="flex items-center gap-2">
                  <InfoIcon />
                  <Typography as="p" className="text-sm">
                    {warningCount} warning threshold breach
                    {warningCount !== 1 ? 'es' : ''} detected
                  </Typography>
                </Box>
              </Alert>
            )}
        ...
        {/* Loading indicator */}
        {isMonitoring && <LinearProgress className="mb-2" />}
        ...
        <Box className="flex items-center gap-4 mt-4 flex-wrap">
          ...
          {showRegressions && (
            <Box className="flex items-center gap-1">
              <Box
                className="rounded-full w-[8px] h-[8px] bg-red-600 border-[2px_solid]"
                style={{ borderColor: 'common.white', borderTop: '1px dashed' }}
              />
              <Typography as="span" className="text-xs text-gray-500">
                Regression
              </Typography>
        ...
        {trendAnalysis && (
          <Box className="mt-4 p-2 rounded bg-gray-50 dark:bg-gray-950">
            ...
            >
              Trend Analysis: {trendAnalysis.direction} trajectory with{' '}
              {(trendAnalysis.confidence * 100).toFixed(0)}% confidence
              {trendAnalysis.regressionPoints &&
                trendAnalysis.regressionPoints.length > 0 &&
