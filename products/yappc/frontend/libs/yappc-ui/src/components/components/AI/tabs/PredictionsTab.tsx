          ...
          <CardContent>
            ...
            <div style={{ marginTop: 12 }}>
              <Typography as="p" className="text-sm">
                Range: {insights.buildTimePrediction.range.min.toFixed(1)} -{' '}
                {insights.buildTimePrediction.range.max.toFixed(1)} minutes
              </Typography>
              <LinearProgress
                variant="determinate"
                value={insights.buildTimePrediction.confidence * 100}
                className="mb-2"
              />
              <Typography as="span" className="text-xs text-gray-500">
                Confidence:{' '}
                {(insights.buildTimePrediction.confidence * 100).toFixed(0)}%
              </Typography>
            </div>
          </CardContent>
        </Card>
