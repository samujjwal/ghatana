...
export const DatePicker = React.forwardRef<HTMLDivElement, DatePickerProps>(
  (props, ref) => {
    ...
    const generateCalendarDays = (): (Date | null)[] => {
      ...
      const firstDay =
        (getFirstDayOfMonth(year, month) - firstDayOfWeek + 7) % 7;
    ...
    const handleDateSelect = (date: Date) => {
      if (
        isDateDisabled(date, minDate, maxDate, disabledDates, shouldDisableDate)
      ) {
        return;
      }
      onChange?.(date);
    ...
    const weekDays = [...Array(7)].map(
      (_, i) => DAYS_SHORT[(i + firstDayOfWeek) % 7]
    );
      ...
      >
        ...
        <button
          ...
          className={cn(
            ...
            value
              ? 'text-grey-900 border-grey-300'
              : 'text-grey-500 border-grey-300'
          )}
        >
        ...
        {isOpen && (
          <div className="absolute z-50 mt-2 bg-white border border-grey-300 rounded-lg shadow-lg p-4 min-w-[280px]">
            ...
            >
              {showWeekNumbers && (
                <div className="text-xs font-medium text-grey-500 text-center p-1">
                  Wk
                </div>
              )}
              {weekDays.map((day) => (
                <div
                  key={day}
                  className="text-xs font-medium text-grey-500 text-center p-1"
                >
                  {day}
                </div>
              ))}
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isDisabled &&
                        'text-grey-300 cursor-not-allowed hover:bg-transparent'
                    )}
                  >
                    {date.getDate()}
                  </button>
