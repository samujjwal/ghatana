...
const BUILTIN_MAPPINGS: FieldMapping[] = [
  // Status
  {
    ...
    field: 'status',
    value: 'open',
    aliases: ['active', 'ongoing'],
  },
  {
    phrase: 'closed',
    field: 'status',
    value: 'closed',
    aliases: ['finished', 'resolved'],
  },
  {
    phrase: 'pending',
    field: 'status',
    value: 'pending',
    aliases: ['waiting', 'on hold'],
  },
  {
    phrase: 'in progress',
    field: 'status',
    value: 'in_progress',
    aliases: ['working on', 'doing'],
  },
  {
    phrase: 'blocked',
    field: 'status',
    value: 'blocked',
    aliases: ['stuck', 'impeded'],
  },
  ...
  { phrase: 'yesterday', field: 'date', value: 'yesterday' },
  {
    ...
    field: 'date',
    value: 'this_week',
    aliases: ['current week'],
  },
  {
    phrase: 'last week',
    field: 'date',
    value: 'last_week',
    aliases: ['previous week'],
  },
  {
    phrase: 'this month',
    field: 'date',
  ...
  // Assignment
  {
    ...
    field: 'assignee',
    value: 'me',
    aliases: ['assigned to me', 'mine'],
  },
  {
    phrase: 'unassigned',
    field: 'assignee',
    value: 'unassigned',
    aliases: ['no assignee', 'nobody'],
  },
  ...
  // Types
  {
    ...
    field: 'type',
    value: 'bug',
    aliases: ['defects', 'issues'],
  },
  {
    phrase: 'features',
    field: 'type',
    value: 'feature',
    aliases: ['enhancements', 'stories'],
  },
  {
    phrase: 'tasks',
    field: 'type',
    value: 'task',
    aliases: ['work items', 'todos'],
  },
];
 ...
 */
function parseQuery(query: string, mappings: FieldMapping[]): QueryIntent {
  ...
  const sortPatterns = [
    {
      pattern: /sort(?:ed)?\s+by\s+(\w+)/i,
      extract: (m: RegExpMatchArray) => m[1],
    },
    {
      pattern: /order(?:ed)?\s+by\s+(\w+)/i,
      extract: (m: RegExpMatchArray) => m[1],
    },
  ...
  if (
    ...
    normalizedQuery.includes('grouped')
  ) {
  ...
  if (intent === 'unknown' && normalizedQuery.length > 2) {
    ...
    for (const mapping of mappings.slice(0, 5)) {
      if (
        mapping.phrase.includes(normalizedQuery) ||
        normalizedQuery.includes(mapping.phrase.substring(0, 3))
      ) {
...
export function useDataTableAI(options: UseDataTableAIOptions = {}) {
  ...
  const allMappings = useMemo(
    () => [...BUILTIN_MAPPINGS, ...fieldMappings],
    [fieldMappings]
  );
  ...
  const recordCorrection = useCallback(
    (originalQuery: string, correctedFilters: Record<string, FilterValue>) => {
        ...
        for (const word of queryWords) {
          if (word.length > 3 && !STOP_WORDS.has(word)) {
            const exists = allMappings.some(
              (m) => m.phrase === word || m.aliases?.includes(word)
            );
