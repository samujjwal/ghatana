...
export const PerformanceDashboard: React.FC<PerformanceDashboardProps> = ({
  ...
  const {
    ...
    alertThresholds: Object.values(alertThresholds).reduce(
        ...
        <Box>
          ...
          <Box className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-8">
            {metricSummaries.map((summary) => (
              <Box key={summary.name}>{renderMetricSummary(summary)}</Box>
            ))}
          </Box>
            ...
            {metrics.slice(0, 4).map((metric) => (
              <Box key={metric}>
                <PerformanceTrendingChart
                  metric={metric}
                  title={
                    metricConfigs[metric as keyof typeof metricConfigs]?.title
                  }
                  timeRange={timeRange}
              ...
              <Paper className="p-4">
                ...
                <Box className="flex flex-col gap-4">
                  ...
                  <Box className="flex justify-between items-center">
                    ...
                    <Chip size="sm" label={alerts.length} variant="outlined" />
                  </Box>
                </Box>
              </Paper>
            </Box>
