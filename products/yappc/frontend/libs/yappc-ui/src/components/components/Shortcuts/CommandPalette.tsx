...
function fuzzyMatch(
  ...
  for (
    ...
    if (textLower[i] === patternLower[patternIndex]) {
      ...
      if (
        matches.length > 1 &&
        matches[matches.length - 1] === matches[matches.length - 2] + 1
      ) {
        score += 0.5;
      }
...
export function CommandPalette({
    ...
    const results = commands
      ...
      .map((cmd) => {
        ...
        const descMatch = cmd.description
          ...
          : { score: 0, matches: [] };
        const score =
          titleMatch.score * 2 + descMatch.score + (cmd.priority || 0);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case 'ArrowUp':
          event.preventDefault();
          setSelectedIndex(
            (prev) => (prev - 1 + totalCommands) % totalCommands
          );
          break;
        case 'Enter':
          event.preventDefault();
  ...
  const itemStyle = (index: number): React.CSSProperties => ({
    ...
    borderLeft:
      selectedIndex === index
        ? '3px solid var(--color-primary, #1976d2)'
