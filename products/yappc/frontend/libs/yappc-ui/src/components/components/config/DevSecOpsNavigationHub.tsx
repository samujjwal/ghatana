...
export default function DevSecOpsNavigationHub({
  ...
  const personaFilteredData = useMemo(() => {
    const personaDomains = domains.filter(
      (domain) => domain.personas?.includes(currentPersona) || !domain.personas
    );

    const personaWorkflows = workflows.filter(
      (workflow) => workflow.persona === currentPersona || !workflow.persona
    );

    const personaPhases =
      lifecycleConfig.phases?.filter(
        (phase) =>
    ...
    const personaTasks = tasks.filter(
      (task) => task.persona === currentPersona || !task.persona
    );
  ...
  const handleDomainSelect = (domainId: string) => {
    ...
    const relevantWorkflow = personaFilteredData.workflows.find(
      (w) => w.domain === domainId
    );
    if (relevantWorkflow) {
  ...
  const renderWorkflowCard = (workflow: unknown) => (
    ...
    >
      <CardContent>
        ...
        <Typography variant="caption" color="text.secondary">
          {workflow.steps?.length || 0} steps • {workflow.phases?.length || 0}{' '}
          phases
        </Typography>
      </CardContent>
    </Card>
        ...
        <Stack spacing={1} mb={2}>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap">
            ...
            {(phase.primary_personas?.length || 0) > 3 && (
              <Chip
                label={`+${(phase.primary_personas?.length || 0) - 3}`}
                size="small"
                variant="outlined"
              />
            )}
          ...
          <Stack direction="row" spacing={2} flexWrap="wrap">
            ...
            {selectedWorkflowId && (
              <Button
                ...
                onClick={() =>
                  navigate(`/devsecops/workflows/${selectedWorkflowId}`)
                }
              >
                Open Workflow
              </Button>
