    ...
    if (compact) {
      return (
        <Box
          ref={ref}
          className={[
            'flex items-center gap-2 p-2 rounded bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700',
            className,
          ]
            .filter(Boolean)
            .join(' ')}
        ...
        >
          ...
          {loading ? (
            <LoadingSkeleton
              compact
              count={Math.min(displayGates.length || 3, 4)}
            />
          ) : (
            <Stack direction="row" spacing={1} alignItems="center">
