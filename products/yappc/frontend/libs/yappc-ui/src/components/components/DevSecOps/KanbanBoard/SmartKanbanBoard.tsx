...
function generateAISuggestions(
  ...
  columns.forEach((col) => {
    itemsByStatus[col.status] = items.filter(
    ...
    if (col.wipLimit) {
      ...
      if (count > col.wipLimit) {
        ...
        suggestions.push({
          ...
          confidence: 1.0,
          reason:
            'WIP limit exceeded - consider moving items or increasing limit',
        });
      }
    }
  });
  ...
  blockedItems.forEach((item) => {
    ...
    const daysSinceUpdate = Math.floor(
      (Date.now() - new Date(item.updatedAt || Date.now()).getTime()) /
        (1000 * 60 * 60 * 24)
    );
      ...
      if (onSuggestionAccept) {
        ...
        if (item && onItemMove) {
          ...
          const targetCol = columns.find(
            (c) => c.status === suggestion.action?.targetStatus
          );
  ...
  return (
    <Box>
      ...
      {enableAI && (activeSuggestions.length > 0 || showInsights) && (
        <Paper className="mb-4 p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
          {/* Header */}
          <Box
            display="flex"
            alignItems="center"
