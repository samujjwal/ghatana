...
export const AIInsightsDashboard: React.FC<AIInsightsDashboardProps> = ({
  ...
  const [implementingIds, setImplementingIds] = useState<Set<string>>(
    new Set()
  );
  ...
  const handleImplementRecommendation = useCallback(
    async (id: string) => {
  ...
  const buildTimeSummary = useMemo(() => {
    ...
    const { predictedTime, confidence, range, factors } =
  ...
  const deploymentRiskSummary = useMemo(() => {
    ...
    const { riskLevel, riskScore, riskFactors, recommendations } =
  ...
  const hasSummaryData = useMemo(
    () =>
      Boolean(
        buildTimeSummary || deploymentRiskSummary || summaryModels.length > 0
      ),
  ...
  const recommendationsByType = useMemo<RecommendationsByType>(() => {
    ...
    return insights.recommendations.reduce<RecommendationsByType>(
      (acc, rec) => {
  ...
  return (
    <Box className={className}>
      ...
      {selectedTab === 2 && insights?.buildTimePrediction && (
        <PredictionsTab
          insights={{ buildTimePrediction: insights.buildTimePrediction }}
        />
      )}
