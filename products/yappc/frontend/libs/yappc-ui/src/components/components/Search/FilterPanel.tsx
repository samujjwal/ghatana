...
export function FilterPanel({
  ...
  const handleMultiSelectChange = useCallback(
    (field: keyof FilterCriteria, value: string, checked: boolean) => {
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <input
                  type="date"
                  value={
                    localCriteria.dateRange?.end?.toISOString().split('T')[0] ||
                    ''
                  }
                  onChange={(e) =>
                    handleCriteriaUpdate({
                ...
                <div style={{ maxHeight: '120px', overflowY: 'auto' }}>
                  {options.status.map((option) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={(localCriteria.status || []).includes(
                          option.value
                        )}
                        onChange={(e) =>
                ...
                <div style={{ maxHeight: '120px', overflowY: 'auto' }}>
                  {options.tags.map((option) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={(localCriteria.tags || []).includes(
                          option.value
                        )}
                        onChange={(e) =>
                          handleMultiSelectChange(
                ...
                <div style={{ maxHeight: '120px', overflowY: 'auto' }}>
                  {options.users.map((option) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={(localCriteria.users || []).includes(
                          option.value
                        )}
                        onChange={(e) =>
                          handleMultiSelectChange(
          ...
          >
            <button
              ...
              style={{
                ...
                cursor:
                  !hasActiveFilters || isLoading ? 'not-allowed' : 'pointer',
              }}
            >
              Clear All
            </button>
