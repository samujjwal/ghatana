...
describe('GateWidget', () => {
  ...
  describe('Overall Status Calculation', () => {
    ...
    it('shows success status when all gates are successful', () => {
      const successGates = mockGates.map((gate) => ({
        ...gate,
  ...
  describe('Loading State', () => {
    it('shows loading skeletons', () => {
      ...
      const container =
        ...
        screen.getByText('Gates').closest('[role="article"]');
  ...
  describe('Interactions', () => {
    ...
    it('opens details URL when details button is clicked', () => {
      ...
      expect(firstDetailsButton).toHaveAttribute(
        'href',
        mockGates[0].detailsUrl
      );
  ...
  describe('Accessibility', () => {
    it('has proper ARIA labels for buttons', () => {
      ...
      expect(
        screen.getByRole('button', { name: 'Refresh gate status' })
      ).toBeInTheDocument();
      expect(
        screen.getByRole('button', { name: 'View details for Build Pipeline' })
      ).toBeInTheDocument();
    });
    ...
    it('has proper heading structure', () => {
      ...
      expect(
        screen.getByRole('heading', { name: 'Test Gates' })
      ).toBeInTheDocument();
    });
  ...
  describe('Custom Props', () => {
    it('forwards custom className', () => {
      renderWithTheme(
        <GateWidget gates={mockGates} className="custom-class" />
      );
