...
function parseNaturalLanguageQuery(
  ...
  if (filters.priority)
    filterDescriptions.push(`priority = "${filters.priority}"`);
  if (filters.date) filterDescriptions.push(`date = "${filters.date}"`);
  if (filters.assignee)
    filterDescriptions.push(`assignee = "${filters.assignee}"`);
  ...
  const confidence =
    matchedTerms.length > 0 ? 0.7 + matchedTerms.length * 0.1 : 0.3;
...
export function SmartDataTable<T = Item>({
  ...
  const [parsedIntent, setParsedIntent] = useState<ParsedQueryIntent | null>(
    null
  );
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const parser =
        queryParser ||
        ((q: string) => parseNaturalLanguageQuery(q, mergedFieldMappings));
