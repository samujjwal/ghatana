...
const StoryCard = React.memo(
  ({
    ...
    const PriorityBadge = () => (
      <Badge
        variant="outline"
        className={cn('text-xs', priorityColors[story.priority])}
      >
        {story.priority}
      </Badge>
    );
              ...
              <Tooltip>
                <TooltipTrigger>
                  <GitPullRequest
                    className={cn(
                      'w-3.5 h-3.5',
                      story.pullRequest.status === 'merged' &&
                        'text-violet-500',
...
const BoardColumnComponent = React.memo(
  ({
    ...
    const isOverLimit = column.wipLimit
      ? column.stories.length > column.wipLimit
      : false;
    const atLimit = column.wipLimit
      ? column.stories.length === column.wipLimit
      : false;
      ...
      <div className="flex flex-col w-[300px] min-w-[300px] bg-zinc-900 rounded-lg">
        ...
        <div className="flex items-center justify-between p-3 border-b border-zinc-800">
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant={
                isOverLimit ? 'destructive' : atLimit ? 'secondary' : 'outline'
              }
              className="text-xs"
            >
...
export const SprintBoard: React.FC<SprintBoardProps> = ({
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const activeIndex = activeColumn.stories.findIndex(
        (s) => s.id === active.id
      );
      ...
      if (story && onStoryMove) {
        ...
        if (targetColumn) {
          const newIndex = targetColumn.stories.findIndex(
            (s) => s.id === active.id
          );
