          ...
          <Card>
            <CardContent>
              ...
              <Typography as="h4" color="info.main">
                {buildTimePrediction.predictedTime}s
              </Typography>
              <Typography as="p" className="text-sm" color="text.secondary">
                ±{' '}
                {Math.ceil(
                  (buildTimePrediction.range.max -
                    buildTimePrediction.range.min) /
                    2
                )}
                s ({(buildTimePrediction.confidence * 100).toFixed(0)}%
                confidence)
              </Typography>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardContent>
            ...
            <Box className="flex gap-2 mt-2">
              {recommendationCounts.critical > 0 && (
                <Chip
                  size="sm"
