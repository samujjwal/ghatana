 ...
 */
export function useDetectReRenders(
  componentName: string,
  ...
  useEffect(() => {
    ...
    if (Object.keys(changedProps).length > 0) {
      console.log(
        `[ReRender] ${componentName} re-rendered due to prop changes:`,
        changedProps
      );
    }
  ...
  const customPropsAreEqual = (
    ...
    if (propsAreEqual) {
      ...
      if (!areEqual) {
        ...
        if (Object.keys(changedProps).length > 0) {
          console.log(
            `[Memo] ${componentName} will re-render due to prop changes:`,
            changedProps
          );
        }
      }
