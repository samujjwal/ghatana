...
describe('Accessibility Utilities', () => {
  ...
  describe('createA11yLabel', () => {
    it('creates label and element props correctly', () => {
      const { labelProps, elementProps } = createA11yLabel(
        'Email',
        'email-input'
      );
  ...
  describe('accessibilityRules', () => {
    ...
    it('has image alt text rule', () => {
      const imgAltRule = accessibilityRules.find(
        (rule) => rule.id === 'a11y-img-alt'
      );
    ...
    it('has button name rule', () => {
      const buttonNameRule = accessibilityRules.find(
        (rule) => rule.id === 'a11y-button-name'
      );
  ...
  describe('runAccessibilityAudit', () => {
    ...
    it('fails audit for inaccessible element', () => {
      ...
      const imgAltIssue = result.issues.find(
        (issue) => issue.code === 'a11y-img-alt'
      );
    ...
    it('audits nested elements', () => {
      ...
      const buttonNameIssue = result.issues.find(
        (issue) => issue.code === 'a11y-button-name'
      );
