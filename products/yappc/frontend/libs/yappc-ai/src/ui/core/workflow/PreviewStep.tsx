  ...
  const buildPreview = useCallback(async () => {
    ...
    try {
      ...
      onChange({
        ...
        buildLog: logs,
        aiSummary:
          'Your changes include a new Login component and authentication hook. The preview shows the login form with email/password inputs.',
      });
    } catch (err) {
      onChange({
        ...value,
    ...
    <Box className="p-6">
      ...
      {isBuilding && (
        <Box className="mb-6">
          ...
          <Paper
            variant="outlined"
            className="mt-4 p-4 overflow-auto max-h-[150px]"
          >
        ...
        <>
          ...
          <Paper variant="outlined" className="mb-4">
            <Box className="p-2 flex items-center gap-2 border-gray-200 dark:border-gray-700 border-b">
              ...
              <Box className="grow" />
              <Chip size="sm" label={value.previewUrl} className="font-mono" />
              ...
              <Tooltip title="Open in new tab">
                <IconButton
                  onClick={() => window.open(value.previewUrl!, '_blank')}
                >
                  <OpenInNewIcon />
                </IconButton>
              </Tooltip>
            ...
            {/* Preview iframe */}
            <Box className="flex justify-center p-4 bg-gray-100">
              <Box
                className="max-w-full rounded overflow-hidden flex items-center justify-center bg-white border border-gray-300"
                style={{
                  width: VIEWPORT_SIZES[value.viewport].width,
                  height: VIEWPORT_SIZES[value.viewport].height,
                }}
              >
      ...
      <Box className="flex justify-between gap-4">
        {onBack && (
          <Button onClick={onBack} disabled={isLoading || isBuilding}>
            Back
          </Button>
        )}
        <Box className="grow" />
