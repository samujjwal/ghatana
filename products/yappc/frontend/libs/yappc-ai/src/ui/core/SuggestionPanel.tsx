...
export type SuggestionType = 'REQUIREMENT' | 'DESIGN' | 'TEST' | 'RISK' | 'ACTION';
...
function getSuggestionIcon(type: SuggestionType): React.ReactNode {
    switch (type) {
        ...
        case 'DESIGN':      return <Code2 size={16} />;
        case 'TEST':        return <TestTube2 size={16} />;
...
function getSuggestionColor(type: SuggestionType): 'default' | 'warning' | 'error' | 'success' | 'info' {
    switch (type) {
...
async function fetchSuggestions(
    ...
    const response = await fetch(`/api/v1/projects/${encodeURIComponent(projectId)}/suggestions`, {
        method: 'POST',
