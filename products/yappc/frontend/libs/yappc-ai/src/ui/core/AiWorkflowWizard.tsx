...
export const AiWorkflowWizard: React.FC<AiWorkflowWizardProps> = ({
  ...
  const handleStepChange = useCallback(
    <K extends keyof WorkflowState>(step: K, data: WorkflowState[K]) => {
  ...
  const handleComplete = useCallback(() => {
    const summary: WorkflowSummary = {
      ...
      filesCreated: state.code.files.filter((f) => f.operation === 'create')
        .length,
      filesModified: state.code.files.filter((f) => f.operation === 'modify')
        .length,
        ...
        return (
          <CompleteStep
            workflowData={{
              workflowId: `wf-${Date.now()}`,
              intent: state.intent.rawIntent,
              filesCreated: state.code.files.filter(
                (f) => f.operation === 'create'
              ).length,
              filesModified: state.code.files.filter(
                (f) => f.operation === 'modify'
              ).length,
              testsGenerated: state.test.tests.length,
              testsPassed: state.test.tests.filter((t) => t.status === 'passed')
                .length,
              testsFailed: state.test.tests.filter((t) => t.status === 'failed')
                .length,
              deploymentUrl: state.deploy.deploymentUrl,
              environment: state.deploy.environment,
              startTime: state.startTime || new Date(),
            ...
            onViewDeployment={() =>
              window.open(state.deploy.deploymentUrl, '_blank')
            }
            onDownloadReport={() => {
  ...
  const content = (
    <Box className="h-full flex flex-col">
      {/* Header */}
      <Box className="p-4 flex items-center gap-2 border-gray-200 dark:border-gray-700 border-b">
        <AIIcon tone="primary" />
      ...
      {/* Step content */}
      <Box className="grow overflow-auto">{renderStep()}</Box>
    </Box>
  );
