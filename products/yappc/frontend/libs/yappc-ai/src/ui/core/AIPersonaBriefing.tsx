...
import {
  useAIInsightsGraphQL,
  usePredictionsGraphQL,
  useAnomaliesGraphQL,
} from '../hooks/useAI.graphql';
 ...
 * AI Persona Briefing Component
 */
export function AIPersonaBriefing({
  persona,
  userId,
  compactMode = false,
}: AIPersonaBriefingProps) {
  ...
  // Generate summary
  const criticalCount =
    anomalies?.filter((a: unknown) => a.severity === 'critical').length || 0;
  const highPriorityInsights =
    insights?.filter(
      (i: unknown) => i.severity === 'high' || i.severity === 'critical'
    ).length || 0;
  const highRiskPredictions =
    predictions?.filter((p: unknown) => p.probability > 0.7).length || 0;
              ...
              {criticalCount > 0 && (
                <Alert severity="error" className="mb-4">
                  ...
                  <Typography as="p" className="text-sm">
                    ...
                    {criticalCount === 1 ? 'issue' : 'issues'} detected that
                ...
                <>
                  ...
                  <Box>
                    ...
                    <Stack direction="row" spacing={1}>
                      ...
                      <Button size="sm" variant="outlined" startIcon={<Info />}>
                        View All Insights
                      </Button>
                    </Stack>
                  </Box>
