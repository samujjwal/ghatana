...
export const DeployStep: React.FC<DeployStepProps> = ({
  ...
  const getStatusIcon = (status: DeploymentStep['status']) => {
    switch (status) {
      case 'success':
        return <SuccessIcon tone="success" />;
      case 'failed':
        return <ErrorIcon tone="danger" />;
      case 'running':
        return <SpeedIcon tone="primary" className="spin" />;
      case 'skipped':
  ...
  const progress =
    (value.steps.filter((s) => s.status === 'success').length /
      value.steps.length) *
    100;
    ...
    <Box className="p-6">
        ...
        <Paper variant="outlined" className="p-6 mb-6">
          <FormControl component="fieldset">
            ...
            <RadioGroup
              ...
              onChange={(e) =>
                handleEnvironmentChange(e.target.value as unknown)
              }
            >
              {ENVIRONMENTS.map((env) => (
                <Paper
                  ...
                  style={{
                    border: value.environment === env.value ? 2 : 1,
                    borderColor:
      ...
      <Box className="flex justify-between gap-4">
        {onBack && (
          <Button
            onClick={onBack}
            disabled={isLoading || value.status === 'deploying'}
          >
            Back
          </Button>
        )}
