...
import {
  ...
  User as AssigneeIcon,
  Label as TagIcon,
  Flag as PriorityIcon,
  ArrowRight as ActionIcon,
  Copy as SimilarIcon,
  Lightbulb as SuggestionIcon,
  ThumbsUp as FeedbackUpIcon,
  ThumbsDown as FeedbackDownIcon,
} from 'lucide-react';
...
import {
  ...
  ListItem,
  ListItemIcon as ListItemAvatar,
  ListItemText,
...
const ListSuggestion: React.FC<{
  ...
  return (
    <ListItem className="hover:bg-black/5">
      <ListItemAvatar>
        <Avatar
      ...
      <ListItemSecondaryAction>
        <Box className="flex gap-1">
          ...
          {onDismiss && (
            <IconButton size="sm" onClick={() => onDismiss(recommendation.id)}>
              <DismissIcon size={16} />
            </IconButton>
          )}
        </Box>
...
export const RecommendationList: React.FC<RecommendationListProps> = ({
  ...
  const [expandedTypes, setExpandedTypes] = useState<Set<RecommendationType>>(
    new Set()
  );
  ...
  return (
    <Paper variant="outlined" className="overflow-hidden">
      <Box className="p-3 flex items-center bg-gray-50">
