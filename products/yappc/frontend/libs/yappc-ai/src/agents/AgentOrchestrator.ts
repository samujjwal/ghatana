...
import type { AgentContext, AgentName, AgentResult, IAIAgent } from './types';
...
export class AgentOrchestrator {
  ...
  getAgent<TInput, TOutput>(
    name: AgentName
  ): IAIAgent<TInput, TOutput> | undefined {
        ...
        stage.steps.map(async (step) => {
          ...
          try {
            ...
            const errorResult: AgentResult<unknown> = {
              ...
              metrics: {
                latencyMs: stepDurations[step.id],
  ...
  async healthCheck(): Promise<
    Record<AgentName, 'healthy' | 'degraded' | 'unhealthy'>
  > {
