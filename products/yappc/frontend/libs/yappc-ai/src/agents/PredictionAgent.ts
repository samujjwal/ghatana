...
> {
   ...
   * Calculate complexity features
   */
  private calculateComplexityFeatures(
    input: PredictionInput
  ): ComplexityFeatures {
    ...
    return {
      estimatedComplexity:
        similarCount > 10 ? 0.8 : similarCount > 5 ? 0.5 : 0.3,
  ...
  private async predictTimeline(
    ...
    const dataQuality = Math.min(features.historical / 20, 1);
    const velocityConfidence =
      features.velocity.velocityVariance < 0.3 ? 0.9 : 0.6;
  ...
  private async calculateRiskScore(
    ...
    const historicalRisk =
      features.historical < 5 ? 0.7 : features.historical < 20 ? 0.4 : 0.2;
