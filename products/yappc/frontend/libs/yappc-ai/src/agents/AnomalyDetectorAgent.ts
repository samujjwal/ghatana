...
> {
  ...
  protected async processRequest(
    ...
    const baseline =
      input.baseline || this.calculateBaseline(input.currentMetrics);
  ...
  private async runLSTMDetection(
    ...
    for (let i = windowSize; i < values.length - windowSize; i++) {
      ...
      const relativeChange =
        baseline.mean !== 0 ? change / baseline.mean : change;
      ...
      if (relativeChange > 0.3) {
        ...
        anomalies.push({
          ...
          deviationPercent:
            Math.round(((nextMean - prevMean) / prevMean) * 1000) / 10,
        });
  ...
  private generateRecommendations(
    ...
    if (warningAnomalies.length > 2) {
      recommendations.push({
        ...
        description:
          'Multiple warning-level anomalies detected. Consider scheduling a review.',
        priority: 'medium',
      });
    }
    ...
    if (velocityAnomalies.length > 0) {
      recommendations.push({
        ...
        description:
          'Velocity anomalies detected. Review team capacity and sprint planning.',
        priority: velocityAnomalies.some((a) => a.severity === 'critical')
          ? 'high'
          : 'medium',
      });
    }
