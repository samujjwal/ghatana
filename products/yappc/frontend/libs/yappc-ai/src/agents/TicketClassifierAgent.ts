...
> {
  ...
  private async suggestAssignees(
    ...
    return assignees
      .filter((assignee) =>
        assignee.expertise.some((exp) =>
          categories.includes(exp as TicketCategory)
        )
      )
  ...
  private async findSimilarTickets(
    ...
    return input.historicalData
      .map((ticket) => {
        ...
        const similarity =
          commonKeywords.length / Math.max(inputKeywords.length, 1);
  ...
  private calculateConfidence(
    ...
    const descriptionBonus =
      input.description && input.description.length > 100 ? 0.1 : 0;

    // Higher confidence with historical data
    const historyBonus =
      input.historicalData && input.historicalData.length > 10 ? 0.1 : 0;
