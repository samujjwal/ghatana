...
> {
  ...
  private buildParsePrompt(input: QueryParserInput): string {
    ...
    if (input.recentQueries?.length) {
      parts.push(
        `Recent queries: ${input.recentQueries.slice(0, 3).join(', ')}`
      );
    }
  ...
  private parseAIResponse(
    content: string
  ): Omit<QueryParserOutput, 'suggestions' | 'alternatives'> {
    try {
  ...
  private validateIntent(intent: unknown): QueryIntent {
    ...
    if (
      ...
      validIntents.includes(intent as QueryIntent)
    ) {
  ...
  private validateFilters(filters: unknown): ItemFilter {
    ...
    if (Array.isArray(f.status)) {
      result.status = f.status.filter(
        (s): s is string => typeof s === 'string'
      );
    }
    if (Array.isArray(f.priority)) {
      result.priority = f.priority.filter(
        (p): p is string => typeof p === 'string'
      );
    }
    if (Array.isArray(f.phaseIds)) {
      result.phaseIds = f.phaseIds.filter(
        (p): p is string => typeof p === 'string'
      );
    }
