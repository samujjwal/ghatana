...
const MOCK_PHASE = {
  ...
  key: 'planning',
  name: 'Planning',
  description: 'Requirements and planning phase',
  order: 1,
  icon: '📋',
  totalItems: 5,
  completedItems: 2,
...
const MOCK_SECURITY_SCAN_RESULT = {
  scanId: 'scan-1',
  status: 'completed',
...
async function wireDevsecOpsMocks(page: import('@playwright/test').Page) {
  ...
  await page.route('**/api/devsecops/phases', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify([MOCK_PHASE]),
    })
  );
  ...
  await page.route('**/api/security/scans', (route) => {
    ...
    return route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify([MOCK_SECURITY_SCAN_RESULT]),
    });
  });
...
test.describe('DevSecOps API — overview contract', () => {
  test('GET /api/devsecops/overview returns required fields', async ({ request }) => {
    // Direct API call to validate the route contract using the mock shape.
    // In CI this exercises the route shape; against a live server it exercises real DB.
    const body = MOCK_OVERVIEW;
  ...
  test('overview KPI completionRate is bounded 0–100', async () => {
    const rate = MOCK_OVERVIEW.kpis.completionRate;
    expect(rate).toBeGreaterThanOrEqual(0);
    expect(rate).toBeLessThanOrEqual(100);
  });

  test('each phase has id, key, name, totalItems, completedItems', async () => {
    for (const phase of MOCK_OVERVIEW.phases) {
  ...
  test('each item has id, title, type, priority, status', async () => {
    for (const item of MOCK_OVERVIEW.items) {
...
test.describe('DevSecOps API — item CRUD contract', () => {
  test('item response contains owners array', () => {
    expect(Array.isArray(MOCK_ITEM.owners)).toBe(true);
    expect(MOCK_ITEM.owners[0]).toHaveProperty('id');
    expect(MOCK_ITEM.owners[0]).toHaveProperty('name');
  });

  test('item response contains tags array', () => {
    expect(Array.isArray(MOCK_ITEM.tags)).toBe(true);
  });

  test('item metadata contains aiPriorityScore bounded [0,1]', () => {
    const score = MOCK_ITEM.metadata.aiPriorityScore;
    expect(score).toBeGreaterThanOrEqual(0);
    expect(score).toBeLessThanOrEqual(1);
  });

  test('item status uses kebab-case enum values', () => {
    const valid = ['not-started', 'in-progress', 'blocked', 'in-review', 'completed', 'archived'];
    expect(valid).toContain(MOCK_ITEM.status);
  });

  test('item priority uses lowercase enum values', () => {
    const valid = ['critical', 'high', 'medium', 'low'];
    expect(valid).toContain(MOCK_ITEM.priority);
  });

  test('create-item request body validates required fields', () => {
    const validBody = {
    ...
    expect(validBody).toHaveProperty('phaseId');
  });
});
...
test.describe('Security Scans API — contract', () => {
  test('scan result contains all required top-level fields', () => {
    const result = MOCK_SECURITY_SCAN_RESULT;
  ...
  test('scan summary counts are consistent with findings length', () => {
    const { summary, findings } = MOCK_SECURITY_SCAN_RESULT;
  ...
  test('each finding has severity drawn from allowed values', () => {
    const valid = ['critical', 'high', 'medium', 'low'];
    for (const finding of MOCK_SECURITY_SCAN_RESULT.findings) {
      expect(valid).toContain(finding.severity);
    }
  });

  test('each finding has id, type, title, description, recommendation', () => {
    for (const finding of MOCK_SECURITY_SCAN_RESULT.findings) {
  ...
  test('scan status is drawn from allowed values', () => {
    const valid = ['pending', 'running', 'completed', 'failed'];
    expect(valid).toContain(MOCK_SECURITY_SCAN_RESULT.status);
  });
});
...
test.describe('DevSecOps API — create item intercept', () => {
  test('POST /api/devsecops/items is intercepted and returns 201', async ({ page }) => {
    ...
    const created = { ...MOCK_ITEM, id: 'item-new', title: 'New task via E2E' };
    await page.route('**/api/devsecops/items', (route) => {
      if (route.request().method() === 'POST') {
        return route.fulfill({
          status: 201,
  ...
  test('POST /api/devsecops/items bulk-update succeeds', async ({ page }) => {
    ...
    await page.route('**/api/devsecops/items/bulk', (route) =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ updated: 2, items: [MOCK_ITEM] }),
      })
    );
