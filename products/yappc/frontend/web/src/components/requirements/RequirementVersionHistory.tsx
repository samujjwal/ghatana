...
export const RequirementVersionHistory: React.FC<RequirementVersionHistoryProps> = ({ versions }) => {
  return (
    <Box className="space-y-2" data-testid="requirement-version-history">
      ...
      {versions.length === 0 && (
        <Typography className="text-sm text-gray-500">No versions available.</Typography>
      )}
      ...
      {versions.map((version) => (
        <Card key={version.id}>
          <CardContent className="space-y-1 p-3">
            ...
            <Typography className="text-sm text-gray-700">{version.summary}</Typography>
            <Typography className="text-xs text-gray-500">By {version.createdBy}</Typography>
