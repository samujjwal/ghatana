...
import type {
  ProductCompletenessGap,
  ProductCompletenessOverlayModel,
  ProductCompletenessRouteRow,
  ProductRouteLifecycle,
} from '@/lib/product/productCompletenessOverlay';

export interface ProductCompletenessOverlayProps {
  model: ProductCompletenessOverlayModel;
  collapsed?: boolean;
  onDismiss?: () => void;
}

const lifecycleTone: Record<ProductRouteLifecycle, string> = {
...
function RouteCoverageRow({ route }: { route: ProductCompletenessRouteRow }) {
  return (
...
function GapRow({ gap }: { gap: ProductCompletenessGap }) {
  return (
...
export function ProductCompletenessOverlay({
  model,
  collapsed = false,
  onDismiss,
}: ProductCompletenessOverlayProps) {
    ...
    return (
      <div className="fixed bottom-12 right-4 z-40">
        ...
        >
          ...
          Product coverage {model.totals.stableCoveragePercent}%
        </Button>
      </div>
    );
  }
  ...
  return (
    <aside
      ...
      aria-label="Product route completeness"
    >
      <header className="flex items-start justify-between gap-3 border-b border-border px-4 py-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <CircleDashed className="h-4 w-4 text-info-color" />
            <h2 className="text-sm font-semibold text-text-primary">Product route completeness</h2>
          </div>
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            variant="ghost"
            size="small"
            aria-label="Collapse Product route completeness"
          ...
          {onDismiss && (
            <Button
              ...
              variant="ghost"
              size="small"
              aria-label="Dismiss Product route completeness"
              onClick={onDismiss}
            >
              <X className="h-4 w-4" />
            </Button>
      ...
      <div className="space-y-3 px-4 py-3">
        ...
        <ul className="space-y-2" aria-label="Lowest scoring product routes">
