...
describe('PHASE_ZONE_POSITIONS', () => {
    ...
    it('has increasing X positions', () => {
        ...
        for (const phase of PHASE_ORDER) {
            const currentX = PHASE_ZONE_POSITIONS[phase].start;
...
describe('usePhaseNavigation', () => {
    ...
    it('returns currentPhaseIndex', () => {
        ...
        expect(result.current.phaseIndex).toBeGreaterThanOrEqual(0);
        expect(result.current.phaseIndex).toBeLessThan(7);
    });
    ...
    it('canNavigatePrevious returns false for first phase', () => {
        ...
        if (result.current.currentPhase === LifecyclePhase.INTENT) {
            expect(result.current.previousPhase).toBeNull();
        }
    });
    ...
    it('provides navigation functions', () => {
        ...
        expect(typeof result.current.isPhaseVisible).toBe('function');
        expect(typeof result.current.isCurrentPhase).toBe('function');
        expect(typeof result.current.getPhasePosition).toBe('function');
    });
