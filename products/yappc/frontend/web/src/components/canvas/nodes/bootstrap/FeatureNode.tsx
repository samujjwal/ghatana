...
const PRIORITY_CONFIG: Record<FeaturePriority, { label: string; color: string; dotColor: string }> = {
  high: { label: 'High', color: 'text-destructive', dotColor: 'bg-destructive-bg' },
  medium: { label: 'Medium', color: 'text-warning-color', dotColor: 'bg-warning-bg' },
  low: { label: 'Low', color: 'text-fg-muted', dotColor: 'bg-surface-muted' },
};
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2">
        <h3 className="text-sm font-medium text-fg line-clamp-2">{data.label}</h3>
        {data.description && (
      ...
      {/* Metadata */}
      <div className="px-3 py-2 border-t border-current/10 flex items-center justify-between text-xs text-fg-muted">
      ...
      {expanded && (
        <div className="px-3 py-2 border-t border-current/10 space-y-2">
          {data.techStack && data.techStack.length > 0 && (
            <div>
              <span className="text-xs font-medium text-fg">Tech Stack:</span>
              <div className="flex flex-wrap gap-1 mt-1">
                {data.techStack.map((tech) => (
                  <span
                    key={tech}
                    className="px-1.5 py-0.5 bg-white/50 rounded text-xs text-fg-muted"
                  >
                    {tech}
                  </span>
                ))}
