...
export class WorkflowAgentApiClient extends BaseDashboardApiClient {
  ...
  async executeBatch(
    requests: ExecuteAgentRequest[]
  ): Promise<ApiResponse<BatchExecutionResponse>> {
    if (this.mode === 'mock') {
      return this.createMockResponse({
    ...
    return this.post<BatchExecutionResponse>('/agents/execute/batch', {
      requests,
    });
  }
        ...
        agents: [
          {
            id: 'agent-1',
            name: 'Code Reviewer',
            role: 'code-reviewer' as WorkflowAgentRole,
            enabled: true,
          ...
          },
          {
            id: 'agent-2',
            name: 'Test Writer',
            role: 'test-writer' as WorkflowAgentRole,
            enabled: true,
            registeredAt: new Date().toISOString(),
          },
          {
            ...
            role: 'security-scanner' as WorkflowAgentRole,
            enabled: true,
            registeredAt: new Date().toISOString(),
          },
        ],
  ...
  async getAgentsByRole(
    role: WorkflowAgentRole
  ): Promise<ApiResponse<AgentsByRoleResponse>> {
    if (this.mode === 'mock') {
      return this.createMockResponse({
        role,
        agents: [
