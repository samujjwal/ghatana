...
export class RequirementsApiClient extends BaseDashboardApiClient {
  ...
  async updateRequirement(
    ...
    return this.put<RequirementResponse>(
      `${this.basePath}/${requirementId}`,
      updates
    );
  }
  ...
  async queryRequirements(
    ...
    return this.get<PaginatedResponse<RequirementResponse>>(
      this.basePath,
  ...
  async getFunnel(
    projectId: string
  ): Promise<ApiResponse<RequirementsFunnelResponse>> {
    return this.get<RequirementsFunnelResponse>(`${this.basePath}/funnel`, {
      projectId,
    });
  }
    ...
    const mockRequirements: RequirementResponse[] = [
      {
        ...
        acceptanceCriteria: [
          {
            id: 'ac-1',
            description: 'Users can login via SSO',
            verified: true,
            verifiedBy: 'user-2',
