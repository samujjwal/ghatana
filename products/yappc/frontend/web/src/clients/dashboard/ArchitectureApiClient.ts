...
export class ArchitectureApiClient extends BaseDashboardApiClient {
  ...
  async getImpactAnalysis(
    resourceId: string
  ): Promise<ApiResponse<ArchitectureImpactResponse>> {
    return this.get<ArchitectureImpactResponse>(
      `${this.basePath}/impact/${resourceId}`
    );
  }
  ...
  async getDependencyGraph(
    resourceId: string
  ): Promise<ApiResponse<DependencyGraphResponse>> {
    return this.get<DependencyGraphResponse>(
      `${this.basePath}/dependencies/${resourceId}`
    );
  }
  ...
  async getTechDebt(projectId: string): Promise<ApiResponse<TechDebtSummary>> {
    return this.get<TechDebtSummary>(`${this.basePath}/tech-debt`, {
      projectId,
    });
  }
   ...
   */
  async getPatternWarnings(
    projectId?: string
  ): Promise<ApiResponse<PatternWarning[]>> {
    return this.get<PatternWarning[]>(`${this.basePath}/patterns`, {
      projectId,
    });
  }
  ...
  async simulateImpact(
    request: SimulateImpactRequest
  ): Promise<ApiResponse<ArchitectureImpactResponse>> {
    return this.post<ArchitectureImpactResponse>(
  ...
  async getCircularDependencies(
    ...
    return this.get<string[][]>(`${this.basePath}/circular-dependencies`, {
      projectId,
    });
  }
   ...
   */
  async getCouplingMetrics(componentId: string): Promise<
    ApiResponse<{
  ...
  private registerDefaultMocks(): void {
    ...
    this.registerMock<ArchitectureImpactResponse>(
      `${this.basePath}/impact/comp-1`,
      {
        resourceId: 'comp-1',
        ...
        patternWarnings: [
          {
            ...
            description:
              'AuthService <-> UserService creates a circular dependency',
            severity: 'HIGH',
          ...
          },
          {
            ...
            recommendation:
              'Split into AuthenticationService and AuthorizationService',
          },
        ],
        techDebt: {
          score: 65,
    ...
    this.registerMock<DependencyGraphResponse>(
      `${this.basePath}/dependencies/service-1`,
      {
        rootId: 'service-1',
        nodes: [
          {
        ...
        clusters: [
          {
            ...
            nodeIds: ['auth-service', 'user-service', 'order-service'],
            color: '#4CAF50',
          },
          {
            id: 'infra-layer',
            label: 'Infrastructure Layer',
            nodeIds: ['user-repo', 'db-client', 'cache-client'],
            color: '#2196F3',
          },
        ],
        statistics: {
    ...
    this.registerMock<PatternWarning[]>(`${this.basePath}/patterns`, [
      {
        pattern: 'Circular Dependency',
        description:
          'AuthService and UserService have bidirectional dependencies',
        severity: 'HIGH',
