...
export class AuthorizationApiClient extends BaseDashboardApiClient {
  ...
  async checkPermission(
    ...
    return this.post<CheckPermissionResponse>(
      `${this.basePath}/check`,
      request
    );
  }
   ...
   */
  async getPersonaPermissions(
    persona: Persona
  ): Promise<ApiResponse<PersonaPermissionsResponse>> {
  ...
  async canAccess(
    ...
    return this.get<boolean>(`${this.basePath}/can-access`, {
      resource,
      action,
    });
  }
  ...
  private registerDefaultMocks(): void {
    ...
    this.registerMock<UserPermissionsResponse>(
      `${this.basePath}/users/user-1/permissions`,
      {
        userId: 'user-1',
        persona: 'Product Owner',
        role: 'PRODUCT_OWNER',
        ...
        deniedPermissions: ['admin:*', 'system:*'],
