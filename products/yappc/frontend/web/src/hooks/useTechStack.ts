...
const techStackAtomFamily = new Map<
  string,
  ReturnType<typeof atomWithStorage<TechStack>>
>();
...
export function useTechStack(
    ...
    (
      ...
      setTechStack((prev) => {
        ...
        const updated = {
          ...
          detectionMethod:
            prev.detectionMethod === 'auto' ? 'mixed' : prev.detectionMethod,
        } as TechStack;
  ...
  const addTechnologyById = useCallback(
  ...
  const updateTechnology = useCallback(
    (techId: string, updates: Partial<Omit<Technology, 'id'>>) => {
      setTechStack((prev) => ({
        ...prev,
    ...
    async (files: string[]) => {
      ...
      if (detected.length > 0) {
        setTechStack((prev) => ({
          ...
          lastUpdated: new Date().toISOString(),
          detectionMethod: prev.technologies.some((t) => t.source === 'manual')
            ? 'mixed'
            : 'auto',
        }));
      }
