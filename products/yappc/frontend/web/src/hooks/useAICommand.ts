...
function parseIntent(intent: string): ParsedIntent {
  ...
  if (
    ...
    lowerIntent.includes('library') ||
    lowerIntent.includes('package') ||
    lowerIntent.includes('component')
  ) {
    projectType = 'UI';
  } else if (
    lowerIntent.includes('landing') ||
    lowerIntent.includes('website') ||
    lowerIntent.includes('page')
  ) {
    projectType = 'UI';
  } else if (
    lowerIntent.includes('desktop') ||
    lowerIntent.includes('windows') ||
    lowerIntent.includes('macos')
  ) {
    projectType = 'DESKTOP';
  }
  ...
  // Smart inferences
  if (
    features.includes('Task Management') ||
    features.includes('CMS Features') ||
    features.includes('E-commerce')
  ) {
  ...
  if (
    ...
    lowerIntent.includes('edit')
  ) {
    type = 'modify';
  } else if (
    lowerIntent.includes('add') ||
    lowerIntent.includes('generate') ||
    lowerIntent.includes('create component')
  ) {
    type = 'generate';
  } else if (
    lowerIntent.includes('go to') ||
    lowerIntent.includes('open') ||
    lowerIntent.includes('show')
  ) {
    type = 'navigate';
  } else if (
    lowerIntent.includes('help') ||
    lowerIntent.includes('how') ||
    lowerIntent.includes('what')
  ) {
    type = 'help';
  }
...
function generateCanvasData(parsed: ParsedIntent) {
  ...
  if (
    parsed.projectType === 'BACKEND' ||
    parsed.projectType === 'FULL_STACK' ||
    parsed.features.includes('API Layer')
  ) {
  ...
  if (
    parsed.features.includes('Database') ||
    parsed.projectType === 'BACKEND' ||
    parsed.projectType === 'FULL_STACK'
  ) {
...
export function useAICommand(): UseAICommandResult {
  ...
  const { mutate: processIntentMutation, isPending: isProcessing } =
    useMutation({
      ...
      onError: (err) => {
        setError(
          err instanceof Error ? err : new Error('Failed to process intent')
        );
        setResponse(null);
      },
    });
  ...
  const updateResponse = useCallback(
    (updates: Partial<AIResponse['details']>) => {
      if (!response) return;
      setResponse({
        ...response,
        details: {
  ...
  const confirmAction = useCallback(async () => {
    ...
    if (response.type === 'create' && response.details.name) {
      try {
        ...
        if (
          response.details.techStack &&
          Array.isArray(response.details.techStack)
        ) {
          response.details.techStack.forEach((tech: string, index: number) => {
            if (index < 3) {
              const compType = componentTypes[index] || {
                type: 'component',
                label: tech,
              };
              elements.push({
