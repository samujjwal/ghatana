...
import { getArtifactsForPhase } from '@/shared/types/lifecycle-artifacts';
import type { LifecycleArtifactService } from './LifecycleArtifactService';
...
export class PhaseGateService {
  ...
  async checkGateStatus(
    ...
    const context = await this.buildGateContext(
      projectId,
      gate.fromPhase,
      gate.toPhase
    );
  ...
  async transition(request: TransitionRequest): Promise<TransitionResult> {
    const { projectId, fromPhase, toPhase, userId, bypass, bypassReason } =
      request;
  ...
  async getPhaseProgress(
    ...
    // Calculate artifact progress derived from LIFECYCLE_ARTIFACT_CATALOG.
    const expectedTotal = Math.max(1, getArtifactsForPhase(phase).length);
    const completedCount =
      phaseIndex < currentIndex
        ? expectedTotal
        : phaseIndex === currentIndex
          ? Math.floor(expectedTotal / 2)
          : 0;
...
export function usePhaseGates(projectId: string) {
  ...
  const [gateStatuses, setGateStatuses] = React.useState<
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error('Failed to load phase state')
      );
    } finally {
      setLoading(false);
    }
