...
function cloneContainer(value: unknown): PatchContainer {
...
function getContainerValue(container: PatchContainer, key: string): unknown {
...
function getPropertyChanges(
  ...
  const previous = isRecord(oldObj) ? oldObj : {};
  const next = isRecord(newObj) ? newObj : {};
...
function applyAdd(doc: unknown, pathParts: string[], value: unknown): unknown {
  const result = cloneContainer(doc);
  let current: PatchContainer = result;

  for (let i = 0; i < pathParts.length - 1; i++) {
    const part = pathParts[i];
    const nextValue = cloneContainer(getContainerValue(current, part));
