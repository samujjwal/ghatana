...
import type { Point, Viewport } from './infinite-canvas';
import {
...
import type { MinimapNode, MinimapConfig } from './minimap';
...
export function MinimapPanel({
  ...
  const renderMinimap = useCallback(() => {
    ...
    nodes.forEach((node) => {
      ...
      const minimapSize = {
        width:
          (node.width / canvasBounds.width) *
          (config.width - 2 * config.padding),
        height:
          (node.height / canvasBounds.height) *
