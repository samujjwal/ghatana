...
export function isNodeInViewport(
  ...
  const viewportRight =
    viewportLeft + viewport.width / viewport.zoom + margin * 2;
  const viewportBottom =
    viewportTop + viewport.height / viewport.zoom + margin * 2;
...
export class SpatialIndex {
  ...
  private getNodeCells(node: HierarchicalNode): string[] {
    ...
    const endX = Math.floor(
      (node.position.x + node.size.width) / this.gridSize
    );
    const endY = Math.floor(
      (node.position.y + node.size.height) / this.gridSize
    );
