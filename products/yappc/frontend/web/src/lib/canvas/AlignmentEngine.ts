...
export class AlignmentEngine {
  ...
  private alignCenterX(nodes: HierarchicalNode[]): void {
    const avgX =
      nodes.reduce((sum, n) => sum + n.position.x + n.size.width / 2, 0) /
      nodes.length;
  ...
  private alignCenterY(nodes: HierarchicalNode[]): void {
    const avgY =
      nodes.reduce((sum, n) => sum + n.position.y + n.size.height / 2, 0) /
      nodes.length;
  ...
  private alignBottom(nodes: HierarchicalNode[]): void {
    const maxBottom = Math.max(
      ...nodes.map((n) => n.position.y + n.size.height)
    );
...
export class SmartGuidesSystem {
  ...
  constructor(private getAllNodes: () => HierarchicalNode[]) {}

  /**
   * Get guides for current drag operation
   */
  getGuidesForNode(
    draggedNode: HierarchicalNode,
    excludeIds: string[] = []
  ): Guide[] {
    const allNodes = this.getAllNodes().filter(
    ...
    guides.push(
      ...this.createGuidesFromPositions(leftEdges, 'vertical', allNodes, 'left')
    );

    // Right edge guides
    const rightEdges = allNodes.map((n) => n.position.x + n.size.width);
    guides.push(
      ...this.createGuidesFromPositions(
        rightEdges,
        'vertical',
        allNodes,
        'right'
    ...
    guides.push(
      ...this.createGuidesFromPositions(topEdges, 'horizontal', allNodes, 'top')
    );
  ...
  snapToGuides(
    ...
    for (const guide of guides.filter((g) => g.type === 'vertical')) {
      ...
      // Check right edge
        ...
        Math.abs(position.x + size.width - guide.position) < this.snapThreshold
      ) {
      ...
      // Check center
        ...
        Math.abs(position.x + size.width / 2 - guide.position) <
        this.snapThreshold
      ) {
    ...
    for (const guide of guides.filter((g) => g.type === 'horizontal')) {
      ...
      // Check bottom edge
        ...
        Math.abs(position.y + size.height - guide.position) < this.snapThreshold
      ) {
      ...
      // Check center
        ...
        Math.abs(position.y + size.height / 2 - guide.position) <
        this.snapThreshold
      ) {
