...
export class ZoomManager {
  ...
  getCurrentZoomRange(): ZoomRange | undefined {
    return ZOOM_RANGES.find(
      (range) =>
        this.currentZoom >= range.range[0] && this.currentZoom < range.range[1]
    );
  }
  ...
  isElementVisible(elementType: ElementType): boolean {
    ...
    return (
      range.showElements.includes(elementType) &&
  ...
  zoomTo(
    ...
    return new Promise((resolve) => {
      ...
      const animate = (currentTime: number) => {
        ...
        this.currentZoom =
          startZoom + (this.targetZoom - startZoom) * easedProgress;
  ...
  onZoomChange(callback: (zoom: number) => void): () => void {
    ...
    return () => {
      this.onZoomChangeCallbacks = this.onZoomChangeCallbacks.filter(
        (cb) => cb !== callback
      );
    };
  }
  ...
  onViewportChange(callback: (viewport: ViewportState) => void): () => void {
    ...
    return () => {
      this.onViewportChangeCallbacks = this.onViewportChangeCallbacks.filter(
        (cb) => cb !== callback
      );
    };
  }
  ...
  private pushHistory(state: ViewportState): void {
    ...
    if (this.currentHistoryIndex < this.zoomHistory.length - 1) {
      this.zoomHistory = this.zoomHistory.slice(
        0,
