...
export class HierarchyManager {
  ...
  private listeners: Map<HierarchyEventType, Array<(data: unknown) => void>> =
    new Map();
  ...
  addChild(
    parentId: string,
    childNode: Partial<HierarchicalNode>
  ): HierarchicalNode {
  ...
  private layoutGrid(
    ...
    const cellWidth =
      (container.size.width -
        ...
        (columns - 1) * container.childSpacing) /
      columns;
