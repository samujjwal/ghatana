...
export class ABTestingService extends EventEmitter {
  ...
  private banditState: Map<string, { successes: number; failures: number }[]> =
    new Map();
   ...
   */
  createExperiment(
    config: Omit<Experiment, 'id' | 'createdAt' | 'updatedAt'>
  ): Experiment {
  ...
  analyzeExperiment(experimentId: string): ExperimentResults {
    ...
    const { winner, confidence, isSignificant } = this.determineWinner(
      variantMetrics,
      experiment
    );
  ...
  private calculateVariantMetrics(
    ...
    const requests = interactions.filter(
      (i) => i.eventType === 'request' || i.eventType === 'response'
    );
    const successful = requests.filter((i) => i.isSuccess);
    const failed = requests.filter((i) => !i.isSuccess);
    const feedbacks = interactions.filter(
      (i) => i.eventType === 'feedback' && i.satisfactionScore
    );
    ...
    return {
      ...
      avgSatisfactionScore:
        feedbacks.length > 0
          ? feedbacks.reduce((sum, f) => sum + f.satisfactionScore!, 0) /
            feedbacks.length
          : 0,
  ...
  private determineWinner(
    ...
    const sorted = [...metrics].sort(
      (a, b) => b.avgSatisfactionScore - a.avgSatisfactionScore
    );
  ...
  private generateRecommendation(
    ...
    const costComparison = metrics
      ...
      .map((m) => {
        const costDiff = (
          ((winnerMetrics.avgCostPerRequest - m.avgCostPerRequest) /
            m.avgCostPerRequest) *
          100
        ).toFixed(1);
  ...
  private updateModelHealth(interaction: InteractionEvent): void {
    ...
    health.errorRate =
      recentInteractions.length > 0 ? failures / recentInteractions.length : 0;
