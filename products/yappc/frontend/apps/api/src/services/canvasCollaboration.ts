...
export class CanvasCollaborationService {
        ...
        try {
          connection.socket.send(
            JSON.stringify({
              type: 'error',
              message:
                'Unsupported WebSocket endpoint. Use /ws/canvas/:projectId',
            })
          );
        } finally {
  ...
  private sendUsersList(ws: WebSocket, room: CanvasRoom) {
    const users = Array.from(room.collaborators.values()).map(
      ({ ws: _, ...user }) => user
    );
  ...
  private handleNodeUpdate(
    ...
    this.broadcast(
      room,
      { type: 'node-update', userId, nodeId, updates },
      userId
    );
  }
  ...
  private cleanupInactiveRooms() {
    ...
    this.rooms.forEach((room, projectId) => {
      if (
        room.collaborators.size === 0 &&
        now - room.lastActivity > inactiveThreshold
      ) {
