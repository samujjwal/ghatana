...
export class SemanticCacheService extends EventEmitter {
  ...
  async lookup(
    ...
    if (bestMatch) {
      ...
      if (options.model) {
        this.hitsByModel[options.model] =
          (this.hitsByModel[options.model] || 0) + 1;
      }
      if (options.feature) {
        this.hitsByFeature[options.feature] =
          (this.hitsByFeature[options.feature] || 0) + 1;
      }
  ...
  getStats(): CacheStats {
    ...
    const avgHitLatency =
      this.totalHits > 0 ? this.totalHitLatencyMicros / this.totalHits : 0;
    const avgSavedLatency =
      this.totalHits > 0 ? this.totalSavedLatencyMs / this.totalHits : 0;
  ...
  generateEdgeCacheKey(embedding: number[], model: string): string {
    ...
    const hash = embedding.reduce(
      (acc, val) => acc + Math.round(val * 1000),
      0
    );
