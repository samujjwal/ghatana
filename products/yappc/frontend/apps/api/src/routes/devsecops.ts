...
export default async function devsecopsRoutes(fastify: FastifyInstance) {
      ...
      const personaDashboards = {
        developer: {
          focus: items
            ...
            .map(transformItem),
          metrics: {
            tasksCompleted: items.filter(
              (i) => i.status === 'COMPLETED' && i.type === 'TASK'
            ).length,
            bugsFixed: items.filter(
              (i) => i.status === 'COMPLETED' && i.type === 'BUG'
            ).length,
          },
        },
        security: {
          focus: items
            ...
            .map(transformItem),
          metrics: {
            openVulnerabilities: items.filter(
              (i) => i.type === 'SECURITY_ISSUE' && i.status !== 'COMPLETED'
            ).length,
            securityTasksCompleted: items.filter(
              (i) => i.type === 'SECURITY_ISSUE' && i.status === 'COMPLETED'
            ).length,
          },
        },
      ...
      const item = await prisma.item.create({
        data: {
          ...
          priority: body.priority
            ? (mapPriorityToEnum(body.priority) as unknown)
            : 'MEDIUM',
          status: body.status
            ? (mapStatusToEnum(body.status) as unknown)
            : 'NOT_STARTED',
          phaseId: phase.id,
  ...
  fastify.patch(
    ...
    async (
      ...
      if (body.priority !== undefined)
        updateData.priority = mapPriorityToEnum(body.priority);
        ...
        data: {
          ...
          items: phase.items.map(
            (item: {
              ...
              title: item.title,
              status:
                item.status?.toLowerCase().replace('_', '-') ?? 'not-started',
        ...
        data: [
          {
            ...
            description: 'High-level overview of DevSecOps progress',
            type: 'summary',
          },
          {
            id: 'release',
  ...
  fastify.get<{ Params: { reportId: string } }>(
    ...
    async (request, reply) => {
      ...
      const completedItems = await prisma.item.count({
        where: { status: 'COMPLETED' },
      });
      const blockedItems = await prisma.item.count({
        where: { status: 'BLOCKED' },
      });
      const inProgressItems = await prisma.item.count({
      ...
      if (blockedItems > 0) {
        risks.push({
          severity: 'high',
          description: `${blockedItems} items are currently blocked`,
        });
      }
      if (completedItems / Math.max(totalItems, 1) < 0.5) {
        risks.push({
          severity: 'medium',
          description: 'Completion rate is below 50%',
        });
      }
      ...
      for (const phase of phases) {
        if (phase.riskScore && phase.riskScore > 70) {
          risks.push({
            severity: 'high',
            description: `${phase.title} phase has elevated risk score`,
          });
        }
      }
      ...
      return reply.send({
        data: {
          id: reportId,
          title:
            reportTitles[reportId] ??
            `${reportId.charAt(0).toUpperCase() + reportId.slice(1)} Report`,
