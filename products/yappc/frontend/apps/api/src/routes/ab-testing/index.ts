...
import {
  ...
  ExperimentStatus,
  AllocationStrategy,
} from '../../services/ab-testing/ABTestingService';
import {
  requirePermission,
  requireRole,
...
export async function abTestingRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get<{ Params: GetAssignmentRequest }>(
    ...
    async (request, reply) => {
      const userId = (request as unknown).user?.id || 'anonymous';
      const variant = abTestingService.assignUser(
  ...
  fastify.get<{ Params: { model: string } }>(
    '/health/:model',
    async (request, reply) => {
      const health = abTestingService.getModelHealth(
        request.params.model as unknown
      );
  ...
  fastify.get<{ Params: { model: string } }>(
    '/fallback/:model',
    async (request, reply) => {
      const fallback = abTestingService.getFallbackModel(
        request.params.model as unknown
      );
