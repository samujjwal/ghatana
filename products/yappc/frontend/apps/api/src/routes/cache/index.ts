...
} from '../../services/cache/SemanticCacheService';
...
export async function semanticCacheRoutes(
  ...
  fastify.post<{ Body: InvalidateRequest }>(
    ...
    async (request, reply) => {
      const pattern: CacheInvalidationPattern = {
        ...
        queryPattern: request.body.queryPattern
          ? new RegExp(request.body.queryPattern)
          : undefined,
        olderThan: request.body.olderThan
          ? new Date(request.body.olderThan)
          : undefined,
      };
