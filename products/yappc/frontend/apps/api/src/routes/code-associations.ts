...
const codeAssociationsRoutes: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.get(
    '/api/artifacts/:artifactId/code-associations',
    async (request, reply) => {
      ...
      try {
        ...
        return reply
          .status(500)
          .send({ error: 'Failed to fetch code associations' });
      }
    }
  );
   ...
   */
  fastify.post(
    '/api/code-associations',
    { preHandler: requirePermission('workflow', 'create') },
    async (request, reply) => {
      const { artifactId, codeArtifactId, relationship, metadata } =
        request.body as {
          artifactId: string;
      ...
      try {
        ...
        return reply
          .status(500)
          .send({ error: 'Failed to create code association' });
      }
    }
  );
   ...
   */
  fastify.delete(
    '/api/code-associations/:associationId',
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (error.code === 'P2025') {
          return reply
            .status(404)
            .send({ error: 'Code association not found' });
        }

        return reply
          .status(500)
          .send({ error: 'Failed to delete code association' });
      }
    }
  );
   ...
   */
  fastify.get(
    '/api/code-associations/stats/:artifactId',
    async (request, reply) => {
      const { artifactId } = request.params as { artifactId: string };
      ...
      try {
        ...
        return reply
          .status(500)
          .send({ error: 'Failed to fetch association stats' });
      }
    }
  );
};
