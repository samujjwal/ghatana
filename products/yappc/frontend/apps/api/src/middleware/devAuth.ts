  ...
  if (!devUserId) {
    ...
    try {
      ...
      if (adminUser) {
        ...
        console.log(
          `[DevAuth] Using admin user: ${adminUser.email} (${adminUser.id})`
        );
      } else {
        ...
        console.log(
          `[DevAuth] Created new admin user: ${newUser.email} (${newUser.id})`
        );
      }
    } catch (err: unknown) {
      ...
      if (envId) {
        ...
        console.warn(
          '[DevAuth] Prisma error; no DEV_USER_ID set. Using generated in-memory dev id:',
          devUserId
        );
      }
    }
...
export async function devAuthBypass(fastify: FastifyInstance): Promise<void> {
...

  {
    ...
    fastify.addHook('onRequest', async (request, reply) => {
      ...
      console.log(
        `[DevAuth] ✓ Set user for ${request.method} ${shortUrl}... → userId: ${request.user.userId}`
      );
    });
