...
export function apiRateLimitMiddleware(
  ...
  const userId =
    ((request as unknown).user as { id?: string })?.id || request.ip;
...
export function aiRateLimitMiddleware(
  ...
  const userId =
    ((request as unknown).user as { id?: string })?.id || request.ip;
  const userTier =
    (((request as unknown).user as { tier?: string })?.tier as
      | 'free'
      | 'pro'
      | 'enterprise') || 'free';
  ...
  reply.header(
    'X-RateLimit-AI-Requests-Remaining',
    result.remaining.requests.toString()
  );
  reply.header(
    'X-RateLimit-AI-Tokens-Remaining',
    result.remaining.tokens.toString()
  );
  reply.header(
    'X-RateLimit-AI-Cost-Remaining',
    result.remaining.cost.toFixed(2)
  );
  reply.header('X-RateLimit-Reset', new Date(result.resetAt).toISOString());
