...
export const YappcCoreResolver = {
  Query: {
    ...
    persona: (_: unknown, { id }: { id: string }) =>
      configService.getPersona(id),
    ...
    flowState: async (_: unknown, { instanceId }: { instanceId: string }) => {
      ...
      return {
        ...
        availableTransitions:
          await flowService.getAvailableTransitions(instanceId),
  ...
  Mutation: {
    startFlow: async (
      _: unknown,
      { flowId, input }: { flowId: string; input: Record<string, unknown> },
      ...
      return {
        ...flow,
        availableTransitions: await flowService.getAvailableTransitions(
          flow.instanceId
        ),
        activeTasks: await flowService.getActiveTasks(flow.instanceId),
      };
    },
    transitionFlow: async (
      _: unknown,
      {
        instanceId,
        event,
      ...
      const userId = requireUserId(context);
      const flow = await flowService.transition(
        instanceId,
        event,
        payload,
        userId
    ...
    cancelFlow: async (
      ...
      const userId = requireUserId(context);
      const flow = await flowService.cancel(
        instanceId,
  ...
  Domain: {
    personas: async (parent: unknown) => {
      ...
      const promises = parent.personas.map((id: string) =>
        configService.getPersona(id)
      );
    ...
    capabilities: async (parent: unknown) => {
      ...
      const capMap = new Map(
        (allCaps.capabilities || []).map((c: CapabilityData) => [c.id, c])
      );
