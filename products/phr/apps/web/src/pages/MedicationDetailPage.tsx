...
export function MedicationDetailPage(): React.ReactElement {
  ...
  useEffect(() => {
    if (!session || !identity || !medicationId) return;
    fetchMedicationDetail(identity.principalId, medicationId, toSessionContext(session))
      .then(setMedication)
      .catch((err: unknown) => setError(toSafeApiErrorState(err, t('medicationDetail.error.load'))))
      .finally(() => setLoading(false));
  }, [session, identity, medicationId]);

  if (loading) return <div className="loading">{t('medicationDetail.loading')}</div>;
  if (error) return (
    <PhrDetailPage
      title={t('medicationDetail.error.load')}
      parentPath="/medications"
      parentLabel={t('phr.routes.medications.label')}
      loading={false}
      error={error}
      data={null}
    />
  ...
  if (!medication) return (
    <PhrDetailPage
      title={t('medicationDetail.notFound')}
      parentPath="/medications"
      parentLabel={t('phr.routes.medications.label')}
      loading={false}
      error={null}
      data={null}
    >
      <p className="muted">{t('medicationDetail.notFound')}</p>
    </PhrDetailPage>
  );
  ...
  return (
    <PhrDetailPage
      ...
      subtitle={t('medicationDetail.subheader')}
      parentPath="/medications"
      parentLabel={t('phr.routes.medications.label')}
      loading={loading}
      error={error}
            ...
            <div className="stack gap-md">
              ...
              {medication.prescribedAt && (
                <div className="detail-row">
                  ...
                  <span className="detail-value"><time dateTime={medication.prescribedAt}>{formatDate(medication.prescribedAt)}</time></span>
                </div>
              )}
            </div>
          </CardContent>
