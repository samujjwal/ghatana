              ...
              return (
                <div key={group.code} className="observation-group">
                  ...
                  >
                    ...
                    <Badge variant={latest.status === 'critical' ? 'destructive' : latest.status === 'normal' ? 'success' : 'secondary'}>
                  ...
                  {selectedObservation === group.code && (
                    <div className="observation-trend">
                      ...
                      {chartData.length > 1 && (
                        <div className="trend-chart">
                          <span className="visually-hidden" aria-hidden="false">
                            {t('observations.chart.summary', { count: chartData.length, metric: latest.name })}
                          </span>
                          <TimeSeriesChart
                            data={chartData}
                      ...
                      <Table>
                        <TableHead>
                          <TableRow>
                            <TableCell component="th">{t('observations.table.date')}</TableCell>
                            <TableCell component="th">{t('observations.table.value')}</TableCell>
                            <TableCell component="th">{t('observations.table.status')}</TableCell>
                          </TableRow>
                        </TableHead>
                        <TableBody>
                          {group.observations.map((observation) => (
                            <TableRow key={observation.id}>
                              <TableCell><time dateTime={observation.effectiveDate}>{formatPhrDate(observation.effectiveDate)}</time></TableCell>
                              <TableCell>{observation.value} <span className="muted">{observation.unit}</span></TableCell>
                              <TableCell>
                                <Badge variant={observation.status === 'critical' ? 'destructive' : observation.status === 'normal' ? 'success' : 'secondary'}>
