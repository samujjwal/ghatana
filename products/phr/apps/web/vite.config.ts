...
const chartsPackagePath = path.resolve(
  __dirname,
  "../../../../platform/typescript/charts",
);
const chartsRechartsPath = path.dirname(
  require.resolve("recharts/package.json", { paths: [chartsPackagePath] }),
);
const lodashPath = path.dirname(
  require.resolve("lodash/package.json", { paths: [chartsRechartsPath] }),
);
const reactRouterPath = path.dirname(
  require.resolve("react-router/package.json", { paths: [reactRouterDomPath] }),
);
const reactRouterEntryPath = path.join(
  reactRouterPath,
  "dist/development/index.mjs",
);
const reactRouterDomEntryPath = path.join(
  reactRouterPath,
  ...
  resolve: {
    ...
    alias: [
      ...
      },
      {
        ...
        replacement: path.resolve(
          __dirname,
          "../../../../platform/typescript/theme/src/provider.tsx",
        ),
      },
      {
        find: "@ghatana/theme",
      ...
      { find: /^lodash\/(.+)$/, replacement: path.join(lodashPath, "$1.js") },
