...
export type MobileScreenKey = 'dashboard' | 'records' | 'consents' | 'notifications' | 'emergency' | 'settings';
...
const canonicalRouteContract = routeContractJson as unknown as RouteContractSource;
...
export function getMobileRoutes(): MobileRouteEntry[] {
  return canonicalRouteContract.routes
    .filter((route) => {
      ...
      const hasScreenKey = route.path in PATH_TO_SCREEN_KEY;
      
      return hasMobileSurface && isStable && hasScreenKey;
    })
    .map((route) => ({
      key: PATH_TO_SCREEN_KEY[route.path],
      label: route.label,
      hint: route.description,
      icon: SCREEN_ICONS[PATH_TO_SCREEN_KEY[route.path]],
      path: route.path,
...
export function isRouteAvailableOnMobile(path: string): boolean {
  ...
  return route.surface?.includes('mobile') === true && route.stability === 'stable';
}
