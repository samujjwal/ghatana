...
describe("offlineStore", () => {
  ...
  it("removes restricted fields recursively before caching", async () => {
    ...
    await saveDashboardOffline(
      dashboardWithRestrictedFields,
      undefined,
      SESSION_IDENTITY,
    );
  ...
  it("getDashboardOfflineTimestamp returns null when no cache exists", async () => {
    mockGet.mockResolvedValueOnce(null);

    await expect(getDashboardOfflineTimestamp()).resolves.toBeNull();
    expect(mockRemove).not.toHaveBeenCalled();
  });

  it("getDashboardOfflineTimestamp returns savedAt for a valid envelope", async () => {
    const savedAt = Date.now();
    const envelope = {
      schemaVersion: 1,
      savedAt,
