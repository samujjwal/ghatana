...
describe('Action Queue', () => {
  ...
  describe('enqueueAction', () => {
    it('should add an action to the queue', async () => {
      const action = await enqueueAction('create_consent', { grantee: 'Hospital A' });
    ...
    it('should generate unique action IDs', async () => {
      const action1 = await enqueueAction('create_consent', { grantee: 'Hospital A' });
      const action2 = await enqueueAction('book_appointment', { providerId: 'provider-1' });
  ...
  describe('getActionQueueStats', () => {
    ...
    it('should count actions by status', async () => {
      const queue = await loadActionQueue();
      queue.push(
        { id: '1', type: 'create_consent', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'pending' },
        { id: '2', type: 'book_appointment', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'pending' },
        { id: '3', type: 'submit_vitals', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'completed' },
        { id: '4', type: 'report_symptom', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'failed' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
  ...
  describe('getPendingActions', () => {
    it('should return only pending actions', async () => {
      ...
      queue.push(
        { id: '1', type: 'create_consent', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'pending' },
        { id: '2', type: 'book_appointment', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'completed' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
  ...
  describe('getFailedActions', () => {
    it('should return only failed actions', async () => {
      const queue = await loadActionQueue();
      queue.push(
        { id: '1', type: 'create_consent', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'failed', error: 'Network error' },
        { id: '2', type: 'book_appointment', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'pending' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
  ...
  describe('clearCompletedActions', () => {
    it('should remove completed actions from queue', async () => {
      const queue = await loadActionQueue();
      queue.push(
        { id: '1', type: 'create_consent', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'completed' },
        { id: '2', type: 'book_appointment', payload: {}, timestamp: new Date().toISOString(), retryCount: 0, status: 'pending' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
  ...
  describe('retryAction', () => {
    it('should reset failed action to pending', async () => {
      const queue = await loadActionQueue();
      queue.push(
        { id: '1', type: 'create_consent', payload: {}, timestamp: new Date().toISOString(), retryCount: 1, status: 'failed', error: 'Network error' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
    ...
    it('should throw if action not found', async () => {
      await expect(retryAction('nonexistent')).rejects.toThrow('Action nonexistent not found');
    });

    it('should throw if max retries exceeded', async () => {
      const queue = await loadActionQueue();
      queue.push(
        { id: '1', type: 'create_consent', payload: {}, timestamp: new Date().toISOString(), retryCount: 3, status: 'failed', error: 'Network error' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
  ...
  describe('syncActions', () => {
    it('should execute pending actions with executor', async () => {
      const queue = await loadActionQueue();
      queue.push(
        { id: '1', type: 'create_consent', payload: { grantee: 'Hospital A' }, timestamp: new Date().toISOString(), retryCount: 0, status: 'pending' },
        { id: '2', type: 'book_appointment', payload: { providerId: 'provider-1' }, timestamp: new Date().toISOString(), retryCount: 0, status: 'pending' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
  ...
  describe('pruneOldActions', () => {
    it('should remove completed actions older than 7 days', async () => {
      ...
      queue.push(
        { id: '1', type: 'create_consent', payload: {}, timestamp: oldDate, retryCount: 0, status: 'completed' },
        { id: '2', type: 'book_appointment', payload: {}, timestamp: recentDate, retryCount: 0, status: 'completed' },
        { id: '3', type: 'submit_vitals', payload: {}, timestamp: oldDate, retryCount: 0, status: 'pending' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
    ...
    it('should not remove pending actions regardless of age', async () => {
      ...
      queue.push(
        { id: '1', type: 'create_consent', payload: {}, timestamp: oldDate, retryCount: 0, status: 'pending' }
      );
      await AsyncStorage.setItem('phr_action_queue', JSON.stringify(queue));
