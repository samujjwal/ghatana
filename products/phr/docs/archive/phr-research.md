# Archived — Comprehensive Feasibility Report: A Patient-Centric Personal Health Record (PHR) App for Nepal

## Expanded Strategic Analysis, Market Intelligence, and Implementation Framework

> **⚠️ Archived / superseded document**
>
> This document is kept for feasibility-phase history only and is no longer authoritative for current architecture or MVP scope.
> Use `phr-consolidated-report-v2.md` for the active strategy document and `README.md` in this folder for the current source-of-truth hierarchy.
>
> **Architectural Evolution Note (added during doc review)**
>
> This research document was written during the early feasibility phase. The following decisions have since been **revised** in the detailed architecture docs:
>
> | Research Phase                        | Current Architecture                                                       | See                                                 |
> | ------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------- |
> | HAPI FHIR Server (Java) as API server | **NestJS** (Node.js) modular monolith with FHIR-shaped Prisma models       | `phr_nestjs_modules_detailed_architecture.md`       |
> | MedicationStatement (FHIR resource)   | **MedicationRequest** (active prescriptions; MedicationStatement deferred) | `phr_mvp_activation_plan.md`                        |
> | React.js (web)                        | **React Router + Jotai + TanStack Query** (web); React Native (mobile)     | `phr_frontend_route_and_component_map.md`           |
> | ELK Stack for logging                 | **Micrometer + OpenTelemetry** via `libs:observability`                    | Platform architecture                               |
> | MinIO / generic object storage        | **Ceph (RADOS Gateway, S3-compatible; LGPL)** for file storage             | `phr_migration_plan_and_module_ownership_matrix.md` |
>
> The regulatory analysis, market study, competitive landscape, and financial projections remain valid.
...
This report expands upon all previous research, incorporates verified corrections regarding the 21 core modules, cloud storage restrictions, privacy-law compliance, and HIB integration requirements, and provides specific guidance on security audit requirements, unit economics, revenue models, and partnership opportunities. The result is a complete feasibility-phase blueprint for building Nepal's leading PHR platform.
...
**New Addition:** Nepal's **Privacy Act 2075 (2018)** is the primary privacy law relevant to PHR operations.
...
5. **MedicationStatement** — Patient-reported medication use _(Note: Current architecture uses **MedicationRequest** for L1 MVP; MedicationStatement deferred to L3+)_
...
| **Logging**         | OpenSearch + OpenSearch Dashboards + Fluent Bit (all Apache 2.0) | Tamper-proof logs, required for security audit            |               |
...
- [ ] Set up centralized logging (OpenSearch + Fluent Bit) before any code development
...
- Encrypted SQLite (lightweight, public-domain, offline-first)
...
- **Vosk (Apache 2.0):** Supports Nepali; lightweight on-premise ASR; accuracy ~80-85% on general speech
- **OpenAI Whisper (MIT):** Open-source batch transcription; strong multilingual support
...
Use **Vosk on-premise ASR as MVP baseline** (Month 7-12), then **build custom Nepali medical ASR model in Year 2** (Phase 4).
...
2. Audio processed locally via Vosk on-premise ASR
...
**Cost:** Vosk runs on-premise — no per-request API fees. Infrastructure cost only (compute).

### Phase 2 Implementation (Months 19-24):

- Collect 1,000+ hours of Nepali medical speech from pilot hospitals
- Train custom model using OpenAI Whisper (MIT) fine-tuning on collected data
- Deploy custom model alongside Vosk baseline for comparison
...
- **Vosk / Whisper Community:** Contribute Nepali medical models and improvements back to open-source projects
...
- **ASR infrastructure (Vosk on-premise):** $0 recurring (compute included in hosting)
...
| **Data Privacy Certification**       | Legal           | 17        | Comply with Privacy Act 2075; certification obtained             |
...
| **Nepali ASR accuracy insufficient** (>15% WER on medical speech)         | Medium         | Low        | Keep Vosk/Whisper fallback; don't commit to custom model until validated                                  |
...
2. **Privacy Policy** (Privacy Act 2075 compliance)
...
7. **Voice ASR Phased Approach**
   - Start with Vosk (Apache 2.0) on-premise ASR for MVP
