...
| **Companion Docs** | [Schema Delta](phr_core_schema_delta_spec.md), [Activation Plan](phr_mvp_activation_plan.md), [NestJS Modules](phr_nestjs_modules_detailed_architecture.md) |

> **📌 What changed in v2.0:** Added data classification per table, encryption column requirements, tenant isolation migration step, rollback procedures, and emergency QR data migration notes.
...
Prisma Migrate should keep the database schema in sync with the Prisma schema as it evolves, preserve existing data, and generate a history of reviewable SQL migration files.
...
- `Appointment` is core MVP; telemedicine app tables are Phase 2
...
## Phase 6A — data input (OCR, audio/voice, form-based, Core MVP)
...
- this phase is part of Core MVP and must land before release exit
...
- `Coverage` and eligibility logging support the core insurance baseline
- `Claim` and `ClaimResponse` are planned Phase 2 / MVP+ activation items in the current rollout
...
Nest modules are feature-oriented and encapsulate related controllers and providers; exported providers form the module’s public interface.
