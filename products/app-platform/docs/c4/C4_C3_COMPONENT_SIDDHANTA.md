...
> This C4 Level 3 Component diagram describes the **Capital Markets (Siddhanta)** domain pack on AppPlatform. Platform-level architecture is in `DOMAIN_PACK_INTERFACE_SPECIFICATION.md`.
...
- **Multi-Calendar**: Every component that persists or publishes timestamps uses a canonical ISO 8601 UTC `timestamp`. BS calendar representation is obtained from the K-15 Multi-Calendar Service via the `CalendarDate` type (populated by the Nepal T1 calendar pack). Components should store `calendar_date JSONB` rather than a separate `timestampBs` field.
- **K-05 Event Envelope**: All domain events published by any component conform to the standard event envelope with `timestamp` (UTC), `calendar_date` (optional, CalendarDate), `source_domain_pack_id`, `tenant_id`, `trace_id`, `causation_id`, and `correlation_id`.
