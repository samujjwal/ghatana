...
**Version**: 2.0.0  
**Date**: 2026-03-12  
**Purpose**: Guide for developing domain packs for the AppPlatform Multi-Domain Operating System

---

## Overview

Domain packs are pluggable modules that provide industry-specific functionality while leveraging the generic AppPlatform Kernel (K-01 through K-19). This guide covers the structure, development process, and best practices for creating domain packs.
...
domainPack:
  ...
  # Domain classification (open string; can be multiple)
  domainTypes:
    - "banking"
    - "financial-services"   # optional second type if pack spans sub-domains
  industry: "banking"
  subdomains:
    - "retail-banking"
    - "corporate-banking"
  ...
  # Required kernel modules with semver constraints
  requiredKernels:
    - { module: "K-01", version: ">=1.0.0" }   # IAM
    - { module: "K-02", version: ">=1.0.0" }   # Config
    - { module: "K-03", version: ">=1.0.0" }   # Rules
    - { module: "K-04", version: ">=1.0.0" }   # Plugin Runtime
    - { module: "K-05", version: ">=1.0.0" }   # Events
    - { module: "K-07", version: ">=1.0.0" }   # Audit
    - { module: "K-15", version: ">=2.0.0" }   # Multi-Calendar (optional for banking)
    - { module: "K-16", version: ">=1.0.0", optional: true }  # Ledger Framework
  
  # Platform CoreDomainCapability values (generic; from the closed CoreDomainCapability enum)
  capabilities:
    - "ENTITY_MANAGEMENT"
    ...
    - "AUDIT_LOGGING"
    - "REPORTING"
  # Industry-specific capabilities (free-form; NOT in the kernel enum)
  ...
  # Business rules (use T2 for sandbox-safe; T3 for executable with permissions)
  businessRules:
    - file: "rules/account-rules.rego"        # T2 (Rego) — sandbox-safe
    - file: "rules/payment-rules.rego"        # T2 (Rego)
    - file: "rules/compliance-rules.rego"     # T2 (Rego)
  ...
  # Author and support — use your own org details
  author:
    name: "[Your Name]"
    email: "[your-team@your-org.example]"
  ...
  certification:
    status: "certified"
    certifiedBy: "AppPlatform Certification Authority"
...
```bash
# Clone domain pack template from the AppPlatform repository
git clone https://github.com/your-org/domain-pack-template.git my-domain-pack
cd my-domain-pack
...
```bash
# Submit to AppPlatform certification service (URL configured per your deployment)
curl -X POST ${APPPLATFORM_CERT_URL}/api/packs/submit \
...
> For Capital Markets (Siddhanta) domain pack specifics, see `domain-packs/capital-markets/DEVELOPMENT_GUIDE.md`.
