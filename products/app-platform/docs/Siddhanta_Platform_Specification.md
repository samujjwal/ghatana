...
> This document describes the **Capital Markets (Siddhanta)** domain pack running on the AppPlatform Kernel. It contains Nepal-specific regulatory, calendar, and market structure details that are NOT part of the generic Platform Kernel. Platform-level contracts are in `DOMAIN_PACK_INTERFACE_SPECIFICATION.md` and the `epics/` / `lld/` directories.
