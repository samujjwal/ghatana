...
- **Multi-Calendar**: Optional `CalendarDate` type on events; active calendars are declared by domain-pack T1 config — the kernel never mandates a specific calendar system
...
**Dependencies**: K-02 (configuration), K-05 (auth events), K-07 (audit), K-15 (multi-calendar)
...
### K-15 Multi-Calendar Service

**File**: `lld/LLD_K15_DUAL_CALENDAR.md`

**Purpose**: Pluggable `CalendarDate` generation, cross-calendar conversion, business day and fiscal year calculation.

**Key Features**:

- Domain-agnostic `CalendarDate` type with open `CalendarId` map
- Pluggable calendar converters registered via T1 config packs (BS, Hijri, Thai Solar, etc.)
- Fiscal year boundaries per jurisdiction (T1 config)
- Business day calculation with holiday calendars (T1 config)
- Timezone-aware conversions
- Settlement date T+n calculation with weekend/holiday awareness
- CBDC/T+0 support
...
All modules use a `CalendarDate` composite type (from K-15) wherever timestamps require calendar-aware representation. The set of active calendars is declared by the domain pack's T1 config — the kernel envelope mandates only the canonical UTC instant.
...
> `calendar_date` is **optional**: it is populated by K-15 only when an active T1 calendar config pack declares additional calendars. Base events carry only `timestamp` (ISO 8601 UTC).

**Calendar Converter Registry**: Converters for specific calendar systems (Bikram Sambat, Hijri, Thai Solar, etc.) are registered via T1 config packs. New calendar support never requires kernel code changes.
...
> `calendar_date` is `null` when no T1 calendar pack is active; populated by K-15 otherwise (see §4.1).
