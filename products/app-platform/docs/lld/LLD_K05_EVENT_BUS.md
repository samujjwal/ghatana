...
The Event Bus provides **append-only event storage, reliable delivery, and saga-based workflow orchestration** for all asynchronous communication across the AppPlatform.
...
- `CalendarDate` timestamping (populated by K-15 when enabled by the active domain pack T1 calendar config)
...
3. `timestamp` in the envelope MUST be a valid ISO 8601 UTC instant; `calendar_date` is optional and populated by K-15
...
Request:
{
  ...
  "metadata": {
    ...
    "user_id": "user_456",
    "trace_id": "abc-123",
    "source_domain_pack_id": "capital-markets-siddhanta"  // optional; set by domain pack producers
  },
  "timestamp": "2025-03-02T10:30:00Z"  // canonical UTC instant; CalendarDate enrichment is optional via K-15
}
...
option java_package = "com.ghatana.platform.events.v1";
...
message PublishEventRequest {
  ...
  google.protobuf.Timestamp timestamp = 7;   // canonical UTC instant (required)
  // calendar_date is optional; set by producers that require multi-calendar representation
  // K-15 enriches it when the domain pack's T1 calendar config is active
  string calendar_date_json = 8;             // JSON-serialized CalendarDate; absent = no multi-calendar
}
...
message Event {
  ...
  google.protobuf.Timestamp timestamp = 9;   // canonical UTC instant
  string calendar_date_json = 10;            // optional; JSON-serialized CalendarDate
}
...
message EventMetadata {
  ...
  string source_domain_pack_id = 6;  // optional; identifies originating domain pack
}
```
...
interface EventBusClient {
   ...
   */
  getEventStream(aggregateType: string, aggregateId: string): Promise<Event[]>;
...
interface EventEnvelope<T> {
  ...
  timestamp: Date; // canonical UTC instant (required)
  calendarDate?: CalendarDate; // optional; populated by K-15 when multi-calendar is active
}
...
interface EventMetadata {
  ...
  sourceDomainPackId?: string; // identifies originating domain pack (P1-4)
}
...
```json
{
  ...
  "metadata": {
    ...
    "source_domain_pack_id": "capital-markets-siddhanta"
  },
  "timestamp": "2025-03-02T10:30:00Z"
  // calendar_date omitted here — optionally enriched by K-15 if calendar pack is active
}
```
...
CREATE TABLE events (
  ...
  data JSONB NOT NULL,
  metadata JSONB NOT NULL,           -- includes source_domain_pack_id if set
  timestamp TIMESTAMPTZ NOT NULL,    -- canonical UTC instant (replaces timestamp_gregorian + timestamp_bs)
  calendar_date JSONB,               -- optional CalendarDate JSON; NULL when no T1 calendar pack active
...
CREATE INDEX idx_events_pack       ON events((metadata->>'source_domain_pack_id'));  -- optional filter by pack
```
...
async function publishEvent(event: Event): Promise<PublishResult> {
  ...
  try {
    // Validate schema
    const validateSpan = tracer.startSpan("event.validate_schema", {
      parent: span,
    });
...
const orderPlacementSaga = defineSaga({
  ...
  steps: [
    ...
    },
    {
      name: "charge_payment",
      action: async (data) => {
        const payment = await paymentService.charge(data.userId, data.amount);
...
class ProjectionRebuilder {
  async rebuild(projectionName: string, fromTimestamp?: Date): Promise<void> {
