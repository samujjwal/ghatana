...
**Version**: 2.0.0  
**Date**: 2026-03-12  
**Purpose**: Define the contract between domain packs and the AppPlatform kernel

> **NORMATIVE DOCUMENT** — This specification is the canonical contract between domain packs and the Platform Kernel. It governs all domain pack integrations.

---

## Overview

This specification defines the interface that domain packs must implement to integrate with the **AppPlatform Multi-Domain Operating System**. It ensures compatibility, security, and proper isolation between domain packs and the generic Platform Kernel.

The Platform Kernel (K-01 through K-19) is domain-agnostic. All industry-specific logic lives exclusively in domain packs. The **Capital Markets (Siddhanta)** pack is the first domain pack built on this platform.
...
interface DomainPack {
  ...
  // Domain classification — one pack may span multiple domain types (e.g., banking + insurance)
  readonly domainTypes: DomainType[];
  readonly industry: string;
  readonly subdomains: string[];

  // Platform compatibility
  readonly platformMinVersion: string; // semver, e.g. "2.0.0"
  readonly platformMaxVersion?: string; // semver, e.g. "3.0.0" (absent = unbounded)

  // Required kernel modules with version constraints
  readonly requiredKernels: KernelModuleConstraint[];

  // Generic platform capabilities (from CoreDomainCapability enum)
  readonly capabilities: CoreDomainCapability[];
  // Industry-specific capability tags (free-form, qualified by domain pack)
  readonly extendedCapabilities?: string[];

  // Pack components
  readonly dataModels: DataModelReference[];
  readonly businessRules: BusinessRuleReference[]; // array of T2RuleReference | T3RuleReference
  ...
  // Feature flags — kernel evaluates these against tenant/environment config
  readonly featureFlags?: FeatureFlagDefinition[];
...
`DomainType` is an **open string type** to allow future domains without schema changes. Use the `WellKnownDomainTypes` namespace for pre-defined values.

```typescript
/**
 * Open string type — domain packs may introduce new domain types without
 * modifying the Platform Kernel. Always use a qualified reverse-DNS form:
 * e.g., "com.example.agri-fintech"
 */
type DomainType = string;

/** Canonical pre-defined domain type constants. Non-exhaustive. */
namespace WellKnownDomainTypes {
  export const FINANCIAL_SERVICES = "financial-services";
  export const BANKING = "banking";
  export const CAPITAL_MARKETS = "capital-markets";
...
/**
 * Generic platform capabilities defined by the KERNEL.
 * These are domain-agnostic — every domain pack can use them.
 * Domain-specific capability strings belong in `extendedCapabilities`.
 */
enum CoreDomainCapability {
  ...
  CALENDAR_AWARE = "calendar_aware", // uses K-15 Multi-Calendar Service
  ROLE_BASED_ACCESS = "role_based_access",
}

/** Kernel module constraint — semver range for platform compatibility. */
interface KernelModuleConstraint {
  readonly module: string; // e.g. "K-05", "K-15"
  readonly version: string; // semver range, e.g. ">=1.2.0 <2.0.0"
  readonly optional?: boolean; // false by default
}
```
...
Business rules are split into two distinct execution tiers based on the plugin taxonomy:

```typescript
/**
 * T2 Rule Reference — declarative, sandbox-safe rules.
 * Languages: Rego (OPA), SQL (read-only), DSL.
 * Platform enforces strict process isolation: no network, no file system.
 */
interface T2RuleReference {
  readonly tier: "T2";
  readonly ruleId: string;
  readonly file: string;
  readonly language: "rego" | "sql" | "dsl";
  readonly version: string;
  readonly dependencies?: string[];
...
/**
 * T3 Rule Reference — executable rules with reduced sandbox guarantees.
 * Languages: Java, Kotlin, Python, JavaScript.
 * Requires explicit kernel-granted permissions; runs in a managed thread pool.
 * Must declare all required capabilities in `permissions`.
 */
interface T3RuleReference {
  readonly tier: "T3";
  readonly ruleId: string;
  readonly file: string;
  readonly language: "java" | "kotlin" | "python" | "javascript";
  readonly version: string;
  readonly permissions: {
    readonly networkAccess?: boolean; // default: false
    readonly fileSystemAccess?: boolean; // default: false
    readonly kernelModules?: string[]; // e.g. ["K-07", "K-08"]
  };
  readonly dependencies?: string[];
  readonly testCases?: RuleTestCase[];
  readonly executionConfig?: RuleExecutionConfig;
...
/** Union type for use in DomainPack.businessRules */
type BusinessRuleReference = T2RuleReference | T3RuleReference;
...
/**
 * Feature Flag Definition — domain packs declare the flags they honour.
 * The Platform Kernel evaluates flags against tenant/environment config
 * and injects the resolved values into the pack's execution context.
 */
interface FeatureFlagDefinition {
  readonly flagId: string; // e.g. "capital-markets.enable-realtime-settlement"
  readonly description: string;
  readonly defaultValue: boolean | string | number;
  readonly valueType: "boolean" | "string" | "number";
  readonly scope: "global" | "tenant" | "user"; // who can override
  readonly rolloutStrategy?: "all" | "percentage" | "allowlist";
}
...
interface DomainRuntime {
  ...
  getRuleExecutionPlan(
    packId: string,
    ruleId: string,
  ): Promise<RuleExecutionPlan>;
}
...
- Developer Forums — via Ghatana platform community
- GitHub Discussions — in the Ghatana monorepo

### Support

- Contact the AppPlatform core team via the internal helpdesk

---

## API Evolution Policy (P2-6)

This section governs how `DomainPack` interface fields may change across `platformMinVersion` ranges.

| Change Type                                       | Allowed Without Major Bump? | Required Action                                                 |
| ------------------------------------------------- | --------------------------- | --------------------------------------------------------------- |
| Add optional field                                | ✅ Yes                      | Document in changelog                                           |
| Add required field                                | ❌ No                       | Bump platform major version; provide migration guide            |
| Remove field                                      | ❌ No                       | Deprecate for 2 minor versions first, then remove in next major |
| Change `CoreDomainCapability` value string        | ❌ No                       | Major bump; run automated migration on existing packs           |
| Add new `WellKnownDomainTypes` constant           | ✅ Yes                      | Backward compatible — open string                               |
| Change `KernelModuleConstraint.version` semantics | ❌ No                       | Major bump                                                      |
| Add new `LifecycleHooks` hook (optional)          | ✅ Yes                      | Document; existing packs ignore it safely                       |
| Change hook signature (parameter types)           | ❌ No                       | Major bump                                                      |

### Deprecation Notice Protocol

1. Mark field `@deprecated` in the TypeScript interface with a `// @deprecated since 2.x.0, remove in 3.0.0` comment.
2. Add entry to `DOMAIN_PACK_DEVELOPMENT_GUIDE.md` migration section.
3. Platform Kernel must log a `WARN` when loading a pack that uses deprecated fields.
4. After one full major cycle, the field is removed in the next major release.

---

_Specification maintained by the AppPlatform Kernel Team. For Capital Markets (Siddhanta) specific conventions, see `domain-packs/capital-markets/DEVELOPMENT_GUIDE.md`._
