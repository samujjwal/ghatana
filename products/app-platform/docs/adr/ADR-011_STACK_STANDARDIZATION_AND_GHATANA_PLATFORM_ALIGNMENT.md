...
The following are not part of the standard AppPlatform baseline and must not be reintroduced in current documents as defaults:
...
This ADR defines the canonical technology stack for the **AppPlatform Multi-Domain Operating System**. It establishes standardization, versioning, and compatibility rules for all platform components while ensuring the kernel remains domain-agnostic.
