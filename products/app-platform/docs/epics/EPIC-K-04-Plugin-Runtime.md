...
8. **FR8 Multi-Calendar Timestamps:** Plugin validity periods must use `CalendarDate` (from K-15) for `valid_from` and `valid_until` fields.
...
- **Multi-Calendar Fields:** `valid_from` and `valid_until` in registration metadata use `CalendarDate`.
