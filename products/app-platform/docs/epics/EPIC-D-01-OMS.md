...
7. **FR7 Multi-Calendar Timestamps:** Order timestamps use `CalendarDate` (from K-15). The kernel event envelope mandates `timestamp` (ISO 8601 UTC); the optional `calendar_date` field is enriched by K-15 when the active T1 calendar config declares additional calendar systems.
...
- **New Entities:**
  - `Order`: `{ order_id: UUID, client_id: UUID, instrument_id: String, side: Enum, qty: Int, type: Enum, status: Enum, submitted_at: CalendarDate, updated_at: CalendarDate }`
  - `Position`: `{ client_id: UUID, instrument_id: String, available_qty: Int, locked_qty: Int }`
- **Multi-Calendar Fields:** `submitted_at` and `updated_at` use `CalendarDate` (populated by K-15 when domain pack T1 calendar config is active).
...
| Payload           | `{ "order_id": "...", "previous_state": "...", "new_state": "...", "timestamp": "2025-03-02T10:30:00Z", "calendar_date": {} }` |
...
- **AI Hook Type:** Copilot Assist / Anomaly Detection / Behavioral Analysis
- **Primary Use Case:** (a) Parse natural-language order intents into structured OrderRequest; (b) Detect fat-finger errors inline before order acceptance; (c) Detect anomalous order sequencing patterns that may indicate front-running or spoofing; (d) Enrich every emitted `OrderPlaced` event with an AI annotation for downstream modules (D-06, D-08).
- **Inference Mode:** Synchronous for (a) and (b) — on the order submission hot path (budget: 1ms P99); Asynchronous for (c) — pattern analysis added as `ai_annotation` to the event after emission.
- **Workflow Steps Exposed:** (a) Chat-to-trade intent capture; (b) Pre-acceptance fat-finger gate; (c) Post-acceptance sequence anomaly enrichment of `OrderPlaced` events; (d) Advisory warning shown to trader on borderline detections.
- **Model Registry Usage:** `trade-intent-parser-v1` (NLP → structured OrderRequest), `fat-finger-detector-v1` (multi-signal: size deviation, price deviation from NBBO, velocity spike), `order-sequencing-analyzer-v1` (temporal pattern detection for spoofing/layering signals)
- **Input Data / Feature Set:** Order fields (ticker, side, qty, price, type); account risk profile; recent order history (last 100 orders for the account); current market NBBO; session velocity metrics.
- **Output / AI Annotation:** For inline checks: `{ blocked: Boolean, confidence: Float, explanation: String }`. For `OrderPlaced` event annotation: `AiAnnotation { model_id: "order-sequencing-analyzer-v1", result: { sequence_risk_score, pattern_label }, confidence, explanation }`.
- **Explainability Requirement:** Blocking explanation surfaced to trader in plain language (e.g., "Order size is 40x your 30-day average — possible fat finger"). Explanation stored in K-07 for every AI-influenced order decision.
- **Confidence Threshold:** Fat-finger block: confidence ≥ 0.90 → hard block (requires force-submit); 0.70–0.89 → soft warning. Sequence anomaly: score ≥ 0.85 → forward to D-08 Surveillance with HIGH priority annotation.
- **Human Override Path:** Trader can acknowledge fat-finger warning and force-submit (force-submit is audited in K-07). Compliance officer can suppress sequence anomaly forwarding for explicitly flagged accounts.
- **Feedback Loop:** Every force-submit (override of a fat-finger block) is labeled as a corrective signal and fed back to `fat-finger-detector-v1` fine-tuning pipeline via K-09.
- **Drift Monitoring:** Weekly: forced-submit ratio; fat-finger model precision/recall vs. post-trade confirmed erroneous orders. Alert if false positive rate exceeds 2%.
- **Fallback Behavior:** Static hard limits (quantity ceiling, price deviation ± 20% from NBBO). Chat-to-trade falls back to structured form input. Sequence anomaly analysis drops gracefully — `OrderPlaced` event emitted without `ai_annotation`.
- **Tenant Isolation:** Account history and sequence analysis is strictly tenant-scoped. Models are shared platform-level; feature extraction is tenant-isolated at K-09 Inference Router.
...
- **Events Emitted:** `OrderPlaced`, `OrderModified`, `OrderCancelled`, `OrderStateChanged`, `OrderAmended`, `OrderRejected` — all conform to K-05 standard envelope (`event_id`, `event_type`, `aggregate_id`, `aggregate_type`, `sequence_number`, `timestamp`, optional `calendar_date`, `payload`)
