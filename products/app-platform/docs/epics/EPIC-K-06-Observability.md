...
3. **FR3 Multi-Calendar Timestamps:** All log entries must include a UTC `timestamp`. When a T1 calendar pack is active, log entries may be enriched with an optional `calendar_date` (`CalendarDate` from K-15) for jurisdiction-native display.
...
- **Multi-Calendar Fields:** Standard log wrapper includes UTC `timestamp`; optional `calendar_date` (CalendarDate) when K-15 enrichment is active.
