...
3. **FR3 Standardized Schema:** Every record must capture: `tenant_id`, `jurisdiction`, `user_id/actor`, `action`, `resource_id`, `before_state`, `after_state`, `timestamp` (ISO 8601 UTC), and — when the active T1 calendar pack is configured — the `calendar_date` field (type `CalendarDate` from K-15).
...
- **New Entities:**
  - `AuditRecord`: `{ record_id: UUID, previous_hash: String, tenant_id: UUID, actor_id: String, action: String, resource_uri: String, before_state: JSON, after_state: JSON, timestamp: Timestamp, calendar_date: CalendarDate | null, hash: String }`
- **Multi-Calendar Fields:** `calendar_date` is populated by K-15 when the active T1 calendar pack is configured. All records store canonical UTC `timestamp` as primary key for ordering.
...
1. **Given** a domain module state change, **When** `AuditClient.record()` is called, **Then** the entry is saved with `before_state`, `after_state`, and a canonical UTC `timestamp` (plus `calendar_date` when K-15 calendar pack is active).
