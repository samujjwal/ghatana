...
6. **FR6 Multi-Calendar Report Metadata:** All report metadata (generation date, reporting period) must use `CalendarDate` (K-15) for jurisdiction-native date representation alongside UTC timestamps.
...
- **New Entities:**
  - `RegulatoryReport`: `{ report_id: UUID, template_id: String, period_start: Date, period_end: Date, period_start_calendar: CalendarDate | null, period_end_calendar: CalendarDate | null, status: Enum, submitted_at: CalendarDate }`
- **Multi-Calendar Fields:** `period_start_calendar`, `period_end_calendar`, `submitted_at` use `CalendarDate` (K-15 enriched).
...
| Payload           | `{ "report_id": "...", "regulator": "SEBON", "submission_ref": "...", "submitted_at": "2025-03-02T10:30:00Z", "submitted_at_calendar": { "bs": "2081-11-18" } }` |
...
| `RegReportSubmitted` | Report sent to regulator portal | `report_id`, `regulator`, `submission_timestamp`, `calendar_date`, `status` |
