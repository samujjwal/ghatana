...
4. **FR4 Multi-Calendar Interface:** All date pickers must allow input in any configured calendar system (e.g., BS or Gregorian), instantly converting via K-15 and displaying the canonical UTC value plus active calendar representations.
...
- **Multi-Calendar Fields:** UI components handle `CalendarDate` natively; the active calendar systems are driven by the domain pack's T1 config.
