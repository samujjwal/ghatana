...
- **Multi-Calendar Fields:** {List which fields use `CalendarDate` from K-15 (e.g., `created_at`, `updated_at`, `executed_at`). Note: required only when the domain pack's T1 calendar config is active.}
...
- **Multi-Calendar Fields:** `submitted_at` and `updated_at` use `CalendarDate` (K-15 enriched when T1 calendar pack is active).
...
| Payload           | `{ "field1": "...", "field2": "...", "timestamp": "2025-03-02T10:30:00Z", "calendar_date": null }` (include UTC `timestamp`; add `calendar_date: CalendarDate` when K-15 enrichment is active) |
...
> **Mandatory**: Every epic MUST complete this section. No epic is exempt from AI integration. The platform is AI-native (Principle 17 — AI as Substrate). If an epic has no direct AI model usage, it still participates via implicit enrichment from K-09 (e.g., anomaly scoring on incoming requests). Mark individual fields N/A only with justification.

- **AI Hook Type:** {Copilot Assist | Anomaly Detection | Predictive Analytics | NLP/NER | Code Generation | Semantic Search | Classification | Recommendation | Summarization | Fraud Detection | N/A (with justification)}
- **Primary Use Case:** {Describe the specific AI/ML problem this module uses AI for — be concrete, not generic.}
- **Inference Mode:** {Synchronous (inline with request path, latency-sensitive) | Asynchronous (background enrichment, fire-and-forget) | Batch (scheduled jobs)}
- **Workflow Steps Exposed:** {Which specific workflow steps have AI integration? List each step and what the AI does.}
- **Model Registry Usage:** `{model-name-v1}` (all models MUST be registered in K-09 AI Governance; direct inference endpoint calls are forbidden)
- **Input Data / Feature Set:** {What data feeds the model? Reference K-08 feature sets if applicable.}
- **Output / AI Annotation:** {What does the model output? Is output stored as `ai_annotation` on the entity/event?}
- **Explainability Requirement:** {How does AI explain its decisions/predictions? Every AI decision written to K-07 must include a human-readable explanation.}
- **Confidence Threshold:** {What minimum confidence score is required before acting on AI output? Below threshold: fall to human review or fallback.}
- **Human Override Path:** {How can humans override AI decisions? Every AI recommendation MUST have a human override path.}
- **Feedback Loop:** {How do human overrides feed back to improve the model? HITL data sent to K-09 fine-tuning pipeline?}
- **Drift Monitoring:** {What metric is used to detect model drift? What PSI/accuracy threshold triggers a re-train or rollback?}
- **Fallback Behavior:** {What deterministic logic runs when AI is unavailable? AI must NEVER be a single point of failure.}
- **Tenant Isolation:** {Can tenants use different model versions? How is AI output scoped to tenant context?}

{If truly no AI integration is possible for this epic, write a brief justification paragraph and list the implicit AI coverage from other modules (e.g., K-09 anomaly scoring on all API calls, K-06 telemetry anomaly detection).}
...
| AI Observability           | {AI model latency P99; override rate; confidence score distribution; drift PSI; fallback activation rate} |
...
| Multi-Calendar Correctness | {Multi-calendar handling accuracy}                                                                        |
...
- **Events Emitted:** {List all events this module emits — must conform to K-05 standard envelope: `event_id`, `event_type`, `aggregate_id`, `aggregate_type`, `sequence_number`, `timestamp` (ISO 8601 UTC), optional `calendar_date` (CalendarDate from K-15), optional `ai_annotation` (AiAnnotation from K-09 when model enrichment is active), `payload`}
...
- Specify field types
- Identify multi-calendar fields (use `CalendarDate` from K-15 for domain-significant dates)

**Section 6 (Event Model):**

- Use past tense for event names
- Include UTC `timestamp` and optional `calendar_date` (CalendarDate) in event envelopes
