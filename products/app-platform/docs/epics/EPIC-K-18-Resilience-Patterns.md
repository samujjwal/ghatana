...
- **Multi-Calendar Fields:** N/A (infrastructure timestamps use standard UTC).
...
- **AI Hook Type:** Predictive Failure Detection / Anomaly Detection
- **Primary Use Case:** AI predicts dependency failures BEFORE circuit breakers open by detecting early warning signals (latency percentile trends, error rate spikes, non-linear degradation patterns) and proactively adjusting circuit breaker thresholds or initiating graceful degradation.
- **Inference Mode:** Asynchronous (continuous time-series scoring on K-06 telemetry); Synchronous only for threshold adjustment (happens between requests, not on hot path).
- **Workflow Steps Exposed:** (a) Continuous dependency health scoring from K-06 latency/error metrics; (b) Proactive threshold tuning for circuit breakers; (c) Retry budget prediction (AI estimates whether retries will succeed or are hitting a persistent failure).
- **Model Registry Usage:** `dependency-health-predictor-v1` (time-series forecasting of dependency health), `retry-success-predictor-v1` (classifies whether a DLQ retry is likely to succeed based on failure pattern), `bulkhead-optimizer-v1` (suggests optimal thread pool sizes based on historical concurrency patterns)
- **Input Data / Feature Set:** P50/P95/P99 latency time series per dependency; error rate per time window; request volume; infrastructure metrics (CPU, GC pause, connection pool saturation).
- **Output / AI Annotation:** Predicted health score (0–1) per dependency, 5-minute forecast horizon; recommended circuit breaker threshold adjustments.
- **Explainability Requirement:** AI must explain which specific metric(s) drove the health score (e.g., "P99 latency increased 3x in the last 2 minutes, preceding 4 of the last 5 circuit openings").
- **Confidence Threshold:** Health score < 0.6 (60% healthy prediction) → alert SRE. Health score < 0.3 → proactively open circuit breaker (circuit breaker AI-assist mode must be explicitly enabled in K-02 config; default: off).
- **Human Override Path:** SRE can manually open/close circuit breakers via K-13 Admin Portal, overriding all AI recommendations.
- **Feedback Loop:** Actual circuit open/close events vs. AI predictions are fed back as labeled samples for `dependency-health-predictor-v1` fine-tuning.
- **Drift Monitoring:** Prediction accuracy (predicted health vs. actual outcome at T+5min) tracked daily; F1-score below 0.85 triggers retraining.
- **Fallback Behavior:** Standard threshold-based circuit breaker operates independently of AI state. AI is purely advisory and can be disabled with a feature flag.
- **Tenant Isolation:** Health prediction models are shared (platform-level telemetry) but circuit breaker configurations remain tenant-isolated.
