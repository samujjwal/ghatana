...
- **New Entities:**
  ...
  - `DailyOHLC`: `{ symbol: String, open: Decimal, high: Decimal, low: Decimal, close: Decimal, volume: BigInt, date: Date, date_calendar: CalendarDate | null }`
- **Multi-Calendar Fields:** `date_calendar` (CalendarDate) in `DailyOHLC` provides jurisdiction-native date representations (e.g., BS date for Nepal fiscal queries).
...
|---|---|---|
| `MarketDataUpdated` | New tick/quote received | `instrument_id`, `bid`, `ask`, `last`, `volume`, `exchange`, `timestamp` |
