...
- **New Entities:**
  - `ChaosExperiment`: `{ experiment_id: UUID, scenario_id: String, status: Enum(PLANNED, RUNNING, COMPLETED, ABORTED), blast_radius: JSON, started_at: CalendarDate, completed_at: CalendarDate, result: Enum(PASS, FAIL, ABORTED) }`
  - `ResilienceScorecard`: `{ module_id: String, score_date: CalendarDate, circuit_breaker_coverage: Float, fallback_coverage: Float, timeout_coverage: Float, chaos_pass_rate: Float, overall_score: Float }`
  - `DrDrillResult`: `{ drill_id: UUID, drill_type: String, rto_actual: Duration, rpo_actual: Duration, status: Enum(PASS, FAIL), completed_at: CalendarDate }`
- **Multi-Calendar Fields:** `started_at`, `completed_at`, `score_date` use `CalendarDate` (K-15 enriched).
