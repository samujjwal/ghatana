...
- **New Entities:**
  ...
  - `Session`: `{ session_id: UUID, user_id: UUID, created_at: Timestamp, expires_at: Timestamp }`
  - `BeneficialOwnershipEdge`: `{ edge_id: UUID, owned_entity_id: UUID, owner_entity_id: UUID, ownership_percentage: Decimal, control_type: Enum(DIRECT, INDIRECT), effective_date: CalendarDate, verified_at: CalendarDate, verified_by: String }` [GAP-002]
  - `UltimateBeneficialOwner`: `{ ubo_id: UUID, entity_id: UUID, person_id: UUID, total_ownership_percentage: Decimal, ownership_chain: List<UUID>, last_updated: CalendarDate }` [GAP-002]
- **Multi-Calendar Fields:** `effective_date`, `verified_at`, `last_updated` in UBO entities use `CalendarDate` (K-15 enriched). Session `created_at`/`expires_at` are standard UTC timestamps.
...
| Payload           | `{ "user_id": "...", "tenant_id": "...", "timestamp": "2025-03-02T10:30:00Z", "auth_method": "OIDC" }` |
