...
- **New Entities:**
  - `SanctionsList`: `{ list_id: String, source: String, version: String, last_updated: CalendarDate, entry_count: Int, status: Enum(ACTIVE, STALE, EXPIRED) }`
  ...
  - `ScreeningResult`: `{ screening_id: UUID, entity_screened: String, lists_checked: List<String>, result: Enum(CLEAR, POTENTIAL_MATCH, CONFIRMED_MATCH, FALSE_POSITIVE), match_score: Float, reviewed_by: String, decision_at: CalendarDate }`
- **Multi-Calendar Fields:** `last_updated` in `SanctionsList`; `decision_at` in `ScreeningResult` use `CalendarDate` (K-15 enriched).
...
- **AI Hook Type:** Pattern Recognition / NLP / Graph Traversal
- **Primary Use Case:** (a) Fuzzy entity name matching with multi-script support (Latin, Devanagari, Arabic transliteration) — AI resolves name aliases across scripts and spelling variations beyond rule-based phonetics; (b) Ultimate Beneficial Owner (UBO) graph traversal — AI traverses corporate ownership graphs to detect indirect sanctions exposure when a direct entity is clean but a parent/subsidiary is sanctioned; (c) Contextual entity disambiguation — AI uses contextual signals (jurisdiction, date of birth, address, associated entities) to distinguish same-name false positives from genuine matches; (d) Proactive list reconciliation — AI monitors newly published sanctions list updates and identifies existing client entities that may be newly affected.
- **Inference Mode:** (a)/(c) Synchronous, on-the-hot-path for real-time screening (budget: 40ms P99 AI component); (b) Asynchronous, on ownership graph update events; (d) Batch, triggered on list update ingestion (within 30 minutes of new list publication).
- **Workflow Steps Exposed:** (a) Entity name submitted → AI-enhanced screening query; (b) Client entity creation/update → UBO graph check; (c) Potential match list → AI scores and re-ranks by contextual confidence; (d) Sanctions list ingested → proactive re-screening of affected clients.
- **Model Registry Usage:** `sanctions-name-matcher-v1` (bi-encoder semantic similarity matching across scripts), `transliteration-resolver-v1` (character-level transformer: Devanagari ♡ Latin ♡ Arabic name normalization), `ubo-graph-traversal-v1` (GNN: multi-hop ownership graph sanctions exposure scoring), `entity-disambiguator-v1` (multi-attribute contextual classifier for false positive filtering)
- **Input Data / Feature Set:** Entity name + jurisdiction + DOB + address; corporate ownership graph (from company registry connectors via K-08); sanctions list embeddings (indexed in K-09 vector store); associated entities and known aliases.
- **Output / AI Annotation:** Ranked match list: `[{ list_entry_id, similarity_score, script_path, contextual_confidence, match_type: EXACT|ALIAS|TRANSLITERATION|UBO_INDIRECT }]`. UBO exposure: `{ exposure_path: [EntityRef], depth, sanctioned_entity_id, exposure_score }`.
- **Explainability Requirement:** For every potential match: AI must show (1) the normalized form of both names, (2) the similarity score breakdown (phonetic, semantic, transliteration), (3) the contextual signals that raised or lowered the confidence. For UBO matches: must display the full ownership chain with percentage shareholding at each hop.
- **Confidence Threshold:** Similarity ≥ 0.95 + contextual confidence ≥ 0.90 → auto-create compliance case (screening result = MATCH, human confirmation required). 0.70 ≤ score < 0.95 → POTENTIAL_MATCH (compliance officer must review). UBO exposure score ≥ 0.80 → flag client for Enhanced Due Diligence regardless of direct-name match result.
- **Human Override Path:** Compliance officer makes the final MATCH / NO_MATCH decision. False positive dismissals are recorded as training negatives. Repeated false positive patterns for a specific name cluster can be added to a jurisdiction-specific exclusion list (audited).
- **Feedback Loop:** Compliance officer MATCH/NO_MATCH decisions form the primary fine-tuning signal for `sanctions-name-matcher-v1` and `entity-disambiguator-v1` via K-09 HITL pipeline.
- **Drift Monitoring:** Ongoing monitoring of FP rate, recall (estimated via periodic ground-truth sample audits), and transliteration accuracy. FP rate > 10% triggers immediate model review.
- **Fallback Behavior:** Rule-based fuzzy matching (Levenshtein + Soundex + phonetic). UBO traversal falls back to depth-2 exact-match parent/subsidiary check. Transliteration falls back to configured alias table.
- **Tenant Isolation:** Sanctions lists are jurisdiction-specific (Nepal tenant: UN, OFAC, NRB lists; India tenant: UN, OFAC, RBI lists). AI models are shared; vector store indexes are tenant-partitioned.
