...
- **AI Hook Type:** Document Verification / Fraud Detection / Risk Scoring / Suitability Assessment
- **Primary Use Case:** (a) Document authenticity and tamper detection (Citizenship, PAN, Passport, Demat account statements) with multi-language OCR for Devanagari and English; (b) Identity network fraud detection — AI detects applicants who share document fields (same address, same photo hash, same device fingerprint) with known fraud profiles; (c) Applicant risk scoring — AI estimates financial crime risk using behavioral and demographic signals beyond document validity; (d) Product suitability assessment — AI evaluates whether the applied-for product matches the applicant's declared risk tolerance, income, and investment horizon.
- **Inference Mode:** (a) Synchronous on document upload (budget: 3 seconds per document page); (b) Asynchronous after identity is verified (runs within 5 minutes of application submission); (c) Asynchronous nightly re-scoring for in-progress applications; (d) Synchronous when applicant reaches the product selection step.
- **Workflow Steps Exposed:** (a) Document upload → AI authenticity check before human review; (b) Application submitted → identity network graph query; (c) Compliance review queue prioritized by AI applicant risk score; (d) Product selection screen shows AI suitability recommendation.
- **Model Registry Usage:** `doc-verification-v1` (computer vision: document tamper detection, MRZ validation, Devanagari OCR), `identity-fraud-detector-v1` (GNN: identity network fraud ring detection), `applicant-risk-scorer-v1` (gradient boosted: financial crime risk from behavioral + demographic signals), `suitability-assessor-v1` (rule-enhanced ML: product suitability vs declared risk profile)
- **Input Data / Feature Set:** Document images + extracted OCR text; applicant device fingerprint; application behavioral signals (time-to-complete, correction frequency, device geolocation vs declared address); income declaration; investment horizon; risk appetite declaration; D-14 sanctions screening result.
- **Output / AI Annotation:** (a) `{ authentic: Boolean, confidence, tamper_regions: [BoundingBox], ocr_fields: { name, dob, id_number } }`. (b) Fraud network score (0–1) + shared attributes found. (c) Applicant risk tier (LOW | MEDIUM | HIGH | VERY_HIGH) + top risk factors. (d) Suitability matrix: `{ product_id, suitable: Boolean, rationale, recommended_alternatives: [ProductRef] }`.
- **Explainability Requirement:** Document verification: AI must mark the specific pixels/regions suspected of tampering. Fraud network: must show the graph path between applicant and known fraud profile. Risk scorer: top-3 behavioral signals driving the score. Suitability: must cite which declared attribute(s) conflict with the product's target suitability criteria.
- **Confidence Threshold:** Document authenticity < 0.70 → flag for mandatory human review; < 0.40 → auto-reject document with re-submission request. Fraud network score ≥ 0.75 → pause application and route to Compliance team. Applicant risk VERY_HIGH → flag for Enhanced Due Diligence before approval. Suitability not matching → show advisory to applicant (non-blocking); applicant must explicitly confirm understanding before proceeding.
- **Human Override Path:** Compliance officer can approve a document flagged by AI (with documented reason, audited). Fraud network flag can be dismissed with senior compliance approval. Suitability advisory can be acknowledged and overridden by applicant (with regulatory risk disclosure).
- **Feedback Loop:** Manual review outcomes (approved despite AI flag, rejected despite AI pass) are fed to `doc-verification-v1` and `identity-fraud-detector-v1` fine-tuning. Risk scorer is calibrated monthly against approved applicants who subsequently triggered compliance flags.
- **Drift Monitoring:** Monthly: document model precision/recall vs. manual review outcomes; fraud detector FP/FN rates; risk score calibration curve (predicted vs. actual compliance flag rate).
- **Fallback Behavior:** Standard manual document review by compliance team. No AI risk scoring — all applications treated as MEDIUM and reviewed manually. Suitability falls back to static product eligibility rules.
- **Tenant Isolation:** Document models handle jurisdiction-specific document types (Nepal: Citizenship Certificate, Nepal PAN; India: Aadhaar, PAN). Risk signals and fraud graph are tenant-scoped. Suitability rules are T2 pack-configurable per jurisdiction.
