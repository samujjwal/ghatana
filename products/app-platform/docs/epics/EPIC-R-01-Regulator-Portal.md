...
- **New Entities:**
  - `RegulatorAccess`: `{ access_id: UUID, regulator_id: String, jurisdiction: String, granted_at: CalendarDate, expires_at: CalendarDate, status: Enum }`
  - `EvidencePackage`: `{ package_id: UUID, regulator_id: String, query_params: JSON, generated_at: CalendarDate, file_path: String, signature: String, status: Enum }`
  - `ComplianceAttestation`: `{ attestation_id: UUID, regulator_id: String, type: Enum, period_start: CalendarDate, period_end: CalendarDate, submitted_at: CalendarDate, status: Enum }`
- **Multi-Calendar Fields:** `granted_at`, `expires_at`, `generated_at`, `period_start`, `period_end`, `submitted_at` use `CalendarDate` (K-15 enriched).
...
| Payload           | `{ "package_id": "...", "regulator_id": "SEBON", "query_params": {...}, "file_size_mb": 125, "signature": "...", "timestamp": "2025-03-02T10:30:00Z" }` |
...
- **AI Hook Type:** Query Assistance / Anomaly Detection / Narrative Generation / Investigation Hint
- **Primary Use Case:** (a) Semantic natural-language query interface — regulator types "show me all suspicious securities trades by PEP-linked entities in Q1 2082" and AI translates to a structured multi-filter query across K-07 and D-07 data; (b) Evidence package narration — AI generates a concise natural-language executive summary of the evidence package for senior regulator/legal review; (c) Access anomaly detection — AI monitors regulator user behavior for unusual data access patterns (excessive downloads, off-hours access, queries targeting specific client groups beyond the scope of an open investigation); (d) Proactive investigation hints — AI analyzes queried evidence and surfaces thematic patterns the regulator has not yet queried (e.g., "3 of the 5 entities in this investigation also had trades in Instrument X on the same day as Entity A").
- **Inference Mode:** (a) Synchronous, interactive (budget: 3 seconds for query translation); (b) On-demand, background generation (budget: 30s for 100-page evidence package); (c) Asynchronous real-time on every regulator action event; (d) On-demand after regulator executes investigation package query.
- **Workflow Steps Exposed:** (a) Query builder: NL input → AI-structured filter suggestion → regulator confirms → query executes; (b) Evidence package ready → AI summary section automatically injected at the top; (c) Access log event → AI behavior score computed → anomalous sessions flagged to portal admin; (d) Investigation package view → "AI Hints" panel with suggested additional queries.
- **Model Registry Usage:** `query-assistant-v1` (LLM + structured finance schema understanding: NL → query DSL), `evidence-summarizer-v1` (LLM: long-document summarization of legal/regulatory evidence), `regulator-access-anomaly-v1` (unsupervised: session behavior baseline + anomaly scoring), `investigation-hint-v1` (graph analytics + LLM: entity co-occurrence pattern discovery within scoped evidence)
- **Input Data / Feature Set:** Regulator query history + role context; evidence package documents (K-07 audit trails, D-07 compliance reports, D-08 surveillance alerts); regulator session/access logs from K-01; entity co-occurrence graph computed over the retrieved evidence set.
- **Output / AI Annotation:** (a) Structured query specification: `{ filters: [...], date_range, entity_ids, instrument_ids, confidence: Float }`. (b) Evidence summary: `{ executive_summary, key_entities: [EntityRef], key_timeline_events: [EventRef], risk_themes: [String] }`. (c) Anomaly alert: `{ anomaly_type, score, suspicious_queries: [QueryRef], recommended_action }`. (d) Hints: `[{ hint_text, suggested_query, entity_refs, relevance_score }]`.
- **Explainability Requirement:** Query assistant: must show the mapping between each NL phrase and the corresponding filter applied, allowing regulator to understand and correct. Evidence summarizer: must cite the specific document/event IDs referenced in each summary statement (no hallucination). Investigation hints: must show the exact co-occurrence data driving the hint.
- **Confidence Threshold:** Query assistant confidence < 0.75 → present the structured query for explicit confirmation before execution (never auto-execute a low-confidence NL query against sensitive financial data). Access anomaly score ≥ 0.80 → alert portal admin and suspend the session pending review.
- **Human Override Path:** Regulator can freely modify the AI-translated query before execution. All access anomaly alerts are reviewed by portal admin before any session suspension action. Investigation hints are purely advisory — regulator chooses whether to follow them.
- **Feedback Loop:** Regulator corrections to AI-translated queries (accepted vs. heavily modified) feed `query-assistant-v1`. Access anomaly false positive flags from portal admins feed `regulator-access-anomaly-v1`.
- **Drift Monitoring:** Query translation accuracy (accepted without modification rate), evidence summary quality (regulator-rated), anomaly FP rate. Monthly report generated in K-09 AI Observability dashboard.
- **Fallback Behavior:** Manual query builder with structured filter form. Evidence packages generated without AI summary (template-based table of contents only). Standard audit log review for access control.
- **Tenant Isolation:** Regulators are strictly jurisdiction-scoped — a SEBON regulator can only access Nepal-tenant data. AI query assistant operates within the regulator’s data scope boundary enforced by K-01 IAM + K-08 residency rules. Investigation hints are scoped to the regulatory case's entity set.
