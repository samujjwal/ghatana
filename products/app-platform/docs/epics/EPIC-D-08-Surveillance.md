...
5. **FR5 Multi-Calendar Context:** Case timelines and reported events use `CalendarDate` timestamps via K-15.
...
- **Multi-Calendar Fields:** `created_at` in alerts uses `CalendarDate`.
...
| Payload           | `{ "alert_id": "...", "pattern": "WASH_TRADE", "confidence": 0.95, "timestamp": "2025-03-02T10:30:00Z", "calendar_date": {} }` |
...
- **AI Hook Type:** Autonomous Agent / Pattern Recognition / Graph Analytics
- **Primary Use Case:** (a) Temporal sequence analysis to detect layering, spoofing, and front-running across the real-time order book and trade feed; (b) Entity relationship graph construction — AI builds undirected graphs connecting accounts, directors, analysts, brokers, and instruments to surface coordinated insider trading rings; (c) Cross-asset correlation anomaly — AI detects abnormal synchronization between option/equity price movements and specific order submissions that precede material news events; (d) Emerging schema detection — AI discovers novel manipulation patterns not yet captured in T2 rule sets and surfaces them for rule authoring.
- **Inference Mode:** (a) Near-real-time streaming (< 50ms lag from live feed, leveraging K-05 event stream); (b) Daily entity graph refresh (overnight batch); (c) Event-triggered via `PriceAnomaly` events from D-05; (d) Weekly unsupervised clustering analysis.
- **Workflow Steps Exposed:** (a) Streaming alert generation; (b) Analyst dashboard: case enriched with relationship graph visualization; (c) Cross-asset correlated alert clustering; (d) Novel pattern library with analyst review workflow.
- **Model Registry Usage:** `market-abuse-detector-v1` (multi-class: LAYERING | SPOOFING | FRONT_RUN | WASH_TRADE | PUMP_DUMP), `entity-relationship-graph-v1` (GNN: k-hop entity relationship scoring for coordinated manipulation), `cross-asset-correlation-v1` (time-series correlation + Granger causality: option order → equity price lead-lag analysis), `pattern-discovery-v1` (unsupervised: DBSCAN clustering of order flow sequences for novel pattern extraction)
- **Input Data / Feature Set:** Order book snapshots (L2 depth); trade confirmations; account → instrument → broker association graph; director/officer entity registry; D-05 `PriceUpdated` events with `ai_annotation.anomaly_score`; news event timestamps.
- **Output / AI Annotation:** `SurveillanceAlert` entity enriched with: `{ abuse_type, confidence_score, evidence_sequence: [EventRef], entity_graph: { nodes, edges }, cross_asset_lead_lag_seconds, pattern_similarity_to_known_cases }`. Every alert carries `ai_annotation` in the emitted `AlertRaised` event.
- **Explainability Requirement:** Temporal pattern: AI must reproduce the exact sequence of orders (with timestamps and sizes) that triggered the pattern match, annotated with which threshold was crossed. Entity graph: AI must show the shortest graph path between flagged accounts. Cross-asset: must show the time series correlation plot data and Granger test p-value.
- **Confidence Threshold:** Confidence ≥ 0.80 → auto-create alert case (human review required). Confidence 0.50–0.79 → add to watchlist for monitoring. Novel pattern discovery: analyst review mandatory before classifying.
- **Human Override Path:** Compliance officer can mark alert as False Positive (with reason). Repeated FP marking for a specific account suppresses low-confidence future alerts for 30 days with escalation to Compliance Head.
- **Feedback Loop:** FP/TP decisions feed `market-abuse-detector-v1` continuous fine-tuning pipeline via K-09 HITL mechanism. Novel patterns approved by analyst are added to `market-abuse-detector-v1` training set.
- **Drift Monitoring:** Weekly FP rate, detection rate (recall on confirmed cases), alert volume trend. If FP rate exceeds 15%, model review triggered immediately.
- **Fallback Behavior:** Deterministic T2 rule-based surveillance continues operating independently of AI. AI is an additive layer. All T2 rules fire regardless of AI model state.
- **Tenant Isolation:** Alert cases and entity graphs are tenant-scoped. Cross-asset correlation operates within the same tenant's instruments. Novel pattern discovery is tenant-partitioned (patterns from Tenant A do not influence Tenant B's alerts).
