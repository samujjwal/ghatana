...
public class SalesPipelinesRegistrar {
    ...
    public Promise<Void> initialize() {
        return Promise.complete().whenResult(() -> {
            pipelines.put("lead-qualification", new PipelineOp("sales-leads", "Lead Qualification",
                    Arrays.asList("lead.generated", "lead.qualified", "contact.initiated")));
            pipelines.put("opportunity-advancement", new PipelineOp("sales-opp", "Opportunity",
                    Arrays.asList("opportunity.created", "opportunity.progressed", "deal.closed")));
            pipelines.put("forecasting", new PipelineOp("sales-forecast", "Forecasting",
                    Arrays.asList("forecast.calculated", "trend.analyzed", "report.published")));
            this.status = Status.READY;
            metrics.incrementCounter("sales.pipelines.registered", "organization", organizationId, "count", "3");
        });
    }
    ...
    public Promise<Void> start() {
        return Promise.complete().whenResult(() -> {
            ...
            metrics.incrementCounter("sales.pipelines.started", "organization", organizationId);
        });
    }
        ...
        return Promise.ofCallback(cb -> {
            pipelines.values().forEach(p -> {
                if (p.matches(event.getType())) {
                    p.recordEvent(event);
                    metrics.incrementCounter("sales.pipeline.matched", "pipeline", p.name, "org", organizationId);
                }
            });
