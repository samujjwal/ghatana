...
import com.ghatana.products.finance.domains.marketdata.service.L1QuoteService;
...
public class MarketDataWebSocketGateway {
    ...
    private final Map<String, IWebSocket> connections = new ConcurrentHashMap<>();
    ...
    public void onL1Update(L1Quote quote) {
        ...
        for (String connectionId : subscribers) {
            IWebSocket ws = connections.get(connectionId);
            if (ws != null) {
                ws.writeMessage(IWebSocket.Message.text(message))
                  .whenException(ex -> {
    ...
    private void handleConnection(IWebSocket webSocket) {
        ...
        if (!validateJwt(token)) {
            webSocket.writeMessage(IWebSocket.Message.text("{\"type\":\"ERROR\",\"code\":\"UNAUTHORIZED\"}"))
        ...
        l1QuoteService.getAllL1()
                .whenResult(quotes -> {
                    for (L1Quote q : quotes) {
                        webSocket.writeMessage(IWebSocket.Message.text(l1UpdateJson(q)));
                    }
                });

        // Message receive loop
        readLoop(connectionId, webSocket);
    }

    private void readLoop(String connectionId, IWebSocket webSocket) {
        webSocket.readMessage()
                .whenResult(msg -> {
                    if (msg != null) {
                        handleMessage(connectionId, webSocket, msg.getText());
                        readLoop(connectionId, webSocket);
                    } else {
                        handleDisconnect(connectionId);
                    }
    ...
    private void handleMessage(String connectionId, IWebSocket webSocket, String text) {
        try {
            ...
            if ("subscribe".equals(action)) {
                try {
                    ...
                    webSocket.writeMessage(IWebSocket.Message.text(
