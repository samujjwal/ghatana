...
/**
 * Unit tests for MarketDataDomainModule.
 *
 * @doc.type test
 * @doc.purpose Verify Market Data domain module behavior
 * @author Ghatana Finance Team
 * @since 1.0.0
 */
@DisplayName("Market Data Domain Module Tests")
class MarketDataDomainModuleTest {

    private MarketDataDomainModule module;
    private KernelContext mockContext;

    @BeforeEach
    void setUp() {
        module = new MarketDataDomainModule();
    ...
    void shouldReturnCorrectModuleId() {
        assertThat(module.getModuleId()).isEqualTo("finance-market-data");
    }
    ...
    void shouldDeclareAllRequiredCapabilities() {
        ...
        assertThat(capabilities.stream()
            ...
            .anyMatch(id -> id.equals("finance.market.data.feeds")))
            .isTrue();
        assertThat(capabilities.stream()
            .map(KernelCapability::getCapabilityId)
            .anyMatch(id -> id.equals("finance.order.book.management")))
            .isTrue();
    }
