...
public class InstrumentService {
    ...
    public Promise<Instrument> create(String symbol, String exchange, String isin,
        ...
        return Promise.ofBlocking(executor, () -> {
            boolean exists = store.existsBySymbolAndExchange(symbol, exchange).getResult();
            if (exists) {
            ...
            store.save(instrument).getResult();
    ...
    public Promise<Optional<Instrument>> findById(UUID id) {
        return Promise.ofBlocking(executor, () -> {
            ...
            Optional<Instrument> found = store.findCurrentById(id).getResult();
            found.ifPresent(i -> {
    ...
    public Promise<Optional<Instrument>> findByIdAsOf(UUID id, LocalDate asOf) {
        return Promise.ofBlocking(executor, () -> store.findByIdAsOf(id, asOf).getResult());
    }
    ...
    public Promise<List<Instrument>> list(InstrumentStatus statusFilter) {
        return Promise.ofBlocking(executor, () -> store.listCurrent(statusFilter).getResult());
    }

    /** Full-text search on symbol, ISIN, or name. */
    public Promise<List<Instrument>> search(String query, int limit) {
        return Promise.ofBlocking(executor, () -> store.search(query, limit).getResult());
    }
    ...
    private static class InstrumentCacheDto {
        ...
        public InstrumentCacheDto(Instrument i) {
            ...
            this.createdAt      = i.createdAtUtc();
            this.calendarDateBs = String.valueOf(i.metadata().getOrDefault("calendarDateBs", ""));
