package com.ghatana.products.finance.domains.corporateactions.service;
...
public class CaInstructionNarrativeService {
    ...
    public Promise<CaNarrative> generateNarrative(String caId, String approverRole) {
        return Promise.ofBlocking(executor, () -> {
            ...
            auditPort.emit(AuditEvent.builder()
                    .tenantId(caId).eventType("NARRATIVE_DRAFT")
                    .resourceType("narrative").resourceId(narrativeId)
                    .detail("caId", caId).detail("prompt", prompt).detail("draft", draft)
                    .build());
            draftCounter.increment();

            String taskId = workflowPort.createApprovalTask(narrativeId, "CA_NARRATIVE", approverRole, "Approve narrative for CA " + caId);
    ...
    public Promise<CaNarrative> approveNarrative(String narrativeId, String approvedBy,
        ...
        return Promise.ofBlocking(executor, () -> {
            ...
            auditPort.emit(AuditEvent.builder().tenantId(existing.caId()).eventType("NARRATIVE_APPROVED").resourceType("narrative").resourceId(narrativeId).build());
    ...
    public Promise<CaNarrative> rejectNarrative(String narrativeId, String rejectedBy,
        ...
        return Promise.ofBlocking(executor, () -> {
            ...
            auditPort.emit(AuditEvent.builder().tenantId(existing.caId()).eventType("NARRATIVE_REJECTED").resourceType("narrative").resourceId(narrativeId).build());
