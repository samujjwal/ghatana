...
/**
 * Unit tests for PricingDomainModule.
 *
 * @doc.type test
 * @doc.purpose Verify Pricing domain module behavior
 * @author Ghatana Finance Team
 * @since 1.0.0
 */
@DisplayName("Pricing Domain Module Tests")
class PricingDomainModuleTest {

    private PricingDomainModule module;
    private KernelContext mockContext;

    @BeforeEach
    void setUp() {
        module = new PricingDomainModule();
    ...
    void shouldReturnCorrectModuleId() {
        assertThat(module.getModuleId()).isEqualTo("finance-pricing");
    }
    ...
    void shouldDeclareAllRequiredCapabilities() {
        ...
        assertThat(capabilities.stream()
            ...
            .anyMatch(id -> id.equals("finance.asset.pricing")))
            .isTrue();
        assertThat(capabilities.stream()
            .map(KernelCapability::getCapabilityId)
            .anyMatch(id -> id.equals("finance.valuation.engine")))
            .isTrue();
    }
