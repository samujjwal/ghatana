package com.ghatana.products.finance.domains.surveillance.service;
...
public class AlertNarrativeGenerationService {
    ...
    public Promise<AlertNarrative> generateDraft(AlertContext ctx) {
        return Promise.ofBlocking(executor, () -> {
            ...
            auditPort.emit(AuditEvent.builder()
                    .tenantId(ctx.alertId()).eventType("NARRATIVE_DRAFTED")
                    .resourceType("NARRATIVE").resourceId(narrativeId)
                    .detail("prompt", prompt).detail("draft", draft)
                    .build());
    ...
    public Promise<AlertNarrative> approveNarrative(String narrativeId, String analystId) {
        return Promise.ofBlocking(executor, () -> {
            ...
            auditPort.emit(AuditEvent.builder()
                    .tenantId(analystId).eventType("NARRATIVE_APPROVED")
                    .resourceType("NARRATIVE").resourceId(narrativeId)
                    .build());
    ...
    public Promise<AlertNarrative> rejectNarrative(String narrativeId, String analystId,
        ...
        return Promise.ofBlocking(executor, () -> {
            ...
            auditPort.emit(AuditEvent.builder()
                    .tenantId(analystId).eventType("NARRATIVE_REJECTED")
                    .resourceType("NARRATIVE").resourceId(narrativeId)
                    .build());
            return loadNarrative(narrativeId);
        });
