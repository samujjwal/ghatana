...
public class VarApiService {
    ...
    private VarResponse buildVarResponse(String portfolioId, String method,
        ...
        var calcResult = varCalculationService.calculateVar(
                portfolioId,
                VarCalculationService.VarMethod.valueOf(method),
                confidence >= 0.99 ? VarCalculationService.ConfidenceLevel.PCT_99 : VarCalculationService.ConfidenceLevel.PCT_95,
                holdingPeriodDays
        ).getResult();
        double varAmount = calcResult.varAmount().doubleValue();
        List<InstrumentContribution> decomp = buildDecomposition(portfolioId, varAmount);
        log.debug("VaR query portfolioId={} method={} var={}", portfolioId, method, varAmount);
        return new VarResponse(portfolioId, method, confidence, holdingPeriodDays,
                               varAmount, LocalDate.now(NST), decomp);
    }
