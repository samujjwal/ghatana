...
/**
 * Unit tests for SanctionsDomainModule.
 *
 * @doc.type test
 * @doc.purpose Verify Sanctions domain module behavior
 * @author Ghatana Finance Team
 * @since 1.0.0
 */
@DisplayName("Sanctions Domain Module Tests")
class SanctionsDomainModuleTest {

    private SanctionsDomainModule module;
    private KernelContext mockContext;

    @BeforeEach
    void setUp() {
        module = new SanctionsDomainModule();
    ...
    void shouldReturnCorrectModuleId() {
        assertThat(module.getModuleId()).isEqualTo("finance-sanctions");
    }
    ...
    void shouldDeclareAllRequiredCapabilities() {
        ...
        assertThat(capabilities.stream()
            ...
            .anyMatch(id -> id.equals("finance.sanctions.screening")))
            .isTrue();
        assertThat(capabilities.stream()
            .map(KernelCapability::getCapabilityId)
            .anyMatch(id -> id.equals("finance.pep.screening")))
            .isTrue();
    }
