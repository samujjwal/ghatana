...
public class ScreeningEngineService {
    ...
    /**
     * Synchronous batch screening entry point.
     * Creates a minimal request using clientId and delegates to screen().
     */
    public boolean screenClient(String clientId) {
        ScreeningRequest request = new ScreeningRequest(
                UUID.randomUUID().toString(), clientId, ScreeningEntityType.INDIVIDUAL,
                null, null, List.of(clientId));
        ScreeningResult result = screen(request, "BATCH-" + clientId).getResult();
