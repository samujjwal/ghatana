...
public class ComplianceScreeningE2eTestSuiteService {
    ...
    public Promise<SuiteResult> runAll() {
        return Promise.ofBlocking(executor, () -> {
            ...
            audit.emit(AuditEvent.builder().tenantId("integration-test").eventType("E2E_COMPLIANCE_SUITE")
                .detail("summary", "passed=" + passed + " failed=" + (results.size() - passed)).build());
    ...
    private void sanctionsBlockOrder(String runId) throws Exception {
        ...
        try {
            ...
            assertStep(runId, "order_blocked", "order blocked by sanctions", "BLOCKED", true, "BLOCKED");
    ...
    private void kycBlockedOrder(String runId) throws Exception {
        ...
        try {
            ...
            assertStep(runId, "order_kyc_blocked", "order blocked by KYC expiry", "BLOCKED", true, "BLOCKED");
            return;
        }
        String status = orderSubmission.getOrderStatus(orderId);
