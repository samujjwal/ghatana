...
`products/aep/aep-engine` is the core AEP execution engine. It owns the runtime primitives that all pipeline stages build on:
...
```bash
./gradlew :products:aep:aep-engine:test
```
