 ...
 * @doc.pattern Test
 */
@DisplayName("HttpHandlerUtils [GH-90000]")
class HttpHandlerUtilsTest {
    ...
    @Test
    @DisplayName("jsonResponse serializes a Map and returns correct status code [GH-90000]")
    void jsonResponse_mapData_returnsJsonWithCorrectStatus() { // GH-90000
        ...
        assertThat(body).contains("\"key\"").contains("\"value\"").contains("42 [GH-90000]");
        assertThat(response.getHeader(io.activej.http.HttpHeaders.CONTENT_TYPE)) // GH-90000
                .contains("application/json [GH-90000]");
    }

    @Test
    @DisplayName("jsonResponse with 404 sets the correct HTTP status code [GH-90000]")
    ...
    @Test
    @DisplayName("errorResponse wraps message in error field and returns the given status [GH-90000]")
    void errorResponse_wrapsMessage() { // GH-90000
        ...
        String body = response.getBody().asString(StandardCharsets.UTF_8); // GH-90000
        assertThat(body).contains("\"error\"").contains("bad request body [GH-90000]");
    }

    @Test
    @DisplayName("toJson converts a simple object to a Map [GH-90000]")
    void toJson_simpleMap_returnsConvertedMap() { // GH-90000
        ...
        assertThat(result).containsKey("name [GH-90000]").containsKey("version [GH-90000]");
        assertThat(result.get("name [GH-90000]")).isEqualTo("test-agent [GH-90000]");
    }

    @Test
    @DisplayName("toJson returns empty map on unserializable input [GH-90000]")
    void toJson_unserializableInput_returnsEmptyMap() { // GH-90000
        ...
        Map<String, Object> result = utils.toJson("not-a-map [GH-90000]");
    ...
    @Test
    @DisplayName("jsonResponse handles serialization failure gracefully with 500 [GH-90000]")
