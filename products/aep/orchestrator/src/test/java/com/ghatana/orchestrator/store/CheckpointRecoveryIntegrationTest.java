...
class CheckpointRecoveryIntegrationTest extends EventloopTestBase {
    ...
    class FullLifecycleTests {
        ...
        void fullSuccessfulExecution() { 
            PipelineCheckpoint cp = store.createExecution(TENANT, PIPELINE, "inst-1", "key-1", Map.of("totalSteps", 3)); 
            ...
            // Step 1: extract
            PipelineCheckpoint after1 = store.updateCheckpoint( 
                    "inst-1",
                    "step-extract",
                    "Extract",
            ...
            // Step 2: transform
            PipelineCheckpoint after2 = store.updateCheckpoint( 
                    "inst-1",
                    "step-transform",
                    "Transform",
                    PipelineCheckpointStatus.STEP_SUCCESS,
            ...
            // Step 3: load
            PipelineCheckpoint after3 = store.updateCheckpoint( 
                    "inst-1",
                    "step-load",
                    "Load",
                    PipelineCheckpointStatus.STEP_SUCCESS,
            ...
            store.completeExecution( 
                    "inst-1", PipelineCheckpointStatus.COMPLETED, Map.of("totalRows", 950, "status", "done")); 
        ...
        void failedExecutionLifecycle() { 
            store.createExecution(TENANT, PIPELINE, "inst-fail", "key-fail", Map.of("totalSteps", 3)); 

            store.updateCheckpoint( 
                    "inst-fail", "step-1", "Step1", PipelineCheckpointStatus.STEP_SUCCESS, Map.of(), Map.of()); 
            ...
            store.completeExecution("inst-fail", PipelineCheckpointStatus.FAILED, Map.of("error", "Step2 timed out")); 
    ...
    class ResumeTests {
        ...
        void resumeFromLastSuccessfulStep() { 
            ...
            store.createExecution(TENANT, PIPELINE, "inst-resume", "key-resume", Map.of("totalSteps", 5)); 
        ...
        void stepHistoryPreservesOrder() { 
            ...
            assertThat(history).extracting(StepCheckpoint::getStepId).containsExactly("step-a", "step-b", "step-c"); 
        }
    }
    ...
    class IdempotencyTests {
        ...
        void duplicateIdempotencyKeyThrows() { 
            ...
            assertThatThrownBy(() -> store.createExecution(TENANT, PIPELINE, "inst-dup-2", "key-dup", Map.of())) 
    ...
    class QueryTests {
        ...
        void findByPipelineIdOrdered() { 
            ...
            assertThat(results).extracting(PipelineCheckpoint::getPipelineId).containsOnly(PIPELINE); 
    ...
    class QueueIntegrationTests {
        ...
        void pollSkipsCompletedJobs() { 
            ...
            PipelineCheckpoint cp =
                    store.findByIdempotencyKey(TENANT, "skip-key").orElseThrow(); 
    ...
    class ConcurrencyTests {
        ...
        void manyConcurrentExecutions() { 
            IntStream.range(0, 50) 
                    .forEach(i -> 
                            store.createExecution(TENANT, PIPELINE, "inst-" + i, "key-" + i, Map.of("totalSteps", 5))); 
    ...
    class EdgeCaseTests {
        ...
        void updateNonExistentThrows() { 
            assertThatThrownBy(() -> store.updateCheckpoint( 
                            "no-such-instance", "s", "S", PipelineCheckpointStatus.STEP_SUCCESS, Map.of(), Map.of())) 
                    .isInstanceOf(RuntimeException.class) 
        ...
        void completeNonExistentThrows() { 
            assertThatThrownBy(() -> store.completeExecution("no-such", PipelineCheckpointStatus.COMPLETED, Map.of())) 
                    .isInstanceOf(RuntimeException.class) 
                    .hasMessageContaining("not found");
        }
        ...
        void stepFailureNoIncrement() { 
            ...
            store.updateCheckpoint("inst-noinc", "s1", "S1", PipelineCheckpointStatus.STEP_FAILED, Map.of(), Map.of()); 
        ...
        void statePreservedAcrossUpdates() { 
            store.createExecution(TENANT, PIPELINE, "inst-state", "key-state", Map.of("initial", "data")); 
        ...
        void nullStatePreservesPrevious() { 
            store.createExecution(TENANT, PIPELINE, "inst-null", "key-null", Map.of("keep", "this")); 

            store.updateCheckpoint("inst-null", "s1", "S1", PipelineCheckpointStatus.STEP_SUCCESS, Map.of(), null); 
    ...
    private void recordSuccessfulStep(String instanceId, String stepId, String stepName, Map<String, Object> output) { 
        ...
        store.updateCheckpoint(instanceId, stepId, stepName, PipelineCheckpointStatus.STEP_SUCCESS, output, Map.of()); 
    }
}
