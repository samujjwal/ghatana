...
public class ExecutionQueueJobRepository {
    ...
    public List<ExecutionQueueJobEntity> pollPendingJobs(int maxJobs) {
        ...
        List<ExecutionQueueJobEntity> jobs = entityManager
                .createNativeQuery(
    ...
    public List<ExecutionQueueJobEntity> findExpiredLeases() {
        return entityManager
                .createQuery(
                        "SELECT j FROM ExecutionQueueJobEntity j " + "WHERE j.status = :status "
    ...
    public long countByStatus(ExecutionQueueJob.JobStatus status, String tenantId) {
        ...
        TypedQuery<Long> query = entityManager.createQuery(jpql, Long.class).setParameter("status", status);
    ...
    public int deleteOldJobs(Instant completedBefore) {
        return entityManager
                .createQuery("DELETE FROM ExecutionQueueJobEntity j " + "WHERE j.completedAt IS NOT NULL "
    ...
    public Long getOldestPendingAgeMs(String tenantId) {
        String jpql = "SELECT MIN(j.enqueuedAt) FROM ExecutionQueueJobEntity j " + "WHERE j.status = :status";

        if (tenantId != null) {
            jpql += " AND j.tenantId = :tenantId";
        }

        TypedQuery<Instant> query = entityManager
                .createQuery(jpql, Instant.class)
