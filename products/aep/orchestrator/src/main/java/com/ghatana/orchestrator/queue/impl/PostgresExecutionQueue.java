...
public class PostgresExecutionQueue implements ExecutionQueue {
            ...
            try (Connection conn = dataSource.getConnection()) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    ...
                    if (rows == 0) {
                        ...
                        logger.info(
                                "Enqueued job: tenantId={}, pipelineId={}, idempotencyKey={}",
                                tenantId,
                                pipelineId,
                                idempotencyKey);
                    }
    ...
    public int size() {
        ...
        try (Connection conn = dataSource.getConnection();
                PreparedStatement ps =
                        conn.prepareStatement("SELECT COUNT(*) FROM execution_queue WHERE status = 'PENDING'")) {
    ...
    private ExecutionJob mapResultSetToJob(ResultSet rs) throws SQLException {
        try {
            ...
            Object triggerData = triggerDataJson != null ? objectMapper.readValue(triggerDataJson, Object.class) : null;

            // Note: instanceId is not in execution_queue table yet, assuming it's generated later or part of
            // triggerData
