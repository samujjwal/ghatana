...
public class PostgresqlCheckpointStore implements CheckpointStore {
    ...
    @Override
    public PipelineCheckpoint createExecution(
            String tenantId,
            String pipelineId,
            String instanceId,
        ...
        logger.info(
                "Creating execution checkpoint: tenantId={}, pipelineId={}, instanceId={}, idempotencyKey={}",
                tenantId,
                pipelineId,
                instanceId,
                idempotencyKey);
        ...
        if (pipelineRepository.existsByIdempotencyKey(tenantId, idempotencyKey)) {
            logger.warn(
                    "Duplicate execution attempt detected: tenantId={}, idempotencyKey={}", tenantId, idempotencyKey);
        ...
        logger.info(
                "Created execution checkpoint: instanceId={}, status={}",
    ...
    @Override
    public PipelineCheckpoint updateCheckpoint(
            String instanceId,
            String stepId,
            String stepName,
        ...
        logger.debug(
                "Updated checkpoint: instanceId={}, completedSteps={}, status={}",
                checkpoint.getInstanceId(),
    ...
    public Optional<PipelineCheckpoint> findByInstanceId(String instanceId) {
        ...
        return pipelineRepository.findById(instanceId).map(PipelineCheckpointEntity::toDomainObject);
    }
    ...
    public List<PipelineCheckpoint> findByPipelineId(String tenantId, String pipelineId, int limit) {
        logger.debug(
                "Finding checkpoints by pipeline ID: tenantId={}, pipelineId={}, limit: {}",
                tenantId,
                pipelineId,
                limit);

        List<PipelineCheckpointEntity> entities =
                pipelineRepository.findByPipelineIdOrderByCreatedAtDesc(tenantId, pipelineId);
    ...
    public int cleanupOldCheckpoints(Instant completedBefore) {
        ...
        logger.info(
                "Cleanup completed: {} pipeline checkpoints, {} step checkpoints deleted",
                deletedPipelines,
                deletedSteps);
    ...
    public Optional<StepCheckpoint> getLastSuccessfulStep(String instanceId) {
        ...
        return stepRepository.findLastSuccessfulStep(instanceId).map(StepCheckpointEntity::toDomainObject);
    }

    @Override
    public void recordStepCheckpoint(String instanceId, StepCheckpoint stepCheckpoint) {
        logger.debug(
                "Recording step checkpoint: instanceId={}, stepId={}, status={}",
                instanceId,
                stepCheckpoint.getStepId(),
                stepCheckpoint.getStatus());

        // Check if step checkpoint already exists
        Optional<StepCheckpointEntity> existingOpt =
                stepRepository.findByInstanceIdAndStepId(instanceId, stepCheckpoint.getStepId());
        ...
        if (existingOpt.isPresent()) {
            ...
            entity = new StepCheckpointEntity(
                    instanceId,
                    stepCheckpoint.getStepId(),
                    stepCheckpoint.getStepName(),
                    stepCheckpoint.getStatus(),
                    stepCheckpoint.getInput());
            entity.updateFrom(stepCheckpoint);
        }
        ...
        logger.debug(
                "Recorded step checkpoint: instanceId={}, stepId={}, status={}",
                instanceId,
                stepCheckpoint.getStepId(),
                stepCheckpoint.getStatus());
    }
