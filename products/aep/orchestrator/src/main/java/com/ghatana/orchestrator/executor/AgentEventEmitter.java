...
public class AgentEventEmitter {
    ...
    public Promise<Void> emitStepResult(AgentStepResult result) {
        ...
        try {
            ...
            logger.debug(
                    "Emitting agent step result event: stepId={}, status={}, duration={}ms",
                    result.getStepId(),
                    result.getStatus(),
                    result.getDurationMs());

            return eventLogClient.publishEvent(tenantId, event).whenComplete((unused, throwable) -> {
                if (throwable != null) {
                    logger.error(
                            "Failed to emit agent step result event for step {}: {}",
                            result.getStepId(),
                            throwable.getMessage());
                } else {
                    logger.debug("Successfully emitted agent step result event for step {}", result.getStepId());
                }
            });
    ...
    private Object buildAgentEvent(AgentStepResult result) {
        ...
        if (result.getMetrics() != null) {
            result.getMetrics().forEach((k, v) -> metricsMap.put(k, v != null ? v.toString() : ""));
        }
