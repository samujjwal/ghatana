...
Each overlay extends the baseline manifests in `products/aep/k8s` and adds:
...
```bash
kustomize build products/aep/k8s/multi-region/eastus | kubectl apply -f -
kustomize build products/aep/k8s/multi-region/westeurope | kubectl apply -f -
```
