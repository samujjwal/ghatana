...
public class PipelineSchemaValidator {
     ...
     */
    public ValidationResult validate(PipelineRegistration pipeline) {
        List<String> errors = new ArrayList<>();
        List<String> warnings = new ArrayList<>();

        String config = pipeline.getConfig();
        if (config == null || config.isBlank()) {
            errors.add("PipelineRegistration configuration is empty");
        ...
        if (!isValidJson(config) && !isValidYaml(config)) {
            errors.add("PipelineRegistration configuration is not valid JSON or YAML");
        }

        // Check for required fields in config
        if (!config.contains("stages") && !config.contains("operators")) {
            warnings.add("PipelineRegistration configuration does not contain 'stages' or 'operators' field");
        }

        // Validate version if present
        if (pipeline.getVersion() < 0) {
            errors.add("PipelineRegistration version must be non-negative");
        }

        // Name length validation
        if (pipeline.getName() != null && pipeline.getName().length() > 255) {
            errors.add("PipelineRegistration name exceeds maximum length of 255 characters");
        }
