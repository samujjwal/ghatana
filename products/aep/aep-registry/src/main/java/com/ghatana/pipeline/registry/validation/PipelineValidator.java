...
public class PipelineValidator {
    ...
    public List<String> validate(PipelineRegistration pipeline, PipelineRegistration existingPipeline) {
        ...
        if (pipeline.getName() == null || pipeline.getName().trim().isEmpty()) {
            ...
            errors.add(String.format("PipelineRegistration name must be at most %d characters", MAX_PIPELINE_NAME_LENGTH));
        ...
        if (pipeline.getConfig() == null || pipeline.getConfig().trim().isEmpty()) {
            ...
            errors.add(String.format("PipelineRegistration configuration exceeds maximum size of %d bytes", MAX_CONFIG_SIZE));
        } else {
    ...
    public List<String> validateDag(String config) {
        ...
        try {
            ...
            if (errors.isEmpty()) {
                ...
                if (!isolated.isEmpty()) {
                    log.warn("PipelineRegistration has isolated stages (no dependencies): {}", isolated);
