...
public final class AepInputValidator {
    ...
    /**
     * Returns {@code true} if the pipeline ID is syntactically valid.
     */
    public static boolean isValidPipelineId(String id) {
        return id != null && !id.isBlank() && id.length() <= MAX_ID_LENGTH
                && RESOURCE_ID_PATTERN.matcher(id).matches();
    }

    /**
     * Returns {@code true} if the tenant ID is syntactically valid.
     */
    ...
    /**
     * Returns {@code true} if the string looks like valid JSON (starts and ends
     * with matching braces/brackets). This is a lightweight pre-check — full
     * parsing is delegated to Jackson.
     */
