...
- [ ] F-007 — High / Correctness / API+UI — Sessions are continuation tokens, not authentication; `AuthContext.isAuthenticated` must require a verified JWT. **(T-12 UI DONE, backend needs verification)**
...
- [ ] F-032 — High / Completeness+Security / Frontend — Role-driven UI gating; lint blocks action buttons without role checks. **(BACKEND PARTIALLY DONE: /api/v1/auth/roles endpoint created, JWT role extraction pending)**
...
- [ ] SIMP-11 — Simplicity / AI — AI confidence routing: low → advisory, mid → reviewable, high → applied with chained audit. **(T-22 envelope DONE, routing logic pending)**
...
- [ ] F-034 — Medium / Correctness / Backend — `/api/v1/compliance/soc2/report` refuses to render if evidence older than configured window. **(T-21 foundation DONE, freshness check pending)**
