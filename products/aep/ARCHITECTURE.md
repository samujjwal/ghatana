...
Modern advancements such as distributed stream processing, probabilistic inference, online learning, RAG, LLMs, and agents extend this model through typed event operators.
...
## Agent as Event Operator

An agent is a first-class `EventOperator`. Agent operators can be used anywhere a normal operator can be used in PatternSpec or PipelineSpec.

Agent operators include:
...
All agent operators must declare input schema, output schema, model policy, tool policy, memory policy, replay policy, guardrails, observability, and uncertainty semantics.

AI and agent behavior is represented as typed `AgentOperator` nodes in the unified operator graph.
...
Data-Cloud must not import PatternSpec, EPL, EventOperator runtime, or adaptive learning semantics.
...
AgentOperator must implement EventOperator.
Side-effecting AgentOperator usage must require tool policy, approval policy, idempotency, and audit.
```
