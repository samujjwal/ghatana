 ...
 */
@DisplayName("Rate Limiting Server Interceptor Tests [GH-90000]")
class RateLimitingServerInterceptorTest {

    @Test
    @DisplayName("Should reject a second in-flight call when concurrency is exhausted [GH-90000]")
    void shouldRejectWhenConcurrencyIsExhausted() { // GH-90000
        ...
        ServerCall<String, String> firstCall = createServerCall("audio.Video/Recognize [GH-90000]");
        ServerCall<String, String> secondCall = createServerCall("audio.Video/Recognize [GH-90000]");
        ...
        assertThat(statusCaptor.getValue().getDescription()).contains("overloaded [GH-90000]");
    }

    @Test
    @DisplayName("Should reject repeated calls when the per-method shared rate limit is exceeded [GH-90000]")
    void shouldRejectWhenRateLimitIsExceeded() { // GH-90000
        ...
        verify(secondCall).close(statusCaptor.capture(), any(Metadata.class)); // GH-90000
        assertThat(statusCaptor.getValue().getCode()).isEqualTo(Status.Code.RESOURCE_EXHAUSTED); // GH-90000
        assertThat(statusCaptor.getValue().getDescription()).contains("Rate limit exceeded [GH-90000]");
    }

    @Test
    @DisplayName("Should allow separate methods to use independent shared rate buckets [GH-90000]")
    void shouldAllowSeparateMethodsToUseIndependentBuckets() { // GH-90000
        RateLimitingServerInterceptor interceptor = new RateLimitingServerInterceptor(1.0, 1L, 2); // GH-90000
        Metadata headers = new Metadata(); // GH-90000
        ServerCall<String, String> transcribeCall = createServerCall("audio.Video/Transcribe [GH-90000]");
        ServerCall<String, String> synthesizeCall = createServerCall("audio.Video/Synthesize [GH-90000]");
        @SuppressWarnings("unchecked [GH-90000]")
        ServerCallHandler<String, String> next = mock(ServerCallHandler.class); // GH-90000
        when(next.startCall(any(), eq(headers))).thenReturn(new ServerCall.Listener<>() {}); // GH-90000
