...
| Finance Kernel | Ghatana Component | Integration Type | Customization Needed | Shared Library Enhancement |
|----------------|-------------------|------------------|---------------------|---------------------------|
| **K-01 IAM** | `auth-service` + `auth-gateway` | Direct + Extension | Finance-specific roles, permissions | Extend auth roles for finance domains |
| **K-02 Config** | `platform:java:config` | Direct | Finance config schemas | Add finance config templates |
| **K-03 Rules** | AEP Rule Engine | Enhanced | Finance rule templates | Generic rule engine with finance DSL |
| **K-04 Plugin** | `platform:java:plugin` | Direct | Finance plugin interfaces | Extend plugin system for finance |
| **K-05 Event Bus** | AEP Platform (uses Event Cloud internally) | Enhanced | Finance event schemas | Add finance event envelope standards to AEP |
| **K-06 Observability** | `platform:java:observability` | Direct | Finance-specific metrics | Add finance metric categories |
| **K-07 Audit** | `platform:java:audit` | Enhanced | 10-year retention, regulatory | Generic audit with configurable retention |
| **K-08 Data Gov** | `data-cloud` platform | Direct | Finance data policies | Add finance data governance policies |
| **K-09 AI Gov** | `ai-registry` + governance | Direct | Finance AI policies | Extend AI governance for finance regulations |
| **K-10 Deploy** | Standard Kubernetes | Direct | Finance deployment configs | Add finance deployment templates |
| **K-11 Gateway** | `platform:java:http` | Enhanced | Finance routing rules | Add finance-specific routing patterns |
| **K-12 SDK** | Ghatana SDK + Finance | Extension | Finance domain SDK | Create finance SDK on top of platform SDK |
| **K-13 Admin** | Finance-specific | New | Domain pack management | Build on admin portal patterns |
| **K-14 Secrets** | `platform:java:security` | Direct | Finance secret policies | Add finance secret classification |
| **K-15 Calendar** | Finance-specific | New | Dual-calendar support | Contribute dual-calendar patterns to platform |
| **K-16 Ledger** | Finance-specific | New | Double-entry accounting | Design for potential platform contribution |
| **K-17 DTC** | AEP Transaction Coordination | Enhanced | Finance transaction rules | Extend transaction patterns for finance |
| **K-18 Resilience** | `platform:java:agent-resilience` | Direct | Finance circuit breakers | Add finance-specific resilience patterns |
| **K-19 DLQ** | AEP Dead Letter Handling | Direct | Finance dead-letter handling | Extend DLQ patterns for finance events |
