...
export default async function entitlementRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.get('/route-entitlements', {
    ...
    const allowedRoutes = flashItRouteContracts.filter((candidate) => roleOrder[role] >= roleOrder[candidate.minimumRole]);
    const routeEntitlements = allowedRoutes.map(({ minimumRole: _minimumRole, ...candidate }) => candidate);
