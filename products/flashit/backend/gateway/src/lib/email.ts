...
function getSesClient(): SESClient {
  if (!sesClient) {
    ...
    sesClient = new SESClient({
      ...
      credentials:
        emailConfig.ses.accessKeyId && emailConfig.ses.secretAccessKey
          ? {
    ...
    if (process.env.NODE_ENV === "production") {
      throw new Error(
        "CRITICAL: EMAIL_PROVIDER=stub is not allowed in production. " +
          "Configure EMAIL_PROVIDER=smtp or EMAIL_PROVIDER=ses with proper credentials. " +
          "Emails are required for authentication, billing, and security features.",
      );
    }
