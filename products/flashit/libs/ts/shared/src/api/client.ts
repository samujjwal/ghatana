...
export class FlashitApiClient {
  ...
  // Moment endpoints
  async getMoments(
    params?: SearchMomentsRequest,
  ): Promise<SearchMomentsResponse> {
    const queryParams = new URLSearchParams();
    if (params?.sphereIds)
      queryParams.append("sphereIds", params.sphereIds.join(","));
    if (params?.query) queryParams.append("query", params.query);
    if (params?.tags) queryParams.append("tags", params.tags.join(","));
    if (params?.emotions)
      queryParams.append("emotions", params.emotions.join(","));
  ...
  async getMoment(id: string): Promise<Moment> {
    const response = await this.request<{ moment: Moment }>(
      `/api/moments/${id}`,
    );
  ...
  async searchMoments(
    queryOrParams: string | SearchMomentsRequest,
  ): Promise<Moment[] | SearchMomentsResponse> {
    if (typeof queryOrParams === "string") {
      const response = await this.getMoments({ query: queryOrParams });
      return response.moments;
    }

    return this.getMoments(queryOrParams);
  }

  async classifySphere(
    data: ClassifySphereRequest,
  ): Promise<ClassifySphereResponse> {
    return this.request<ClassifySphereResponse>(
      "/api/moments/classify-sphere",
      {
        method: "POST",
        body: JSON.stringify(data),
      },
