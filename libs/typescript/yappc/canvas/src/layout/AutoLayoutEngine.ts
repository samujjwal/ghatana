...
export class AutoLayoutEngine {
   ...
   */
  calculateLayout(elements: LayoutElement[], edges: LayoutEdge[]): LayoutResult {
   ...
   */
  private hierarchicalLayout(elements: LayoutElement[], edges: LayoutEdge[]): LayoutResult {
    ...
    layers.forEach((layer, layerIndex) => {
      ...
      const maxElementSize = isVertical 
        ? Math.max(...layer.map(id => elements.find(e => e.id === id)?.width || 0))
        : Math.max(...layer.map(id => elements.find(e => e.id === id)?.height || 0));
      ...
      layer.forEach((elementId, index) => {
        ...
        if (isVertical) {
          const y = layerIndex * (maxElementSize + spacing) + this.config.padding!;
          const totalWidth = layerSize * element.width + (layerSize - 1) * spacing;
          const startX = -totalWidth / 2;
          const x = startX + index * (element.width + spacing) + element.width / 2;
          positions.set(elementId, { x, y });
        } else {
          const x = layerIndex * (maxElementSize + spacing) + this.config.padding!;
          const totalHeight = layerSize * element.height + (layerSize - 1) * spacing;
          const startY = -totalHeight / 2;
          const y = startY + index * (element.height + spacing) + element.height / 2;
          positions.set(elementId, { x, y });
        }
      });
    });
   ...
   */
  private forceDirectedLayout(elements: LayoutElement[], edges: LayoutEdge[]): LayoutResult {
    const positions = new Map<string, { x: number; y: number }>();
      ...
      elements.forEach(element1 => {
        ...
        edges.forEach(edge => {
          if (edge.source === element1.id || edge.target === element1.id) {
            const otherId = edge.source === element1.id ? edge.target : edge.source;
   ...
   */
  private treeLayout(elements: LayoutElement[], edges: LayoutEdge[]): LayoutResult {
    ...
    const layoutNode = (nodeId: string, depth: number): number => {
      ...
      if (node.children.length === 0) {
        ...
        const childYs = node.children.map(childId => layoutNode(childId, depth + 1));
    ...
    for (let i = 0; i < layers.length - 1; i++) {
      ...
      nextLayer.forEach(nodeId => {
        ...
        if (connectedFromCurrent.length > 0) {
          const avg = connectedFromCurrent.reduce((a, b) => a + b, 0) / connectedFromCurrent.length;
      ...
      // Sort by barycenter
      nextLayer.sort((a, b) => (barycenters.get(a) || 0) - (barycenters.get(b) || 0));
    }
  }
...
export function useAutoLayout() {
  const applyLayout = (elements: LayoutElement[], edges: LayoutEdge[], config: LayoutConfig) => {
