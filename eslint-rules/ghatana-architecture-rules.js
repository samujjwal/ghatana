  ...
  rules: {
    ...
    'no-cross-product-imports': {
      meta: {
        ...
        schema: [],
        messages: {
          crossProductImport: "🚫 Cross-product import detected: '{{import}}' belongs to '{{targetProduct}}'. Products must not depend on each other. Use shared-services or platform instead.",
        },
      },
      ...
      create(context) {
        ...
        function checkImport(node, importPath) {
          ...
          if (targetProduct && currentProduct && targetProduct !== currentProduct) {
            // Check if allowed
    ...
    'enforce-platform-boundaries': {
      meta: {
        ...
        schema: [],
        messages: {
          invalidLayerDependency: "🚫 Invalid layer dependency: '{{import}}' (layer {{targetLayer}}) cannot be imported by '{{current}}' (layer {{currentLayer}}). Lower layers cannot depend on higher layers.",
        },
      },
          ...
          for (const banned of BANNED_LIBRARIES) {
            if (importPath === banned || importPath.startsWith(`${banned}/`)) {
              context.report({
                ...
                data: {
                  library: banned,
                  alternative: ALTERNATIVES[banned] || 'Find an alternative in the approved tech stack',
                },
              });
            }
          }
    ...
    'no-deprecated-ghatana-ui': {
      meta: {
        ...
        schema: [],
        messages: {
          deprecatedPackage: "🚫 @ghatana/ui is deprecated. Use @ghatana/design-system for platform components or create product-local components.",
        },
      },
      ...
      create(context) {
        return {
          ImportDeclaration(node) {
            ...
            if (importPath === '@ghatana/ui' || importPath.startsWith('@ghatana/ui/')) {
              context.report({
                node,
    ...
    'prefer-platform-utils': {
      meta: {
        ...
        schema: [],
        messages: {
          usePlatformUtils: "⚠️ Consider using @ghatana/platform-utils instead of external libraries for common utilities.",
        },
      },
